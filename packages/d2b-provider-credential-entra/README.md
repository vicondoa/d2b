...
```bash
cd packages && cargo check -p d2b-provider-credential-entra
cd packages && cargo test -p d2b-provider-credential-entra --lib --test lifecycle --test conformance --test faults --test canary --test delivery --test placement
