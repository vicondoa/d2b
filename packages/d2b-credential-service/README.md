...
`ADR046-credential-002`. Nothing recorded here is a design statement.

## Provider identity

Not yet declared. Filled by `ADR046-credential-002`.
...
Not yet declared. Filled by `ADR046-credential-002`. Until then the standing Provider rules
...
```bash
cd packages && cargo test -p d2b-credential-service
cd packages && cargo clippy -p d2b-credential-service --all-targets
```
