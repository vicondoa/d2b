...
//! <workload>.nixling                  local workload, current-node form
...
//! ```
//!
//! Label-count rules (labels before the reserved `.nixling` suffix):
//!
...
//! suffix word `nixling` in a non-suffix position, malformed labels, and
//! ambiguous multi-label inputs without the `.nixling` suffix are rejected
...
pub const TARGET_SUFFIX: &str = "nixling";
...
pub enum TargetParseError {
    /// The input (after stripping `nl://`) was empty.
    Empty,
    /// A multi-label human form omitted the reserved `.nixling` suffix, which
    /// makes the workload/node/realm split ambiguous (e.g. `build-vm.work`).
    MissingSuffix,
    /// The input was only the reserved suffix (`nixling` / `nl://nixling`).
    MissingWorkload,
    /// A label was a list-only selector (`all` or `*`), which never names a
    /// single persisted target.
    SelectorNotAllowed,
    /// The reserved suffix word `nixling` appeared in a non-suffix label.
    ReservedLabel,
...
impl TargetName {
    /// Parse a human or canonical (`nl://...`) target address, fail-closed.
    ///
    /// The `nl://` scheme prefix is optional; when present the fully-qualified
    /// `.nixling` suffix is required (it is the canonical machine form).
    pub fn parse(raw: &str) -> Result<Self, TargetParseError> {
        let (body, had_scheme) = match raw.strip_prefix("nl://") {
            Some(rest) => (rest, true),
            None => (raw, false),
        };
        if body.is_empty() {
        ...
        if had_suffix {
            ...
            // The canonical `nl://` form is always fully qualified, and any
            // multi-label human form must carry the `.nixling` suffix so the
    ...
    /// Render the canonical `nl://<workload>.<node>.<realm-path>.nixling`
...
mod tests {
    ...
    fn bare_workload_defaults_to_this_local() {
        ...
        assert_eq!(t.to_canonical(), "nl://demo.this.local.nixling");
    }

    #[test]
    fn workload_suffix_form_is_this_local() {
        let t = parsed("demo.nixling");
        assert_eq!(t.workload.as_str(), "demo");
        assert_eq!(t.node.as_str(), "this");
        assert_eq!(t.realm, RealmPath::local());
    }
    ...
    fn two_labels_are_workload_and_node_local_realm() {
        // Per ADR: `build-vm.work.nixling` is workload `build-vm` on local
        // node `work`, NOT realm `work`.
        let t = parsed("build-vm.work.nixling");
    ...
    #[test]
    fn nl_scheme_round_trips_to_canonical() {
        let t = parsed("nl://demo.aca.work.nixling");
        assert_eq!(t.to_canonical(), "nl://demo.aca.work.nixling");
    ...
    fn with_local_node_resolves_this() {
        ...
        let t = parsed("personal-dev.nixling").with_local_node(local.clone());
        assert_eq!(t.node, local);
        assert!(!t.node_is_this());
        assert_eq!(t.to_canonical(), "nl://personal-dev.laptop.local.nixling");
        // an explicit node is left untouched.
        let explicit = parsed("demo.aca.work.nixling").with_local_node(local);
    ...
    fn multi_label_without_suffix_is_rejected() {
        ...
        assert_eq!(
            TargetName::parse("nl://demo.aca.work"),
            Err(TargetParseError::MissingSuffix)
        );
    }
    ...
    fn selectors_and_reserved_labels_are_rejected() {
        assert_eq!(
            TargetName::parse("all.nixling"),
            Err(TargetParseError::SelectorNotAllowed)
        );
        assert_eq!(
            TargetName::parse("demo.all.nixling"),
            Err(TargetParseError::SelectorNotAllowed)
        );
        assert_eq!(
            TargetName::parse("*.nixling"),
            Err(TargetParseError::SelectorNotAllowed)
        );
        // `nixling` only valid as the suffix.
        assert_eq!(
            TargetName::parse("nixling.nixling"),
    ...
    fn empty_and_suffix_only_are_rejected() {
        ...
        assert_eq!(TargetName::parse("nl://"), Err(TargetParseError::Empty));
        assert_eq!(
            TargetName::parse("nixling"),
    ...
    fn malformed_labels_are_rejected() {
        assert!(matches!(
            TargetName::parse("Demo.nixling"),
            Err(TargetParseError::BadLabel(_))
        ));
        assert!(matches!(
            TargetName::parse("demo.-bad.nixling"),
            Err(TargetParseError::BadLabel(_))
        ));
    }
}
