...
const API_ADMISSION: &str = include_str!("../../d2b-resource-api/src/admission.rs");
...
fn admission_capability_has_exactly_one_native_evaluator_call_site() {
    ...
    assert!(!STORE_LIB.contains("AdmissionIssuer"));
    assert!(!API_ADMISSION.contains("pub fn admission_pair"));
    assert!(!API_ADMISSION.contains("pub struct AdmissionIssuer"));
}
