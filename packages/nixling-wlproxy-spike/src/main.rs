...
//! nixling-wayland-filter binary (Lane C) without implementing the full
...
//!    The nixling filter binary (Lane C) should document this as a known
...
//!    to treat proxy death as a fatal VM error (triggering nixlingd's pidfd
...
fn main() {
    println!("nixling-wlproxy-spike: compile-time feasibility proof only. Not a runnable binary.");
}
...
/// The listen socket is `/run/nixling-wlproxy/<vm>/wayland.sock`.
///
...
impl FilterPolicy {
    ...
    pub fn emit_warnings(&self) {
        // Baseline required globals for nixling graphics VMs.
...
impl XdgToplevelHandler for FilterToplevel {
    fn handle_set_app_id(&mut self, slf: &Rc<XdgToplevel>, app_id: &str) {
        ...
        // Enforce prefix: strip any existing nixling prefix first to avoid
...
mod tests {
    ...
    fn app_id_prefixing_idempotent() {
        ...
        assert_eq!(prefix, "nixling.work-aad.");
        // First application
        ...
        assert_eq!(re_prefixed, "nixling.work-aad.org.mozilla.firefox");
    }
