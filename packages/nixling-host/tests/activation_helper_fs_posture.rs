//! Filesystem-posture coverage for the `nixling-activation-helper` binary,
...
//! binary via `CARGO_BIN_EXE_nixling-activation-helper` against an isolated
...
const HELPER: &str = env!("CARGO_BIN_EXE_nixling-activation-helper");
...
fn run(args: &[&str]) -> Option<i32> {
    Command::new(HELPER)
        .args(args)
        .output()
        .expect("spawn nixling-activation-helper")
        .status
        .code()
}
...
fn help_exits_zero_and_missing_verb_exits_one() {
    ...
    assert!(
        help.contains("nixling-activation-helper"),
