//! `nixling-host` is the disjoint host-prepare API surface. The modules
...
// Lives here (not nixling-core) so DeviceClass is available without
// a dep-graph cycle; nixling-priv-broker converts CompiledSeccompProgram
...
// VM; consumed by nixlingd via SpawnRunner with RunnerRole::Gpu).
pub mod gpu_argv;
...
// ownership matrix under /var/lib/nixling/vms/<vm>/. CRITICAL: includes
