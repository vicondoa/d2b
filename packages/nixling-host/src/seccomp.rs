...
//! `SECCOMP_SET_MODE_FILTER`. This module lives in `nixling-host` (not
//! `nixling-core`) so that [`DeviceClass`] is available without creating
//! a dep-graph cycle; `nixling-priv-broker` depends on `nixling-host` and
...
/// `nixling-host` (which forbids unsafe code); the broker's quarantined
...
///
/// Safe to construct and inspect in `nixling-host`.  Unsafe syscall
