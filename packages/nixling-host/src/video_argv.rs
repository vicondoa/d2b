...
//! ```
//!
//! The video sidecar runs as the graphics VM's `nixling-<vm>-gpu`
...
/// Video decode backend. The nixling video host module wires VAAPI
...
/// `nixling-<vm>-video` (per `nixos-modules/components/video/host.nix`).
pub fn exec_arg0(input: &VideoArgvInput) -> Result<String, VideoArgvError> {
    if input.vm_name.is_empty() {
        return Err(VideoArgvError::EmptyVmName);
    }
    Ok(format!("nixling-{}-video", input.vm_name))
}
...
mod tests {
    ...
    fn audit_parity_minimal() {
        ...
        assert!(joined.contains("--socket-path /run/nixling-video/corp-desktop/video.sock"));
