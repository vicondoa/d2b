...
//! paths) and emits the `Vec<String>` argv that `nixlingd` will exec
...
//!   under `/run/nixling/vms/<vm>/ch-api.sock`);
...
mod tests {
    ...
    ///
    /// - API socket lives at `/run/nixling/vms/corp-vm/ch-api.sock`
    ...
    fn audit_input() -> ChArgvInput {
        ChArgvInput {
            ...
            fs_shares: vec![
                ...
                ChFsShare { socket: "corp-vm-virtiofs-nl-meta.sock".to_owned(),     tag: "nl-meta".to_owned() },
                ChFsShare { socket: "corp-vm-virtiofs-nl-hkeys.sock".to_owned(),    tag: "nl-hkeys".to_owned() },
                ChFsShare { socket: "corp-vm-virtiofs-nl-ssh-host.sock".to_owned(), tag: "nl-ssh-host".to_owned() },
            ],
    ...
    fn headless_audit_parity_minimal() {
        ...
        assert!(joined.contains("socket=corp-vm-virtiofs-nl-meta.sock,tag=nl-meta"));
        assert!(joined.contains("socket=corp-vm-virtiofs-nl-hkeys.sock,tag=nl-hkeys"));
        assert!(joined.contains("socket=corp-vm-virtiofs-nl-ssh-host.sock,tag=nl-ssh-host"));
    ...
    fn extra_vsock_emits_socket_only_form() {
        let mut input = audit_input();
        input.primary_vsock = None;
        input.extra_vsock = vec!["/run/nixling/vms/corp-vm/obs.vsock".to_owned()];
        let argv = generate_ch_argv(&input).expect("extra-only vsock is valid");
        let joined = argv.join(" ");
        // socket=... form (no cid=...)
        assert!(joined.contains("--vsock socket=/run/nixling/vms/corp-vm/obs.vsock"));
    }
    ...
    fn extra_args_appended_in_order() {
        ...
        input.extra_args = vec![
            "--tpm".to_owned(),
            "socket=/run/nixling/vms/corp-vm/swtpm.sock".to_owned(),
            "--gpu".to_owned(),
            "socket=/run/nixling/vms/corp-vm/gpu.sock".to_owned(),
        ];
    ...
    fn multiple_oem_strings_join_with_comma() {
        ...
        input.platform_oem_strings = vec![
            "io.systemd.credential:vmm.notify_socket=vsock-stream:2:8888".to_owned(),
            "io.nixling.vm=corp-vm".to_owned(),
        ];
        let argv = generate_ch_argv(&input).expect("multi-oem valid");
        let joined = argv.join(" ");
        assert!(joined.contains(
            "oem_strings=[io.systemd.credential:vmm.notify_socket=vsock-stream:2:8888,io.nixling.vm=corp-vm]"
        ));
    }
