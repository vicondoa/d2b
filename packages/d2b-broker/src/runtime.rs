...
fn cutover_relaunch_allowed(
    ...
    let d2b_contracts_broker::broker_wire::BrokerRequest::LaunchCutoverRunner(request) = request
    else {
        return Ok(false);
    };
    let capabilities = load_cutover_capabilities(config)?;
            ...
            let (outcome, identity) = match req.effect {
                ...
                d2b_contracts_broker::broker_wire::CutoverEffectKind::Verification
                    if matches!(
                        &req.payload,
                        Some(
                            d2b_contracts_broker::broker_wire::CutoverEffectPayload::Verification(
                                _
                            )
                        )
                    ) =>
...
fn launch_cutover_runner(
    ...
    targeted_reap_runner(&runner_id, outcome.pidfd.as_fd());
    Ok(
        d2b_contracts_broker::broker_wire::LaunchCutoverRunnerResponse {
