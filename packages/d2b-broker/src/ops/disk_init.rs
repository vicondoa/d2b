...
//!   `/var/lib/d2b/vms/` (path-safety check).
...
//! exists, the broker validates that it is the expected d2b-owned
...
use d2b_core::bundle_resolver::{BundleResolver, ResolvedDiskInitOp};

/// Permitted root prefix for all disk-init target paths.
const PERMITTED_ROOT: &str = "/var/lib/d2b/vms/";
...
/// `D2B_BROKER_MKFS_EXT4_BINARY` (default
...
fn validate_mkfs_ext4_binary(raw: &str) -> io::Result<&std::path::Path> {
    ...
    if !p.is_absolute() {
        return Err(io::Error::new(
            ...
            format!("disk-init: D2B_BROKER_MKFS_EXT4_BINARY must be absolute (got {raw:?})"),
        ));
    }
    if p.components()
        .any(|c| matches!(c, std::path::Component::ParentDir))
    {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!("disk-init: D2B_BROKER_MKFS_EXT4_BINARY may not contain `..` (got {raw:?})"),
        ));
    }
    Ok(p)
}
...
fn resolve_mkfs_ext4_binary() -> io::Result<MkfsTool> {
    let raw = std::env::var("D2B_BROKER_MKFS_EXT4_BINARY")
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let action = match self.kind {
            DiskInitErrorKind::UnexpectedPosture => {
                "inspect the file ownership, permissions, and size, then restore the declared d2b image posture or remove the conflicting file before retrying"
            }
...
mod tests {
    use super::*;
    use d2b_core::bundle_resolver::ResolvedDiskInitOp;
    use std::fs;
