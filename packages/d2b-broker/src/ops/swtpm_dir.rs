...
/// state Volume directory under the trusted root, which
/// [`derive_resource_backed_paths`] fences the launch's argv against.
///
/// The spawn hook reads the directory's presence from here: the derivation
/// itself stays a pure function of the plan and the identity.
            ...
            let marker_result = match existing_marker.clone() {
                Some(marker) => {
                    ...
                    if marker.origin == MarkerOrigin::LegacyMigration {
                        // The v2 legacy swtpm adoption journal module was
                        // removed with the v2 Bundle/migration surface.
                        // Without a trusted journal to verify the
                        // commitment, a legacy-origin marker cannot be
