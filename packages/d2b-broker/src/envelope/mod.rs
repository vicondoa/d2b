...
mod tests {
    ...
    fn a_spinning_handler_is_aborted_to_timed_out_while_an_innocent_operation_answers() {
        ...
        // 2 s spin - so the refusal proves the abort fired at the 400 ms
        // budget while the handler was still spinning. The handler signals
        // when it is actually spinning, so the innocent operation provably
        // answers while the spinner is in flight. Wall-clock load cannot
        // stretch either side of the proof.
        ...
        // the code alone proves the abort fired at the budget while the
        // handler was still spinning - no wall-clock window is read.
