...
impl NoiseHandshake {
    pub fn new(
        ...
        let bootstrap_identity = credentials.into_bootstrap_identity();
        Ok(Self {
            state,
            bootstrap_identity,
            role,
...
impl HandshakeCredentials {
    fn into_bootstrap_identity(self) -> Option<BootstrapIdentityBinding> {
        match self {
            Self::IkPsk2Initiator { psk, .. } | Self::IkPsk2Responder { psk, .. } => {
                Some(psk.into_identity())
            }
            Self::Nn | Self::Kk { .. } => None,
        }
    }
}
...
pub(crate) struct EstablishedAuthentication {
    pub(crate) transcript_hash: [u8; 32],
    pub(crate) remote_static_key: Option<[u8; 32]>,
    pub(crate) bootstrap_identity: Option<BootstrapIdentityBinding>,
    pub(crate) generation: u64,
}
...
impl EstablishedHandshake {
    ...
    pub(crate) fn take_bootstrap_identity(&mut self) -> Option<BootstrapIdentityBinding> {
        self.bootstrap_identity.take()
    }

    pub(crate) fn take_authentication(&mut self) -> EstablishedAuthentication {
        EstablishedAuthentication {
            transcript_hash: self.transcript_hash,
            remote_static_key: self.remote_static_key,
            bootstrap_identity: self.take_bootstrap_identity(),
            generation: self.generation,
        }
    }
}
