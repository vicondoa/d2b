...
fn usage() -> Err {
    "usage: d2b-gateway-enroll <enroll|rotate> <credential-path> <seal-key-path> [not-after-unix]"
        .into()
}
...
fn read_material() -> Result<d2b_gateway_runtime::GatewayCredentialMaterial, Err> {
...
fn validate_paths(credential_path: &PathBuf, seal_key_path: &PathBuf) -> Result<(), Err> {
    let state_dir = std::env::var("D2B_GATEWAY_STATE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/var/lib/d2b/gateways"));
