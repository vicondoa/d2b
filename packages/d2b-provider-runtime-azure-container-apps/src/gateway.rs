...
//! Identity) and the d2b-internal per-session credential (plane 3) live in
...
use d2b_contracts::{Capability, CapabilitySet, ErrorKind, OpaquePayload, WorkloadState};
use d2b_contracts::{
    ids::{ExecutionId, NodeId, ProviderId, RealmId, WorkloadId},
    realm::RealmPath,
...
pub enum AcaDiskImageSource {
    ...
    ContainerImage {
        /// Container image reference, e.g. `registry.azurecr.io/d2b-wayland:mi`.
        image: String,
...
impl AcaWorkloadProvider {
    ...
    /// `d2b-workload=<alias>` Azure Container Apps label plus configured realm/default labels
    ...
    async fn create_disk_image(
        ...
        labels.insert("d2b-workload".to_owned(), workload.as_str().to_owned());
...
impl AcaWorkloadProvider {
    ...
    pub async fn list(&self, selector: ListSelector) -> ProviderResult<Vec<WorkloadSummary>> {
        ...
        Ok(sandboxes
            ...
            .filter_map(|sandbox| {
                let workload = sandbox.labels.get("d2b-workload")?;
                let realm = sandbox
                    .labels
                    .get("d2b-realm")
...
fn sandbox_state(sandbox: &AcaSandbox) -> WorkloadState {
    match sandbox.state.as_deref().map(str::to_ascii_lowercase) {
        Some(state) if state == "running" || state == "ready" => {
            WorkloadState::Running
        }
        Some(state) if state == "stopping" => WorkloadState::Stopping,
        Some(state) if state == "starting" || state == "creating" => {
            WorkloadState::Starting
        }
        Some(state) if state == "failed" => WorkloadState::Failed,
...
mod tests {
    ...
    fn cfg() -> AcaConfig {
        AcaConfig {
            ...
            resource_group: "rg-d2b-centralus".into(),
            sandbox_group: "casbx-d2b-test".into(),
    ...
    fn lifecycle_defaults() -> AcaSandboxDefaults {
        ...
        labels.insert("d2b-realm".to_owned(), "work".to_owned());
        let mut disk_labels = BTreeMap::new();
        disk_labels.insert("d2b-image-tag".to_owned(), "mi".to_owned());
        AcaSandboxDefaults {
            disk_image: AcaDiskImageSource::ContainerImage {
                image: "cr.example.azurecr.io/d2b-wayland:mi".to_owned(),
                name: "d2b-wayland-mi".to_owned(),
    ...
    fn url_builder_matches_adc_contract() {
        ...
        assert_eq!(
            ...
            [
                ...
                "scriptions/24f3458d-0000-",
                "0000-0000-000000000000/resourceGroups/rg-d2b-centralus/",
                "sandboxGroups/casbx-d2b-test/sandboxes/sbx-1/executeShellCommand",
        ...
        assert!(
            c.list_sandboxes_url(Some("d2b-workload=demo,d2b-realm=work"))
                .ends_with("/sandboxes?api-version=2026-02-01-preview&labels=d2b-workload%3Ddemo%2Cd2b-realm%3Dwork")
        );
        assert!(
            c.disk_images_url()
    ...
    async fn workload_exec_maps_nonzero_exit_to_typed_error() {
        ...
        let req = ExecStartRequest {
            ...
            tty: false,
            command: OpaquePayload::new(b"false".to_vec()).unwrap(),
        };
    ...
    async fn workload_exec_denies_tty_before_rest_call() {
        ...
        let req = ExecStartRequest {
            workload: WorkloadId::parse("sbx-1").unwrap(),
            tty: true,
            command: OpaquePayload::new(b"sh".to_vec()).unwrap(),
        };
        let err = p.exec(req).await.unwrap_err();
    ...
    async fn lifecycle_workload_exec_resolves_alias_to_sandbox_id() {
        ...
        let req = ExecStartRequest {
            ...
            tty: false,
            command: OpaquePayload::new(b"true".to_vec()).unwrap(),
        };
    ...
    async fn workload_exec_id_is_derived_from_request_not_elapsed_time() {
        ...
        let req = ExecStartRequest {
            workload: WorkloadId::parse("sbx-1").unwrap(),
            tty: false,
            command: OpaquePayload::new(b"echo hi".to_vec()).unwrap(),
        };
        ...
        let mut changed = req;
        changed.command = OpaquePayload::new(b"echo bye".to_vec()).unwrap();
    ...
    async fn create_sandbox_uses_rest_disk_and_sandbox_contract() {
        let created_disk = disk_item("disk-1", "d2b-wayland-mi", "work").to_string();
        ...
        assert!(
            calls[0].1.contains(
                "/sandboxes?api-version=2026-02-01-preview&labels=d2b-realm%3Dwork%2Cd2b-workload%3Ddemo"
            )
        );
        ...
        assert!(calls[1].1.contains("name%3Dd2b-wayland-mi"));
        assert!(calls[1].1.contains("d2b-workload%3Ddemo"));
        ...
        assert_eq!(
            ...
            "cr.example.azurecr.io/d2b-wayland:mi"
        );
        assert_eq!(disk_body["labels"]["name"], "d2b-wayland-mi");
        assert_eq!(disk_body["labels"]["d2b-workload"], "demo");
        assert!(
        ...
        assert_eq!(sandbox_body["labels"]["d2b-workload"], "demo");
        assert_eq!(sandbox_body["labels"]["d2b-realm"], "work");
    }
    ...
    async fn malformed_list_items_fail_closed() {
        let (p, _) = lifecycle_provider_seq(vec![(
            200,
            r#"[{"state":"Running","labels":{"d2b-workload":"demo"}}]"#,
        )]);
        let err = p.list(ListSelector::All).await.unwrap_err();
        assert_eq!(err.kind(), ErrorKind::MalformedFrame);
    }
    ...
    async fn workload_lookup_rejects_cross_realm_stale_label() {
        ...
        let work_disk = disk_item("disk-1", "d2b-wayland-mi", "work").to_string();
    ...
    async fn workload_lookup_rejects_cross_workload_stale_labels() {
        ...
        let demo_disk = disk_item("disk-1", "d2b-wayland-mi", "work").to_string();
        let (p, http) = lifecycle_provider_seq(vec![
    ...
    async fn list_maps_aca_labels_to_workload_summaries() {
        ...
        assert_eq!(list[0].state, WorkloadState::Running);
    }
