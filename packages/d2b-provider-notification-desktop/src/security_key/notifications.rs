...
//! - Per-action nonce tokens (from [`crate::security_key::nonce`]) embedded in the action
...
use crate::security_key::events::{BlockReason, BusyDetail, SecurityKeyEvent};
use crate::security_key::nonce::action_key_for;

/// An action button on a desktop notification.
///
/// The `action_key` encodes the nonce in the `d2b-sk-<verb>:<hex>` format
/// produced by [`crate::security_key::nonce::action_key_for`].  The notification daemon
/// passes this string back on `ActionInvoked`; the callback looks it up with
/// [`crate::security_key::nonce::ActionNonceStore::validate_and_consume`].
...
mod tests {
    ...
    fn test_nonce() -> String {
        String::from_utf8(vec![b'a'; crate::security_key::nonce::NONCE_BYTES * 2])
