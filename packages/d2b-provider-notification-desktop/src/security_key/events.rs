...
//! via the durable state file ([`crate::security_key::state`]) and must not perform any
//! privileged host mutations in response - all callbacks route through
//! `d2bd`/CLI via action nonces ([`crate::security_key::nonce`]).
...
/// ([`crate::security_key::nonce`]) so that `Cancel request` callbacks cannot be replayed
