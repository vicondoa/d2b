...
mod tests {
    ...
    fn open_uhid_pair() -> (UhidDevice, std::os::unix::net::UnixStream) {
        use std::os::fd::OwnedFd;
        use std::os::unix::net::UnixStream;

        let (read, write) = UnixStream::pair().expect("uhid test socketpair");
        read.set_nonblocking(true)
            .expect("nonblocking uhid read end");
    ...
    async fn read_event_dispatches_by_type_and_returns_none_on_clean_eof() {
        ...
        write.write_all(&output).expect("write output event");
        ...
        write.write_all(&get_report).expect("write get-report event");
    ...
    async fn read_event_rejects_a_short_header_as_unexpected_eof() {
        ...
        let (mut dev, mut write) = open_uhid_pair();
        write.write_all(&[1u8, 2]).expect("write short header");
