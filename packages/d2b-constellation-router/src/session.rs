...
use d2b_constellation_provider::types::{ByteStream, TransportSession};
...
fn parse_codec_id(id: &str) -> ProviderResult<d2b_constellation_core::ProtocolToken> {
    d2b_constellation_core::ProtocolToken::parse(id)
...
mod tests {
    ...
    use d2b_constellation_provider::types::{NodeRegistration, TransportTarget};
    ...
    async fn client_receives_explicit_handshake_rejection_fail_closed() {
        ...
        impl ProtocolCodec for SkewedSchemaCodec {
            fn codec_id(&self) -> &str {
                d2b_constellation_codec_protobuf::CODEC_ID
            }
    ...
    async fn peer_binding_mismatch_is_rejected_fail_closed() {
        ...
        let forged = Handshake {
            ...
            schema_fingerprint: parse_codec_id(&codec.schema_fingerprint()).unwrap(),
            capabilities: CapabilitySet::empty().negotiation(),
            peer: Some(d2b_constellation_core::PeerContext {
