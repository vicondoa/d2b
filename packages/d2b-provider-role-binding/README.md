...
This is the crate root for the `RoleBinding` resource type. It owns the type's identity
...
| Field | Value |
| --- | --- |
| Provider name | `role-binding` |
| ResourceType | `RoleBinding` |
| Package | `packages/d2b-provider-role-binding/` |
| Driver declaration | `role_binding_descriptor` -> `DriverDescriptor` |

## Config schema

The type declares no provider config schema: `RoleBinding` rows carry the JSON spec
object the core rows store, and nothing outside it decodes at validate.

## Exported resource types

`RoleBinding` is not exportable. `ResourceExport` admits only qualified
`*.d2bus.org.*Service` types, so a `RoleBinding` row is never an export subject. The
declaration carries `exportable: false`.

## Controllers / services / workers / binaries

The crate ships the type's declaration, not a standalone process. The shared
driver serves `RoleBinding` rows through the `ResourceDriver` verbs: `validate` decodes the
...
The registration surface is `role_binding_descriptor`: it declares the type through the shared
...
`RoleBinding` names no placement anchor, so a `RoleBinding` row is reconciled on its
...
```bash
cargo test -p d2b-provider-role-binding
```
