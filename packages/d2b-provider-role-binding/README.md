...
This is the crate root for the `RoleBinding` resource type. It owns the type's
...
serves `RoleBinding` rows through the `ResourceDriver` verbs: `validate` decodes the
...
`RoleBindingDriverFactory` is the registration surface; `role_binding_descriptor` carries
