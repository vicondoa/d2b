...
//!
//! `RoleBinding` is the binding of a role to its subjects: the driver converges it as metadata once its desired state is admitted.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`RoleBindingDriverFactory`] registration through the type's
//!   descriptor ([`role_binding_descriptor`]).
...
pub const ROLE_BINDING_TYPE_NAME: &str = "RoleBinding";

/// The resource verbs the `RoleBinding` type supports.
///
...
/// The execution domains the `RoleBinding` type can be reconciled in.
///
/// Derived from the placement contract: `RoleBinding` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `RoleBinding` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const ROLE_BINDING_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `RoleBinding` driver reads while reconciling.
///
...
const ROLE_BINDING_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `RoleBinding` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn role_binding_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `RoleBinding` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct RoleBindingDriverFactory {
    types: [ResourceTypeName; 1],
}

impl RoleBindingDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(ROLE_BINDING_TYPE_NAME)],
        }
    }
}

impl Default for RoleBindingDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for RoleBindingDriverFactory {
    fn resource_types(&self) -> &[ResourceTypeName] {
        &self.types
    }

    async fn create(&self, _key: &ResourceKey) -> Box<dyn DynResourceDriver> {
        Box::new(RoleBindingDriver)
    }
}
...
/// One `RoleBinding` resource's driver.
pub struct RoleBindingDriver;

#[async_trait]
impl ResourceDriver for RoleBindingDriver {
...
/// `*.d2bus.org.*Service` types, so a `RoleBinding` row is never an export subject.
pub fn role_binding_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        resource_type: WellKnownType::ROLE_BINDING,
        allowed_sources: AllowedSources::BUILTIN,
        verbs: ROLE_BINDING_VERBS,
        execution: ROLE_BINDING_EXECUTION_DOMAINS,
        exportable: false,
        reads: ROLE_BINDING_READS,
        operations: &[],
        creations: &[],
        startup: &[],
        services: &[],
        decoder: role_binding_spec_decoder(),
        factory: Arc::new(RoleBindingDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        ROLE_BINDING_TYPE_NAME, RoleBindingDriverFactory, role_binding_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", ROLE_BINDING_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = RoleBindingDriverFactory::new()
            .create(&ResourceKey::new("work", ROLE_BINDING_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
