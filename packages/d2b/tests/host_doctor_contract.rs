...
//!
//! Spawns the real `d2b` binary and exercises `host doctor
//! --read-only --json` against on-disk daemon-state report fixtures
//! written into a sandboxed scratch directory. Every probe is
//! redirected via env knobs (`D2B_BROKER_SOCKET`,
//! `D2B_PUBLIC_SOCKET`, `D2B_DAEMON_STATE_DIR`,
//! `D2B_METRICS_URL`, `D2B_MANIFEST_PATH`) so the test never
//! touches real `/run` or `/var/lib` state and is fully hermetic.
//!
//! Unlike the `list` contract test, this gate needs no D2B_FIXTURES
...
const MANIFEST_JSON: &str = r#"{"_manifest":{"manifestVersion":6},"_observability":{"enabled":false,"vmName":"sys-obs","obsVsockCid":1000,"obsVsockHostSocket":"/var/lib/d2b/vms/sys-obs/vsock.sock","signozUrl":"http://10.40.0.10:8080","signozOtlpGrpcPort":4317,"signozOtlpHttpPort":4318}}"#;
  ...
  "markers": [
    {
      ...
      "severity": "fail",
      "remediation": "fix the in-guest shutdown path and retry d2b vm stop",
      "elapsedMs": 90000
    },
    {
      "vm": "media-vm",
...
impl Sandbox {
    ...
    fn doctor_command(&self) -> Command {
        ...
        cmd.env("D2B_BROKER_SOCKET", &self.broker_socket)
            .env("D2B_PUBLIC_SOCKET", &self.public_socket)
            .env("D2B_DAEMON_STATE_DIR", &self.state_dir)
            // Closed loopback port -> predictable "unreachable" metrics probe.
            .env("D2B_METRICS_URL", "http://127.0.0.1:1/metrics")
            .env("D2B_MANIFEST_PATH", &self.manifest_path);
        cmd
    }
