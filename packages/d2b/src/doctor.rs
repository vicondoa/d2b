...
fn probe_http_metrics(url: &str) -> Result<u16, String> {
    ...
    let req = format!(
        "GET {path} HTTP/1.1\r\nHost: {host}:{port}\r\nUser-Agent: d2b-host-doctor/0.4\r\nConnection: close\r\nAccept: text/plain\r\n\r\n",
        path = parsed.path,
        host = parsed.host,
        port = parsed.port,
    );
    ...
    let bytes = match std::fs::read(&path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            report.push(
                ...
                DoctorStatus::Warn,
                format!(
                    "daemon storage-lifecycle-report.json missing at {}; daemon may not have run the startup contract check; remediation: restart d2bd after rebuilding host configuration",
                    path.display()
                ),
            );
            return;
        ...
        Err(err) => {
            report.push(
                ...
                DoctorStatus::Warn,
                format!(
                    "read {}: {err}; remediation: restart d2bd to rewrite the storage lifecycle report",
                    path.display()
                ),
            );
            return;
    ...
    let parsed = match serde_json::from_slice::<StorageLifecycleReport>(&bytes) {
        ...
        Err(err) => {
            report.push(
                ...
                DoctorStatus::Warn,
                format!(
                    "parse {}: {err}; remediation: restart d2bd to rewrite the storage lifecycle report",
                    path.display()
                ),
            );
            return;
...
/// Return `true` for interface names matching the d2b bridge
/// naming convention: `br-<env>-lan` or `br-<env>-up`.
fn is_d2b_bridge_name(name: &str) -> bool {
...
fn check_bridge_ipv6_sysctl(daemon_state_dir: &Path, report: &mut DoctorReport) {
    ...
    if bridges.is_empty() {
        report.push(
            "bridge-ipv6-sysctl",
            DoctorStatus::Warn,
            "no d2b bridges found (envs.json absent and no br-*-{lan,up} interfaces visible); IPv6 sysctl not verifiable",
        );
        return;
    }
...
mod tests {
    ...
    fn pidfd_loose_parser_extracts_known_runners() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("d2b-doctor-pidfd-{}.json", std::process::id()));
    ...
    #[test]
    fn d2b_bridge_name_matcher() {
        assert!(is_d2b_bridge_name("br-corp-lan"));
        assert!(is_d2b_bridge_name("br-work-up"));
        assert!(!is_d2b_bridge_name("eth0"));
        assert!(!is_d2b_bridge_name("br-corp-mgmt"));
        assert!(!is_d2b_bridge_name("virbr0"));
    }
