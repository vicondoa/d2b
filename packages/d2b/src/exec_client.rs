...
pub const EXEC_CLI_CHUNK_BYTES: u64 = d2b_contracts::public_wire::EXEC_MAX_CHUNK_BYTES;
...
pub fn expect_detached_create(
    resp: ExecOpResponse,
) -> Result<d2b_contracts::public_wire::ExecDetachedCreateResult, ExecClientError> {
    match resp {
...
pub fn expect_detached_list(
    resp: ExecOpResponse,
) -> Result<d2b_contracts::public_wire::ExecDetachedListResult, ExecClientError> {
    match resp {
...
pub fn expect_detached_logs(
    resp: ExecOpResponse,
) -> Result<d2b_contracts::public_wire::ExecDetachedLogsResult, ExecClientError> {
    match resp {
...
pub fn expect_detached_status(
    resp: ExecOpResponse,
) -> Result<d2b_contracts::public_wire::ExecDetachedStatusResult, ExecClientError> {
    match resp {
...
pub fn expect_detached_kill(
    resp: ExecOpResponse,
) -> Result<d2b_contracts::public_wire::ExecDetachedKillResult, ExecClientError> {
    match resp {
...
mod tests {
    use super::*;
    use d2b_contracts::public_wire::ExecControlResult;
    use std::collections::VecDeque;
            ...
            match op {
                ...
                ExecOp::Close(_) => {
                    ...
                    Ok(ExecOpResponse::Close(
                        d2b_contracts::public_wire::ExecCloseResult { stdin_closed: true },
                    ))
                }
    ...
    fn malformed_output_error_never_echoes_the_guest_bytes() {
        const SENTINEL: &str = "D2B_SECRET_LEAK_CANARY";
        let mut transport = FakeTransport::terminal(ExecTerminalStatus::Exited { code: 0 });
