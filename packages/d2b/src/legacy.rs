//! The retained legacy command tree and its compatibility helpers.
...
pub(super) const DEFAULT_GUEST_CONFIG_PATH: &str = "/var/lib/d2b-guest/guest-config.nix";
/// Exit code surfaced for every guest-control config-read failure on the CLI.
pub(super) const EXIT_GUEST_CONTROL_CONFIG: i32 = 70;
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(super) struct ReadGuestConfigResponseFrame {
    #[serde(rename = "type")]
    _type_name: String,
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(super) struct ShellResponseFrame {
    #[serde(rename = "type")]
    _type_name: String,
    #[serde(flatten)]
...
pub(super) fn shell_gateway_attach_failure(raw: &str, json: bool) -> CliFailure {
...
pub(super) fn shell_action_word(action: ShellAction) -> &'static str {
    match action {
...
pub(super) fn gateway_shell_argv(
    args: &ShellArgs,
    action: ShellAction,
...
pub(super) fn cmd_gateway_shell(
...
pub(super) fn cmd_shell(context: &LegacyContext, args: &ShellArgs) -> Result<i32, CliFailure> {
...
pub(super) fn shell_terminal_size(
    (rows, cols): (u32, u32),
...
pub(super) struct ShellOwnerTransport {
...
pub(super) fn shell_owner_transport(
    context: &LegacyContext,
) -> Result<ShellOwnerTransport, CliFailure> {
...
pub(super) fn encode_shell_op_frame(op: &ShellOp, op_id: u64) -> Result<Vec<u8>, CliFailure> {
...
pub(super) fn close_shell_attach<T>(transport: &mut T, session: &str) -> Result<(), CliFailure>
where
...
pub(super) fn is_close_attach_transport_unavailable(err: &CliFailure) -> bool {
    err.exit_code == 69
        && (err
            .message
...
pub(super) fn run_shell_fsm<T, H, S>(
    transport: &mut T,
    host: &mut H,
    signals: &mut S,
    session: &str,
...
pub(super) fn shell_round_trip(
    context: &LegacyContext,
    op: ShellOp,
) -> Result<ShellOpResponse, CliFailure> {
...
pub(super) fn parse_shell_reply(bytes: &[u8]) -> Result<ShellOpResponse, CliFailure> {
...
pub(super) fn shell_name(value: &str) -> Result<IpcShellName, CliFailure> {
...
pub(super) fn shell_name_option(value: Option<&str>) -> Result<Option<IpcShellName>, CliFailure> {
    value.map(shell_name).transpose()
}

pub(super) fn shell_state_str(state: ShellSessionState) -> &'static str {
    match state {
...
pub(super) fn shell_close_cause_str(cause: public_wire::ShellCloseCause) -> &'static str {
    match cause {
...
pub(super) fn shell_attach_intro(vm: &str, attach: &public_wire::ShellAttachResult) -> String {
...
pub(super) fn shell_trailing_command_hint(raw_args: &[OsString]) -> Option<&'static str> {
...
#[derive(Debug, Args)]
pub(super) struct ConfigSyncArgs {
...
#[derive(Debug, Args)]
pub(super) struct ConfigDiffArgs {
    /// VM name (must match the static manifest).
    pub(crate) vm: String,
...
#[derive(Debug, Args)]
pub(super) struct ConfigApproveArgs {
    /// VM name (must match the static manifest).
    pub(crate) vm: String,
...
#[derive(Debug, Args)]
pub(super) struct ConfigRejectArgs {
    /// VM name (must match the static manifest).
    pub(crate) vm: String,
    /// Emit a JSON envelope.
    #[arg(long)]
...
#[derive(Debug, Args)]
pub(super) struct ConfigStatusArgs {
...
pub(super) fn config_validate_remote_path(p: &str) -> Result<(), CliFailure> {
...
pub(super) fn vm_uses_guest_control(context: &LegacyContext, vm: &str) -> Result<bool, CliFailure> {
...
pub(super) fn guest_control_config_failure(
    kind: &str,
...
pub(super) fn guest_control_config_failure_from_daemon(
    error: DaemonErrorEnvelope,
    is_json: bool,
) -> CliFailure {
...
pub(super) fn reject_ssh_only_flags_on_guest_control(
    args: &ConfigSyncArgs,
) -> Result<(), CliFailure> {
...
pub(super) enum GuestConfigReadOutcome {
...
pub(super) fn read_guest_config_via_socket(
    context: &LegacyContext,
    vm: &str,
...
#[derive(Debug)]
pub(super) struct ConfigSyncStaged {
    bytes: usize,
    sha256: String,
}
...
/// into the error.
pub(super) fn finish_config_sync_from_reply(
    reply: &[u8],
    staging: &Path,
    is_json: bool,
...
pub(super) fn cmd_config_diff(args: &ConfigDiffArgs) -> Result<i32, CliFailure> {
    config_validate_vm_name(&args.vm)?;
    let staging = config_staging_path(&args.vm);
    if !staging.exists() {
        return Err(CliFailure::new(
...
pub(super) fn cmd_config_approve(args: &ConfigApproveArgs) -> Result<i32, CliFailure> {
    config_validate_vm_name(&args.vm)?;
    let staging = config_staging_path(&args.vm);
...
pub(super) fn cmd_config_reject(args: &ConfigRejectArgs) -> Result<i32, CliFailure> {
    config_validate_vm_name(&args.vm)?;
    let staging = config_staging_path(&args.vm);
...
pub(super) fn cmd_config_status(args: &ConfigStatusArgs) -> Result<i32, CliFailure> {
    let base = config_staging_base();
...
pub(super) fn local_vm_shell_target(workload: &public_wire::WorkloadPublicSummary) -> &str {
...
pub(super) fn shell_launch_target(
...
pub(super) fn require_unsafe_local_shell_feature(
    capabilities: &[d2b_contracts::FeatureFlag],
) -> Result<(), CliFailure> {
    if capabilities
        .iter()
...
pub(super) fn try_resolve_direct_shell_target(
    context: &LegacyContext,
    requested: &str,
