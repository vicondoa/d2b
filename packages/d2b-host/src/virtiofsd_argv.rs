...
/// as the microvm.nix default; d2b keeps the same default.
...
pub struct VirtiofsdArgvInput {
    ...
    /// `d2b-virtiofs` group per the ADR 0003 minijail split.
...
mod tests {
    ...
    fn all_four_audit_shares_render_independently() {
        ...
        // virtiofsd programs for `corp-vm`: ro-store, d2b-meta,
        // d2b-hkeys, d2b-ssh-host. Each should render an independent
        ...
        let shares = [
            ...
            (
                "d2b-meta",
                "corp-vm-virtiofs-d2b-meta.sock",
                "/var/lib/d2b/vms/corp-vm/store-meta",
                false,
            ),
            (
                "d2b-hkeys",
                "corp-vm-virtiofs-d2b-hkeys.sock",
                "/var/lib/d2b/vms/corp-vm/host-keys",
                false,
            ),
            (
                "d2b-ssh-host",
                "corp-vm-virtiofs-d2b-ssh-host.sock",
                "/var/lib/d2b/vms/corp-vm/sshd-host-keys",
                false,
            ),
        ];
