    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidPrefix { prefix } => {
                write!(f, "invalid d2b ifname prefix: {prefix:?}")
            }
...
/// Constructs an [`IfName`] enforcing IFNAMSIZ-1 + d2b alphabet,
...
/// Validates a d2b ifname prefix (`<=8` bytes,
...
///
/// Guaranteed `<= 15` bytes for the default prefix (`d2b-` + `b` +
...
/// Returns whether `name` matches the d2b-derived shape (prefix +
/// role tag + base32 suffix). Used by host-LAN-CIDR derivation to
/// skip d2b-owned links.
...
mod tests {
    ...
    fn derives_within_ifnamsiz_for_default_prefix() {
        ...
        assert!(n.as_str().starts_with("d2b-b"), "{}", n.as_str());
    }
    ...
    #[test]
    fn looks_d2b_owned_recognises_default_prefix() {
        let n = derive_from_env_vm("e", Some("v"), DerivedRole::Bridge, None).unwrap();
        assert!(looks_d2b_owned(n.as_str(), DEFAULT_PREFIX));
        assert!(!looks_d2b_owned("br-foreign-0", DEFAULT_PREFIX));
        assert!(!looks_d2b_owned("lo", DEFAULT_PREFIX));
    }
    ...
    fn invalid_prefix_rejected() {
        ...
        assert!(validate_prefix("d2b-").is_ok());
    }
    ...
    fn detect_collisions_flags_duplicate_bridge() {
        let bridge = new_validated("d2b-bAAAAAAAA").unwrap();
        let mappings = vec![
            IfNameMapping {
                env: "e1".into(),
                vm: None,
        ...
        match err {
            IfNameError::IfNameCollision(detail) => {
                assert_eq!(detail.ifname, "d2b-bAAAAAAAA");
    ...
    fn detect_collisions_flags_bridge_vs_tap() {
        let n = new_validated("d2b-bXXXXX").unwrap();
        let mappings = vec![
            IfNameMapping {
                env: "e".into(),
                vm: Some("v1".into()),
            ...
            IfNameMapping {
                env: "e".into(),
                vm: Some("v2".into()),
                bridge: new_validated("d2b-bYYYYY").unwrap(),
                tap: Some(n),
                role: None,
            },
        ];
    ...
    fn nix_emitted_ifnames_pass_looks_d2b_owned() {
        ...
        eprintln!(
            "nix_emitted_ifnames_pass_looks_d2b_owned: {} derivedIfname values accepted",
            mappings.len()
        );
    }
}
