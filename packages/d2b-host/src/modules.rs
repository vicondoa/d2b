...
use d2b_core::host_w3::{KernelModuleEntry, ModuleRequirementW3};
    ...
    let bridge_nf_recommendations = if br_netfilter_present {
        vec![
            BridgeNfRecommendation {
                ...
                rationale: "br_netfilter loaded; pin to 0 so iptables cannot route around inet d2b unless ADR opts in".to_owned(),
            },
            BridgeNfRecommendation {
                key: "net.bridge.bridge-nf-call-ip6tables".to_owned(),
                recommended: "0".to_owned(),
                rationale: "br_netfilter loaded; pin to 0 so ip6tables cannot route around inet d2b unless ADR opts in".to_owned(),
            },
        ]
    } else {
        Vec::new()
