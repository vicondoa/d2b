...
//!
//! - the deterministic `inet d2b` chain layout
...
//! - drift detection via canonical hashing of the `inet d2b` table
//!   JSON;
...
//! NOT ship libnftd2b-dev. The fallback approach: this module produces
...
//! `nft list table inet d2b -j`. The fake backend behind
...
pub enum NftError {
    ...
    /// priority that would shadow the d2b decision.
    ForeignNftRuleShadowsD2b { details: String },
    ...
    /// Attempted to flush a foreign nft table/chain. D2b NEVER
    /// flushes foreign rules; this error is fail-closed.
    NftForeignRuleFlushAttempted { target: String },
    /// The post-apply hash of `inet d2b` does not match the
    /// pre-apply hash recorded in host.json.
    InetD2bDrift { before: Sha256, after: Sha256 },
}
...
impl NftError {
    ...
    pub const fn as_kebab_case(&self) -> &'static str {
        match self {
            ...
            Self::InetD2bDrift { .. } => "inet-d2b-drift",
        }
    }
...
/// Parser error for the limited `nft -f -` script dialect d2b emits
/// for the managed `inet d2b` table.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// match the `inet d2b` chain layout:
///
/// | Chain        | Priority |
/// | ------------ | -------- |
...
/// Chain hook kinds permitted under `inet d2b`. The variants
...
/// A single chain in the `inet d2b` table.
...
/// A batch of declarative nft state for the `inet d2b` table.
///
...
impl NftBatch {
    ...
    /// drift / preservation gate uses it to distinguish d2b-managed
    /// from foreign state.
    pub const COMMENT_PREFIX: &'static str = "d2b managed: ";
    ...
    pub fn parse(script: &str) -> Result<Self, ParseNftScriptError> {
        ...
        for (index, raw_line) in script.lines().enumerate() {
            ...
            if !saw_table {
                ...
                if !family.eq_ignore_ascii_case("inet") || table != "d2b" {
                    return Err(ParseNftScriptError::new(format!(
                        "line {line_no}: only `table inet d2b` is supported, got `table {family} {table}`"
                    )));
                }
                saw_table = true;
        ...
        if !saw_table {
            return Err(ParseNftScriptError::new("missing `table inet d2b` header"));
        }
        ...
        if !saw_table_end {
            return Err(ParseNftScriptError::new(
                "unterminated `table inet d2b` block",
            ));
        }
    ...
    pub fn add_usbip_carveout_expr(
        ...
        let chain = self
            ...
            .iter_mut()
            .find(|c| c.hook == hook)
            .ok_or_else(|| NftError::ForeignNftRuleShadowsD2b {
                details: format!("{} chain missing from inet d2b batch", hook.as_str()),
            })?;
...
/// Build the canonical `inet d2b` chain layout. Used by the broker
...
///
/// This split keeps `d2b-host` `#![forbid(unsafe_code)]` and
...
/// Hash the live `inet d2b` table for drift detection. Real
/// callers invoke `nft list table inet d2b -j` and pipe the output
...
pub fn hash_inet_d2b_table(nft_list_json: &[u8]) -> Sha256 {
...
/// fresh `inet d2b` batch. Ordering invariant (specific BEFORE
...
/// Assert that none of the chains under `inet d2b` accidentally
...
mod tests {
    ...
    #[test]
    fn build_inet_d2b_chains_layout_matches_plan() {
    ...
    fn comment_marker_prefix_on_every_managed_rule() {
        ...
        for rule in &forward.rules {
            assert!(
                ...
                "rule comment '{c}' missing d2b-managed marker",
                c = rule.comment
            );
        }
    }
    ...
    fn drift_detection_strips_volatile_fields() {
        ...
        assert_eq!(hash_inet_d2b_table(a), hash_inet_d2b_table(b));
    }
    ...
    fn drift_detection_catches_real_change() {
        ...
        assert_ne!(hash_inet_d2b_table(a), hash_inet_d2b_table(b));
    }
    ...
    /// rules MUST NOT appear in the rendered d2b batch (we never
    ...
    fn fake_backend_refuses_carveout_after_generic() {
        ...
        assert_eq!(err.as_kebab_case(), "foreign-nft-rule-shadows-d2b");
    }
    ...
    fn idempotency_hash_table_stable() {
        let nft_json = br#"{
            "nftables": [
                { "table": { "family": "inet", "name": "d2b" } },
                { "chain": { "family": "inet", "table": "d2b", "name": "forward", "type": "filter", "hook": "forward", "prio": -150, "policy": "drop" } },
                { "rule": { "family": "inet", "table": "d2b", "chain": "forward", "expr": [ { "match": {} } ] } }
            ]
        }"#;
        let h1 = hash_inet_d2b_table(nft_json);
        let h2 = hash_inet_d2b_table(nft_json);
    ...
    /// fields in the `nft list table inet d2b -j` output are
    ...
    fn idempotency_hash_volatile_stripped() {
        ...
        let with_volatile = br#"{
            "nftables": [
                { "table": { "family": "inet", "name": "d2b", "handle": 17 } },
                { "chain": { "family": "inet", "table": "d2b", "name": "forward", "type": "filter", "hook": "forward", "prio": -150, "policy": "drop", "handle": 3, "index": 7 } }
            ]
        }"#;
        let h_clean = hash_inet_d2b_table(without_volatile);
        let h_volatile = hash_inet_d2b_table(with_volatile);
        assert_eq!(
            h_clean, h_volatile,
