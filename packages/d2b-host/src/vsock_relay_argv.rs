...
//! sidecars used by `d2b.observability.*`. The observability work
...
pub struct VsockRelayArgvInput {
    ...
    /// uses `d2b-otel-relay@<vm>` for guest→host relays and
    /// `d2b-otel-vsock-in` for the obs stack VM listener.
...
/// observability harness path namespace (`/run/d2b/...`,
/// `/var/lib/d2b/...`) only ever contains ASCII letters / digits /
/// `_-./`; refusing
...
mod tests {
    ...
    /// `socat -d -d VSOCK-LISTEN:14317,fork,max-children=16,reuseaddr UNIX-CONNECT:/run/d2b/obs-ingress.sock`
    ...
    fn audit_guest_egress() -> VsockRelayArgvInput {
        VsockRelayArgvInput {
            socat_binary_path: "/nix/store/SOCATSOCATSOCATSOCATSOCAT-socat/bin/socat".to_owned(),
            relay_name: "d2b-otel-egress@corp-vm".to_owned(),
            source: SocatEndpoint::UnixListen {
                path: "/run/d2b/otlp.sock".to_owned(),
                max_children: Some(16),
                mode: 0o660,
            },
    ...
    fn stack_vsock_in_parity() {
        ...
        assert_eq!(argv[4], "UNIX-CONNECT:/run/d2b/obs-ingress.sock");
    }
    ...
    fn guest_egress_parity() {
        ...
        assert_eq!(
            argv[3],
            "UNIX-LISTEN:/run/d2b/otlp.sock,fork,reuseaddr,max-children=16,mode=0660"
        );
    ...
    fn unix_listen_to_unix_connect_shape_is_supported() {
        let input = VsockRelayArgvInput {
            socat_binary_path: "/nix/store/SOCATSOCATSOCATSOCATSOCAT-socat/bin/socat".to_owned(),
            relay_name: "d2b-host-bridge".to_owned(),
            source: SocatEndpoint::UnixListen {
                path: "/run/d2b/host-egress.sock".to_owned(),
                max_children: None,
                mode: 0o660,
            },
            sink: SocatEndpoint::UnixConnect {
                path: "/run/d2b/forward.sock".to_owned(),
            },
            extra_args: Vec::new(),
        };
        let argv = generate_vsock_relay_argv(&input).unwrap();
    ...
    fn rejects_path_with_brackets() {
        let mut input = audit_stack_vsock_in();
        input.sink = SocatEndpoint::UnixConnect {
            path: "/run/d2b/[evil].sock".to_owned(),
        };
        assert!(matches!(
            generate_vsock_relay_argv(&input),
    ...
    fn rejects_path_with_nul() {
        let mut input = audit_stack_vsock_in();
        input.sink = SocatEndpoint::UnixConnect {
            path: "/run/d2b/\0evil.sock".to_owned(),
        };
        assert!(matches!(
            generate_vsock_relay_argv(&input),
            Err(VsockRelayArgvError::PathContainsSocatMetachar { .. })
