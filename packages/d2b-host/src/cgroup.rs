...
//!    code path as the dropped-privilege `d2bd` view.
//!
//! ## Backend model
//!
...
/// Identifier for the delegated d2b slice path.
pub fn d2b_slice_path() -> PathBuf {
    Path::new(UNIFIED_HIERARCHY_ROOT).join(D2B_SLICE_NAME)
}
...
pub fn require_non_root_delegation<B: CgroupBackend>(backend: &B) -> Result<(), CgroupError> {
    if backend.current_uid() == 0 {
        return Err(CgroupError::CgroupDelegationRefused {
            detail: "delegation must be driven through the non-root d2bd; refuse uid 0".to_owned(),
        });
    }
    Ok(())
}
...
/// Step 6: chown the entire delegated subtree to `d2bd`. Performs
/// fd-based `fchown` (via the backend); never path-based per filesystem
/// path-safety rules.
pub fn chown_subtree_to_d2bd<B: CgroupBackend>(
    backend: &B,
    path: &Path,
    uid: u32,
    gid: u32,
...
mod tests {
    ...
    fn refuses_when_unified_hierarchy_missing() {
        let backend = FakeCgroupBackend::new(d2bd_uid());
    ...
    fn rejects_partition_root_writes() {
        let path = Path::new("/sys/fs/cgroup/d2b.slice");
