...
impl std::fmt::Display for RoutePreflightError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Ipv6AddressPresentOnD2bLink { ifname } => write!(
                f,
                "route-preflight: IPv6 address present on d2b link {ifname:?}"
            ),
            ...
            Self::InetD2bTableDrift { expected, observed } => write!(
                f,
                "route-preflight: inet d2b table drift (expected={expected}, observed={observed})"
            ),
        }
    }
}
...
// -------------------------------------------------------------------
// Predicate 2: no IPv6 address on any d2b link.
// -------------------------------------------------------------------
...
pub fn check_no_ipv6_on_d2b_links(
    snapshot: &AddrSnapshot,
    prefix: Option<&str>,
) -> Result<(), RoutePreflightError> {
    let prefix = prefix.unwrap_or(DEFAULT_PREFIX);
    for row in &snapshot.rows {
        if row.family == AddrFamily::V6 && looks_d2b_owned(&row.ifname, prefix) {
            return Err(RoutePreflightError::Ipv6AddressPresentOnD2bLink {
...
pub fn check_firewall_coexistence(
    ...
    // `inet d2b` table integrity: hash the live JSON dump and
    // compare against the bundle-recorded digest.
    let observed = hash_inet_d2b_table(nft_list_json).to_string();
    if observed != expected_table_hash {
        return Err(RoutePreflightError::InetD2bTableDrift {
...
pub struct RoutePreflightInput<'a> {
    ...
    /// Bundle-recorded hash of `nft list table inet d2b -j` from
    /// the last successful apply.
    pub expected_inet_d2b_hash: &'a str,
    /// Live `nft list table inet d2b -j` output.
...
pub fn run_route_preflight(input: &RoutePreflightInput<'_>) -> Result<(), RoutePreflightError> {
    ...
    check_no_ipv6_on_d2b_links(input.addrs, None)?;
...
pub struct HostLanCidrs {
    ...
    /// must override via `d2b.site.hostLanCidrs`.
...
pub fn detect_host_lan_cidrs(snapshot: &RouteTableSnapshot, prefix: Option<&str>) -> HostLanCidrs {
    ...
    for r in &snapshot.routes {
        ...
        if looks_d2b_owned(&r.device, prefix) {
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn ipv6_address_on_d2b_link_fails_closed() {
        let d2b_link = lan_if();
        let snap = AddrSnapshot {
            rows: vec![AddrRow {
                ifname: d2b_link.as_str().to_owned(),
                family: AddrFamily::V6,
                address: "fe80::1".into(),
            }],
        };
        let err = check_no_ipv6_on_d2b_links(&snap, None).unwrap_err();
        assert!(matches!(
            err,
            RoutePreflightError::Ipv6AddressPresentOnD2bLink { .. }
        ));
    }
    ...
    fn ipv6_address_on_foreign_link_ignored() {
        ...
        check_no_ipv6_on_d2b_links(&snap, None).unwrap();
    }
    ...
    #[test]
    fn host_lan_cidrs_skip_d2b_and_known_foreign() {
        let d2b_link = lan_if();
        let snap = RouteTableSnapshot {
            routes: vec![
                RouteRow {
                ...
                RouteRow {
                    ...
                    via: None,
                    device: d2b_link.as_str().to_owned(),
                    scope: RouteScope::Link,
                    family: AddrFamily::V4,
                    up: true,
                    point_to_point: false,
    ...
    fn empty_inet_d2b_json() -> Vec<u8> {
        // Canonical empty `nft list table inet d2b -j` output.
        br#"{"nftables":[]}"#.to_vec()
    }
    ...
    fn firewall_coexistence_passes_when_detector_matches_policy_and_hash() {
        ...
        let json = empty_inet_d2b_json();
        let expected = hash_inet_d2b_table(&json).to_string();
    ...
    fn firewall_coexistence_fails_on_table_hash_drift() {
        ...
        let err = check_firewall_coexistence(&declared, FirewallManager::None, "deadbeef", &json)
            .unwrap_err();
        assert!(matches!(err, RoutePreflightError::InetD2bTableDrift { .. }));
    }
    ...
    fn run_route_preflight_for_vm_threads_through_firewall_check() {
        ...
        let json = empty_inet_d2b_json();
        let expected_hash = hash_inet_d2b_table(&json).to_string();
        let input = RoutePreflightInput {
            ...
            expected_inet_d2b_hash: &expected_hash,
            nft_list_json: &json,
        };
