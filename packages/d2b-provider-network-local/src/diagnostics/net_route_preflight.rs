//! Daemon-side replacement for the `d2b-net-route-preflight.service`
//! host singleton.
//!
...
//! `packages/d2b-provider-network-local/nix/network.nix`) that ran at boot, shelled out to
...
//! diagnostic check inside `d2bd` itself. Startup misses do not block
...
///
/// This function is called only for d2b-declared bridges
...
impl PreflightHistory {
    ...
    /// successful `d2b host reconcile --network --apply` so
...
mod tests {
    ...
    fn all_envs_pass_when_bridges_up() {
        let envs = vec![env("corp", "d2b-corp"), env("personal", "d2b-personal")];
        let probe = FakeProbe::default()
            .with("d2b-corp", EnvPreflightStatus::Pass)
            .with("d2b-personal", EnvPreflightStatus::Pass);
    ...
    fn missing_bridge_marks_env_failed() {
        ...
        let probe = FakeProbe::default()
            .with("d2b-corp", EnvPreflightStatus::Pass)
            .with("d2b-personal", EnvPreflightStatus::BridgeMissing);
        let report = run_net_route_preflight_for_envs(&envs, &probe);
        assert!(!report.is_ok());
    ...
    fn down_bridge_is_failure() {
        let envs = vec![env("corp", "d2b-corp")];
