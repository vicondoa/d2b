...
fn typed_volume_and_agent_shapes_match_the_contract() {
    ...
    assert_eq!(
        ...
        [
            d2b_contracts_zone_session::v3::volume::ViewRight::Read,
            d2b_contracts_zone_session::v3::volume::ViewRight::Traverse,
        ]
    );
    let agent = guest_agent_process_spec("net-vm").unwrap();
    assert_eq!(
        agent.execution().process_class(),
        d2b_contracts_zone_session::v3::process::ProcessClass::Worker
    );
    assert_eq!(agent.execution().sandbox().namespace_classes(), []);
    assert_eq!(
        agent.execution().sandbox().capability_classes(),
        [
            d2b_contracts_zone_session::v3::process::CapabilityClass::NetworkAdmin,
            d2b_contracts_zone_session::v3::process::CapabilityClass::NetworkBind,
            d2b_contracts_zone_session::v3::process::CapabilityClass::NetworkRaw,
        ]
    );
    assert_eq!(agent.execution().mounts().len(), 1);
    assert_eq!(
        agent.execution().mounts()[0].access(),
        d2b_contracts_zone_session::v3::process::MountAccess::ReadOnly
    );
...
fn rendered_config_keeps_the_mandatory_host_blocklist() {
    ...
    for cidr in d2b_contracts_zone_session::v3::network::DEFAULT_HOST_BLOCKLIST {
