...
/// in `$1`.
const ARGV0_SENTINEL: &str = "d2b-exec";
...
/// e.g. `d2b-exec-<pid>-<counter>-<salt>.service`.
///
...
pub fn unique_exec_unit_name() -> String {
    ...
    format!("d2b-exec-{pid}-{counter}-{salt:016x}.service")
}
...
";
    ...
    fn pipe_mode_runs_command_through_login_shell_as_user() {
        ...
        assert_eq!(
            ...
            vec![
                "--uid=john",
                "--unit=d2b-exec-1-0-abcdef.service",
                "--quiet",
                "--collect",
    ...
    fn unique_exec_unit_name_is_unique_and_well_formed() {
        ...
        for name in [&a, &b] {
            assert!(name.starts_with("d2b-exec-"));
    ...
    fn systemctl_kill_args_spells_out_the_full_kill_argv() {
        let unit = "d2b-exec-1-0-abcdef.service";
    ...
    fn systemctl_kill_unit_retries_once_after_non_success() {
        ...
        for result in [Some(false), None] {
            ...
            let unit = "d2b-exec-2-0-abcdef.service";
    ...
    fn run_systemctl_kill_once_reports_none_when_spawn_fails() {
        ...
        assert_eq!(
            run_systemctl_kill_once(missing, "d2b-exec-4-0-abcdef.service"),
            None
        );
    }
}
