...
//!
//! Security/safety posture: d2b guest binaries are built fully static and
...
//! (`d2b-exec-runner --tty-exec`), which performs the setup in ordinary
...
pub mod linux {
    ...
    /// `d2b-exec-runner` binary (invoked in `--tty-exec` mode), the
