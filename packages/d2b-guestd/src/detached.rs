...
use d2b_exec_runner::paths::RUN_DIR;
...
pub struct RunnerUnitPaths {
    ...
    /// Base directory for slot dirs (production: `/run/d2b-exec`).
    pub run_dir: PathBuf,
}
...
pub trait TransientUnitManager: Send + Sync + 'static {
    /// Start `d2b-exec-<slot>.service`. Blocks (on the runtime) until the
    ...
    /// Enumerate the managed `d2b-exec-*` units currently known to systemd.
...
/// of managed `d2b-exec-<NN>.service` units. Columns are
...
/// Extract the slot/kind from `d2b-exec-<NN>.service` or
/// `d2b-exec-<NN>-w.service`; `None` if it does not match the bounded
/// slot-keyed names.
fn parse_slot_from_unit(unit: &str) -> Option<(u32, ManagedUnitKind)> {
    let rest = unit.strip_prefix("d2b-exec-")?;
...
/// Stable unit name for a slot: `d2b-exec-<NN>.service` (zero-padded). The
/// opaque exec id never appears in the unit name (journald cardinality bound).
pub fn unit_name(slot: u32) -> String {
    format!("d2b-exec-{slot:02}.service")
}

/// Stable workload unit name for a slot: `d2b-exec-<NN>-w.service`.
pub fn workload_unit_name(slot: u32) -> String {
    format!("d2b-exec-{slot:02}-w.service")
}
...
fn systemd_run_argv(slot: u32, ceiling_sec: u64, exec_runner_path: &Path) -> Vec<OsString> {
    let unit = format!("d2b-exec-{slot:02}");
    ...
    let mut argv: Vec<OsString> = vec![
        ...
        OsString::from("--slice=d2b-exec.slice"),
        OsString::from("-p"),
...
mod tests {
    ...
    fn unit_name_is_slot_keyed_and_id_free() {
        assert_eq!(unit_name(0), "d2b-exec-00.service");
        assert_eq!(unit_name(31), "d2b-exec-31.service");
    }

    #[test]
    fn systemd_run_argv_is_slot_keyed_and_id_free() {
        let runner = PathBuf::from("/nix/store/abc-d2b-exec-runner/bin/d2b-exec-runner");
        let argv = systemd_run_argv(7, 0, &runner);
        let joined: Vec<String> = argv
            .iter()
            .map(|a| a.to_string_lossy().into_owned())
            .collect();
        assert!(joined.contains(&"--unit=d2b-exec-07".to_string()));
        assert!(joined.contains(&"--slice=d2b-exec.slice".to_string()));
    ...
    fn systemd_run_argv_emits_runtime_max_sec_only_when_set() {
        let runner = PathBuf::from("/nix/store/abc/bin/d2b-exec-runner");
    ...
    fn parse_slot_from_unit_matches_only_bounded_names() {
        assert_eq!(
            parse_slot_from_unit("d2b-exec-00.service"),
            Some((0, ManagedUnitKind::Runner))
        );
        assert_eq!(
            parse_slot_from_unit("d2b-exec-31.service"),
            Some((31, ManagedUnitKind::Runner))
        );
        assert_eq!(
            parse_slot_from_unit("d2b-exec-07.service"),
            Some((7, ManagedUnitKind::Runner))
        );
        assert_eq!(
            parse_slot_from_unit("d2b-exec-07-w.service"),
            Some((7, ManagedUnitKind::Workload))
        );
        assert_eq!(parse_slot_from_unit("other.service"), None);
        assert_eq!(parse_slot_from_unit("d2b-exec-.service"), None);
        assert_eq!(parse_slot_from_unit("d2b-exec-xx.service"), None);
    }

    #[test]
    fn parse_managed_units_reads_active_column() {
        let text = "\
d2b-exec-00.service loaded active   running Detached exec 00
d2b-exec-00-w.service loaded active running Detached exec 00 workload
d2b-exec-01.service loaded inactive dead    Detached exec 01
d2b-exec-02.service loaded activating start Detached exec 02
...
";
    ...
    fn parse_managed_units_tolerates_leading_status_glyph() {
        let text = "● d2b-exec-05.service loaded failed failed Detached exec 05\n";
    ...
    fn parse_show_blocks_extracts_id_slice_and_exec_start() {
        let text = "\
Id=d2b-exec-07.service
Slice=d2b-exec.slice
ExecStart={ path=/nix/store/abc/bin/d2b-exec-runner ; argv[]=/nix/store/abc/bin/d2b-exec-runner --serve-exec --slot 07 ; ignore_errors=no }
BindsTo=d2b-exec-07.service
PartOf=d2b-exec-07.service
After=d2b-exec-07.service

Id=d2b-exec-08.service
Slice=other.slice
ExecStart={ path=/bin/false ; argv[]=/bin/false ; ignore_errors=no }
";
        let blocks = parse_show_blocks(text);
        assert_eq!(blocks.len(), 2);
        assert_eq!(blocks[0].id, "d2b-exec-07.service");
        assert_eq!(blocks[0].slice.as_deref(), Some("d2b-exec.slice"));
        assert!(
            blocks[0]
                .exec_start
                .as_deref()
        ...
        assert_eq!(blocks[0].binds_to.as_deref(), Some("d2b-exec-07.service"));
        assert_eq!(blocks[0].part_of.as_deref(), Some("d2b-exec-07.service"));
        assert_eq!(blocks[0].after.as_deref(), Some("d2b-exec-07.service"));
    ...
    fn parse_show_blocks_keeps_only_first_exec_start_and_skips_empty() {
        let text = "\
Id=d2b-exec-00.service
Slice=
ExecStart=
";
        let blocks = parse_show_blocks(text);
        assert_eq!(blocks.len(), 1);
        assert_eq!(blocks[0].id, "d2b-exec-00.service");
    ...
    fn parse_show_blocks_flags_multiline_continuation_as_malformed() {
        ...
        let text = "\
Id=d2b-exec-03-w.service
Slice=d2b-exec.slice
ExecStart={ path=/run/current-system/sw/bin/bash ; argv[]=/run/current-system/sw/bin/bash -l -c exec \"$@\" d2b-exec sh -c echo a ; ignore_errors=evil }
echo b ; ignore_errors=no ; start_time=[n/a] }
BindsTo=d2b-exec-03.service
PartOf=d2b-exec-03.service
After=d2b-exec-03.service
";
        let blocks = parse_show_blocks(text);
        assert_eq!(blocks.len(), 1);
        assert!(
...
Id=d2b-exec-03-w.service
Slice=d2b-exec.slice
ExecStart={ path=/bin/bash ; argv[]=/bin/bash -l -c exec \"$@\" d2b-exec id ; ignore_errors=no }
BindsTo=d2b-exec-03.service
";
    ...
    fn parse_exec_start_extracts_exe_and_argv_tokens() {
        let parsed = parse_exec_start(
            "{ path=/nix/store/abc/bin/d2b-exec-runner ; \
             argv[]=/nix/store/abc/bin/d2b-exec-runner --serve-exec --slot 07 ; \
             ignore_errors=no }",
        )
        .expect("structured ExecStart parses");
        assert_eq!(parsed.exe, "/nix/store/abc/bin/d2b-exec-runner");
        assert_eq!(
            parsed.argv,
            vec![
                "/nix/store/abc/bin/d2b-exec-runner".to_owned(),
                "--serve-exec".to_owned(),
                "--slot".to_owned(),
                "07".to_owned(),
            ]
    ...
    fn parse_exec_start_preserves_quoted_argv_token_boundaries() {
        let parsed = parse_exec_start(
            r#"{ path=/run/current-system/sw/bin/bash ; argv[]=/run/current-system/sw/bin/bash -l -c "exec \"$@\"" d2b-exec "/bin/echo" "hello world" ; ignore_errors=no }"#,
        )
    ...
    fn debug_impls_redact_exec_start_argv() {
        ...
        let identity = UnitIdentity::Queried {
            ...
            binds_to: Some("d2b-exec-03.service".to_owned()),
            part_of: Some("d2b-exec-03.service".to_owned()),
            after: Some("d2b-exec-03.service".to_owned()),
        };
        ...
        let show = ShowEntry {
            id: "d2b-exec-03-w.service".to_owned(),
            slice: Some("d2b-exec.slice".to_owned()),
            exec_start: Some(format!(
                "{{ path=/bin/bash ; argv[]=/bin/bash -lc {secret} ; ignore_errors=no }}"
            )),
