...
fn run(args: Vec<OsString>) -> Result<(), d2b_guestd::service::GuestdServiceError> {
    match args.first().and_then(|arg| arg.to_str()) {
        ...
        Some("--serve") => {
            ...
            let token = d2b_guestd::service::load_token_from_credentials_env()?;
            let mut config = d2b_guestd::service::GuestdServeConfig::with_exec_policy(
                parsed.vm_id,
                token,
                parsed.exec_policy,
            )?;
            ...
            let runtime = tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .build()
                .map_err(|_| d2b_guestd::service::GuestdServiceError::Ttrpc)?;
            runtime.block_on(d2b_guestd::service::serve_vsock(config))
        }
        _ => Err(d2b_guestd::service::GuestdServiceError::Ttrpc),
    }
}
...
fn parse_serve_args(
    args: &[OsString],
) -> Result<ServeArgs, d2b_guestd::service::GuestdServiceError> {
    ...
    let mut activation_status_dir: PathBuf = PathBuf::from("/run/d2b-guestd/activations");
        ...
        match arg.to_str() {
            ...
            Some("--exec-user") => {
                let user = iter
                    ...
                    .ok_or(d2b_guestd::service::GuestdServiceError::Ttrpc)?;
            ...
            _ => return Err(d2b_guestd::service::GuestdServiceError::Ttrpc),
        }
    }
    let vm_id = vm_id
        .filter(|value| {
    ...
    let shell_policy = if shell_enabled {
        if shell_max_attached > shell_max_sessions {
            return Err(d2b_guestd::service::GuestdServiceError::Ttrpc);
        }
        Some(ShellPolicy {
            enabled: true,
...
) -> Result<PathBuf, d2b_guestd::service::GuestdServiceError> {
    let path = value
        .map(PathBuf::from)
        .ok_or(d2b_guestd::service::GuestdServiceError::Ttrpc)?;
    if !path.is_absolute() {
