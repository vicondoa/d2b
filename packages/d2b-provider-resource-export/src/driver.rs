...
//!
//! `ResourceExport` declares that a resource is exported to another zone: the driver converges it as metadata once its desired state is admitted.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`ResourceExportDriverFactory`] registration through the type's
//!   descriptor ([`resource_export_descriptor`]).
...
pub const RESOURCE_EXPORT_TYPE_NAME: &str = "ResourceExport";

/// The resource verbs the `ResourceExport` type supports.
///
...
/// this declaration.
const RESOURCE_EXPORT_VERBS: &[&str] = &[
    "get",
    "list",
    "watch",
    "create",
...
/// The execution domains the `ResourceExport` type can be reconciled in.
///
/// Derived from the placement contract: `ResourceExport` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `ResourceExport` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const RESOURCE_EXPORT_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `ResourceExport` driver reads while reconciling.
///
...
const RESOURCE_EXPORT_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `ResourceExport` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn resource_export_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `ResourceExport` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct ResourceExportDriverFactory {
    types: [ResourceTypeName; 1],
}

impl ResourceExportDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(RESOURCE_EXPORT_TYPE_NAME)],
        }
    }
}

impl Default for ResourceExportDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for ResourceExportDriverFactory {
    fn resource_types(&self) -> &[ResourceTypeName] {
        &self.types
    }

    async fn create(&self, _key: &ResourceKey) -> Box<dyn DynResourceDriver> {
        Box::new(ResourceExportDriver)
    }
}
...
/// One `ResourceExport` resource's driver.
pub struct ResourceExportDriver;

#[async_trait]
impl ResourceDriver for ResourceExportDriver {
...
/// `*.d2bus.org.*Service` types, so a `ResourceExport` row is never an export subject.
pub fn resource_export_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        resource_type: WellKnownType::RESOURCE_EXPORT,
        allowed_sources: AllowedSources::BUILTIN,
        verbs: RESOURCE_EXPORT_VERBS,
        execution: RESOURCE_EXPORT_EXECUTION_DOMAINS,
        exportable: false,
        reads: RESOURCE_EXPORT_READS,
        operations: &[],
        creations: &[],
        startup: &[],
        services: &[],
        decoder: resource_export_spec_decoder(),
        factory: Arc::new(ResourceExportDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        RESOURCE_EXPORT_TYPE_NAME, ResourceExportDriverFactory, resource_export_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", RESOURCE_EXPORT_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = ResourceExportDriverFactory::new()
            .create(&ResourceKey::new("work", RESOURCE_EXPORT_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
