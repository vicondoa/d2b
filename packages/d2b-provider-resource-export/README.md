...
| Field | Value |
| --- | --- |
| Provider name | `resource-export` |
| ResourceType | `ResourceExport` |
| Package | `packages/d2b-provider-resource-export/` |
| Driver declaration | `resource_export_descriptor` -> `DriverDescriptor` |

## Config schema

The type declares no provider config schema: `ResourceExport` rows carry the JSON spec
object the core rows store, and nothing outside it decodes at validate.

## Exported resource types

`ResourceExport` is not exportable. `ResourceExport` admits only qualified
`*.d2bus.org.*Service` types, so a `ResourceExport` row is never an export subject. The
