...
//!
//! `Zone` is the zone's own row: the plane's metadata convergence for it, with the production Zone status projection (the mandatory system-core handler pair) in `zone_status`.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`ZoneDriverFactory`] registration through the type's
//!   descriptor ([`zone_descriptor`]).
...
pub const ZONE_TYPE_NAME: &str = "Zone";

/// The resource verbs the `Zone` type supports.
///
...
/// The execution domains the `Zone` type can be reconciled in.
///
/// Derived from the placement contract: `Zone` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `Zone` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const ZONE_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `Zone` driver reads while reconciling.
///
...
const ZONE_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `Zone` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn zone_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `Zone` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct ZoneDriverFactory {
    types: [ResourceTypeName; 1],
}

impl ZoneDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(ZONE_TYPE_NAME)],
        }
    }
}

impl Default for ZoneDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for ZoneDriverFactory {
...
/// One `Zone` resource's driver.
pub struct ZoneDriver;

#[async_trait]
impl ResourceDriver for ZoneDriver {
...
pub fn zone_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        ...
        startup: &[],
        services: &[],
        decoder: zone_spec_decoder(),
        factory: Arc::new(ZoneDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        ZONE_TYPE_NAME, ZoneDriverFactory, zone_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", ZONE_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = ZoneDriverFactory::new()
            .create(&ResourceKey::new("work", ZONE_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
