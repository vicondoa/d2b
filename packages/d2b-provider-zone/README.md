...
This is the crate root for the `Zone` resource type. It owns the type's identity
...
driver serves `Zone` rows through the `ResourceDriver` verbs: `validate` decodes the
...
The registration surface is `zone_descriptor`: it declares the type through the shared
...
`d2b-contracts-resource` and `d2b-contracts-zone-session` for the status
projection's vocabulary (`zone_status`), and on `tokio` for its tests.
