...
This is the crate root for the `Quota` resource type. It owns the type's identity
...
`Quota` is a scarce-resource scope claim. The driver converges it as metadata once its desired state is admitted; the Host-global authority index that arbitrates the claim's class (beside every other scarce-resource class the session admits) is controller-session machinery and stays in `d2b-core-controller`.
...
| Provider name | `quota` |
| ResourceType | `Quota` |
| Package | `packages/d2b-provider-quota/` |
| Driver declaration | `quota_descriptor` -> `DriverDescriptor` |

## Config schema

The type declares no provider config schema: `Quota` rows carry the JSON spec
object the core rows store, and nothing outside it decodes at validate.

## Exported resource types

`Quota` is not exportable. `ResourceExport` admits only qualified
`*.d2bus.org.*Service` types, so a `Quota` row is never an export subject. The
declaration carries `exportable: false`.

## Controllers / services / workers / binaries

The crate ships the type's declaration, not a standalone process. The shared
driver serves `Quota` rows through the `ResourceDriver` verbs: `validate` decodes the
...
The registration surface is `quota_descriptor`: it declares the type through the shared
...
`Quota` names no placement anchor, so a `Quota` row is reconciled on its
...
```bash
cargo test -p d2b-provider-quota
```
