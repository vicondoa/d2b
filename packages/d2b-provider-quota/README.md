...
This is the crate root for the `Quota` resource type. It owns the type's
...
serves `Quota` rows through the `ResourceDriver` verbs: `validate` decodes the
...
`QuotaDriverFactory` is the registration surface; `quota_descriptor` carries
