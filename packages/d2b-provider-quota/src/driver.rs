...
//! - `describe` -> [`QuotaDriverFactory`] registration through the type's
//!   descriptor ([`quota_descriptor`]).
...
pub const QUOTA_TYPE_NAME: &str = "Quota";

/// The resource verbs the `Quota` type supports.
///
...
/// The execution domains the `Quota` type can be reconciled in.
///
/// Derived from the placement contract: `Quota` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `Quota` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const QUOTA_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `Quota` driver reads while reconciling.
///
...
const QUOTA_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `Quota` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn quota_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `Quota` resource type. Construction is
...
impl QuotaDriverFactory {
    ...
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(QUOTA_TYPE_NAME)],
        }
    }
}

impl Default for QuotaDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn quota_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        ...
        factory: Arc::new(QuotaDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        QUOTA_TYPE_NAME, QuotaDriverFactory, quota_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", QUOTA_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = QuotaDriverFactory::new()
            .create(&ResourceKey::new("work", QUOTA_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
