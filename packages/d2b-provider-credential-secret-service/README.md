...
`ADR046-cred-ss-003`. Nothing recorded here is a design statement.

## Provider identity

Not yet declared. Filled by `ADR046-cred-ss-003`.
...
Not yet declared. Filled by `ADR046-cred-ss-003`. Until then the standing Provider rules
...
cd packages && cargo clippy -p d2b-provider-credential-secret-service --all-targets
```
