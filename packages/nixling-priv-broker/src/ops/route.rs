...
use nixling_core::bundle_resolver::ResolvedRouteIntent;
use nixling_core::host_w3::RouteIntent;
...
/// Owner-table snapshot: every nixling-owned route key currently
...
) -> Result<ApplyRouteDiff, nixling_host::routes::RoutePreflightError> {
    nixling_host::routes::run_route_preflight(preflight)?;
...
/// [`nixling_host::routes::run_route_preflight_for_vm`]. The broker
...
pub fn pre_vm_start(
    vm_id: &str,
    preflight: &nixling_host::routes::RoutePreflightInput<'_>,
) -> Result<(), nixling_host::routes::RoutePreflightError> {
    nixling_host::routes::run_route_preflight_for_vm(vm_id, preflight)
}
...
mod tests {
    ...
    fn diff_detects_add_remove_unchanged() {
        let prev = OwnerTable::from_intents(&[ri("10.0.0.0/24", Some("nl-bX"))]);
        let next = OwnerTable::from_intents(&[
            ri("10.0.0.0/24", Some("nl-bX")),
            ri("10.0.1.0/24", Some("nl-bY")),
        ]);
    ...
    fn route_key_is_deterministic() {
        let a = ri("10.0.0.0/24", Some("nl-bX"));
        let b = ri("10.0.0.0/24", Some("nl-bX"));
