...
//! (`nixling_host::devices::DeviceNodeEntry`) and, on success, returns
//! an `OwnedFd` that the runtime hands back to `nixlingd` via
    ...
    DeviceMatrixEntry {
        ...
        entry: DeviceNodeEntry {
            ...
            group_required: match class {
                DeviceClass::Kvm => "kvm",
                DeviceClass::NetTun => "nixlingd",
                DeviceClass::VhostNet => "kvm",
                DeviceClass::Fuse => "nixlingd",
...
mod tests {
    use super::*;
    use nixling_host::devices::DeviceNodeKind;
