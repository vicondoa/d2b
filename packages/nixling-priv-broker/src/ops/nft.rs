...
pub struct ApplyNftablesInputs {
    ...
    /// `nft list table inet nixling -j` of the live table before
...
pub fn apply_nftables(inputs: &ApplyNftablesInputs) -> Result<ApplyNftablesDecision, NftError> {
    ...
    let table_hash_before = hash_inet_nixling_table(&inputs.live_table_json);
...
/// Re-hash the live `inet nixling` table for drift detection. Run
...
pub fn rehash_for_drift(live_table_json: &[u8]) -> Sha256 {
    hash_inet_nixling_table(live_table_json)
}

pub const DEFAULT_HOST_RUNTIME_PATH: &str = "/var/lib/nixling/runtime/host-runtime.json";
...
pub fn persist_live_nft_hash(
    ...
    let live_hash = hash_inet_nixling_table(&live_json).to_string();
...
fn persisted_nft_hash_path() -> PathBuf {
    env::var_os("NIXLING_BROKER_NFT_HASH_PATH")
        .map(PathBuf::from)
...
/// have to import `nixling-host` directly.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DriftDetected { expected, observed } => write!(
                f,
                "apply-nftables: canonical inet nixling hash drift detected (expected={expected}, observed={observed})"
            ),
...
mod tests {
    ...
    fn usbip_script() -> (String, String) {
        ...
        batch
            .add_usbip_carveout(&nixling_host::nftables::BusId::new("1-1.2"))
            .expect("carveout");
        let script = batch.render_nft_script();
        let hash = batch.canonical_hash().to_string();
        (script, hash)
    ...
    fn apply_with_coexistence_refuses_on_carveout_misorder() {
        ...
        let script = concat!(
            ...
            "    drop comment \"nixling managed: broad-drop\"\n",
            "    meta iifname \"usbip-3-1\" accept comment \"nixling managed: owner-1\"\n",
            "  }\n",
            "}\n"
        );
        let err = apply_with_coexistence_inner(
        ...
        assert!(matches!(
            ...
            ApplyWithCoexistenceError::CarveoutOrderingViolation(
                NftError::ForeignNftRuleShadowsNixling { .. }
            )
        ));
        assert!(exec.take_log().is_empty());
    }
