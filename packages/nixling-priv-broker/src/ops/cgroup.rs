...
pub enum CgroupOpError {
    ...
    /// under the delegated nixling.slice (`path-class = foreign`).
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CgroupOpError::PathClassForeign { requested } => write!(
                f,
                "OpenCgroupDir: requested path outside nixling.slice: {}",
                requested.display()
            ),
...
impl PathClass {
    pub fn as_str(&self) -> &'static str {
        match self {
            PathClass::NixlingSlice => "nixling-slice",
    ...
    A: AuditSink,
{
    ...
    let mut fields = AuditFields {
        ...
        owner_uid: Some(context.nixlingd_uid),
        ..AuditFields::default()
    };
    ...
    let slice = create_nixling_slice(
        ...
        context.nixlingd_uid,
        context.nixlingd_gid,
    )
    .map_err(|err| {
        audit.record(
            "DelegateCgroupV2",
    ...
    Ok(DelegateCgroupV2Outcome {
        slice_path: slice,
        owner_uid: context.nixlingd_uid,
    ...
    A: AuditSink,
{
    ...
    // subject name (e.g. "nixling-slice" or a vm id). The broker
    // maps that to a canonical path under nixling.slice.
    let (canonical_path, class) =
        if requested_subject == NIXLING_SLICE_NAME || requested_subject == "nixling-slice" {
            (context.slice_path(), PathClass::NixlingSlice)
...
pub(crate) fn create_nixling_slice<B: CgroupBackend>(
    backend: &B,
    ...
    host_cgroup::chown_subtree_to_nixlingd(backend, parent_slice, nixlingd_uid, nixlingd_gid)?;
...
mod tests {
    ...
    fn cgroup_kill_ancestor_refused() {
        ...
        let err = handle_cgroup_kill(&b, &ctx, "nixling.slice", &audit).unwrap_err();
        match err {
