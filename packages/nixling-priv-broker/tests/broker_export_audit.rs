...
fn export_audit_requires_admin_and_exports_op_audit_records() {
    ...
    broker.probe_hello(broker.nixlingd_uid()).assert_success();
    broker
        .probe_stub(broker.nixlingd_uid(), "ApplyNftables")
        .assert_success();
    ...
    let unauthorized = broker.probe_export_audit(broker.nixlingd_uid(), "not-authorized");
    ...
    let export = broker.probe_export_audit(broker.nixlingd_uid(), "admin:9000");
    ...
    assert_eq!(apply_record["peer_uid"], broker.nixlingd_uid());
