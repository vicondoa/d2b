...
pub const NIXLINGD_UID: u32 = 4242;
...
impl TestBroker {
    pub fn spawn(prefix: &str) -> Self {
        ...
        let run_dir = scratch.path().join("run/nixling");
        let audit_dir = scratch.path().join("var/lib/nixling/audit");
        ...
        let child = Command::new(BROKER_BIN)
            ...
            .spawn()
            .expect("spawn nixling-priv-broker serve");
