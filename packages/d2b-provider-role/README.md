...
This is the crate root for the `Role` resource type. It owns the type's
...
serves `Role` rows through the `ResourceDriver` verbs: `validate` decodes the
...
`RoleDriverFactory` is the registration surface; `role_descriptor` carries
