...
//!
//! `Role` is an authority role: the driver converges it as metadata once its desired state is admitted, and the crate carries the revision-bound positive authorization decision cache (`rbac`).
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`RoleDriverFactory`] registration through the type's
...
pub const ROLE_TYPE_NAME: &str = "Role";

/// The resource verbs the `Role` type supports.
///
...
/// The execution domains the `Role` type can be reconciled in.
///
/// Derived from the placement contract: `Role` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `Role` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const ROLE_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `Role` driver reads while reconciling.
///
...
const ROLE_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `Role` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn role_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `Role` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct RoleDriverFactory {
    types: [ResourceTypeName; 1],
}

impl RoleDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(ROLE_TYPE_NAME)],
        }
    }
}

impl Default for RoleDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for RoleDriverFactory {
...
/// One `Role` resource's driver.
pub struct RoleDriver;

#[async_trait]
impl ResourceDriver for RoleDriver {
...
pub fn role_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        ...
        startup: &[],
        services: &[],
        decoder: role_spec_decoder(),
        factory: Arc::new(RoleDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        ROLE_TYPE_NAME, RoleDriverFactory, role_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", ROLE_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = RoleDriverFactory::new()
            .create(&ResourceKey::new("work", ROLE_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
