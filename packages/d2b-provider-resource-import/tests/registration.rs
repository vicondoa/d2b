//! ResourceImport family registration boundary: the driver declaration is what the plane
//! registers, and the registry serves this type's decoder and factory from it.

use d2b_provider_resource_import::RESOURCE_IMPORT_TYPE_NAME;
...
#[tokio::test]
async fn descriptor_declares_and_registers_the_resource_import_type() {
    let descriptor = descriptor();
    assert_eq!(descriptor.resource_type, WellKnownType::RESOURCE_IMPORT);
    assert_eq!(
        descriptor.resource_type.to_resource_type_name().as_str(),
        RESOURCE_IMPORT_TYPE_NAME
    );
    assert_eq!(
    ...
    assert!(!descriptor.exportable, "a ResourceImport row is never an export subject");
    assert_eq!(
        descriptor.execution,
        &["host"],
        "a ResourceImport row carries no placement anchor, so the plane reconciles it on its Host"
    );
    ...
    assert_eq!(
        ...
        vec![ResourceTypeName::new(RESOURCE_IMPORT_TYPE_NAME)]
    );
    assert!(
        providers
            .decoders()
            .contains_key(&ResourceTypeName::new(RESOURCE_IMPORT_TYPE_NAME)),
        "the registry serves the type's decoder from the declaration"
    );
    let factory = providers
        .lookup(&ResourceTypeName::new(RESOURCE_IMPORT_TYPE_NAME))
        .expect("the registry serves the declared factory");
    assert_eq!(factory.resource_types().len(), 1);
    assert_eq!(factory.resource_types()[0].as_str(), RESOURCE_IMPORT_TYPE_NAME);
    factory
...
async fn a_second_registration_of_the_type_is_refused() {
    ...
    assert!(matches!(
        ...
        ProviderDirectoryError::DuplicateType(type_name)
            if type_name.as_str() == RESOURCE_IMPORT_TYPE_NAME
    ));
}
