...
//!
//! `ResourceImport` declares that a remote resource is imported into this zone: the driver converges it as metadata once its desired state is admitted.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`ResourceImportDriverFactory`] registration through the type's
//!   descriptor ([`resource_import_descriptor`]).
...
pub const RESOURCE_IMPORT_TYPE_NAME: &str = "ResourceImport";

/// The resource verbs the `ResourceImport` type supports.
///
...
/// this declaration.
const RESOURCE_IMPORT_VERBS: &[&str] = &[
    "get",
    "list",
    "watch",
    "create",
...
/// The execution domains the `ResourceImport` type can be reconciled in.
///
/// Derived from the placement contract: `ResourceImport` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `ResourceImport` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const RESOURCE_IMPORT_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `ResourceImport` driver reads while reconciling.
///
...
const RESOURCE_IMPORT_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `ResourceImport` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn resource_import_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `ResourceImport` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct ResourceImportDriverFactory {
    types: [ResourceTypeName; 1],
}

impl ResourceImportDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(RESOURCE_IMPORT_TYPE_NAME)],
        }
    }
}

impl Default for ResourceImportDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for ResourceImportDriverFactory {
    fn resource_types(&self) -> &[ResourceTypeName] {
        &self.types
    }

    async fn create(&self, _key: &ResourceKey) -> Box<dyn DynResourceDriver> {
        Box::new(ResourceImportDriver)
    }
}
...
/// One `ResourceImport` resource's driver.
pub struct ResourceImportDriver;

#[async_trait]
impl ResourceDriver for ResourceImportDriver {
...
/// converted core types without it, so it must be registered before the
/// plane opens. The driver serves no broker operations and creates no
/// children through this declaration.
///
/// The type is not exportable: `ResourceExport` admits only qualified
/// `*.d2bus.org.*Service` types, so a `ResourceImport` row is never an export subject.
pub fn resource_import_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        resource_type: WellKnownType::RESOURCE_IMPORT,
        allowed_sources: AllowedSources::BUILTIN,
        verbs: RESOURCE_IMPORT_VERBS,
        execution: RESOURCE_IMPORT_EXECUTION_DOMAINS,
        exportable: false,
        reads: RESOURCE_IMPORT_READS,
        operations: &[],
        creations: &[],
        startup: &[],
        services: &[],
        decoder: resource_import_spec_decoder(),
        factory: Arc::new(ResourceImportDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        RESOURCE_IMPORT_TYPE_NAME, ResourceImportDriverFactory, resource_import_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", RESOURCE_IMPORT_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = ResourceImportDriverFactory::new()
            .create(&ResourceKey::new("work", RESOURCE_IMPORT_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
