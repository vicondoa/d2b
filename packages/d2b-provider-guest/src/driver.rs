//! The Guest family's driver: one `Guest` driver over the four runtime
//! Providers that realize it - cloud-hypervisor, qemu-media,
//! azure-container-apps, and azure-virtual-machine.
//!
//! The driver serves the `Guest` resource type through the plane's verbs:
//! - [`ResourceDriver::validate`]: the spec decodes and names a runtime
//!   Provider this family owns;
//! - [`ResourceDriver::recover`]: owned-child / provider adoption evidence;
...
//!   the framework controllers' per-kind finalize) runs until the Provider's
//!   internal finalizer gate clears, then the owned children retire.
//!
...
//!
//! Every runtime Provider identity, controller reference, and child Provider
//! reference comes from the realizer crate that owns it, so the family table
//! cannot drift from the Providers it serves. What each kind creates, and on
//! whose custody, is declared by [`guest_descriptor`]; the production effects
...
///
/// The Provider references are the realizer crates' own constants, so a
/// renamed or retired Provider identity cannot drift from the family table.
...
pub enum GuestKind {
    /// The Cloud Hypervisor provider (`runtime-cloud-hypervisor`).
    CloudHypervisor,
    /// The qemu-media provider (`runtime-qemu-media`).
    QemuMedia,
    /// The azure-container-apps provider (`runtime-azure-container-apps`).
    AzureContainerApps,
    /// The azure-virtual-machine provider (`runtime-azure-virtual-machine`).
    AzureVirtualMachine,
}
...
impl GuestKind {
    ...
    /// The family row of this kind.
    pub const fn registration(self) -> GuestRegistration {
        GUEST_REGISTRATIONS[self.index()]
    }

    /// This kind's position in the family table.
    ...
    /// The runtime-only effect id this kind's calls are named with.
    ...
    /// The Provider reference this kind selects its row by.
    ...
    pub const fn children_owned_by_controller(self) -> bool {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GuestEffectPhase {
    /// The Provider reported the guest converged.
    Ready,
    /// The Provider is still converging.
    Pending,
}
...
impl GuestEffectOutcome {
    /// An outcome carrying only the reported phase.
    pub const fn phase(phase: GuestEffectPhase) -> Self {
        Self {
            phase,
...
#[derive(Debug, Clone)]
/// Closed driver error: the failure class, the operation, and the structured detail.
pub struct GuestDriverError {
...
pub struct GuestSpecDecodeError;

/// The manager-wired decode hook for `Guest`.
pub fn guest_spec_decoder() -> Arc<dyn SpecDecoder> {
...
pub fn decode_metadata(raw: &[u8]) -> Result<Value, GuestEffectError> {
    if raw.is_empty() {
        return Ok(json!({}));
    }
...
#[async_trait]
pub trait GuestChildSurface: Send + Sync {
...
impl<'a> ContextChildSurface<'a> {
    /// Wrap the calling resource's own context.
...
/// stale `Ready`.
pub fn view_phase(view: &d2b_resource_runtime::manager::ResourceView) -> &'static str {
...
pub struct GuestEffectRequest<'a> {
...
/// state machines.
#[async_trait]
pub trait GuestDriverEffects: Send + Sync + 'static {
...
/// the `Host` whose runtime carries the guest (the Cloud Hypervisor and
/// qemu-media kinds) or the containing `Guest` for a nested provider scope
/// (the azure-container-apps kind), so both domains admit the type.
const GUEST_EXECUTION_DOMAINS: &[&str] = &["host", "guest"];

/// The resource types the Guest driver reads while reconciling.
///
/// Derived from the driver's row reads: the Provider row the spec selects
/// (its `/config` carries the framework kinds' settings), the owned children
/// the family ensures or observes (the runtime Volume, the VMM Process, the
/// control Endpoint), and the references one Guest spec declares - its
/// execution target, its owning Guest, its volumes and bindings, its device
/// and network attachments.
const GUEST_READS: &[WellKnownType] = &[
...
const GUEST_CREATIONS: &[ChildCreation] = &[
    ChildCreation {
...
/// The `Guest` type's driver declaration.
///
...
impl GuestDriverFactory {
    /// Build the factory for the one type the family serves.
...
impl ResourceDriver for GuestDriver {
    ...
    async fn recover(&mut self, ctx: &mut ResourceContext) -> Result<RecoveryOutcome, Self::Error> {
        ...
        if kind.children_owned_by_controller() {
            let vmm_ref = d2b_provider_guest_cloud_hypervisor::deterministic_child_ref(
                &key_ref(ctx.key()),
...
pub fn declared_dependency_refs(spec: &Value) -> Vec<ResourceRef> {
...
fn qemu_child_ensures(
    ...
    let config = serde_json::from_value::<d2b_provider_guest_qemu_media::ProviderConfig>(
        provider
            .pointer("/config")
            .cloned()
...
fn aca_child_ensures(
    ...
    let target = ResourceRef::parse(&format!("Endpoint/{}-sandbox-agent", owner.name().as_str()))
        .map_err(|_| error(GuestDriverErrorKind::SpecInvalid))?;
    let spec = json!({
        "providerRef": d2b_provider_guest_azure_container_apps::PROVIDER_REF,
        ...
        "consumerPolicy": {
            "allowedSubjects": [d2b_provider_guest_azure_container_apps::PROVIDER_REF],
