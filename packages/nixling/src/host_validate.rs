...
pub struct ValidateRequest {
    ...
    /// `NIXLING_VALIDATE_SCRIPTS_DIR` env knob.
    pub scripts_dir: PathBuf,
...
impl ValidateRequest {
    pub fn from_env_defaults(mode: ValidateMode) -> Self {
        let evidence_dir = std::env::var_os("NIXLING_VALIDATE_EVIDENCE_DIR")
            .map(PathBuf::from)
...
fn resolve_default_scripts_dir() -> PathBuf {
    if let Some(p) = std::env::var_os("NIXLING_VALIDATE_SCRIPTS_DIR") {
        return PathBuf::from(p);
    }
    // Production: NixOS module installs validator scripts under the
    // system profile share dir.
    let installed = PathBuf::from("/run/current-system/sw/share/nixling/tests");
    if installed.is_dir() {
        return installed;
    }
...
mod tests {
    ...
    fn tempdir() -> PathBuf {
        ...
        let path = base.join(format!("nixling-host-validate-test-{pid}-{nanos}"));
