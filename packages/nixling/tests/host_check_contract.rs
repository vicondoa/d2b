...
//!
//!   * CLI-local (hermetic): drive the real `nixling host check --read-only`
...
//!     `nixling_contracts::cli_output::HostCheckOutputV2` (the `deny_unknown_fields` DTOs make this
...
//!     severity grouping.
//!
//!   * Daemon-backed: spawn a real, KVM-free `nixlingd serve --once` wired to
//!     the fixture bundle, talk to it through the bundled `nixlingd
...
//!     (`NIXLING_TEST_NIXLINGD_BIN`) is unavailable.
//!
//! Hermeticity note: the committed fixture-smoke `bundle.json` declares
//! ABSOLUTE artifact paths (`/etc/nixling/host.json`, ...). On a deployed
//! nixling host those files exist, so the bundle loader would resolve them and
...
use nixling_contracts::cli_output::{HostCheckOutputV2, HostCheckSeverityV2};
...
/// Reproduce, in Rust, the `_nl_host_check_sysctls_json` derivation from
...
/// `nl_write_host_check_fixture_pass`), parameterised by the bundle-derived
/// `sysctls` map.
...
impl Scenario {
    ...
    /// Run `nixling host check --read-only <args>` against this scenario.
    fn run(&self, args: &[&str], extra_env: &[(&str, &str)]) -> Output {
        ...
        cmd.args(["host", "check", "--read-only"])
            .args(args)
            .env("NIXLING_MANIFEST_PATH", self.tree.join("manifest.json"))
            .env("NIXLING_BUNDLE_PATH", self.tree.join("bundle.json"))
            .env("NIXLING_HOST_CHECK_FIXTURE", &self.fixture);
        for (key, value) in extra_env {
            cmd.env(key, value);
        }
        cmd.output().expect("spawn nixling host check")
    }
}
...
fn host_check_ufw_active_warns() {
    ...
    let scenario = Scenario::new(&fixtures, false, "host-pass.json", |_| {});
    let out = scenario.run(&["--json"], &[("NIXLING_TEST_UFW_ACTIVE", "1")]);
...
fn host_check_systemctl_unavailable_warns() {
    ...
    let scenario = Scenario::new(&fixtures, false, "host-pass.json", |_| {});
    let out = scenario.run(&["--json"], &[("NIXLING_TEST_SYSTEMCTL_UNAVAILABLE", "1")]);
...
fn host_check_hard_failure_exits_two() {
    ...
    let scenario = Scenario::new(&fixtures, false, "host-fail.json", |value| {
        value["kernelRelease"] = json!("6.5.0-nixling");
    });
    let out = scenario.run(&["--json"], &[]);
...
fn host_check_nft_probe_error_envelope() {
    assert_probe_error_envelope("host-nft-error.json", "forced nft probe failure", |value| {
        value["nftHasNixlingTable"] = Value::Null;
...
fn host_check_usage_error_exits_three() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_nixling"))
        ...
        .output()
        .expect("spawn nixling host check --bogus");
...
fn daemon_host_check_pass_runs_full_probe_battery() {
    ...
    let scenario = Scenario::new(&fixtures, false, "host-pass.json", |_| {});
    let Some(daemon) =
        common::spawn_nixlingd_host_check(&scenario.tree, &scenario.fixture, &TestPeer::launcher())
    else {
