...
//!
//! Spawns the real `nixling` binary and drives `auth status` across the three
...
//!     `nixling_contracts::cli_output::AuthStatusOutputV2` (`deny_unknown_fields` makes a successful
...
//! and never reads the bundle/manifest, so this test needs no NL_FIXTURES and
//! always runs (no skip path).

use std::process::Command;

use nixling_contracts::cli_output::{AuthRoleV2, AuthStatusOutputV2};
...
fn run_auth_status(
    ...
    cmd.args(["auth", "status", "--test-uid"])
        ...
        .env("NIXLING_AUTH_STATUS_FIXTURE", fixture);
    for (key, value) in role_env {
        cmd.env(key, value);
    }
    cmd.output().expect("spawn nixling auth status")
}

fn parse_json(out: &std::process::Output) -> AuthStatusOutputV2 {
    assert!(
        out.status.success(),
        "`nixling auth status --json` exited {:?}; stderr:\n{}",
        out.status.code(),
...
fn auth_status_roles_match_schema_and_authz() {
    ...
    assert!(
        ...
        "`nixling auth status --human` exited {:?}; stderr:\n{}",
        human.status.code(),
