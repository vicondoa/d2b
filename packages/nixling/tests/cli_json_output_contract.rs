...
use nixling_contracts::cli_output::UsbProbeOutputV1;
...
impl FixtureEnv {
    ...
    fn run_with_shape(&self, shape: &str, args: &[&str]) -> Output {
        ...
        cmd.env("NIXLING_MANIFEST_PATH", self.tree.join("manifest.json"))
            .env("NIXLING_BUNDLE_PATH", self.tree.join("bundle.json"))
            .env("NIXLING_DAEMON_STATE_DIR", &self.daemon_state)
            .env("NIXLING_TEST_DEPLOYMENT_SHAPE", shape);
        cmd.output()
            .unwrap_or_else(|err| panic!("spawn nixling {}: {err}", args.join(" ")))
    }
...
fn assert_matches_golden(out: &Output, golden_name: &str, label: &str) {
    ...
    assert_eq!(
        actual, expected,
        "`nixling {label}` drifted from tests/golden/cli-output/{golden_name}"
    );
}
...
fn normalized_runtime_golden(name: &str) -> String {
    ...
    expected
        .replace(
            ...
            "routes through nixlingd → broker.",
        )
        .replace(
            ...
            "nixling keys rotate --dry-run: planned operation. --apply routes through nixlingd → broker RunKeysRotate with broker audit.",
        )
        .replace(
            ...
            "nixling keys trust --dry-run: planned operation. --apply routes through nixlingd → broker RunKeysRotate with broker audit.",
        )
        .replace(
            ...
            "nixling keys rotate-known-host --dry-run: planned operation. --apply routes through nixlingd → broker RunKeysRotate with broker audit.",
        )
...
fn list_output_matches_cli_json_drift_goldens() {
    ...
    let human = env.run(
        &["list", "--human"],
        &[("NIXLING_TEST_SYSTEM_STATE_JSON", &env.system_state)],
    );
...
fn audit_output_matches_cli_json_drift_goldens() {
    ...
    let human = base_command(&["audit", "--human"], &home, &runtime)
        .env("NIXLING_PUBLIC_SOCKET", &human_socket)
        .env("NIXLING_AUDIT_TESTMODE_KVM_MODE", "660")
        .output()
        .expect("spawn nixling audit --human");
    ...
    let json = base_command(&["audit", "--json"], &home, &runtime)
        .env("NIXLING_PUBLIC_SOCKET", &json_socket)
        .env("NIXLING_AUDIT_TESTMODE_KVM_MODE", "660")
        .output()
        .expect("spawn nixling audit --json");
...
fn host_check_and_auth_status_outputs_match_goldens() {
    ...
    for (args, envs, golden_name, label) in [
        (
            ...
            vec![("NIXLING_AUTH_STATUS_FIXTURE", env.auth_status.as_path())],
        ...
        cmd.env("NIXLING_MANIFEST_PATH", env.tree.join("manifest.json"))
            .env("NIXLING_BUNDLE_PATH", env.tree.join("bundle.json"))
            .env("NIXLING_TEST_LAUNCHER_UIDS", "1000");
        for (key, value) in envs {
            cmd.env(key, value);
        }
        let out = cmd
            .output()
            .unwrap_or_else(|err| panic!("spawn nixling {label}: {err}"));
