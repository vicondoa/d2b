...
//! `d2b-<vm>-snd.service` audio sidecar per
//! `packages/d2b-provider-audio-pipewire/nix/host.nix`. The sidecar publishes
...
/// is supported per `packages/d2b-provider-audio-pipewire/nix/host.nix`; the
...
pub struct AudioArgvInput {
    ...
    /// `/run/d2b/vms/<vm>/snd.sock`. The audio component module
    /// asserts the parent dir is `RuntimeDirectory = d2b/vms/<vm>`
    /// with mode 0700 owned by `d2b-<vm>-snd`.
...
fn expected_audio_sidecar_path(vm_name: &str) -> String {
    format!("/run/d2b/vms/{vm_name}/d2b-{vm_name}")
}
...
/// `d2b-<vm>-snd` (per `packages/d2b-provider-audio-pipewire/nix/host.nix`).
pub fn exec_arg0(input: &AudioArgvInput) -> Result<String, AudioArgvError> {
    if input.vm_name.is_empty() {
        return Err(AudioArgvError::EmptyVmName);
    }
    Ok(format!("d2b-{}-snd", input.vm_name))
}
...
mod tests {
    ...
    fn audit_input() -> AudioArgvInput {
        AudioArgvInput {
            sidecar_binary_path: "/run/d2b/vms/corp-desktop/d2b-corp-desktop".to_owned(),
            vm_name: "corp-desktop".to_owned(),
            socket_path: "/run/d2b/vms/corp-desktop/snd.sock".to_owned(),
    ...
    fn exec_arg0_matches_systemd_unit_name() {
        assert_eq!(exec_arg0(&audit_input()).unwrap(), "d2b-corp-desktop-snd");
    }
    ...
    fn rejects_other_vms_per_vm_copy() {
        let mut input = audit_input();
        input.sidecar_binary_path = "/run/d2b/vms/other/d2b-other".to_owned();
        assert!(matches!(
            generate_audio_argv(&input),
            Err(AudioArgvError::SidecarBinaryPathNotPerVmCopy { .. })
        ));
