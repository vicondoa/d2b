...
# the audio Provider host module installs a rule that reads the same state file and
...
let
  ...
  # in the Provider host module). That means the runner is always launched
...

{
  ...
  # in the Provider host module that uses `stream.rules` for client streams).
