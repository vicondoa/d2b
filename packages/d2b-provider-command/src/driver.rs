...
//!
//! `Command` is a declared launch shape: fixed at startup, family-declared, it materializes the spawn operation the process controller serves. The type is declared here and its rows materialize in the policy-rows unit, so this crate ships the declaration and the driver shell.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`CommandDriverFactory`] registration through the type's
...
pub const COMMAND_TYPE_NAME: &str = "Command";

/// The resource verbs the `Command` type supports.
///
...
/// The execution domains the `Command` type can be reconciled in.
///
/// Derived from the placement contract: `Command` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `Command` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const COMMAND_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `Command` driver reads while reconciling.
///
...
const COMMAND_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `Command` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn command_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `Command` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct CommandDriverFactory {
    types: [ResourceTypeName; 1],
}

impl CommandDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(COMMAND_TYPE_NAME)],
        }
    }
}

impl Default for CommandDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for CommandDriverFactory {
    fn resource_types(&self) -> &[ResourceTypeName] {
        &self.types
    }

    async fn create(&self, _key: &ResourceKey) -> Box<dyn DynResourceDriver> {
        Box::new(CommandDriver)
    }
}
...
/// One `Command` resource's driver.
pub struct CommandDriver;

#[async_trait]
impl ResourceDriver for CommandDriver {
...
/// `*.d2bus.org.*Service` types, so a `Command` row is never an export subject.
pub fn command_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        resource_type: WellKnownType::COMMAND,
        allowed_sources: AllowedSources::BUILTIN,
        verbs: COMMAND_VERBS,
        execution: COMMAND_EXECUTION_DOMAINS,
        exportable: false,
        reads: COMMAND_READS,
        operations: &[],
        creations: &[],
        startup: &[],
        services: &[],
        decoder: command_spec_decoder(),
        factory: Arc::new(CommandDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        COMMAND_TYPE_NAME, CommandDriverFactory, command_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", COMMAND_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = CommandDriverFactory::new()
            .create(&ResourceKey::new("work", COMMAND_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
