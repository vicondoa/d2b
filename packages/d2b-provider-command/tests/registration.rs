//! Command registration boundary: the driver declaration is what the plane
...
/// The declared `Command` type satisfies the shared declaration-only metadata
/// driver contract: the one type its crate owns, registered through its
/// declared decoder and factory.
#[allow(clippy::disallowed_methods, reason = "cfg(test) helper")]
#[tokio::test]
async fn the_declaration_registers_the_command_type() {
    assert_metadata_registration(
        &d2b_provider_command::command_descriptor(),
        WellKnownType::COMMAND,
    )
    .await;
}
