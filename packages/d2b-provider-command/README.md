...
This is the crate root for the `Command` resource type. It owns the type's
...
serves `Command` rows through the `ResourceDriver` verbs: `validate` decodes the
...
`CommandDriverFactory` is the registration surface; `command_descriptor` carries
