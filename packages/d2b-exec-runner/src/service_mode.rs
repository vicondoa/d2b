//! `d2b-exec-runner --serve-exec --slot NN` service mode.
//!
//! This is the per-slot detached supervisor systemd launches as
//! `d2b-exec-<NN>.service`. It reads the SpecCodec spec guestd wrote into
...
use d2b_exec_runner::atomicio::{atomic_write, read_file_nofollow};
use d2b_exec_runner::filering::FileRing;
use d2b_exec_runner::paths::{RunnerPaths, Stream};
use d2b_exec_runner::record::{StatusPhase, StatusRecord};
use d2b_exec_runner::spec::{ExecSpec, SpecCodec};
...
pub fn main_service(slot: u32) -> i32 {
    if slot >= DETACHED_RETAINED_PER_VM as u32 {
        eprintln!("d2b-exec-runner: slot out of range");
        return 64;
    }
    ...
    if validate_slot_dir(&paths).is_err() {
        ...
        eprintln!("d2b-exec-runner: slot directory validation failed");
        return 71;
    }
...
mod production {
    ...
    impl StdSpawner {
        pub fn new(slot: u32, systemctl_path: PathBuf) -> Self {
            Self {
                runner_unit_name: format!("d2b-exec-{slot:02}.service"),
                systemctl_path,
    ...
    fn show_entry_slice_check(show_text: &str, unit_name: &str) -> SliceCheck {
        for block in show_text.split("\n\n") {
            ...
            if id == Some(unit_name) {
                ...
                // systemd nests `d2b-exec.slice` under `d2b.slice`
                // (a `-`-keyed slice name denotes cgroup hierarchy), so the
                // live `ControlGroup` is `/d2b.slice/d2b-exec.slice/
                // <unit>`, NOT a top-level `/d2b-exec.slice/<unit>`. Verify
                // the IMMEDIATE parent slice segment is `d2b-exec.slice`
                // regardless of ancestor slices; the authoritative `Slice=`
                // property above already pins the slice unit assignment.
                let expected_suffix = format!("/d2b-exec.slice/{unit_name}");
                return match cgroup {
...
mod tests {
    ...
    fn spec(argv0: &str, max_runtime_sec: u64) -> ExecSpec {
        ExecSpec {
            ...
            workload_unit_name: "d2b-exec-03-w.service".to_owned(),
            systemd_run_args: vec![
                "--uid=alice".to_owned(),
                "--unit=d2b-exec-03-w.service".to_owned(),
                "--quiet".to_owned(),
                "--collect".to_owned(),
                "--expand-environment=no".to_owned(),
                "--slice=d2b-exec.slice".to_owned(),
                ...
                "--wait".to_owned(),
                "--property=PAMName=login".to_owned(),
                "--property=BindsTo=d2b-exec-03.service".to_owned(),
                "--property=PartOf=d2b-exec-03.service".to_owned(),
                "--property=After=d2b-exec-03.service".to_owned(),
    ...
    fn read_stream_until_eof(
        paths: &RunnerPaths,
        stream: Stream,
    ) -> d2b_exec_runner::filering::RingChunk {
    ...
    fn slice_verification_requires_runner_and_workload_in_exec_slice() {
        // Production reality: systemd nests `d2b-exec.slice` under
        // `d2b.slice`, so the live `ControlGroup` paths carry the
        // `/d2b.slice/d2b-exec.slice/<unit>` prefix.
        let ok = "\
Id=d2b-exec-03.service
Slice=d2b-exec.slice
ControlGroup=/d2b.slice/d2b-exec.slice/d2b-exec-03.service

Id=d2b-exec-03-w.service
Slice=d2b-exec.slice
ControlGroup=/d2b.slice/d2b-exec.slice/d2b-exec-03-w.service
";
...
Id=d2b-exec-03.service
Slice=d2b-exec.slice
ControlGroup=/d2b-exec.slice/d2b-exec-03.service

Id=d2b-exec-03-w.service
Slice=d2b-exec.slice
ControlGroup=/d2b-exec.slice/d2b-exec-03-w.service
";
        assert!(production::units_in_expected_slice(
            flat,
            "d2b-exec-03.service",
        ...
        // A workload whose immediate parent slice is NOT `d2b-exec.slice`
        // (even if `d2b-exec.slice` appears as a non-adjacent ancestor)
        // must fail closed.
        let wrong_parent = "\
Id=d2b-exec-03.service
Slice=d2b-exec.slice
...
Id=d2b-exec-03-w.service
Slice=d2b-exec.slice
ControlGroup=/d2b.slice/d2b-exec.slice/evil.slice/d2b-exec-03-w.service
";
        assert!(!production::units_in_expected_slice(
            wrong_parent,
            "d2b-exec-03.service",
...
";
    ...
    fn std_spawner_fails_closed_when_slice_not_verified_even_if_wrapper_exited() {
        for check in [
            ...
            assert_eq!(
                killed,
                vec!["d2b-exec-03-w.service".to_owned()],
    ...
    fn cancel_sentinel_terminates_and_records_cancelled() {
        ...
        assert!(
            proc.killed_units()
                .contains(&"d2b-exec-03-w.service".to_owned())
        );
        std::fs::remove_dir_all(&dir).ok();
    }
