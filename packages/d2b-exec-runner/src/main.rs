//! `d2b-exec-runner` binary entry point.
//!
...
fn run(args: &[String]) -> i32 {
    match args.first().map(String::as_str) {
        Some("--version") if args.len() == 1 => {
            println!("d2b-exec-runner {}", env!("CARGO_PKG_VERSION"));
            0
        }
        Some("--serve-exec") => match parse_slot(&args[1..]) {
            Some(slot) => service_mode::main_service(slot),
            None => {
                eprintln!("d2b-exec-runner: --serve-exec requires --slot <0..32>");
                64
            }
        },
        ...
        _ => {
            eprintln!("d2b-exec-runner: unsupported invocation");
            78
        }
    }
}
