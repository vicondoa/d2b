...
/// The 12 reservedPaths declared across every component in
/// `tests/unit/nix/eval-cases/w8-integration-wave-plan.nix`. These are the
/// most sensitive planned integration seams (the file each component may not
/// let any other component touch), so the manifest's fingerprint matrix must
/// cover every one of them regardless of which category (generated,
/// dependency, or contract) carries it.
