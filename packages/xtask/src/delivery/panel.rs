...
//!   provider, model, and reasoning effort from
//!   [`PANEL_PROVIDER_POLICY`](super::model::PANEL_PROVIDER_POLICY),
//!   [`PANEL_MODEL_POLICY`](super::model::PANEL_MODEL_POLICY), and
//!   [`PANEL_REASONING_EFFORT_POLICY`](super::model::PANEL_REASONING_EFFORT_POLICY).
//! * `panel-attest` validates a directory holding exactly one strict record
//!   per role, each carrying `model_version` and `reasoning_effort` and bound
//!   to the same `candidate_id`, `content_id`, and `snapshot_sha256`,
//!   rejecting a wrong model or reasoning effort, a missing or duplicate role,
...
//! * Provider, model, and reasoning-effort fields exist only inside the
//!   external delivery-state directory. They never reach a committed file, a
//!   pull-request body, or a release archive.
//!
