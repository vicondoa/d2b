...
//! directory. Excluding commit history from `candidate_id` is deliberate:
//! history-only rebases preserve the candidate's content identity. Do not
//! "fix" it by folding base or head object IDs into either identifier.
//!
...
/// existing snapshots, seals, and delivery artifacts.
...
/// reference, a commit subject, or a checkpoint, none of which
...
pub(crate) mod fixtures {
    ...
    pub fn material() -> CandidateMaterial {
        CandidateMaterial {
            ...
            dependency_fingerprints: vec![fingerprint("cargo-lock", "Cargo.lock", 3)],
