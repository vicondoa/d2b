...
use d2b_realm_core::{
    ...
    ConstellationError, ConstellationFrame, ControllerGenerationCredentialRef,
    ControllerGenerationId, ControllerGenerationMetadata, ControllerGenerationStatus,
    CorrelationId, DefaultRealmSelectionMetadata, DefaultRealmSelectionSource, DescendantRoute,
    DirectShortcutAuthorizationMetadata, DirectShortcutState, DirectShortcutTeardownMetadata,
    DirectShortcutTeardownReason, DiscoveryIngressClass, DiscoveryQueueDropPolicy,
    DiscoveryQueuePolicy, EnrollmentId, EnrollmentReason, EnrollmentRecord, EnrollmentStatus,
    ...
    IdempotencyKey, IdentityAuditEventKind, IdentityAuditEventMetadata, KeyFingerprint, KeyPin,
    KeyRotationEvent, KeyRotationEventKind, KeyRotationId, KeyRotationPlan, KeyRotationReason,
    KeyRotationStatus, KeyRotationSubject, KeyRotationSubjectKind, LegacySurface,
    MigrationErrorEnvelope, MigrationLegacyId, MigrationReasonCode, NodeId, NodeSummary,
    OperationId, OperationKind, OperationRequest, OperationResponse, ParentTrustAnchor,
    PreAuthAdmissionOutcome, PrincipalId, ProviderId, ProviderRegistryEntry,
    ...
    RealmAccessTargetInput, RealmControllerPlacement, RealmId, RealmIdentityConfigEntry,
    RealmIdentityConfigInvariants, RealmIdentityConfigJson, RealmIdentityConfigRuntimeState,
    RealmIdentityFingerprint, RealmIdentityMetadata, RealmIdentityRef, RealmIdentityStatus,
    RealmKeyRole, RealmPath, RealmTarget, RealmTransportBinding, RealmTreeEdge, RecoveryProcedure,
    RecoveryProcedureId, RecoveryReason, RecoveryStatus, ReplayWindowMetadata, RevocationId,
    RevocationList, RevocationListId, RevocationListStatus, RevocationReason, RevocationRecord,
    RevocationStatus, RevocationTarget, RouteAdvertisement, RouteAdvertisementEnvelope,
    RouteAuditEventKind, RouteAuditLabels, RouteFailClosedReason, RouteId,
    RouteNamespaceAllocation, RoutePlacementClass, RoutePolicyRuleId, RouteRealmClass,
    RouteReplayWindowId, RouteSignature, RouteTelemetryBatch, RouteTelemetryCounterKind,
    RouteTelemetryLabels, RouteTelemetrySample, SessionAdmissionAttemptMetadata,
    SessionAdmissionOutcome, SessionTeardownDirective, SessionTeardownReason, ShellAttachId,
    ...
    ShellName, ShellSessionInstanceId, ShellState, ShellSummary, ShortcutAuthorizationId,
    SignatureRef, StreamCursor, StreamId, StreamResume, TreeRouteDecision,
    TreeRouteDecisionOutcome, TreeRouteHop, TreeRouteHopDirection, TreeRoutePath, UnixSocketPath,
    UnverifiedPeerAdmissionAttemptMetadata, UnverifiedPeerRef, WorkloadId, WorkloadPlacement,
...
enum D2bRealmCoreSchema {
    ...
    RealmIdentityRef(RealmIdentityRef),
    ControllerGenerationCredentialRef(ControllerGenerationCredentialRef),
    ...
    RealmIdentityFingerprint(RealmIdentityFingerprint),
    RealmIdentityStatus(RealmIdentityStatus),
    RealmIdentityMetadata(RealmIdentityMetadata),
    ControllerGenerationStatus(ControllerGenerationStatus),
    ControllerGenerationMetadata(ControllerGenerationMetadata),
    RealmKeyRole(RealmKeyRole),
    KeyPin(KeyPin),
    ParentTrustAnchor(ParentTrustAnchor),
    ChildKeyPin(ChildKeyPin),
    EnrollmentStatus(EnrollmentStatus),
    EnrollmentReason(EnrollmentReason),
    EnrollmentRecord(EnrollmentRecord),
    KeyRotationSubjectKind(KeyRotationSubjectKind),
    KeyRotationSubject(KeyRotationSubject),
    KeyRotationReason(KeyRotationReason),
    KeyRotationStatus(KeyRotationStatus),
    KeyRotationPlan(KeyRotationPlan),
    KeyRotationEventKind(KeyRotationEventKind),
    KeyRotationEvent(KeyRotationEvent),
    RevocationTarget(RevocationTarget),
    RevocationReason(RevocationReason),
    RevocationStatus(RevocationStatus),
    RevocationRecord(RevocationRecord),
    RevocationListStatus(RevocationListStatus),
    RevocationList(RevocationList),
    SessionTeardownReason(SessionTeardownReason),
    SessionTeardownDirective(SessionTeardownDirective),
    RecoveryReason(RecoveryReason),
    RecoveryStatus(RecoveryStatus),
    RecoveryProcedure(RecoveryProcedure),
    IdentityAuditEventKind(IdentityAuditEventKind),
    IdentityAuditEventMetadata(IdentityAuditEventMetadata),
    RealmIdentityConfigJson(RealmIdentityConfigJson),
    RealmIdentityConfigRuntimeState(RealmIdentityConfigRuntimeState),
    RealmIdentityConfigEntry(RealmIdentityConfigEntry),
    RealmIdentityConfigInvariants(RealmIdentityConfigInvariants),
    SignatureRef(SignatureRef),
    UnverifiedPeerRef(UnverifiedPeerRef),
    RoutePolicyRuleId(RoutePolicyRuleId),
    RouteReplayWindowId(RouteReplayWindowId),
    ShortcutAuthorizationId(ShortcutAuthorizationId),
    DiscoveryQueueDropPolicy(DiscoveryQueueDropPolicy),
    DiscoveryQueuePolicy(DiscoveryQueuePolicy),
    DiscoveryIngressClass(DiscoveryIngressClass),
    PreAuthAdmissionOutcome(PreAuthAdmissionOutcome),
    UnverifiedPeerAdmissionAttemptMetadata(UnverifiedPeerAdmissionAttemptMetadata),
    ReplayWindowMetadata(ReplayWindowMetadata),
    SessionAdmissionAttemptMetadata(SessionAdmissionAttemptMetadata),
    SessionAdmissionOutcome(SessionAdmissionOutcome),
    RealmTreeEdge(RealmTreeEdge),
    DescendantRoute(DescendantRoute),
    RouteSignature(RouteSignature),
    RouteAdvertisement(RouteAdvertisement),
    RouteAdvertisementEnvelope(RouteAdvertisementEnvelope),
    RouteNamespaceAllocation(RouteNamespaceAllocation),
    TreeRouteHopDirection(TreeRouteHopDirection),
    TreeRouteHop(TreeRouteHop),
    TreeRoutePath(TreeRoutePath),
    RouteFailClosedReason(RouteFailClosedReason),
    TreeRouteDecisionOutcome(TreeRouteDecisionOutcome),
    TreeRouteDecision(TreeRouteDecision),
    DirectShortcutState(DirectShortcutState),
    DirectShortcutAuthorizationMetadata(DirectShortcutAuthorizationMetadata),
    DirectShortcutTeardownMetadata(DirectShortcutTeardownMetadata),
    DirectShortcutTeardownReason(DirectShortcutTeardownReason),
    RouteAuditEventKind(RouteAuditEventKind),
    RouteRealmClass(RouteRealmClass),
    RoutePlacementClass(RoutePlacementClass),
    RouteAuditLabels(RouteAuditLabels),
    RouteTelemetryCounterKind(RouteTelemetryCounterKind),
    RouteTelemetryLabels(RouteTelemetryLabels),
    RouteTelemetrySample(RouteTelemetrySample),
    RouteTelemetryBatch(RouteTelemetryBatch),
...
fn gen_schemas() -> Result<Vec<PathBuf>, Box<dyn std::error::Error>> {
    ...
    let schemas: [(&str, RootSchema); 20] = [
        ...
        (
            "realm-identity.json",
            schemars::schema_for!(RealmIdentityConfigJson),
        ),
        (
