...
//! directive).
//!
//! d2b stores **no Azure secret of its own**. Authentication to and
...
use d2b_constellation_core::{Capability, GatewayId, OperationId, RealmPath, StreamId, WorkloadId};
use schemars::{
    JsonSchema,
...
pub enum CredentialPlane {
    ...
    /// Gateway-minted per-session credential (d2b-internal peer/display).
    D2bInternal,
}
