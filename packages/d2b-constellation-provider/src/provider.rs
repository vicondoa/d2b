...
pub trait WorkloadProvider: Send + Sync {
    ...
    ) -> ProviderResult<d2b_constellation_core::ExecutionId>;
}
...
/// and opaque stream/payload contracts from `d2b-constellation-core`.
#[async_trait]
pub trait DurableExecutionProvider: Send + Sync {
    ...
    async fn start(
        &self,
        req: d2b_constellation_core::ExecStartRequest,
    ) -> ProviderResult<d2b_constellation_core::ExecutionSummary>;
    /// Attach or reconnect to a durable execution after generation validation.
    async fn attach(
        &self,
        req: d2b_constellation_core::ExecAttachRequest,
    ) -> ProviderResult<d2b_constellation_core::ExecutionSummary>;
    /// Fetch retained log metadata under a bounded request.
    async fn logs(
        &self,
        req: d2b_constellation_core::ExecLogsRequest,
    ) -> ProviderResult<d2b_constellation_core::ExecutionSummary>;
    /// Cancel an execution idempotently. `false` means it was already terminal
    /// or unknown to this provider scope.
    async fn cancel(&self, req: d2b_constellation_core::ExecCancelRequest) -> ProviderResult<bool>;
}
...
pub trait StreamMux: Send + Sync {
    ...
    async fn close_stream(&self, id: d2b_constellation_core::StreamId) -> ProviderResult<()>;
}
