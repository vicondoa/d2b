...
fn guest_spec_requires_the_runtime_provider() {
    ...
    assert!(
        d2b_contracts_zone_session::v3::ResourceSpec::new(
            Some(d2b_contracts_zone_session::v3::ResourceRef::parse("Provider/other").unwrap()),
            None,
            d2b_contracts_zone_session::v3::CanonicalJsonObject::empty(),
            None,
        )
        .is_ok()
    );
