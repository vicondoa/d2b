...
impl ProviderAgentProcess {
    ...
    /// # Errors
    ///
    /// Returns [`ProviderAgentError::InvalidInput`] when the zone or
    /// source is empty, the capacity is zero, or a zone label or Provider
    ...
    /// # Errors
    ///
    /// Returns [`ProviderAgentError::InvalidInput`] when the event or
    /// authorization decision is not one of its closed tokens or the
    ...
    pub fn session_connect(
        ...
        let method = parse_closed_token(&event, &["connect", "reconnect", "close"])?;
    ...
    /// # Errors
    ///
    /// Returns [`ProviderAgentError::InvalidInput`] when the event,
    /// provider, or domain is not one of its closed tokens or the outcome
    /// is not recognized.
    pub fn process_effect(
        ...
        let outcome = outcome.into();
        let method = parse_closed_token(&event, &["launch", "stop", "adopt", "quarantine"])?;
...
fn parse_token(value: &str) -> Result<BoundedToken, ProviderAgentError> {
