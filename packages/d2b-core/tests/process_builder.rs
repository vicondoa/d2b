...
use d2b_core::processes::{ProcessRole, ReadinessPredicate, RoleProfile, SpawnRunnerPlanOp};
...
fn profile() -> RoleProfile {
    RoleProfile {
        ...
        cgroup_placement: CgroupPlacement {
            subtree: "d2b.slice/test-vm/cloud-hypervisor".to_owned(),
...
fn process_builder_constructs_existing_process_node_shape() {
    let disk_op = disk_init_plan_op(
        "/var/lib/d2b/vms/test-vm/disk.img",
        1024 * 1024,
        0o600,
        60_100,
        60_100,
    ...
    let node = ProcessNodeBuilder::new(ProcessRole::CloudHypervisorRunner, profile())
        ...
        .with_unit("d2bd.service")
        .with_binary_path("/run/current-system/sw/bin/cloud-hypervisor")
        .with_argv(["microvm@test-vm", "--api-socket", "/run/d2b/ch.sock"])
        .with_env(["D2B_TEST=1"])
...
fn readiness_and_disk_init_helpers_emit_existing_variants() {
    ...
    assert_eq!(
        readiness_unix_socket_exists("/run/d2b/test.sock"),
        ReadinessPredicate::UnixSocketExists("/run/d2b/test.sock".to_owned())
    );
    assert_eq!(
    ...
    assert_eq!(
        disk_init_plan_op("/var/lib/d2b/vms/test-vm/root.img", 4096, 0o600, 1, 2, true),
        SpawnRunnerPlanOp::DiskInit {
            target_path: "/var/lib/d2b/vms/test-vm/root.img".into(),
            size_bytes: 4096,
            mode: 0o600,
            owner_uid: 1,
            owner_gid: 2,
...
fn pre_start_hooks_are_not_long_lived_spawnable_runners() {
    let node = ProcessNodeBuilder::new(ProcessRole::SwtpmPreStartFlush, profile())
        ...
        .with_argv(["d2b-swtpm-flush@test-vm"])
        .build()
