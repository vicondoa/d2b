...
fn build_synthetic_resolver() -> BundleResolver {
    ...
    let host = HostJson {
        ...
        nftables: NftablesModel {
            ...
            ownership_id: "d2b-fixture-001".to_owned(),
        },
        ...
        vms: vec![VmProcessDag {
            ...
            nodes: vec![ProcessNode {
                ...
                argv: vec![
                    ...
                    "--api-socket".to_owned(),
                    "/var/lib/d2b/vms/work-vm/work-vm.sock".to_owned(),
                ],
                env: Vec::new(),
                profile: {
                    #[cfg(feature = "test-support")]
                    {
                        d2b_core::test_support::RoleProfileBuilder::new()
                            ...
                            .with_cgroup_placement(CgroupPlacement {
                                subtree: "d2b/work-vm/ch-runner".to_owned(),
                                controllers: Vec::new(),
                                delegated: true,
                            })
                            .build()
...
fn bundle_resolver_round_trips_nft_intents() {
    ...
    assert!(host_intent.script_body.contains("table inet d2b"));
    assert!(host_intent.script_body.contains("chain input"));
    assert!(host_intent.desired_hash.starts_with("fnv1a64:"));
    assert_eq!(host_intent.ownership_id, "d2b-fixture-001");
...
fn bundle_resolver_round_trips_hosts_intent() {
    ...
    assert!(intent.managed_block.contains("d2b managed begin"));
    assert!(intent.managed_block.contains("d2b managed end"));
...
fn bundle_resolver_round_trips_usbip_intents() {
    ...
    assert_eq!(
        ...
        Some("/run/d2b/locks/usbip/pending")
    );
}
...
fn bundle_resolver_uses_real_bus_ids_when_present() {
    ...
    assert_eq!(bind.vm_name, "work-vm");
    assert_eq!(bind.lock_path.to_str(), Some("/run/d2b/locks/usbip/1-1.5"));
    assert!(
...
fn bundle_resolver_round_trips_socket_intents() {
    ...
    assert_eq!(
        ...
        Some("/run/d2b/vms/work-vm/ch-runner.sock")
    );
