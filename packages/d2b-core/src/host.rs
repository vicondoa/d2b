...
pub struct HostJson {
    ...
    /// Exact nftables `inet d2b` table declaration.
...
/// `d2b_host::ifname::detect_collisions` re-validates uniqueness over
...
/// Exact nftables table, hook priorities, and chain layout owned by d2b.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct NftablesModel {
    ...
    /// Drift-detection digest of the applied `inet d2b` table.
    ...
    /// (`comment "d2b managed: <ownership-id>"`). Empty
...
mod tests {
    use super::{
        BridgePortFlags, HostJson, IfName, Ipv6SysctlEntry, NmReloadBehavior, TapRole,
        UsbipBusidLock, UsbipLockOwner, UsbipLockScope, VendorProductPair,
    };
    ...
    fn ipv6_sysctl_entry_defaults_arp_ignore_to_one() {
        let entry = serde_json::from_str::<Ipv6SysctlEntry>(
            r#"{"ifName":"d2b-bAAAA000","disableIpv6":1,"acceptRa":0,"autoconf":0,"addrGenMode":1}"#,
        )
        .expect("deserialize");
