...
/// Run the host check on the bounded loader worker
/// ([`crate::loader_worker`]).
///
...
/// are owned, so the whole check runs on the worker.
pub async fn run_on_loader_worker(
    host: HostJson,
    closures: Vec<ClosureMetadata>,
    strict: bool,
) -> Result<HostCheckReport, ProbeError> {
    crate::loader_worker::run(move || run(&host, closures.iter(), strict))
        .await
        .map_err(|refusal| {
