...
use super::*;
use d2b_contracts_resource::v3::{CanonicalJsonValue, ZoneRevision};
...
fn relationship_validation_rejects_missing_guest_and_cross_service() {
    ...
    let binding = AudioBindingSpec::new(service_ref.clone(), guest_ref.clone(), zone.as_str());
...
impl AudioMediatorSource for FailingMediatorSource {
    fn build(&self, _vm_name: &str, _projection: bool) -> Option<Box<dyn AudioMediator>> {
        Some(Box::new(
            d2b_provider_audio_pipewire::FakeAudioMediator::unavailable(),
        ))
    }
}
...
fn reconcile_binding_resource_validates_relationships_and_reconciles() {
    ...
    // A valid trio reconciles: no audio capability on the target publishes
    // the honest Degraded/Unavailable status and the binding row enters the
    // shared registry.
    let mut runtime = AudioResourceRuntime::new(zone.clone(), Arc::new(NoAudioSource));
    let status = runtime
...
fn product_is_audio_resource_and_decode_spec_are_pinned_directly() {
    ...
    // A non-audio type, an unreadable envelope, and another provider's rows
    // are all unusable audio identities.
    assert_eq!(
        is_audio_resource(
    ...
    assert!(matches!(
        ...
        Err(AudioResourceRuntimeError::InvalidSpec(_))
    ));
