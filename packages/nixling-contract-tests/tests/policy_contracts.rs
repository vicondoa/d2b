...
// host-prep DAG variant + ordering edges in nixling_host::host_prep_dag, and
// the ChNetHandoffMode enum in nixling-core. Pure grep over committed sources;
...
fn tap_dag_contract_doc_matches_implementation() {
    ...
    const IFNAME: &str = "packages/nixling-host/src/ifname.rs";
    const TAP_OPS: &str = "packages/nixling-priv-broker/src/ops/tap.rs";
    const DAG: &str = "packages/nixling-host/src/host_prep_dag.rs";
    const HOST_DTO: &str = "packages/nixling-core/src/host.rs";
    ...
    assert!(
        ifname.contains(r#"pub const DEFAULT_PREFIX: &str = "nl-";"#),
        "tap-dag-contract-doc-eval: ifname.rs DEFAULT_PREFIX is not \"nl-\""
    );
    assert!(
        doc.contains("`nl-`"),
        "tap-dag-contract-doc-eval: doc must mention default prefix `nl-`"
    );
    ...
    assert!(
        doc.contains("looks_nixling_owned"),
        "tap-dag-contract-doc-eval: doc must reference looks_nixling_owned"
    );
    assert!(
        ifname.contains("pub fn looks_nixling_owned"),
        "tap-dag-contract-doc-eval: ifname.rs missing pub fn looks_nixling_owned"
    );
    ...
    // broker public socket sits behind the daemon-only `nixling` group
    // declared by host-daemon.nix; sanity check that's still true.
    assert!(
        doc.contains("nixling"),
        "tap-dag-contract-doc-eval: doc must reference daemon-only nixling group"
    );
    assert!(
        read_repo_file("nixos-modules/host-daemon.nix").contains("users.groups.nixling"),
        "tap-dag-contract-doc-eval: host-daemon.nix no longer declares nixling group"
    );
}
...
//     (no opaque exec id in unit name/argv), scoped to nixling-exec.slice,
...
fn guest_exec_runtime_static() {
    const GUESTD_SRC: &str = "packages/nixling-guestd/src";
    ...
    const RUNNER_SRC: &str = "packages/nixling-exec-runner/src";
    ...
    assert_file_has_line(
        &detached_unit_src,
        r"nixling-exec-\{slot",
        "guest-exec-runtime-static: detached units must be slot-keyed (nixling-exec-<NN>)",
    );
    ...
    assert_file_has_line(
        &detached_unit_src,
        r"nixling-exec\.slice",
        "guest-exec-runtime-static: detached units must be scoped to nixling-exec.slice",
    );

    // The retained-log path is truncation-bounded (drop-oldest accounting).
    assert_file_has_line(
        "packages/nixling-exec-runner/src/filering.rs",
        r"truncated|dropped",
    ...
    assert_file_has_line(
        ...
        r"/run/nixling-exec",
        "guest-exec-runtime-static: guest module must declare /run/nixling-exec parent dir",
    );
    assert_file_has_line(
        "nixos-modules/guest-control.nix",
        r"nixling-exec",
        "guest-exec-runtime-static: guest module must declare the nixling-exec slice",
    );
