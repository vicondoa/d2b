...
//! `NL_FIXTURES_FULL` is unset (e.g. the plain `cargo test` pass, or a
...
//!     host's `/etc/nixling/minijail-profiles/*.json`, which the bash gate
...
/// header-documented bind set ("per-VM /var/lib/nixling/vms/<vm>/vsock.sock, no
...
fn vsock_relay_rendered_profile_shape() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP: NL_FIXTURES_FULL unset (vsock-relay rendered profile shape)");
        return;
    };
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            assert!(
                writable
                    .iter()
                    .any(|w| *w == format!("/var/lib/nixling/vms/{}", dag.vm)),
            ...
            assert!(
                writable
                    .iter()
                    .any(|w| w.starts_with("/var/lib/nixling/vms/")),
            ...
            assert_eq!(
                ...
                format!("nixling.slice/{}/vsock-relay", dag.vm),
...
/// replaces the per-VM nixling-otel-relay@<vm>.service"; this pins that contract
/// against the rendered DAG node: a `socat` binary, the
/// `nixling-otel-relay@<vm>` process title, and the `UNIX-LISTEN:` + `EXEC:`
/// (ch-vsock-connect) socat pipe.
#[test]
fn vsock_relay_rendered_argv_socat_shape() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP: NL_FIXTURES_FULL unset (vsock-relay rendered argv shape)");
        return;
    };
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            assert!(
                node.argv
                    .first()
                    .is_some_and(|a| a.starts_with("nixling-otel-relay@")),
                "vsock-relay {} (vm {}) argv[0] must be the nixling-otel-relay@<vm> title; got {:?}",
                node.profile.profile_id,
                dag.vm,
                node.argv
            );
            ...
            assert!(
                node.argv
                    .iter()
                    .any(|a| a.starts_with("EXEC:") && a.contains("nixling-ch-vsock-connect")),
...
/// role that replaces the singleton `nixling-otel-host-bridge.service`.
#[test]
...
/// profile JSON (`/etc/nixling/minijail-profiles/host-otel-host-bridge.json`)
/// behind `NL_LIVE=1`; they port here as typed `RoleProfile` field checks over
...
fn otel_host_bridge_rendered_profile_shape() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP: NL_FIXTURES_FULL unset (otel-host-bridge rendered profile shape)");
        return;
    };
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            assert!(
                writable.contains(&"/run/nixling/otel"),
            ...
            // Host-scoped cgroup placement + namespace isolation.
            assert_eq!(
                p.cgroup_placement.subtree, "nixling.slice/host/otel-host-bridge",
...
/// this pins that contract: a `socat` binary, the `nixling-otel-host-bridge`
/// process title, the `UNIX-LISTEN:/run/nixling/otel/host-egress.sock` listen
/// endpoint, and the `EXEC:` ch-vsock-connect forward leg.
#[test]
fn otel_host_bridge_rendered_argv_socat_shape() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP: NL_FIXTURES_FULL unset (otel-host-bridge rendered argv shape)");
        return;
    };
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            assert_eq!(
                ...
                Some("nixling-otel-host-bridge"),
                "otel-host-bridge {} (vm {}) argv[0] must be the nixling-otel-host-bridge title; got {:?}",
                node.profile.profile_id,
                dag.vm,
                node.argv
            );
            assert!(
                node.argv
                    .iter()
                    .any(|a| a.starts_with("UNIX-LISTEN:/run/nixling/otel/host-egress.sock")),
