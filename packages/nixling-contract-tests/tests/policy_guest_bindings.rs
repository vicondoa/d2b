...
fn guest_ttrpc_bindings_are_xtask_only_and_ipc_stays_ttrpc_free() {
    ...
    assert!(
        ...
        "guest-ttrpc-bindings: nixling-guestd must not generate ttRPC bindings during normal builds"
    );
    ...
    assert_no_line_matches(
        ...
        "guest-ttrpc-bindings: nixling-guestd must keep ttRPC code generation in xtask only",
    );
