...
//! `nixling_contract_tests::read_repo_file` / `repo_root` helpers) and asserts a
...
fn changelog_keeps_unreleased_above_latest_dated_release() {
    ...
    // via NIXLING_REQUIRE_EMPTY_UNRELEASED=1. PR CI (default off) allows
    // active-development entries from concurrent work.
    if std::env::var("NIXLING_REQUIRE_EMPTY_UNRELEASED").as_deref() == Ok("1") {
...
fn changelog_v1_0_0_section_enumerates_breaking_changes_with_adr_and_migration_xref() {
    ...
    assert!(
        v1_section.contains("docs/how-to/migrate-nixling-v0-to-v1.md"),
...
// tests/vfsd-watchdog-retired-eval.sh
//
// Asserts the `nixling-vfsd-watchdog@.{service,timer}` definitions (and per-VM
...
fn vfsd_watchdog_units_retired_from_store_module() {
    ...
    let service_decl = Regex::new(r#"(?m)^\s*"nixling-vfsd-watchdog@"\s*="#).unwrap();
    assert!(
        !service_decl.is_match(&store_module),
        r#""nixling-vfsd-watchdog@" service template still declared in nixos-modules/store.nix"#
    );

    // (b) Productive timer template absent.
    let timer_decl = Regex::new(r#"systemd\.timers\."nixling-vfsd-watchdog@"\s*="#).unwrap();
    assert!(
        !timer_decl.is_match(&store_module),
        r#"systemd.timers."nixling-vfsd-watchdog@" still declared in nixos-modules/store.nix"#
    );

    // (c) Per-VM enabling units absent (literal "${name}" key).
    assert!(
        !store_module.contains(r#""nixling-vfsd-watchdog-${name}-enable""#),
        r#"per-VM enabling unit "nixling-vfsd-watchdog-${{name}}-enable" still declared in nixos-modules/store.nix"#
    );
}
