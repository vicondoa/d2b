...
//!     the typed `nixling_core::static_invariants` validators directly. These
//!     live here (not as `nixling-core` `#[cfg(test)]` unit tests) because
...
use nixling_contract_tests::{load_bundle_resolver_from_env, load_manifest_value_from_env};
use nixling_core::static_invariants::{
