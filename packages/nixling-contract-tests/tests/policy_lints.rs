...
fn daemon_experimental_option_documents_default_with_migration() {
    ...
    let guide = read_repo_file("docs/how-to/migrate-nixling-v1-0-to-v1-1.md");
    assert!(
        guide.contains("Remove `nixling.daemonExperimental.enable`"),
        "the v1.0->v1.1 migration guide must instruct removing nixling.daemonExperimental.enable"
    );
}
