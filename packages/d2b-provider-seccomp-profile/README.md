...
This is the crate root for the `SeccompProfile` resource type. It owns the type's
...
serves `SeccompProfile` rows through the `ResourceDriver` verbs: `validate` decodes the
...
`SeccompProfileDriverFactory` is the registration surface; `seccomp_profile_descriptor` carries
