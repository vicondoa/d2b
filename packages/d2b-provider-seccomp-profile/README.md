...
This is the crate root for the `SeccompProfile` resource type. It owns the type's identity
...
| Field | Value |
| --- | --- |
| Provider name | `seccomp-profile` |
| ResourceType | `SeccompProfile` |
| Package | `packages/d2b-provider-seccomp-profile/` |
| Driver declaration | `seccomp_profile_descriptor` -> `DriverDescriptor` |

## Config schema

The type declares no provider config schema: `SeccompProfile` rows carry the JSON spec
object the core rows store, and nothing outside it decodes at validate.

## Exported resource types

`SeccompProfile` is not exportable. `ResourceExport` admits only qualified
`*.d2bus.org.*Service` types, so a `SeccompProfile` row is never an export subject. The
declaration carries `exportable: false`.

## Controllers / services / workers / binaries

The crate ships the type's declaration, not a standalone process. The shared
driver serves `SeccompProfile` rows through the `ResourceDriver` verbs: `validate` decodes the
...
The registration surface is `seccomp_profile_descriptor`: it declares the type through the shared
...
`SeccompProfile` names no placement anchor, so a `SeccompProfile` row is reconciled on its
...
```bash
cargo test -p d2b-provider-seccomp-profile
```
