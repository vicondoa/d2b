...
use nixling_constellation_core::{EntrypointMode, RealmPath, TargetName};
...
impl RealmEntrypoint {
    /// A host-resident entrypoint (dispatched on the local `nixlingd`).
...
pub enum DispatchTarget {
    /// The realm entrypoint is this host: local `nixlingd` applies the realm
...
mod tests {
    use super::*;
    use nixling_constellation_core::RealmId;
    ...
    fn example_table() -> RealmEntrypointTable {
        ...
        t.gateway_backed(realm(&["work"]), target("work-gateway.laptop.nixling"));
        t.gateway_backed(realm(&["ops"]), target("ops-gateway.laptop.nixling"));
        t
    }
    ...
    fn local_realm_is_host_resident() {
        let t = example_table();
        let d = t.resolve(&target("demo.nixling")).unwrap();
        assert_eq!(
            d,
            DispatchTarget::HostResident {
                target: target("demo.nixling")
            }
        );
    }
    ...
    fn host_resident_named_realm() {
        let t = example_table();
        // dev-vm.laptop.personal.nixling -> personal -> host-resident.
        let d = t
            .resolve(&target("dev-vm.laptop.personal.nixling"))
            .unwrap();
    ...
    fn gateway_backed_realm_returns_its_gateway() {
        let t = example_table();
        let d = t.resolve(&target("demo.aca.work.nixling")).unwrap();
        match d {
            DispatchTarget::GatewayBacked {
                gateway,
                target: tgt,
            } => {
                assert_eq!(gateway, target("work-gateway.laptop.nixling"));
                assert_eq!(tgt, target("demo.aca.work.nixling"));
            }
    ...
    fn nested_child_realm_matches_parent_by_longest_suffix() {
        ...
        let t = example_table();
        let d = t
            .resolve(&target("api.build.payments.work.nixling"))
            .unwrap();
    ...
    fn more_specific_child_entry_overrides_parent() {
        ...
        let sibling = t
            .resolve(&target("api.build.billing.work.nixling"))
            .unwrap();
    ...
    fn unknown_realm_fails_closed() {
        let t = example_table();
        let err = t.resolve(&target("x.y.unknown.nixling")).unwrap_err();
    ...
    fn gateway_backed_without_gateway_fails_closed() {
        ...
        let err = t.resolve(&target("demo.aca.work.nixling")).unwrap_err();
