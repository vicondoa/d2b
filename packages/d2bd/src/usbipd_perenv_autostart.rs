...
//! `d2b host doctor` and the integration test layer can pin the
...
pub enum PerEnvUsbipdRole {
    ...
    /// interfaces; the broker-managed `inet d2b` input chain drops
...
/// vocabulary in `d2b host doctor` whether they're looking at
...
mod tests {
    ...
    fn manifest_with(vms: &[(&str, Option<&str>, bool, Option<&str>)]) -> ManifestV04 {
        ...
        for (name, env, usbip, host_ip) in vms {
            ...
            vm.insert(
                "gpuSocket".into(),
                Value::String(format!("/var/lib/d2b/vms/{name}/{name}-gpu.sock")),
            );
            ...
            vm.insert(
                ...
                json!({
                    ...
                    "enabled": false,
                    "vsockCid": 100,
                    "vsockHostSocket": format!("/var/lib/d2b/vms/{name}/vsock.sock"),
                }),
            );
            vm.insert(
                "runtime".into(),
            ...
            vm.insert(
                "stateDir".into(),
                Value::String(format!("/var/lib/d2b/vms/{name}")),
            );
            vm.insert(
                "staticIp".into(),
