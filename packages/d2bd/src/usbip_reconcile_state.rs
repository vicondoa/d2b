    ...
    pub const fn remediation(self) -> &'static str {
        match self {
            Self::MissingExactTuple | Self::AmbiguousSameEnvStreams => {
                "run `d2b usb probe`; if the stream is still ambiguous, stop the VM so it drains"
            }
            Self::CleanupUnsupported => {
...
pub enum UsbipVmCarrierCleanupMode {
    ...
    VmStopOrRestart,
    /// Explicit `d2b usb detach`: tear down active state and release the
...
fn scrub_usb_hex_id(value: Option<&str>) -> Option<String> {
    d2b_contracts::usbip::sanitize_usb_hex_id(value)
}
...
fn scrub_public_claim_ref(value: &str) -> String {
    ...
    if valid_opaque
        && !looks_like_trace_id(value)
        && d2b_contracts::usbip::validate_bus_id(value).is_err()
    {
        value.to_owned()
    } else {
...
mod tests {
    ...
    fn usbip_step_failure_classification_uses_bounded_other_fallback() {
        assert_eq!(
            classify_usbip_step_failure("lock", "busy for /run/d2b/locks/usbip/1-2.4"),
