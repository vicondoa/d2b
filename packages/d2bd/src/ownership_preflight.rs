//! VM-start preflight that invokes the `d2b_host::ownership_matrix`
...
const CANONICAL_MATRIX: &[EntrySpec] = &[
    ...
    EntrySpec {
        path: "swtpm",
        owner_template: "d2b-<vm>-swtpm",
        group_template: "d2b-<vm>-swtpm",
        mode: 0o0700,
        kind: EntryKind::Dir,
        required: true,
        recursive: false,
...
/// templates (e.g. the `store-view/live/.d2b-marker-<vm>` leaf).
...
mod tests {
    ...
    fn render_drift_message_lists_axes() {
        let drift = vec![OwnershipMismatch::Drift {
            path: "/var/lib/d2b/vms/vm1/state".into(),
            expected: d2b_host::ownership_matrix::Ownership {
                uid: 100,
                gid: 200,
                mode: 0o0750,
            },
            actual: d2b_host::ownership_matrix::Ownership {
                uid: 0,
                gid: 0,
                mode: 0o0755,
            },
            drift_reason: d2b_host::ownership_matrix::DriftReason {
                owner: true,
                group: true,
                mode: true,
            },
    ...
    fn render_drift_message_includes_kind_mismatch() {
        let drift = vec![OwnershipMismatch::KindMismatch {
            path: "/var/lib/d2b/vms/vm1/store-view/sync.lock".into(),
    ...
    fn render_drift_message_marks_non_enoent_stat_failure() {
        ...
        let drift = vec![OwnershipMismatch::StatFailed {
            path: "/var/lib/d2b/vms/vm1/store-view/state".into(),
