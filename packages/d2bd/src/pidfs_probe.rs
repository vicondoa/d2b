...
//! `d2bd` requires Linux ≥ 6.9 because the BootedNotify identity
...
/// is via the `D2B_ALLOW_PIDFS_PROBE_SOFT_FAIL` env var (for
/// CI / development hosts that don't actually run VMs).
pub fn enforce_probe_outcome(outcome: &PidfsProbeOutcome) -> Result<(), TypedError> {
    let allow_soft_fail = std::env::var_os("D2B_ALLOW_PIDFS_PROBE_SOFT_FAIL").is_some();
...
/// `D2B_ALLOW_PIDFS_PROBE_SOFT_FAIL` env var; tests pass it directly so
