...
//! `d2b-known-hosts-refresh@<vm>.service` systemd
//! oneshot (defined in `nixos-modules/host-known-hosts.nix`).
//!
//! That unit ran a shell script that:
//!
//!   1. Looked up the VM in `share/d2b/vms.json`, skipping net
//!      VMs (no host-accessible sshd) and entries lacking a
//!      `staticIp`.
//!   2. Read the authoritative host pubkey from
//!      `/var/lib/d2b/vms/<vm>/sshd-host-keys/ssh_host_ed25519_key.pub`.
//!   3. Under a flock on `/var/lib/d2b/known_hosts.d2b.lock`,
//!      replaced any line matching the static IP in
//!      `/var/lib/d2b/known_hosts.d2b` with the fresh
...
use d2b_contracts::broker_wire::{RunRotateKnownHostRequest, RunRotateKnownHostResponse};
...
pub enum SkipReason {
    ...
    /// keyed the `known_hosts.d2b` line on the static IP, so
...
impl RefreshOutcome {
    ...
    /// rewrote `known_hosts.d2b`; that's
...
mod tests {
    ...
    fn vm_entry(name: &str, is_net_vm: bool, static_ip: Option<&str>) -> VmEntry {
        VmEntry {
            api_socket: Some(format!("/run/d2b/vms/{name}/api.sock")),
            audio: false,
            ...
            observability: VmObservability {
                agent_socket: Some(format!("/run/d2b/vms/{name}/agent.sock")),
                enabled: false,
                vsock_cid: Some(42),
