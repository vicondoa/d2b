...
impl ProviderExecutor {
    ...
    pub async fn list(&self) -> ProviderResult<usize> {
        Ok(self
            .provider
            .list(d2b_realm_core::WorkloadSelector::All)
            .await?
            .len())
    }
}
...
impl PeerDaemon {
    ...
    ) -> Result<RemoteDispatchOutcome, d2b_realm_router::RemoteNodeError> {
...
mod tests {
    ...
    impl RemotePeerClient for FakePeerClient {
        fn send_operation(
            &mut self,
            _route: &d2b_realm_router::RemoteRoute,
        ...
        fn query_operation(
            ...
            _route: &d2b_realm_router::RemoteRoute,
            _req: &OperationRequest,
        ) -> Result<d2b_realm_router::RemotePeerStatus, RemoteNodeError> {
            Ok(d2b_realm_router::RemotePeerStatus::InProgress)
        }
    }
    ...
    fn peer_router_routes_through_the_codec_neutral_router() {
        let r = PeerOperationRouter::with_capabilities(
            new_shared_router(),
            CapabilitySet::empty().with(d2b_realm_core::Capability::Lifecycle),
        );
    ...
    fn reconnecting_sessions_share_one_dedup_owner() {
        ...
        let caps = CapabilitySet::empty().with(d2b_realm_core::Capability::Lifecycle);
    ...
    fn peer_daemon_denies_missing_capability_before_peer_send() {
        ...
        assert_eq!(
            err.kind,
            d2b_realm_router::RemoteNodeErrorKind::CapabilityDenied
        );
    ...
    #[test]
    fn realm_seam_imports_no_broker_guest_control_or_provider_credentials() {
        let src = include_str!("realm_stubs.rs");
        let imports = src
            .lines()
        ...
        for forbidden in [
            ...
            assert!(
                ...
                "realm seam import leaked forbidden dependency {forbidden}: {imports}"
            );
        }
    }
}
