...
//!   [`OperationRouter`](d2b_constellation_router::OperationRouter). It
...
use d2b_constellation_provider::provider::{ProtocolCodec, WorkloadProvider};
use d2b_constellation_router::{
...
impl ProviderExecutor {
    ...
    pub async fn list(&self) -> ProviderResult<usize> {
        Ok(self
            .provider
            .list(d2b_constellation_core::WorkloadSelector::All)
            .await?
            .len())
    }
}
...
impl PeerDaemon {
    ...
    ) -> Result<RemoteDispatchOutcome, d2b_constellation_router::RemoteNodeError> {
...
/// Which role a single `d2bd` instance plays. There is exactly one
...
/// authority, while a realm gateway runs its own `d2bd` in gateway mode
...
/// The mode-relevant slice of a `d2bd` instance's resolved config. It is
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            DaemonModeError::HostCarriesRealmConfig => write!(
                f,
                "host-mode d2bd must not carry realm provider/relay config"
            ),
            DaemonModeError::GatewayCarriesHostLifecycle => write!(
                f,
                "gateway-mode d2bd must not carry host-broker/local-lifecycle responsibility"
            ),
        }
    }
}
...
mod tests {
    ...
    use d2b_constellation_router::{RemoteNodeError, RemoteNodeRegistration};
    ...
    impl RemotePeerClient for FakePeerClient {
        fn send_operation(
            &mut self,
            _route: &d2b_constellation_router::RemoteRoute,
        ...
        fn query_operation(
            ...
            _route: &d2b_constellation_router::RemoteRoute,
            _req: &OperationRequest,
        ) -> Result<d2b_constellation_router::RemotePeerStatus, RemoteNodeError> {
            Ok(d2b_constellation_router::RemotePeerStatus::InProgress)
        }
    }
    ...
    fn peer_router_routes_through_the_codec_neutral_router() {
        let r = PeerOperationRouter::with_capabilities(
            new_shared_router(),
            CapabilitySet::empty().with(d2b_constellation_core::Capability::Lifecycle),
        );
    ...
    fn reconnecting_sessions_share_one_dedup_owner() {
        ...
        let caps = CapabilitySet::empty().with(d2b_constellation_core::Capability::Lifecycle);
    ...
    fn peer_daemon_denies_missing_capability_before_peer_send() {
        ...
        assert_eq!(
            err.kind,
            d2b_constellation_router::RemoteNodeErrorKind::CapabilityDenied
        );
