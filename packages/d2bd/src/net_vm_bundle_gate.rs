...
use d2b_core::bundle_resolver::{BundleResolver, intent_id_hosts_host, intent_id_nft_env};
use sha2::Digest as _;

/// Default parent dir holding `<env>.conf` for each net VM.
pub const DEFAULT_DNSMASQ_DIR: &str = "/var/lib/d2b/dnsmasq";
...
impl BundleGateDrift {
    ...
    /// canonical `/var/lib/d2b/dnsmasq/` parent.
...
pub fn compute_expected_dnsmasq_hash(resolver: &BundleResolver, env: &str) -> String {
    ...
    hasher.update(b"d2b-dnsmasq:v1\n");
...
mod tests {
    ...
    fn build_resolver() -> BundleResolver {
        ...
        let net_vm = VmEntry {
            api_socket: Some("/run/d2b/vms/sys-work-net/api.sock".to_owned()),
            audio: false,
            ...
            shell: None,
            ssh_user: None,
            state_dir: "/var/lib/d2b/vms/sys-work-net".to_owned(),
    ...
    fn write_matching_conf(dir: &Path, env: &str, resolver: &BundleResolver) -> PathBuf {
        ...
        let mut buf = Vec::new();
        buf.extend_from_slice(b"d2b-dnsmasq:v1\n");
        let nft_id = intent_id_nft_env(env);
        let nft_body = resolver
            .find_nft_intent(&nft_id)
    ...
    fn drift_path_accessor_returns_offending_path() {
        ...
        assert_eq!(
            drift.path(),
            PathBuf::from("/var/lib/d2b/dnsmasq/work.conf")
        );
