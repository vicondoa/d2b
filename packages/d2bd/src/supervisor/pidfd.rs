...
//! `d2b-<vm>-virtiofsd@<share>.service` ExecStopPost-style bash
//! health check + `d2b-vfsd-watchdog@<vm>` timer was the only thing
...
mod tests {
    ...
    fn virtiofsd_signal_exit_treated_as_unexpected() {
        ...
        assert_eq!(outcome.events.len(), 3);
        assert_vfsd_died(&outcome.events[0], "alpha", "virtiofsd-d2b-meta", exit);
        assert_share_degraded(&outcome.events[1], "alpha", "virtiofsd-d2b-meta");
        assert_stop_ch(&outcome.events[2], "alpha");
