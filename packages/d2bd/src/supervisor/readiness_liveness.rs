...
use d2b_contracts::broker_wire::ChildExitStatus;
...
mod tests {
    ...
    fn stale_same_role_reap_for_different_pid_does_not_fast_fail_live_runner() {
        ...
        use d2b_contracts::broker_wire::{ChildExitStatus, ChildReapedNotification};
        ...
        let table = PidfdTable::new(
            std::env::temp_dir().join(format!("d2b-readiness-liveness-test-{pid}.json")),
        );
        table
            .register(
                "work".into(),
