...
pub(crate) struct ProductionBindingDriverEffects {
    ready: Arc<dyn for<'a> Fn(&'a SocketIdentity) -> ServingEffectFuture<'a, bool> + Send + Sync>,
    remove: Arc<
        dyn for<'a> Fn(&'a SocketIdentity) -> ServingEffectFuture<'a, Result<(), String>>
            + Send
            + Sync,
    >,
...
impl ProductionBindingDriverEffects {
    pub(crate) fn new(
        ready: Arc<
            dyn for<'a> Fn(&'a SocketIdentity) -> ServingEffectFuture<'a, bool> + Send + Sync,
        >,
        remove: Arc<
            dyn for<'a> Fn(&'a SocketIdentity) -> ServingEffectFuture<'a, Result<(), String>>
                + Send
