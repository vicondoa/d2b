...
//!
//! U14: the redb-backed operator fixtures (the seeded store, the resource-API
//! bootstrap and the durable CRUD/drain scenarios) retired with the durable
//! store. The manager-only replacement matrix lands in U15.
//!
//! The surviving Provider-lifecycle fixtures build their authorization with
//! `d2bd`'s test-only constructors, so this target runs with
//! `cargo test -p d2bd --features test-support`.
...
use d2b_contracts_resource::v3::{ResourceName, ResourceRef, ZoneId};
