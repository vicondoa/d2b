...
//! files (via the `d2b_contract_tests` repo-file helpers) and asserts a
...
//! overrides, `D2B_*_IN_NIX_SHELL` python3 re-entry, `d2b_mktemp` scratch
...
fn broker_delegates_bundle_validation_to_core() {
    ...
    assert!(
        tree.contains("d2b_core::manifest"),
        "broker-validate-bundle: expected d2b_core::manifest import under {src_rel}"
    );
...
//   backtick `d2b ...` spans, UNION the operation-enum variants parsed from
...
fn declared_cli_ops(found: &mut BTreeSet<String>) {
    ...
    let cmd_re = Regex::new(r"`(d2b[^`]*)`").expect("valid cmd regex");
    let version_re = Regex::new(r"^v\d+(?:\.\d+)*$").expect("valid version regex");
    let audio_on_re = Regex::new(r"^d2b\s+audio\s+on\|off(?:\s|$)").expect("valid audio regex");
    let audio_mic_re =
        Regex::new(r"^d2b\s+audio\s+mic\s+on\|off(?:\s|$)").expect("valid audio mic regex");
    let audio_speaker_re =
        Regex::new(r"^d2b\s+audio\s+speaker\s+on\|off(?:\s|$)").expect("valid audio speaker regex");
    ...
    for cap in cmd_re.captures_iter(&cli) {
        ...
        if raw_cmd.starts_with("d2b audit") && raw_cmd.contains("--human") {
