...
/// (`${...}`), so both `"d2b-foo-snd"` and `"d2b-${m.name}-snd"` match.
...
fn denylist_patterns() -> Vec<(String, Regex)> {
    let raw = vec![
        ...
        format!("d2b-{NAMEPART}-snd"),
        format!("d2b-{NAMEPART}-video"),
        format!("d2b-{NAMEPART}-gpu"),
        format!("d2b-{NAMEPART}-store-sync"),
        r"d2b-known-hosts-refresh@".to_string(),
        r"d2b-otel-relay@".to_string(),
        r"d2b-net-route-preflight".to_string(),
        r"d2b-audit-check\.service".to_string(),
        r"d2b-audit-check\.timer".to_string(),
        r"d2b-ch-exporter".to_string(),
        r"d2b-otel-host-bridge\.service".to_string(),
        format!("d2b-sys-{NAMEPART}-usbipd-"),
        ...
        // BOTH the attr-name form (`systemd.services."d2b-<vm>-swtpm"`)
        // AND the unit-string form (`d2b-<vm>-swtpm.service`).
        format!("d2b-{NAMEPART}-swtpm(\\.service)?"),
    ];
    raw.into_iter()
        .map(|p| {
            let re =
...
fn classify(rel: &str, idx: usize, lines: &[String]) -> Verdict {
    ...
    // dedicated file allowlist: bundle-metadata `*Service = "d2b-..."` strings.
    if line.contains("audioService = \"d2b-")
        || line.contains("videoService = \"d2b-")
        || line.contains("gpuService = \"d2b-")
        || line.contains("tpmService = \"d2b-")
    {
        return Verdict::Skip;
    }
...
fn legacy_unit_denylist_classify_semantics() {
    ...
    assert_eq!(
        classify(
            ...
            0,
            &[r#"    services.foo = "d2b-bar-gpu"; # retired: transitional"#.to_string()]
        ),
        Verdict::Skip,
    ...
    assert_eq!(
        classify(
            ...
            0,
            &[r#"    gpuService = "d2b-${m.name}-gpu";"#.to_string()]
        ),
        Verdict::Skip,
    ...
    assert_eq!(
        classify(
            ...
            0,
            &[r#"        unit_pattern = "d2b-otel-relay@*.service";"#.to_string()]
        ),
        Verdict::Skip,
    ...
    assert_eq!(
        classify(
            "nixos-modules/components/observability/host.nix",
            0,
            &[r#"    systemd.services."d2b-ch-exporter" = {"#.to_string()]
        ),
        Verdict::Live,
    ...
    assert_eq!(
        ...
        Verdict::Skip,
        "a `*Service = \"d2b-...\"` bundle-metadata line is exempt anywhere"
    );
...
fn legacy_unit_denylist_patterns_match_representative_refs() {
    ...
    let samples: &[&str] = &[
        ...
        r#"audioService = "d2b-${m.name}-snd""#,
        "d2b-work-video.service",
        "d2b-work-gpu.service",
        "d2b-work-store-sync.service",
        "d2b-known-hosts-refresh@work.service",
        "d2b-otel-relay@work.service",
        "d2b-net-route-preflight.service",
        "d2b-audit-check.service",
        "d2b-audit-check.timer",
        "g:d2b-ch-exporter",
        "d2b-otel-host-bridge.service",
        "d2b-sys-work-usbipd-3-1.service",
        // swtpm: attr-name form (no .service suffix)
        r#"systemd.services."d2b-work-swtpm" = {"#,
    ];
    let patterns = denylist_patterns();
    assert_eq!(
        patterns.len(),
    ...
    assert!(
        swtpm_pat.is_match(r#"systemd.services."d2b-work-swtpm" = {};"#),
        "swtpm denylist pattern must catch the attr-name form (no .service suffix)"
    );
    assert!(
        swtpm_pat.is_match("d2b-work-swtpm.service"),
        "swtpm denylist pattern must catch the .service-string form"
    );
    assert!(
        swtpm_pat.is_match(r#"systemd.services."d2b-${name}-swtpm" = {};"#),
...
fn legacy_unit_denylist_swtpm_forms() {
    // Attr-name form (`systemd.services."d2b-<vm>-swtpm"`) in a
    ...
    // Unit-string form (`d2b-<vm>-swtpm.service`) in a non-exempt
    // module must also be flagged LIVE.
    assert_eq!(
        classify(
            "nixos-modules/host.nix",
            0,
            &["    d2b-work-swtpm.service".to_string()]
        ),
        Verdict::Live,
    ...
    assert_eq!(
        classify(
            ...
            0,
            &[r#"        unit = "d2b-${name}-swtpm.service";"#.to_string()]
        ),
        Verdict::Skip,
    ...
    assert_eq!(
        classify(
            "nixos-modules/manifest.nix",
            0,
            &[r#"    tpmService = "d2b-${m.name}-swtpm";"#.to_string()]
        ),
        Verdict::Skip,
