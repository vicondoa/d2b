...
//! `fixture-smoke-full` corpus (D2B_FIXTURES_FULL) so it validates the actual
//! emitted artifact, not just the source. It skips cleanly when
//! D2B_FIXTURES_FULL is unset (plain `cargo test` pass, non-x86_64 host, or CI
...
fn swtpm_readiness_uses_unix_socket_listening() {
    ...
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP {test}: D2B_FIXTURES_FULL unset (swtpm readiness fixture unavailable)");
        return;
    };
