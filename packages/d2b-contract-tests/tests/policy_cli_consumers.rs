...
const THIS_TEST: &str = "packages/d2b-contract-tests/tests/policy_cli_consumers.rs";
...
fn is_legacy_grep_excluded(rel: &str) -> bool {
    rel.split('/').any(|component| {
        ...
        ) || component.starts_with(".d2b-smoke-cache.")
    })
}
...
fn cli_nix_live_consumer_bindings_absent() {
    ...
    assert!(
        ...
        "cli-nix-consumers-eval: d2b.store.package/generations referenced \
