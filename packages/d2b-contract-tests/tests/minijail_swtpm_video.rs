...
//! layer (this crate, gated by the D2B_FIXTURES / D2B_FIXTURES_FULL contract
...
//! `fixture-smoke-full` resolver (D2B_FIXTURES_FULL; the `corp-full` VM emits
//! `vm-corp-full-swtpm`, `vm-corp-full-swtpm-flush`, and `vm-corp-full-video`).
//! That resolver is `None` when D2B_FIXTURES_FULL is unset (a plain `cargo
...
//!     Nix module.
//!   * The bash gates' opt-in live phases (`D2B_LIVE=1`) are runtime execution
//!     tests that require root, a live host, `minijail0`, `swtpm`/`tpm2-tools`,
//!     and write `/var/lib/d2b/validated/p1-*.json` evidence. They are NOT
...
fn full_resolver_or_skip(test: &str) -> Option<BundleResolver> {
    match load_full_bundle_resolver_from_env() {
        Some(r) => Some(r),
        None => {
            eprintln!("SKIP {test}: D2B_FIXTURES_FULL unset (feature-rich fixture unavailable)");
            None
        }
    }
}
...
// tests/minijail-validator-swtpm.sh
//
// Phase 1 (eval-only, always-on) ports below. Phase 2 (D2B_LIVE=1) is NOT
...
// /var/lib/d2b/vms/<vm>/swtpm), probes an undeclared syscall (ptrace) for a
// SIGSYS seccomp kill, and writes /var/lib/d2b/validated/p1-swtpm.json
...
/// `d2b.slice/<vm>/{swtpm,swtpm-flush}` cgroup subtree.
#[test]
fn swtpm_rendered_umask_state_dir_cgroup() {
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            let p = &node.profile;
            let state_dir = format!("/var/lib/d2b/vms/{}/swtpm", dag.vm);
            ...
            match node.role {
                ...
                ProcessRole::SwtpmPreStartFlush => {
                    ...
                    assert_eq!(
                        p.cgroup_placement.subtree,
                        format!("d2b.slice/{}/swtpm-flush", dag.vm),
...
fn swtpm_source_principal_and_no_tmpfs() {
    ...
    // verbatim as the bash `grep -q 'stablePrincipalId "d2b-${name}-swtpm"'`.
    assert!(
        src.contains(r#"stablePrincipalId "d2b-${name}-swtpm""#),
        "swtpm userNamespace must reference the swtpm principal \
         (stablePrincipalId \"d2b-${{name}}-swtpm\"), not the runner principal, in \
...
// tests/minijail-validator-video.sh
//
// Layer-1 (always-on) ports below. Layer-2 (D2B_LIVE=1) is NOT ported: it
...
// ACL, and writes /var/lib/d2b/validated/p1-video.json evidence. That is a
...
/// `d2b-<vm>-video` principal, NOT gpu). The `videoNvidiaDecode` gating
        ...
        for node in &dag.nodes {
            ...
            assert!(
                ...
                "video {} (vm {}) must run as a DEDICATED principal distinct from gpu \
                 (AGENTS: d2b-<vm>-video, NOT gpu); video uid {} collides with a gpu uid \
                 in {:?}",
                p.profile_id,
                dag.vm,
                p.uid,
...
/// and bind the role to the dedicated `d2b-${name}-video` principal. The
...
fn video_source_nvidia_gating_namespace_principal() {
    ...
    assert!(
        window.contains(r#"principal = "d2b-${name}-video""#),
        "video profile must bind the dedicated d2b-${{name}}-video principal (NOT gpu) in \
         {MINIJAIL_PROFILES_NIX}"
    );
}
...
/// /run/d2b-video`, the `u:$uid:---` deny entries, and the
