...
//! D2B_FIXTURES / D2B_FIXTURES_FULL contract step in
...
//! bundle (D2B_FIXTURES_FULL), whose `corp-full` VM enables graphics + video +
...
//! D2B_FIXTURES_FULL is unset (a non-x86_64 host, where the graphics platform
...
//!         (`ptrace(PTRACE_TRACEME)` must raise SIGSYS), the `D2B_LIVE=1` bare
//!         GET_CAPS hardware smoke on the host render node, and the
//!         `/var/lib/d2b/validated/p1-gpu.json` evidence write.
//!       - wayland-proxy: the `D2B_LIVE=1` Layer-2 `minijail0 /bin/true` positive
//!         probe and the `/var/lib/d2b/validated/p1-wayland-proxy.json`
//!         evidence write.
//!
//! Spec corrections / smoke-fixture gaps:
//!   * gpu `SOURCE_WAYLAND` (`/run/user/<uid>/wayland-0`) and `BIND_TARGET`
//!     (`/run/d2b-gpu/<vm>/wayland-0`): the bash gate's lines 100-103 are
...
//!     `packages/d2b-priv-broker/src/{live_handlers,sys}.rs`, and
...
const BUNDLE_RESOLVER_RS: &str = "packages/d2b-core/src/bundle_resolver.rs";
const LIVE_HANDLERS_RS: &str = "packages/d2b-priv-broker/src/live_handlers.rs";
const SYS_RS: &str = "packages/d2b-priv-broker/src/sys.rs";
const PROCESSES_RS: &str = "packages/d2b-core/src/processes.rs";
const BROKER_WIRE_RS: &str = "packages/d2b-contracts-broker/src/broker_wire.rs";
...
/// in-sandbox wayland BIND_TARGET convention: `/run/d2b-gpu/<vm>` is a
/// writable path, so `/run/d2b-gpu/<vm>/wayland-0` lives under it. The
...
fn gpu_rendered_device_bind_matrix_and_wayland_target() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP: D2B_FIXTURES_FULL unset (feature-rich fixture unavailable)");
        return;
    };
    let mut seen = 0usize;
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            // `/run/d2b-gpu/<vm>` must be a writable path.
            let target_parent = format!("/run/d2b-gpu/{}", dag.vm);
...
/// reference, the `d2b.slice/<vm>/gpu` cgroup subtree, mount+ipc namespace
...
fn gpu_render_node_minijail_profile_source_shape() {
    ...
    assert!(
        src.contains(r#"stablePrincipalId "d2b-${name}-gpu""#),
...
///   * the writable path `/run/d2b-wlproxy` is present;
...
fn wayland_proxy_profile_source_shape() {
    ...
    assert!(
        src.contains("/run/d2b-wlproxy"),
        "writable path /run/d2b-wlproxy not found in {MINIJAIL_PROFILES_NIX}"
    );
...
/// `packages/d2b-priv-broker/src/live_handlers.rs` MUST carry a
...
/// role variant MUST exist in both `packages/d2b-core/src/processes.rs`
/// (`ProcessRole::WaylandProxy`) and `packages/d2b-contracts-broker/src/broker_wire.rs`
...
/// gate, which scanned `/etc/d2b/minijail-profiles/*wayland-proxy*.json` and
...
/// profile bind), the dedicated runtime dir `/run/d2b-wlproxy/<vm>` as the
...
fn wayland_proxy_rendered_profile_shape() {
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            let runtime_dir = format!("/run/d2b-wlproxy/{}", dag.vm);
            assert!(
                p.mount_policy
                    .writable_paths
                    .iter()
