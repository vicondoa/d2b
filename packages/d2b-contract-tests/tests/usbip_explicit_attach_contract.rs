...
fn explicit_attach_uses_same_busid_validation_as_declared_path() {
    ...
    // `d2b_contracts::usbip::validate_bus_id` as the declared path.
...
#[test]
fn lock_path_for_busid_is_scoped_to_run_d2b_locks_usbip() {
...
fn explicit_firewall_rule_carries_per_env_uplink_not_shared_subnet() {
    ...
    // exactly the target env bridge, not all d2b envs.
    let req = UsbipExplicitFirewallRuleRequest {
        bus_id: "1-2".to_owned(),
        env: "corp".to_owned(),
        host_uplink_ip: "192.0.2.1".to_owned(),
    ...
    // this env's bridge, not open a wildcard across all d2b envs.
    assert!(
        !req.env.is_empty(),
...
fn explicit_ops_appear_in_broker_runtime_dispatch() {
    let runtime = read_repo_file("packages/d2b-priv-broker/src/runtime.rs");
    assert!(
...
fn usb_detach_dispatches_host_unbind_instead_of_static_ambiguous_refusal() {
    ...
    assert!(
        ...
        "d2b usb detach must dispatch the broker UsbipUnbind path so stale host claims can be released"
    );
    assert!(
    ...
    assert!(
        ...
        "d2b usb detach must not hardcode an ambiguous-flow refusal before trying broker cleanup"
    );
}
