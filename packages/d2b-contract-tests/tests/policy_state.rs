...
//!
//! NOTE: the retired launcher group-name literals (`d2b-` + `launcher{,s}`)
...
//
// Asserts the canonical `d2b` lifecycle-group rename semantics: the group
// is declared, launcher users join it, the daemon's publicSocketGroup is
// `d2b`, the state-dir ACL launcher group is `d2b`, the ACL helper
...
fn group_rename_semantic() {
    ...
    let legacy = concat!("d2b-", "launcher"); // singular tombstone group name
    let legacy_plural = concat!("d2b-", "launchers"); // plural tombstone group name

    require_match(
        &host_users,
        r"d2b = \{ \};",
        "users.groups.d2b declaration missing",
    );
    require_match(
        &host_users,
    ...
    require_match(
        &host_daemon,
        r#"publicSocketGroup = "d2b";"#,
        "daemon publicSocketGroup is not d2b",
    );
    require_match(
        &host_activation,
        r"LAUNCHER_GROUP=d2b",
        "state-dir ACL launcher group is not d2b",
    );
    require_match(
        &acl_helper,
...
fn group_migration_fresh_install() {
    ...
    for needle in [
        ...
        r#"lib.stringAfter [ "users" "d2bGroupMigration" ]"#,
    ] {
        assert!(
    ...
    // awk port: extract the d2bGroupMigration block (from the line declaring
    ...
    for line in activation.lines() {
        if line.contains("system.activationScripts.d2bGroupMigration") {
            in_block = true;
        }
        if in_block {
...
//
// v1.1 invariant gate over `/var/lib/d2b` state-dir posture:
//   (a) declared `0750 root d2bd` in host-daemon.nix tmpfiles (NOT 0755);
//   (b) tmpfiles ACL rules grant per-user/per-group `--x` traversal
//       (incl. `kvm` as a GROUP and the `d2b` lifecycle group);
...
fn state_dir_acl() {
    ...
    if !line_matches(&daemon, r#""d /var/lib/d2b 0750 root d2bd"#) {
        failures.push("/var/lib/d2b not declared `0750 root d2bd`".into());
    }
    // (a) 0755 workaround absent (in either module).
    if line_matches(&daemon, r#""d /var/lib/d2b 0755"#)
        || line_matches(&activation, r#""d /var/lib/d2b 0755"#)
    {
        failures.push("found `0755 /var/lib/d2b` workaround".into());
    }
    // (b) tmpfiles `u:<user>:--x` traversal grants on the state dir.
    if !line_matches(
        &activation,
        r#"tmpfilesAcl[[:space:]]+"/var/lib/d2b"[[:space:]]+"u:[^"]+:--x""#,
    ) {
    ...
    if !line_matches(
        &activation,
        r#"tmpfilesAcl[[:space:]]+"/var/lib/d2b"[[:space:]]+"g:kvm:--x""#,
    ) {
        failures.push("tmpfiles `g:kvm:--x` grant missing (kvm is a Linux group)".into());
    }
    // (b) `d2b` lifecycle group traversal grant.
    if !line_matches(
        &activation,
        r#"tmpfilesAcl[[:space:]]+"/var/lib/d2b"[[:space:]]+"g:d2b:--x""#,
    ) {
        failures.push("tmpfiles `g:d2b:--x` /var/lib/d2b traversal grant missing".into());
    }
    // (c) NO default ACL on the state-dir root (it would widen per-VM subdir
    // surface). Comment lines are excluded.
    let default_acl =
        Regex::new(r#"tmpfilesAcl[[:space:]]+"/var/lib/d2b"[[:space:]]+"default:"#).unwrap();
    let comment = Regex::new(r"^[[:space:]]*#").unwrap();
    for line in activation.lines() {
        if default_acl.is_match(line) && !comment.is_match(line) {
            failures.push(format!(
                "found tmpfiles default ACL on /var/lib/d2b root: {line}"
            ));
        }
    }
...
fn store_sync_export() {
    ...
    let wants: &[(&str, &str)] = &[
        ...
        (
            "StoreSync resource marks service.name as d2b-store-sync",
            r#"key[[:space:]]*=[[:space:]]*"service.name";[[:space:]]*value[[:space:]]*=[[:space:]]*"d2b-store-sync""#,
        ),
        (
        ...
        (
            ...
            r#"setfacl -m u:d2b-host-otel-collector:rw,m::rw \$\{hostEgressSocket\}"#,
        ),
    ];
    ...
    let denies: &[(&str, &str)] = &[
        ...
        (
            ...
            r#"setfacl.*d2b-host-otel-collector.*audit"#,
        ),
    ];
...
fn host_otlp_ingest_socket_isolation() {
    ...
    let wants: &[(&str, &str)] = &[
        ...
        (
            ...
            r#"d \$\{otelIngestDir\} \$\{ingestDirMode\} d2b-host-otel-collector \$\{ingestGroup\}"#,
        ),
    ];
...
fn host_journald_cursor_provisioned() {
    ...
    let wants: &[(&str, &str)] = &[
        (
            ...
            r#"install -d -m 0700 -o d2b-host-otel-collector -g d2b-host-otel-collector \$\{journaldStorageDir\}"#,
        ),
        (
