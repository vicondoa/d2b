...
//! the fixture-contract layer (this crate, gated by the D2B_FIXTURES /
//! D2B_FIXTURES_FULL contract steps in `tests/tools/rust-workspace-checks.sh`), not the
...
//! (`D2B_FIXTURES_FULL`, the `corp-full` VM with graphics+video+audio+tpm+usbip+
...
//!   * Each gate's opt-in `D2B_LIVE=1` Layer-2 phases (spawning `minijail0` +
//!     `vhost-device-sound` / `usbip version`, the SYS_ptrace -> SIGSYS probe,
//!     optional busid bind/unbind, and writing
//!     `/var/lib/d2b/validated/p1-{audio,usbip}.json` evidence) are runtime
...
//!     faithfully; the only skipped coverage is the documented `D2B_LIVE=1`
...
/// `d2b.slice/<vm>/audio` cgroup subtree.
///
/// Skips when `D2B_FIXTURES_FULL` is unset (the audio profile only renders on the
...
fn audio_rendered_profile_broker_pre_ns_shape() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!(
            "SKIP: D2B_FIXTURES_FULL unset; audio profile only renders on fixture-smoke-full"
        );
        return;
    };
    let mut seen = 0usize;
...
fn usbip_proxy_source_has_no_host_privilege_surface() {
    ...
    assert!(
        any_line_matches(&proxy_profile, r#"principal\s*=\s*"d2b-\$\{vmId\}-proxy""#),
...
/// `d2b.slice/sys-work-usbipd/{backend,proxy}`.
///
/// Skips when `D2B_FIXTURES_FULL` is unset (usbip only renders when an env has a
/// `usbip.yubikey` VM, which the default fixture lacks).
#[test]
fn usbip_rendered_usbipd_backend_proxy_shape() {
    let Some(resolver) = load_full_bundle_resolver_from_env() else {
        eprintln!("SKIP: D2B_FIXTURES_FULL unset; usbipd nodes only render on fixture-smoke-full");
        return;
    };
    ...
    for dag in &resolver.processes.vms {
        for node in &dag.nodes {
            ...
            if p.profile_id.ends_with("-usbipd-backend") {
                ...
                assert_eq!(
                    p.cgroup_placement.subtree,
                    format!("d2b.slice/{}/backend", dag.vm),
