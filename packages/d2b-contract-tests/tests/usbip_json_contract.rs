...
fn public_usb_probe_json_schema_matches_dto_semantics() {
    let payload = UsbipProbeResponse {
        entries: vec![
            ...
            UsbipProbeEntry {
                ...
                lock_path: "/run/d2b/locks/usbip/1-2.3".to_owned(),
                ...
                follow_up_command: Some("d2b usb attach media 1-2.3 --apply".to_owned()),
                ..Default::default()
            },
        ],
    };
    ...
    assert_eq!(entries[0]["durableClaim"]["state"], "held-by-desired-owner");
    assert_eq!(entries[0]["host"]["bind"], "bound-to-usbip-host");
    assert_eq!(entries[0]["guest"]["import"], "imported");
    assert_eq!(entries[0]["topologyPolicy"]["policy"], "allowed");
    assert!(
