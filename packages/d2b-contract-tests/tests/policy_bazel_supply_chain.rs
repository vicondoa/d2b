...
fn generated_policy_inputs_are_not_forged_as_a_second_workspace_authority() {
    if repo_path_exists("packages/policy-inputs") {
        ...
        fn validate_policy_metadata(
            ...
            for (node_id, node) in &node_by_id {
                ...
                if dependency_ids.len() != dependencies.len()
                    || !dependency_ids.is_subset(&node_id_set)
                {
                    ...
                    for kind in kinds {
                        ...
                        if let Some(target) = field(kind, "target") {
                            if !target.is_null() && !target.is_string() {
                                return Err(format!(
                                    "{node_id} dependency target cfg is malformed"
                                ));
                            }
                        }
                    }
        ...
        fn validate_selected_policy_context(
            context: SelectedPolicyContext,
        ) -> BTreeSet<PolicyIdentity> {
            ...
            let identities = validate_policy_metadata(&policy, &policy_lock, context)
        ...
        fn selected_policy_artifact_mutations_fail_closed() {
            ...
            document["resolve"]["nodes"][root_node]["deps"][0]["pkg"] =
                Value::String(alternate.to_owned());
            assert!(!valid(&document, &lock));
            ...
            document = read_json(&format!("{root}/metadata.json"));
            document["resolve"]["nodes"][root_node]["deps"][0]["dep_kinds"] =
                Value::Array(Vec::new());
            assert!(!valid(&document, &lock));
