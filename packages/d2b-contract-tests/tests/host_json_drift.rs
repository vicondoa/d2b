...
//! `d2b_core::host::HostJson` DTO shape in places, so the golden checks
...
fn baseline_host_json_required_fields_and_ifnames() {
    ...
    let ifname_re =
        Regex::new(r"^d2b-[a-z][a-z0-9-]*-(XXXXXXXX|[0-9a-f]{8})$").expect("valid ifname regex");
    ...
    for env in environments {
        ...
        assert!(
            ...
            "baseline-host.json: bridge {bridge:?} does not match d2b-<env>-(XXXXXXXX|[0-9a-f]{{8}})"
        );
        assert!(
            bridge.len() <= 15,
...
//
// The shell gate skipped this assertion when d2b_smoke_bundle_host_json was not
// available. This Rust migration preserves that behavior by checking
// D2B_FIXTURES/host.json only when the fixture environment is provided; the
// always-run committed fixture checks above do not require D2B_FIXTURES.
// ---------------------------------------------------------------------------
#[test]
