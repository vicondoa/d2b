...
//! | role socket     | `socket:vm:<vm>:role:<role_id>`                           | [`crate::processes::ProcessNode`] (derived `/run/nixling/vms/...`)|
//!
...
impl HostRuntimeArtifact {
    pub fn new(runtime: HostRuntime) -> Self {
        Self {
            path: PathBuf::from("/var/lib/nixling/runtime/host-runtime.json"),
            runtime,
        }
    }
...
impl BundleVerifyPolicy {
    /// Production policy: owned by `root:nixlingd`, mode 0640.
    ///
    /// Looks up the `nixlingd` group by parsing `/etc/group`.
    /// If the group does not exist the GID check is skipped.
    pub fn production() -> Self {
        Self {
            required_uid: 0,
            required_gid: lookup_group_gid("nixlingd"),
            required_mode: 0o640,
        }
    }
...
fn verify_bundle_hash(path: &Path, raw_bytes: &[u8]) -> Result<(), Error> {
    ...
    let expected = match value.as_object_mut().and_then(|o| o.remove("bundleHash")) {
        None => {
            ...
            eprintln!(
                "nixling: warning: bundle artifact {} has no bundleHash field; \
...
impl BundleResolver {
    ...
    ///
    /// Uses [`BundleVerifyPolicy::production`]: owned by `root:nixlingd`,
    ...
    pub fn resolve_prepare_dir_intent(
        ...
        let base_dir = if runtime_dir {
            PathBuf::from(format!("/run/nixling/vms/{vm_id}"))
        } else {
    ...
    pub fn validate_minijail_profiles(&self) -> Result<usize, MinijailProfileViolation> {
        ...
        for dag in &self.processes.vms {
            for node in &dag.nodes {
                ...
                if !p.cgroup_placement.subtree.starts_with("nixling/")
                    && !p.cgroup_placement.subtree.starts_with("nixling.slice/")
                    && !p.cgroup_placement.subtree.is_empty()
                {
                    return Err(MinijailProfileViolation::CgroupSubtreeOutsideNixling {
                        profile_id: p.profile_id.clone(),
...
impl std::fmt::Display for MinijailProfileViolation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CgroupSubtreeOutsideNixling {
                profile_id,
                subtree,
            } => write!(
                f,
                "minijail profile {profile_id} cgroup subtree {subtree} is outside the delegated nixling root"
            ),
        }
    }
}
...
fn render_host_nft_script(host: &HostJson) -> String {
    ...
    let comment = if model.ownership_id.is_empty() {
        ...
        format!(" comment \"nixling managed: {}\"", model.ownership_id)
    };
    buf.push_str(&format!(
        "table {} {} {{\n",
...
fn render_env_nft_subset(host: &HostJson, env: &NetEnv) -> String {
    ...
    buf.push_str(&format!(
        "# nixling env nft subset for {} (bridge {})\n",
...
fn render_hosts_managed_block(host: &HostJson) -> String {
    ...
    buf.push('\n');
    buf.push_str("# managed by nixling broker — do not edit by hand\n");
    for env in &host.environments {
        buf.push_str(&format!(
            "# env {} bridge {} mtu {}\n",
            env.env,
...
fn build_nm_unmanaged_intents(host: &HostJson) -> BTreeMap<String, ResolvedNmUnmanagedIntent> {
    let mut out = BTreeMap::new();
    let mut contents = String::from("# managed by nixling broker — do not edit by hand\n");
    ...
    for env in &host.environments {
        for lock in &env.usbip_busid_locks {
            for bus_id in synthesize_bus_ids(lock) {
                ...
                let lock_path = PathBuf::from(format!("/run/nixling/locks/usbip/{}", &bus_id));
                out.insert(
                    intent_id.clone(),
...
fn build_socket_intents(processes: &ProcessesJson) -> BTreeMap<String, ResolvedSocketIntent> {
    ...
    for dag in &processes.vms {
        for node in &dag.nodes {
            ...
            let socket_path =
                PathBuf::from(format!("/run/nixling/vms/{}/{}.sock", dag.vm, node.id.0));
            out.insert(
                intent_id.clone(),
...
fn build_installer_intents(bundle: &Bundle) -> BTreeMap<String, ResolvedInstallerIntent> {
    let mut out = BTreeMap::new();
    let bundle_path = PathBuf::from("/var/lib/nixling/current-bundle/manifest.json");
    let artifacts = vec![
        InstallerArtifact {
            path: PathBuf::from("/etc/systemd/system/nixlingd.service"),
            mode: 0o644,
            purpose: "nixlingd systemd unit (non-NixOS host)".to_owned(),
        },
        InstallerArtifact {
            path: PathBuf::from("/etc/nixling/daemon-config.json"),
            mode: 0o640,
            purpose: "daemon configuration file consumed by nixlingd".to_owned(),
        },
        InstallerArtifact {
    ...
    out.insert(
        ...
        ResolvedInstallerIntent {
            ...
            unit_path: PathBuf::from("/etc/systemd/system/nixlingd.service"),
            service_name: "nixlingd.service".to_owned(),
            daemon_config_path: PathBuf::from("/etc/nixling/daemon-config.json"),
            bundle_path,
            artifacts,
        },
    );
...
mod tests {
    ...
    fn build_personal_dev_bundle(root: &Path) -> BundleResolver {
        ...
        let runtime_dir = "/run/nixling/personal-dev";
    ...
    fn host_reconcile_and_store_preflight_emit_executable_vm_start_intents() {
        ...
        assert!(matches!(
            ...
            ] if runtime.base_dir == Path::new("/run/nixling/vms/personal-dev")
                && state.base_dir == Path::new("/var/lib/nixling/vms/personal-dev")
        ));
        ...
        assert!(matches!(
            ...
            [ResolvedVmStartAction::PrepareStoreView(intent)]
                if intent.hardlink_farm_path == Path::new("/var/lib/nixling/vms/personal-dev/store-view")
        ));
