...
pub(crate) struct HostCheckFixture {
    ...
    nft_has_nixling_table: Option<bool>,
...
impl ServiceProbeKind {
    ...
    const fn active_env(self) -> &'static str {
        match self {
            Self::Firewalld => "NIXLING_TEST_FIREWALLD_ACTIVE",
            Self::Ufw => "NIXLING_TEST_UFW_ACTIVE",
        }
    }
...
impl ProbeSource {
    fn from_env() -> Result<Self, ProbeError> {
        let fixture = match env::var_os("NIXLING_HOST_CHECK_FIXTURE") {
            Some(path) => {
    ...
    fn nft_has_nixling_table(&self) -> Result<bool, ProbeError> {
        if let Some(detail) = self
            .fixture
            .as_ref()
        ...
        if let Some(value) = self
            .fixture
            .as_ref()
            .and_then(|fixture| fixture.nft_has_nixling_table)
        {
            return Ok(value);
        }
                    ...
                    entry
                        ...
                        .map(|table| {
                            table.get("family").and_then(Value::as_str) == Some("inet")
                                && table.get("name").and_then(Value::as_str) == Some("nixling")
                        })
                        .unwrap_or(false)
                })
            })
    ...
    fn service_state(&self, kind: ServiceProbeKind) -> Result<ServiceProbeState, ProbeError> {
        ...
        if env_bool("NIXLING_TEST_SYSTEMCTL_UNAVAILABLE").unwrap_or(false)
            || self
                .fixture
                .as_ref()
    ...
    I: IntoIterator<Item = &'a ClosureMetadata>,
{
    ...
    let nft_has_nixling_table = probe.nft_has_nixling_table()?;
    ...
    for module in &host.kernel_modules {
        ...
        // iptables/ip6tables/arptables outside the inet nixling
    ...
    findings.push(if nft_has_nixling_table {
        finding(
            "nftables-table",
            HostCheckSeverity::Pass,
            "`nft list ruleset --json` shows an `inet nixling` table".to_owned(),
            "No action required.",
        )
    } else {
        finding(
        ...
        for sysctl in &env.ipv6_sysctls {
            for (field, expected) in [
                ...
                findings.push(HostCheckFinding {
                    ...
                    remediation: "Ensure the declared nixling IPv6 sysctls are applied to every env bridge and tap.".to_owned(),
                    vm: None,
                    detail: None,
                    details,
                });
...
mod module_sysctl_tests {
    ...
    fn br_netfilter_host(loaded: bool) -> HostJson {
        ...
        host.kernel_modules
            .push(crate::host::KernelModulesEntry {
                ...
                gate: "Optional, but if present nixling fails closed unless all bridge-nf-call sysctls are zero.".to_owned(),
                sysctls: vec![
    ...
    fn baseline_fixture(loaded_br_netfilter: bool) -> HostCheckFixture {
        let mut fixture = HostCheckFixture {
            kernel_release: Some("6.8.12-nixling".to_owned()),
            ...
            nft_has_nixling_table: Some(true),
            nft_error: None,
