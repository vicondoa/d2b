...
//! (and Nix emitters under `nixos-modules/`) and the nixling-host
...
pub struct RouteIntent {
    ...
    /// Whether this route is owned (and managed) by nixling.
    #[serde(default)]
    pub owned: bool,
}
...
pub struct FirewallCoexistencePolicy {
    ...
    /// What nixling does when this manager is detected.
...
/// What nixling does when a given firewall manager is present.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "kebab-case")]
pub enum CoexistencePolicy {
    /// Coexist: install `inet nixling` table alongside the manager's
    ...
    /// manager covering nixling interfaces before proceeding.
    RequireUnmanaged,
}
