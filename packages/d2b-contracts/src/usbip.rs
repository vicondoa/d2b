...
//!
//! `d2b usb attach <vm> <present-busid> --apply` allows any present USB
...
/// or from an explicit `d2b usb attach <vm> <busid> --apply` invocation.
///
...
pub enum UsbipClaimSource {
    ...
    /// Originated from an explicit `d2b usb attach <vm> <busid> --apply`
...
/// `/run/d2b/locks/usbip/<busid>` that the broker opens/acquires for the
...
impl UsbipDaemonClaimRecord {
    ...
    pub fn lock_path_for_busid(busid: &str) -> String {
        format!("/run/d2b/locks/usbip/{busid}")
    }
}
