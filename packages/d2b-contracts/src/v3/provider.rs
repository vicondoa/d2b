...
//!
//! This module will own the `Provider` ResourceType surface named by
//! `ADR-046-provider-model-and-packaging` work item `ADR046-provider-001`:
//! the Provider resource spec and status, the signed Provider manifest and
//! its component descriptors, the Provider dependency and service
//! declarations, and the trust and compatibility bindings a Zone evaluates
//! before it admits a Provider.
//!
...
/// `ADR046-provider-001` should delete it rather than build on it.
