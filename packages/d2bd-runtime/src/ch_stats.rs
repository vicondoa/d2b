...
//!
//! Background. The legacy `d2b-ch-exporter.service` was a host
...
//! the d2bd daemon already owns a public socket and an HTTP
...
//! `d2b_vm_ch_api_up`, `d2b_vm_state`, `d2b_vm_running`.
//! Two new gauges describe the CH-reported VM shape:
//! `d2b_vm_ch_vcpu_count` and `d2b_vm_ch_memory_bytes`.
//!
...
pub fn render_ch_stats(
    ...
    out.push_str(
        "# HELP d2b_vm_ch_api_up Whether the VM Cloud Hypervisor API responded to /vmm.ping.\n",
    );
    out.push_str("# TYPE d2b_vm_ch_api_up gauge\n");
    out.push_str(
        "# HELP d2b_vm_state Cloud Hypervisor VM state exported as a one-hot gauge per state label.\n",
    );
    out.push_str("# TYPE d2b_vm_state gauge\n");
    out.push_str("# HELP d2b_vm_running Whether the host currently considers the VM running.\n");
    out.push_str("# TYPE d2b_vm_running gauge\n");
    out.push_str(
        "# HELP d2b_vm_ch_vcpu_count Boot vCPU count reported by Cloud Hypervisor vm.info.\n",
    );
    out.push_str("# TYPE d2b_vm_ch_vcpu_count gauge\n");
    out.push_str("# HELP d2b_vm_ch_memory_bytes Configured guest memory in bytes reported by Cloud Hypervisor vm.info.\n");
    out.push_str("# TYPE d2b_vm_ch_memory_bytes gauge\n");
    ...
    for (input, stats) in &scrapes {
        ...
        for st in &states {
            ...
            out.push_str(&format!(
                "d2b_vm_state{{{labels},state=\"{escaped}\"}} {v}\n"
            ));
        }

        let running_v = if running.is_running(&input.vm) { 1 } else { 0 };
        out.push_str(&format!("d2b_vm_running{{{labels}}} {running_v}\n"));

        if let Some(vcpus) = stats.vcpu_count {
            out.push_str(&format!("d2b_vm_ch_vcpu_count{{{labels}}} {vcpus}\n"));
        }
        if let Some(memory) = stats.memory_bytes {
            out.push_str(&format!("d2b_vm_ch_memory_bytes{{{labels}}} {memory}\n"));
        }
    }
...
fn opaque_label(value: &str) -> String {
    d2b_contracts_resource::v3::canonical_digest("d2b:daemon-metric-label:v1", value.as_bytes())
}
...
mod tests {
    ...
    fn vm(name: &str, env: &str, role: &str) -> ChVmInput {
        ChVmInput {
            ...
            env: env.to_owned(),
            role: role.to_owned(),
            api_socket: format!("/run/d2b/{name}.sock"),
        }
    }
    ...
    fn render_emits_help_and_type_headers() {
        ...
        assert!(body.contains("# HELP d2b_vm_ch_api_up"));
        assert!(body.contains("# TYPE d2b_vm_ch_api_up gauge"));
        assert!(body.contains("# HELP d2b_vm_state"));
        assert!(body.contains("# TYPE d2b_vm_state gauge"));
        assert!(body.contains("# HELP d2b_vm_running"));
        assert!(body.contains("# TYPE d2b_vm_running gauge"));
        assert!(body.contains("# HELP d2b_vm_ch_vcpu_count"));
        assert!(body.contains("# HELP d2b_vm_ch_memory_bytes"));
    }
    ...
    fn no_vcpu_or_memory_lines_when_missing() {
        ...
        assert!(!body.contains("d2b_vm_ch_vcpu_count{vm=\"noinfo\""));
        assert!(!body.contains("d2b_vm_ch_memory_bytes{vm=\"noinfo\""));
    }
