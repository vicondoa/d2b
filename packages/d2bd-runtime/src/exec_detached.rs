...
//! [`ResourceDetachedClient`] and the authenticated Resource API. The former
//! guest-control RPC adapter remains available only to test characterization
//! callers so the daemon composition cannot reach it.
...
use d2b_contracts_control::guest_wire::ExecState as PublicExecState;
...
mod tests {
    ...
    async fn run_detached_request_bridges_runtime_without_panicking() {
        let params = ProbeParams {
            vm_id: "vm-a".to_owned(),
            socket_path: PathBuf::from("/nonexistent/d2b/vm-a/vsock.sock"),
            state_root: PathBuf::from("/nonexistent/d2b/vm-a"),
