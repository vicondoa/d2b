...
use nixling_constellation_core::{
    Capability, CapabilitySet, StreamDescriptor, StreamKind, StreamOpen,
};
use nixling_constellation_provider::capabilities::DisplayCapabilitySet;
use nixling_constellation_provider::error::{ProviderError, ProviderResult};
use nixling_constellation_provider::provider::DisplayProvider;
...
impl Default for WaypipeRunnerConfig {
    fn default() -> Self {
        Self {
            ...
            gateway_relay_bin: "nixling-gateway-relay".into(),
            host_socket: "/run/nixling/gateway-display/waypipe.sock".into(),
            display_name: "wayland-nl".into(),
...
pub fn host_waypipe_service(cfg: &WaypipeRunnerConfig) -> WaypipeSystemdService {
    WaypipeSystemdService {
        name: "nixling-gateway-waypipe-client.service".into(),
...
pub fn guest_waypipe_service(cfg: &WaypipeRunnerConfig, socket: &str) -> WaypipeSystemdService {
    WaypipeSystemdService {
        name: "nixling-gateway-waypipe-server.service".into(),
...
impl DisplayProvider for WaypipeDisplayProvider {
    fn provider_id(&self) -> nixling_constellation_core::ProviderId {
        nixling_constellation_core::ProviderId::parse("waypipe").expect("valid provider id")
    }
...
mod tests {
    ...
    fn argv_shapes_are_shm_only_no_gpu() {
        ...
        assert!(guest.windows(2).any(|w| w == ["--display", "wayland-nl"]));
        assert_eq!(
            gated_relay_sender_argv(&cfg),
            ["nixling-gateway-relay", "sender"]
        );
    }
