...
This is the crate root for the `EmergencyPolicy` resource type. It owns the type's
...
serves `EmergencyPolicy` rows through the `ResourceDriver` verbs: `validate` decodes the
...
`EmergencyPolicyDriverFactory` is the registration surface; `emergency_policy_descriptor` carries
