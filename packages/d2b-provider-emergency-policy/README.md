...
This is the crate root for the `EmergencyPolicy` resource type. It owns the type's identity
...
| Field | Value |
| --- | --- |
| Provider name | `emergency-policy` |
| ResourceType | `EmergencyPolicy` |
| Package | `packages/d2b-provider-emergency-policy/` |
| Driver declaration | `emergency_policy_descriptor` -> `DriverDescriptor` |

## Config schema

The type declares no provider config schema: `EmergencyPolicy` rows carry the JSON spec
object the core rows store, and nothing outside it decodes at validate.

## Exported resource types

`EmergencyPolicy` is not exportable. `ResourceExport` admits only qualified
`*.d2bus.org.*Service` types, so a `EmergencyPolicy` row is never an export subject. The
declaration carries `exportable: false`.

## Controllers / services / workers / binaries

The crate ships the type's declaration, not a standalone process. The shared
driver serves `EmergencyPolicy` rows through the `ResourceDriver` verbs: `validate` decodes the
...
The registration surface is `emergency_policy_descriptor`: it declares the type through the shared
...
`EmergencyPolicy` names no placement anchor, so a `EmergencyPolicy` row is reconciled on its
...
```bash
cargo test -p d2b-provider-emergency-policy
```
