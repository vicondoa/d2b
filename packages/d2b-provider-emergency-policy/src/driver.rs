...
//!
//! `EmergencyPolicy` is a zone's emergency posture row: the driver converges it as metadata once its desired state is admitted.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`EmergencyPolicyDriverFactory`] registration through the type's
//!   descriptor ([`emergency_policy_descriptor`]).
...
pub const EMERGENCY_POLICY_TYPE_NAME: &str = "EmergencyPolicy";

/// The resource verbs the `EmergencyPolicy` type supports.
///
...
/// this declaration.
const EMERGENCY_POLICY_VERBS: &[&str] = &[
    "get",
    "list",
    "watch",
    "create",
...
/// The execution domains the `EmergencyPolicy` type can be reconciled in.
///
/// Derived from the placement contract: `EmergencyPolicy` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `EmergencyPolicy` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const EMERGENCY_POLICY_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `EmergencyPolicy` driver reads while reconciling.
///
...
const EMERGENCY_POLICY_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `EmergencyPolicy` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn emergency_policy_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `EmergencyPolicy` resource type. Construction is
...
impl EmergencyPolicyDriverFactory {
    ...
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(EMERGENCY_POLICY_TYPE_NAME)],
        }
    }
}

impl Default for EmergencyPolicyDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
/// One `EmergencyPolicy` resource's driver.
...
/// `*.d2bus.org.*Service` types, so a `EmergencyPolicy` row is never an export subject.
pub fn emergency_policy_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        ...
        verbs: EMERGENCY_POLICY_VERBS,
        execution: EMERGENCY_POLICY_EXECUTION_DOMAINS,
        exportable: false,
        reads: EMERGENCY_POLICY_READS,
        operations: &[],
        creations: &[],
        startup: &[],
        services: &[],
        decoder: emergency_policy_spec_decoder(),
        factory: Arc::new(EmergencyPolicyDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        EMERGENCY_POLICY_TYPE_NAME, EmergencyPolicyDriverFactory, emergency_policy_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", EMERGENCY_POLICY_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = EmergencyPolicyDriverFactory::new()
            .create(&ResourceKey::new("work", EMERGENCY_POLICY_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
