...
//!   backend trait (`d2b_host::netlink::NetlinkBackend`) so the
...
fn ip_binary_path() -> PathBuf {
    std::env::var_os("D2B_BROKER_IP_BINARY")
        .map(PathBuf::from)
...
mod tests {
    ...
    fn live_bridge_flag_resolver() -> BundleResolver {
        ...
        let host = BundleHostJson {
            ...
            environments: vec![NetEnv {
                ...
                ipv6_sysctls: vec![d2b_core::host::Ipv6SysctlEntry {
            ...
            nftables: NftablesModel {
                ...
                ownership_id: "d2b-test".to_owned(),
            },
            network_manager: d2b_core::host::NetworkManagerUnmanaged {
                file_path: "/etc/NetworkManager/conf.d/00-d2b-unmanaged.conf".to_owned(),
                match_criteria: Vec::new(),
                ownership: d2b_core::host::OwnershipRule {
                    owner: "root".to_owned(),
                    group: "root".to_owned(),
                    mode: "0644".to_owned(),
            ...
            if_name_mappings: vec![
                IfNameMapping {
                    ...
                    derived_ifname: BundleIfName::new("d2b-bWORK000").expect("derived bridge"),
                },
                IfNameMapping {
                    env: "work".to_owned(),
                    ...
                    derived_ifname: BundleIfName::new("d2b-tWORK010").expect("derived tap"),
                },
            ],
            qemu_media: None,
            ch: None,
    ...
    fn build_bridge_port_ip_args_uses_bridge_slave_netlink_surface() {
        ...
        .join(" ");
        assert!(rendered.starts_with("link set dev d2b-tWORK010 type bridge_slave"));
        for needle in [
            "hairpin off",
            "learning on",
            "flood off",
