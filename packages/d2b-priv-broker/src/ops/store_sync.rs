...
//! `/var/lib/d2b/vms/<vm>/store/` hardlink farm.
//!
//! Replaces the `d2b-<vm>-store-sync.service` per-VM systemd
//! oneshot (today: a bash script that hardlink-farms `/nix/store`
//! closure entries into `/var/lib/d2b/vms/<vm>/store`) with a
//! typed broker op.
//!
...
mod tests {
    ...
    fn happy_path_populates_split_layout_and_swaps_currents() {
        ...
        // d2bd/d2b/users principals resolve to the current uid/gid.
    ...
    fn build_error_stage_classifies_topology_vs_materialise() {
        ...
        assert_eq!(
            build_error_stage(&HardlinkFarmError::CrossMountLink {
                ...
                destination: "/var/lib/d2b/x".to_owned(),
                dev: 1,
            }),
            ErrorStage::Stage
        );
