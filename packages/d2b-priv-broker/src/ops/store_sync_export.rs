...
//! is `0640 root:d2bd` and carries host-only context the
...
//! unified `/var/lib/d2b/audit/broker-*.jsonl` stream.
//!
...
//! `/var/lib/d2b/observability/store-sync/`). The host Nix/Alloy
...
pub const DEFAULT_STORE_SYNC_EXPORT_DIR: &str = "/var/lib/d2b/observability/store-sync";
...
mod tests {
    ...
    fn redaction_fields_are_absent_from_every_terminal_shape() {
        ...
        for audit in &records {
            ...
            assert!(
                !text.contains("/var/lib/d2b/vms/corp-vm/store-view"),
