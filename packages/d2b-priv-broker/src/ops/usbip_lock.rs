...
//!
//! Per `d2b_core::host::UsbipBusidLock`, every USBIP-capable VM
//! claims its busid via a broker-owned exclusivity record at
//! `/run/d2b/locks/usbip/<bus_id>`. The broker is the single
//! writer of that file; the daemon may read it via the `d2bd`
...
//!   `d2b down` does not release USBIP locks.
