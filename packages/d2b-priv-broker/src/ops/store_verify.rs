...
//!
//! The CLI reaches this only through d2bd -> broker. Verification reads
...
fn verify_locked(intent: &ResolvedStoreViewIntent, repair: bool) -> StoreVerifyResponse {
    ...
    let live_marker = hardlink_farm::live_dir(store_root).join(format!(".d2b-marker-{vm}"));
...
mod tests {
    use super::*;
    use d2b_host::hardlink_farm::{GenerationMarker, StoreViewLinkCounts};
    ...
    fn nonzero_live_marker_returns_drift() {
        ...
        let generation_id = publish(&intent);
        std::fs::write(
            hardlink_farm::live_dir(&intent.hardlink_farm_path).join(".d2b-marker-vm-a"),
            b"payload-is-drift",
        )
        .unwrap();
        ...
        .unwrap();
        assert!(raw.contains(".d2b-marker-vm-a"));
    }
    ...
    fn repair_nonzero_live_marker_replants_zero_length_marker() {
        ...
        let intent = intent(tmp.path(), "vm-a", closure);
        publish(&intent);
        let marker = hardlink_farm::live_dir(&intent.hardlink_farm_path).join(".d2b-marker-vm-a");
