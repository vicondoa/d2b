...
pub fn live_open_zone_store(
    ...
    zone_store_id: &d2b_contracts::v3::storage::ZoneStoreId,
...
pub fn live_run_keys_rotate(
    ...
    let GeneratedSshKey {
        ...
        .run_ssh_keygen(&intent.key_path, &format!("d2b:{}", intent.vm))
        .map_err(LiveHandlerError::ReconcileExec)?;
...
pub fn live_run_migrate(
    ...
    live_run_migrate_with_marker_dir(executor, intent, Path::new("/var/lib/d2b/migrate"))
}
...
mod tests {
    ...
    fn rollback_target_view_path_uses_rolled_back_marker_basename() {
        let intent = ResolvedStoreViewIntent {
            ...
            vm: "alpha".to_owned(),
            generation: 99,
            hardlink_farm_path: PathBuf::from("/var/lib/d2b/vms/alpha/store-view"),
            target_view_path: PathBuf::from(
                "/var/lib/d2b/vms/alpha/store-view/generations/99/current-system",
            ),
        ...
        assert_eq!(
            ...
            PathBuf::from("/var/lib/d2b/vms/alpha/store-view/generations/7/old-system"),
        );
        ...
        assert_eq!(
            ...
            PathBuf::from("/var/lib/d2b/vms/alpha/store-view/generations/7/current-system"),
        );
    ...
    fn sample_host_key_trust_intent(root: &TestDir) -> ResolvedHostKeyTrustIntent {
        ResolvedHostKeyTrustIntent {
            ...
            known_hosts_path: root.join("known_hosts.d2b"),
    ...
    fn live_run_keys_rotate_drives_executor() {
        ...
        assert!(matches!(
            ...
            ReconcileOp::RunSshKeygen { key_path, comment }
                if key_path == &intent.key_path && comment == "d2b:alpha"
        ));
    }
    ...
    fn live_run_trust_rewrites_known_hosts_atomically() {
        ...
        std::fs::write(
            ...
            b"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFAKE alpha@d2b\n",
        )
        .expect("write host pubkey");
        std::fs::write(
            &intent.known_hosts_path,
            b"10.20.0.10 ssh-ed25519 AAAAOLD alpha@d2b\n192.0.2.1 ssh-ed25519 AAAAOTHER beta@d2b\n",
        )
        ...
        assert!(matches!(
            ...
            ReconcileOp::WriteAtomicFile { path, mode: 0o644, contents }
                if path == &intent.known_hosts_path
                    && String::from_utf8_lossy(contents).contains("10.20.0.10 ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFAKE alpha@d2b")
                    && String::from_utf8_lossy(contents).contains("192.0.2.1 ssh-ed25519 AAAAOTHER beta@d2b")
        ));
    }
    ...
    fn live_run_rotate_known_host_rewrites_known_hosts_atomically() {
        ...
        assert!(matches!(
            ...
            ReconcileOp::WriteAtomicFile { path, mode: 0o644, contents }
                if path == &intent.known_hosts_path
                    && !String::from_utf8_lossy(contents).contains("10.20.0.10 ssh-ed25519 AAAAOLD alpha@d2b")
                    && String::from_utf8_lossy(contents).contains("192.0.2.1 ssh-ed25519 AAAAOTHER beta@d2b")
        ));
    }
    ...
    fn live_run_host_install_propagates_systemctl_failure() {
        ...
        let err = live_run_host_install_with_runtime_and_systemctl(
            ...
            |args| {
                ...
                Err("systemctl enable failed: d2bd.service: permission denied".to_owned())
            },
        )
        .unwrap_err();
        ...
        assert!(matches!(
            ...
            LiveHandlerError::HostInstall(ref detail)
                if detail == "systemctl enable failed: d2bd.service: permission denied"
        ));
        assert_eq!(calls.into_inner(), vec!["enable d2bd.service".to_owned()]);
        ...
        assert!(log.iter().any(|op| matches!(
            ...
            ReconcileOp::WriteAtomicFile {
                ...
                && String::from_utf8_lossy(contents).contains("# d2bd.service")
        )));
        assert!(log.iter().any(|op| matches!(
            op,
            ReconcileOp::WriteAtomicFile {
