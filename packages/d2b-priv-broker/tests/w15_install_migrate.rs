...
fn writable_install_intent() -> ResolvedInstallerIntent {
    ResolvedInstallerIntent {
        ...
        unit_path: writable_artifact_path("etc/systemd/system/d2bd.service"),
        service_name: "d2bd.service".to_owned(),
        daemon_config_path: writable_artifact_path("etc/d2b/daemon-config.json"),
        bundle_path: writable_artifact_path("current-bundle/manifest.json"),
        artifacts: vec![
            InstallerArtifact {
                path: writable_artifact_path("etc/systemd/system/d2bd.service"),
                mode: 0o644,
                purpose: "d2bd systemd unit (test override)".to_owned(),
            },
            InstallerArtifact {
                path: writable_artifact_path("etc/d2b/daemon-config.json"),
                mode: 0o640,
                purpose: "daemon configuration file (test override)".to_owned(),
            },
            InstallerArtifact {
                path: writable_artifact_path("share/d2b/vms.json"),
                mode: 0o644,
...
impl ReconcileExecutor for FailingWriteExecutor {
    ...
    fn prepare_store_view(
        &self,
        _intent: &d2b_core::bundle_resolver::ResolvedStoreViewIntent,
    ) -> Result<(), ReconcileExecError> {
    ...
    fn run_ssh_keygen(
        ...
        _key_path: &Path,
        _comment: &str,
    ) -> Result<d2b_priv_broker::ops::exec_reconcile::GeneratedSshKey, ReconcileExecError> {
...
fn bundle_resolver_unavailable_returns_typed_error() {
    ...
    assert_eq!(
        ...
        BrokerResponse::Error(BrokerErrorResponse {
            ...
            operation: "BundleResolver".to_owned(),
            target_wave: Some("W12".to_owned()),
            message: "broker operation failed; details are available only in the redacted audit channel".to_owned(),
