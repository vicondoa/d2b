...
//!
//! `Operation` is a broker operation row: it declares the payload schema, the authority profile, the audit facet, and the handler reference the generic envelope dispatches. The type is declared here and its rows materialize in the policy-rows unit, so this crate ships the declaration and the driver shell.
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`OperationDriverFactory`] registration through the type's
...
pub const OPERATION_TYPE_NAME: &str = "Operation";

/// The resource verbs the `Operation` type supports.
///
...
/// The execution domains the `Operation` type can be reconciled in.
///
/// Derived from the placement contract: `Operation` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `Operation` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const OPERATION_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `Operation` driver reads while reconciling.
///
...
const OPERATION_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `Operation` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn operation_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `Operation` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct OperationDriverFactory {
    types: [ResourceTypeName; 1],
}

impl OperationDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(OPERATION_TYPE_NAME)],
        }
    }
}

impl Default for OperationDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for OperationDriverFactory {
    fn resource_types(&self) -> &[ResourceTypeName] {
        &self.types
    }

    async fn create(&self, _key: &ResourceKey) -> Box<dyn DynResourceDriver> {
        Box::new(OperationDriver)
    }
}
...
/// One `Operation` resource's driver.
pub struct OperationDriver;

#[async_trait]
impl ResourceDriver for OperationDriver {
...
/// `*.d2bus.org.*Service` types, so a `Operation` row is never an export subject.
pub fn operation_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        resource_type: WellKnownType::OPERATION,
        allowed_sources: AllowedSources::BUILTIN,
        verbs: OPERATION_VERBS,
        execution: OPERATION_EXECUTION_DOMAINS,
        exportable: false,
        ...
        startup: &[],
        services: &[],
        decoder: operation_spec_decoder(),
        factory: Arc::new(OperationDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        OPERATION_TYPE_NAME, OperationDriverFactory, operation_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", OPERATION_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = OperationDriverFactory::new()
            .create(&ResourceKey::new("work", OPERATION_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
