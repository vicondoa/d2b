...
/// through LIST/relist (R24). Owner hints are a durable-plane concept; the
/// hub's change notices carry no owner edge, so manager-plane frames carry
/// the changed key only (Phase A minimal WATCH; the midpoint proof covers
/// LIST).
...
impl core::fmt::Debug for ManagerWatchStreams {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str("ManagerWatchStreams(<redacted>)")
    }
}
