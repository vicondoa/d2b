...
pub fn nixlingd_bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_nixlingd"))
}
...
impl DaemonFixture {
    pub fn new(prefix: &str) -> Self {
        let parent = PathBuf::from("target/nixlingd-integration-tests");
        fs::create_dir_all(&parent).expect("create nixlingd integration temp parent");
        let tmp = Builder::new()
            .prefix(prefix)
            .tempdir_in(&parent)
            .expect("create nixlingd integration tempdir");
...
pub fn spawn_nixlingd_serve(
    ...
    let mut command = Command::new(nixlingd_bin());
    command
        .arg("serve")
        .arg("--config")
    ...
    let child = command.spawn().expect("spawn nixlingd serve");
...
pub fn spawn_lock_only(config: &Path, state_lock: &Path, hold_seconds: u64) -> SpawnedProcess {
    let child = Command::new(nixlingd_bin())
        .arg("lock-only")
        .arg("--config")
        .arg(config)
        .arg("--state-lock")
        ...
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn nixlingd lock-only");
    SpawnedProcess::from_child(child)
}
...
pub fn test_client(socket: &Path, frames: &[&str]) -> (i32, String) {
    ...
    let output = command.output().expect("spawn nixlingd test-client");
...
pub fn run_lock_only(config: &Path, state_lock: &Path, _locks_dir: &Path) -> (i32, String) {
    let output = Command::new(nixlingd_bin())
        .arg("lock-only")
        .arg("--config")
        .arg(config)
        .arg("--state-lock")
