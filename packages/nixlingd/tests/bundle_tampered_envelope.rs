...
//!     → nixling_core::Error::Bundle(BundleError::Tampered { path, reason })
...
use nixlingd::typed_error::TypedError;
use std::fs;
...
/// Apply the same mapping that `load_bundle_resolver` in `nixlingd/src/lib.rs`
/// uses to convert a `nixling_core::Error` into a `TypedError`.
...
fn bundle_tampered_remediation_contains_rebuild_guidance() {
    let typed = TypedError::BundleTampered {
        path: PathBuf::from("/var/lib/nixling/current-bundle/bundle.json"),
