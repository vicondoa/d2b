...
/// Persistent payload of `/run/nixling/version`. Written exactly
...
pub struct DaemonVersionFile {
    ...
    /// installs this is the Nix store path of `nixlingd`. The CLI
...
pub enum DaemonRestartStatus {
    ...
    /// the running daemon. A `systemctl restart nixlingd` will pick
    /// up the new binary.
    PendingRestart {
        running_path: String,
        on_disk_path: String,
    },
    /// `/run/nixling/version` is missing. Either the daemon is not
    ...
    DaemonNotRunning,
    /// `/run/nixling/version` is present but unparseable JSON. The
...
mod tests {
    ...
    impl FakeFs {
        ...
        fn with_unparseable_version(self) -> Self {
            *self.version.lock().unwrap() =
                Some(Err("parsing /run/nixling/version: expected JSON".to_owned()));
            self
        }
    }
    ...
    fn up_to_date_when_paths_match() {
        let fs = FakeFs::with_version_running("/nix/store/abc-nixlingd-0.4.0/bin/nixlingd")
            .with_on_disk("/nix/store/abc-nixlingd-0.4.0/bin/nixlingd");
        assert_eq!(compute_restart_status(&fs), DaemonRestartStatus::UpToDate);
    }

    #[test]
    fn pending_restart_when_paths_differ() {
        let fs = FakeFs::with_version_running("/nix/store/old-nixlingd-0.4.0/bin/nixlingd")
            .with_on_disk("/nix/store/new-nixlingd-0.4.1/bin/nixlingd");
        ...
        match status {
            DaemonRestartStatus::PendingRestart {
                ...
                assert_eq!(running_path, "/nix/store/old-nixlingd-0.4.0/bin/nixlingd");
                assert_eq!(on_disk_path, "/nix/store/new-nixlingd-0.4.1/bin/nixlingd");
            }
    ...
    fn missing_install_path_treats_as_up_to_date() {
        // If /run/current-system/sw/bin/nixlingd is gone (Tier-0
        ...
        let fs = FakeFs::with_version_running("/nix/store/abc-nixlingd/bin/nixlingd");
    ...
    fn version_file_round_trip_serializable() {
        let original = DaemonVersionFile {
            server_version: "0.4.0".to_owned(),
            binary_path: "/nix/store/abc-nixlingd-0.4.0/bin/nixlingd".to_owned(),
            started_at: "2026-05-29T03:00:00Z".to_owned(),
            protocol_version: 3,
        };
    ...
    fn version_file_rejects_unknown_fields() {
        ...
        let json = serde_json::json!({
            "serverVersion": "0.4.0",
            "binaryPath": "/nix/store/abc/bin/nixlingd",
