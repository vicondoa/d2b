...
/// `/var/lib/nixling/state/host-runtime.json` (nftables ownership
/// hashes) + `/run/nixling/locks/usbip/` (busid carriers).
...
impl StopDagOwner {
    ...
    /// Plan the reconcile that should run when nixlingd starts.
    ///
...
mod tests {
    ...
    use nixling_core::host::{HostJson, UsbipBusidLock, UsbipLockOwner, UsbipLockScope};
