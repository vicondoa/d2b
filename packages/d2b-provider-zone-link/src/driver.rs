...
//!
//! `ZoneLink` is the link between two zones: the driver converges it as metadata once its desired state is admitted, and the crate carries the child-local enrollment-and-session state machine (`zone_links`) and the authority-owned cursor envelope (`zonelink`).
//!
//! Conversion mapping (spec section 13):
//! - `describe` -> [`ZoneLinkDriverFactory`] registration through the type's
//!   descriptor ([`zone_link_descriptor`]).
...
pub const ZONE_LINK_TYPE_NAME: &str = "ZoneLink";

/// The resource verbs the `ZoneLink` type supports.
///
...
/// The execution domains the `ZoneLink` type can be reconciled in.
///
/// Derived from the placement contract: `ZoneLink` names no placement anchor
/// (`PlacementAnchor::canonical_for` resolves none), so a `ZoneLink` row never
/// carries the canonical `spec.executionRef` and the plane reconciles it on
/// its own Host domain.
const ZONE_LINK_EXECUTION_DOMAINS: &[&str] = &["host"];
/// The resource types the `ZoneLink` driver reads while reconciling.
///
...
const ZONE_LINK_READS: &[WellKnownType] = &[];


/// The manager-wired decode hook for `ZoneLink` rows: the stored spec envelope is
/// the JSON spec object the core types store (they have no typed core spec,
/// and the old handler worked from canonical JSON).
pub fn zone_link_spec_decoder() -> Arc<dyn SpecDecoder> {
...
/// [`ResourceDriverFactory`] for the `ZoneLink` resource type. Construction is
/// infallible by contract (R3): the driver carries no fallible setup and no
/// target-local state.
pub struct ZoneLinkDriverFactory {
    types: [ResourceTypeName; 1],
}

impl ZoneLinkDriverFactory {
    /// Construct the one-type factory.
    pub fn new() -> Self {
        Self {
            types: [ResourceTypeName::new(ZONE_LINK_TYPE_NAME)],
        }
    }
}

impl Default for ZoneLinkDriverFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ResourceDriverFactory for ZoneLinkDriverFactory {
    fn resource_types(&self) -> &[ResourceTypeName] {
        &self.types
    }

    async fn create(&self, _key: &ResourceKey) -> Box<dyn DynResourceDriver> {
        Box::new(ZoneLinkDriver)
    }
}
...
/// One `ZoneLink` resource's driver.
pub struct ZoneLinkDriver;

#[async_trait]
impl ResourceDriver for ZoneLinkDriver {
...
/// `*.d2bus.org.*Service` types, so a `ZoneLink` row is never an export subject.
pub fn zone_link_descriptor() -> DriverDescriptor {
    DriverDescriptor {
        resource_type: WellKnownType::ZONE_LINK,
        allowed_sources: AllowedSources::BUILTIN,
        verbs: ZONE_LINK_VERBS,
        execution: ZONE_LINK_EXECUTION_DOMAINS,
        exportable: false,
        reads: ZONE_LINK_READS,
        operations: &[],
        creations: &[],
        startup: &[],
        services: &[],
        decoder: zone_link_spec_decoder(),
        factory: Arc::new(ZoneLinkDriverFactory::new()),
    }
}
...
mod tests {
    ...
    use super::{
        ZONE_LINK_TYPE_NAME, ZoneLinkDriverFactory, zone_link_spec_decoder,
    };
    ...
    fn target_row(spec: serde_json::Value) -> StoredDesiredResource {
        StoredDesiredResource {
            key: ResourceKey::new("work", ZONE_LINK_TYPE_NAME, "sample"),
            uid: TARGET_UID,
            generation: 1,
            owner_uid: None,
    ...
    async fn driver(spec: serde_json::Value, manager: Arc<ScriptedManager>) -> (ResourceContext, Box<dyn DynResourceDriver>) {
        ...
        let driver = ZoneLinkDriverFactory::new()
            .create(&ResourceKey::new("work", ZONE_LINK_TYPE_NAME, "sample"))
            .await;
        (ctx, driver)
    }
