//! ZoneLink registration boundary: the driver declaration is what the plane
...
/// The declared `ZoneLink` type satisfies the shared declaration-only metadata
/// driver contract: the one type its crate owns, registered through its
/// declared decoderand factory.
#[allow(clippy::disallowed_methods, reason = "cfg(test) helper")]
#[tokio::test]
async fn the_declaration_registers_the_zone_link_type() {
    assert_metadata_registration(
        &d2b_provider_zone_link::zone_link_descriptor(),
        WellKnownType::ZONE_LINK,
    )
    .await;
}
