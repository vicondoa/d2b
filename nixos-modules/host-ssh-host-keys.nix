# Host-managed sshd host keys for nixling VMs.
#
# Why
# ---
...
#   - the host's `nixling-known-hosts-refresh.service` pins the
#     first observed key in `known_hosts.nixling` and refuses to
...
# but for the *server* side. For every enabled `nixling.vms.<name>`:
#
...
#     `/run/nixling-sshd-host-keys/` (virtiofs tag `nl-ssh-host`).
#
...
let
  ...
  perVmGenScript = name: _vm: ''
    ### nixling-generate-sshd-host-keys: ${name}
    ...
    if [ ! -f "$priv" ]; then
      ...
      if ! ${pkgs.openssh}/bin/ssh-keygen \
            -t ed25519 -N "" \
            -C "nixling:${name}:sshd-host-key" \
            -f "$staging" >/dev/null; then
        echo "nixling-generate-sshd-host-keys: FAILED to generate $priv" >&2
        ${pkgs.coreutils}/bin/rm -f "$staging" "$staging.pub"
        exit 1
      fi
    ...
    # opens /run/nixling-sshd-host-keys/ssh_host_ed25519_key as
...
in
{
  ...
  # any nixling-* group for this module, but using the same anchor
  # avoids two parallel activation scripts both creating
  # `${site.stateDir}/vms/<name>/`.
  system.activationScripts.nixlingGenerateSshdHostKeys = lib.stringAfter [ "users" "nixlingGenerateKeys" ] ''
    set -u
