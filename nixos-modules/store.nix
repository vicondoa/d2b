# Per-VM nix store for d2b microVMs.
#
# Background
# ----------
...
#     <hash>-nixos-system-…/
#     …
#   store-meta/                     virtiofs → guest /run/d2b-store-meta
...
# Hardlinks from /nix/store/<x> to /var/lib/d2b/<y> fail with EXDEV
...
let
  ...
  # to reach scopes spawned by interactive `d2b up`. When
  ...
  # child virtiofsd processes also run as `d2b-virtiofs-<vm>`. The
  # sanitised conf is written to /var/lib/d2b/vms/<vm>/ which is
  ...
  # /var/lib/d2b/vms/<vm>/virtiofsd-<base>-hardened so the host's
  # /nix/store remains untouched.
  d2bVfsRunnerOf = name: pkgs.writeShellApplication {
    name = "d2b-virtiofsd-run-${name}";
    ...
    text = ''
      ...
      #        /var/lib/d2b/vms/<vm>/store/.d2b-marker-<vm>
      #      -- proving d2b-store-sync actually planted the file.
      #
      #   2. The check below runs as ExecStart (inside the unit's
      #      mount namespace, AFTER BindReadOnlyPaths has remapped
      #      /nix/store -> /var/lib/d2b/vms/<vm>/store) and tests the
      #      BIND-VIEW marker at
      #        /nix/store/.d2b-marker-<vm>
      ...
      # directory without going through d2b-store-sync. Together
      # the two layers establish: the marker was planted by the sync
      # helper AND the daemon is serving that exact view.
      if [ ! -e "/nix/store/.d2b-marker-${name}" ]; then
        echo "d2b-virtiofsd-run-${name}: bind-view marker /nix/store/.d2b-marker-${name} missing; BindReadOnlyPaths did not take effect or the per-VM store is empty -- refusing to start virtiofsd" >&2
        exit 3
      fi

      ORIG=${cfg.store.stateDir}/${name}/current/bin/virtiofsd-run
      if [ ! -x "$ORIG" ]; then
        echo "d2b-virtiofsd-run-${name}: missing $ORIG" >&2
        exit 2
      fi
      CONF=$(sed -nE 's|.*--configuration[[:space:]]+([^[:space:]]+).*|\1|p' "$ORIG" | head -n1)
      if [ -z "$CONF" ] || [ ! -f "$CONF" ]; then
        echo "d2b-virtiofsd-run-${name}: could not locate supervisord conf in $ORIG" >&2
        exit 2
      fi

      # Hardened scripts live in /run (root-owned tmpfs, 0700) instead
      # of /var/lib/d2b/vms/<vm>/ which is mode 2775 group=kvm for
      ...
      install -d -m 0700 -o root -g root /run/d2b/${name}
      ...
      # level. The d2b guests treat /nix/.ro-store and
      # /run/d2b-store-meta as read-only mounts, so this is
      # never reached by legitimate guest traffic.
      while IFS= read -r origcmd; do
        [ -n "$origcmd" ] || continue
        if [ ! -f "$origcmd" ]; then
          echo "d2b-virtiofsd-run-${name}: command path missing: $origcmd" >&2
          exit 2
        fi
        ...
        if [ "$needs_hardening" = 1 ]; then
          ...
          do
            if ! grep -qF -- "$flag" "$patched.tmp"; then
              echo "d2b-virtiofsd-run-${name}: hardening flag $flag missing from patched script $base; refusing to exec" >&2
              rm -f "$patched.tmp"
              exit 4
            fi
          done
  ...
  vmGenerationOf = name:
    ...
    in
    pkgs.runCommand "d2b-${name}-generation" { } ''
      mkdir -p $out
      ln -s ${top} $out/system
  ...
  #      hardlinks from /nix/store/<x> to /var/lib/d2b/<x> work
  ...
  # ---------------------------------------------------------------------------
  d2bStoreSync = pkgs.writeShellApplication {
    name = "d2b-store-sync";
    runtimeInputs = with pkgs; [
      coreutils
      util-linux
    ...
    text = ''
      ...
      if [ "$(id -u)" -ne 0 ]; then
        echo "d2b-store-sync: must run as root (need mount-namespace + chown)." >&2
        exit 1
      fi
      ...
      if [ -z "$VM" ] || [ -z "$GEN_SRC" ]; then
        echo "usage: d2b-store-sync <vm> <generation-derivation-dir>" >&2
        exit 2
      fi
      if [ ! -d "$GEN_SRC" ] || [ ! -f "$GEN_SRC/store-paths" ]; then
        echo "d2b-store-sync: '$GEN_SRC' is not a valid generation dir." >&2
        exit 2
      fi
      ...
      if [ -z "''${D2B_STORE_SYNC_IN_NS:-}" ]; then
        export D2B_STORE_SYNC_IN_NS=1
      ...
      if [ -L "$CURRENT_LINK" ]; then
        ...
        if [ -f "$CUR_META" ]; then
          ...
          if [ "$CUR_TOP" = "$NEW_TOP" ] && [ "$CUR_SRC" = "$NEW_SRC" ]; then
            marker="$STORE_DIR/.d2b-marker-$VM"
            missing=0
            ...
            if [ "$missing" -eq 0 ] && [ -e "$marker" ]; then
              echo "d2b-store-sync: $VM already at generation $CURRENT_GEN ($NEW_TOP); nothing to do."
              exit 0
            fi
          fi
        fi
      ...
      echo "d2b-store-sync: $VM → generation $NEXT_GEN"
      NEW_TOP=$(readlink "$GEN_SRC/system")
      ...
      printf '{"closureHash":"toplevel:%s","d2bVersion":"activation","activatedAt":"%s","vm":"%s","generationNumber":%d}\n' \
      ...
      install -d -m 0755 /nix/var/nix/gcroots/per-user/root/d2b 2>/dev/null || true
      ln -sfT "$ROOT_DIR/generation-$NEXT_GEN" \
        /nix/var/nix/gcroots/per-user/root/d2b/"$VM"-generation-"$NEXT_GEN" 2>/dev/null || true
      ...
      # /var/lib/d2b/vms/<vm>/booted points at the runner; the runner
      ...
      find "$STORE_DIR" -mindepth 1 -maxdepth 1 -printf '%f\n' \
        | ${pkgs.gawk}/bin/awk -v marker=".d2b-marker-$VM" '$0 != marker { print }' \
      ...
      while IFS= read -r base; do
        [ -n "$base" ] || continue
        case "$base" in
          .d2b-marker-*|live.stage.*) continue ;;
        esac
      ...
      for d in "$GEN_DIR"/*; do
        ...
        if [ "$keep" -eq 0 ]; then
          ...
          rm -f "/nix/var/nix/gcroots/per-user/root/d2b/$VM-generation-$g" 2>/dev/null || true
        fi
      done

      # Permissions: align store-view directory inodes with
      # the daemon ownership matrix (`d2bd:users 0755`). The owner
      # (`d2bd`) can mutate directory entries during StoreSync; the
      ...
      find "$META_DIR" -type d -exec chown d2bd:users {} + 2>/dev/null || true
      ...
      # the unit to start, so a hand-crafted /var/lib/d2b/vms/<vm>/store-view/live/
      # populated by anything other than d2b-store-sync cannot be
      # served by virtiofsd. Mode 0444 so the unprivileged virtiofsd
      # user (d2b-virtiofs-<vm>) can read it but not modify it.
      MARKER="$STORE_DIR/.d2b-marker-$VM"
      : > "$MARKER"
      chmod 0444 "$MARKER"
      ...
      echo "d2b-store-sync: $VM done (generation $NEXT_GEN active)."
    '';
  };
...

{
  options.d2b.store = {
    ...
    # options `d2b.store.package` and `d2b.store.generations`
    # were retired together with the bash CLI. The only consumer was
    # `cli.nix` (the `d2b switch` codepath); store.nix itself
    # uses the `d2bStoreSync` derivation and `vmGenPaths`
  ...
  config = lib.mkIf cfg.store.enable {
    ...
    # d2b-vfsd-watchdog@ template + per-VM enable units
    ...
    # retired
    #   - systemd.services."d2b-vfsd-watchdog@"
    #   - systemd.services."d2b-vfsd-watchdog-<vm>-enable" (per-VM)
    #   - systemd.timers."d2b-vfsd-watchdog@"
    ...
    #      task); just tell her.
    #
    #   2. Periodic healthcheck (`d2b-vfsd-watchdog@<vm>.timer` /
    #      `.service`): every 60s, check the CH journal for the
    #      NEEDS_RESET pattern. If present, kill microvm@<vm> so the
    #      autorestart (workload VMs) or the operator's manual `d2b up`
    #      (graphics VMs) replaces the wedged CH with a healthy one.
    # ---------------------------------------------------------------------------

    system.activationScripts.d2bVfsdRestartHint = lib.stringAfter [ "etc" ] ''
      set -u
      ${lib.concatStringsSep "\n" (lib.mapAttrsToList
        (name: _: ''
