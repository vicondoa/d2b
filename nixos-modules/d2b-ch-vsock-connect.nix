...
# with `EXEC:"d2b-ch-vsock-connect <base> <port>"`.
{ pkgs, ... }:

pkgs.writeShellApplication {
  name = "d2b-ch-vsock-connect";
  runtimeInputs = with pkgs; [ python3 ];
  text = ''
    exec ${pkgs.python3}/bin/python3 -u ${pkgs.writeText "d2b-ch-vsock-connect.py" ''
      ...
      Plug into socat with:
        EXEC:"d2b-ch-vsock-connect <base> <port>"
      """
      import os
      import socket
      import sys
      ...
      def main():
          if len(sys.argv) != 3:
              sys.stderr.write("usage: d2b-ch-vsock-connect <base-socket> <port>\n")
              sys.exit(2)
          ...
          def refresh_deadline():
              ...
              if remaining <= 0:
                  sys.stderr.write("d2b-ch-vsock-connect: connect-timeout\n")
                  sys.exit(1)
          ...
          try:
              ...
              sys.stderr.write("d2b-ch-vsock-connect: transport-unreachable\n")
              sys.exit(1)
          try:
              refresh_deadline()
          ...
          while not reply.endswith(b"\n"):
              ...
              try:
                  ...
                  sys.exit(1)
              if not chunk:
                  sys.stderr.write("d2b-ch-vsock-connect: eof-before-ack\n")
                  sys.exit(1)
              reply += chunk
              if len(reply) > 128:
                  sys.stderr.write("d2b-ch-vsock-connect: ack-too-long\n")
                  sys.exit(1)
          if not reply.startswith(b"OK ") or not reply.endswith(b"\n"):
              sys.stderr.write("d2b-ch-vsock-connect: connect-refused\n")
              sys.exit(1)
          local_port = reply[3:-1]
          if not local_port.isdigit() or int(local_port) > 0xffffffff:
              sys.stderr.write("d2b-ch-vsock-connect: malformed-ack\n")
              sys.exit(1)
