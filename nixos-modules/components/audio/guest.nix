# Audio support for nixling VMs (virtio-snd + vhost-user-sound +
# PipeWire). Imported into the GUEST config by host.nix whenever a
# VM sets `nixling.vms.<name>.audio.enable = true`.
#
# Host-side wiring (the per-VM sidecar systemd service, the
# WirePlumber rule, the activation script that materialises per-VM
# state files) lives in modules/nixling/audio-host.nix and is imported
# at the host scope from modules/nixling/default.nix.
#
# Architecture
# ------------
...
# named `nixling-<vm>` — giving the user a normal per-stream mute/
...
# service (`nixling-<vm>-snd.service`, declared in audio-host.nix)
...
# tiny shell helper that reads /var/lib/nixling/<vm>/audio-state.json
...
#   1. Asks systemd to start `nixling-<vm>-snd.service` if not running.
...
let
  ...
  #
  # Important: in the nixling framework, an `audio.enable = true` VM
  # is asserted to also have `autostart = false` (see the assertion
  # in audio-host.nix). That means the runner is always launched
  # interactively via `nixling up` running as the host's Wayland
  # user (`nixling.site.waylandUser`), never via microvm@<vm>.service.
  audioArgsScript = pkgs.writeShellScript "nixling-audio-args-${vmName}" ''
    set -eu
    # shellcheck source=/dev/null
    . ${nl.nixlingReadAudioState}
    _a_result=$(nixling_read_audio_state "${vmName}")
    ...
    # Best-effort: ensure the sidecar's service is running. `nixling
    ...
    ${pkgs.systemd}/bin/systemctl reset-failed \
      "nixling-${vmName}-snd.service" >/dev/null 2>&1 || true
    ...
    if [ ! -S "$sock" ]; then
      echo "nixling-audio: sidecar socket $sock did not appear; skipping audio device" >&2
      exit 0
    fi
...

{
  ...
  # `nixling.vms.<name>.audio.users` (default `[ ssh.user ]`) via
  ...
  options.nixling.audio.users = lib.mkOption {
    ...
    description = ''
      ...
      `nixling.vms.<name>.audio.users`.
    '';
  };

  config = {
  # Cloud-hypervisor is the only path: qemu+vhost-user-snd would
  # work too but the rest of nixling already targets CH. mkDefault so
  ...
  # sessions, but nixling guests don't run a graphical login manager
  ...
  # `nixling.vms.<name>.audio.users` and propagated into
  # `config.nixling.audio.users` here by host.nix. We add `audio` to
  ...
  services.pipewire.wireplumber.extraConfig."91-nixling-virtio-snd" = {
    "monitor.alsa.rules" = [
      {
        matches = [
