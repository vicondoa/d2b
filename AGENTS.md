...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its approved-API list, and the compile-fail fixtures under `packages/{d2b-bus,d2b-controller-toolkit,d2b-resource-api}/tests/ui/external-seals/` | The set of public APIs that can mint a capability is **allowlisted and gate-enforced**: the guard enumerates the crate's real public API from `cargo doc` output and fails when an unapproved constructor, factory, or accessor appears. Adding a public API that yields `ComponentSessionAdmission` or `CommittedRevisionProof` is a deliberate widening of the trust boundary and requires updating the approved list in the same change, with a stated reason. The guard exists because this defect relocated six times: each fix closed one mint route while a later refactor opened another, and a compile-fail fixture can only ever prove the one route it exercises. Seal fixtures must fail at the capability boundary (for example `E0277` on the private seal, or `E0451` on private fields) and explicitly assert the absence of `error[E0432]`, since a fixture that fails on an unresolved import passes for the wrong reason and proves nothing. |
