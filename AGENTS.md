...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its `approved-public-api.txt` and `approved-capability-api.txt`, and the compile-fail fixtures under `packages/{d2b-bus,d2b-controller-toolkit,d2b-resource-api}/tests/ui/external-seals/` | Two distinct checks live here, and confusing them is how the boundary kept moving. **The capability inventory is workspace-wide**: it discovers every workspace library crate through `cargo metadata` and classifies capability and claim types by *type identity*, not by crate, path, or identifier name - that coupling is what let the mint route relocate repeatedly into whichever crate the guard was not reading. Its snapshot is `approved-capability-api.txt`. **The full-API snapshot is narrower**, covering only `d2b-bus`, `d2b-session` and `d2b-session-unix` in `approved-public-api.txt`. `CommittedRevisionProof` in `d2b-controller-toolkit` is covered by neither snapshot; it has its own compile-fail seal. Widening any of these surfaces is a deliberate trust-boundary change and must update the matching gate in the same commit, with a stated reason. Methods reached through `Deref` are excised from the snapshot because they come from the standard library and would otherwise drift with the compiler version; only that region is removed, since rustdoc emits trait implementations after it and a capability-returning trait method is exactly what would hide there. The gate renders the workspace once and fails when an approved crate yields no public items, because a partial rustdoc build otherwise reports that crate's entire API as removed and reads like a capability change. Seal fixtures must fail because the capability is genuinely unreachable, each asserting its own exact diagnostic: a deliberately unexported item fails with `error[E0432]` naming it, a missing method with `error[E0599]`, a private item with `error[E0603]`, reading a private field with `error[E0616]`, and constructing a struct with private fields with `error[E0451]`. The `CommittedRevisionProof` fixture additionally asserts the *absence* of `E0432`, because it must fail on private fields rather than on a missing import. An earlier fixture asserted `no AdmittedComponentSession in the root` against a type the crate had renamed, so it passed for entirely the wrong reason and reported a boundary as proven while the mint path stayed open: a fixture must name types that currently exist unless absence is itself the invariant. |
