...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its `approved-public-api.txt`, `approved-capability-api.txt`, and `approved-hidden-public-api.txt`, the mutation fixture under `tests/ui/public-api-mutations/`, and the compile-fail seals under `packages/{d2b-bus,d2b-controller-toolkit,d2b-resource-api}/tests/ui/external-seals/` | Three checks with different scopes. The **capability inventory** discovers every workspace library crate through `cargo metadata`; it seeds roots from two fixed name lists - `CAPABILITY_TYPE_IDENTITIES` (`ComponentSessionAdmission`, `AuthenticatedComponentSession`, `SessionAcceptor`, `SessionRegistrationCapability`, `VerifiedUnixPeer`) and `CLAIM_TYPE_IDENTITIES` (`ResourceRef`, `ResourceUid`) - and then propagates by **type identity**, including through private field types, so a renamed or opaque wrapper in any crate is still classified. Everything beyond those roots is discovered, not listed: `CommittedRevisionProof` is not a seeded root, it is reached through its private `ResourceUid` field and then pinned in the capability snapshot. Adding a root is therefore a much larger act than adding a snapshot entry. Its snapshot is `approved-capability-api.txt`. The **full-API snapshot** is narrower, covering `d2b-bus`, `d2b-session` and `d2b-session-unix` in `approved-public-api.txt`. The **hidden-API snapshot** independently inventories exact signatures for every public `doc(hidden)` function and method. Rustdoc 1.94.1 requires unstable options to render hidden items, so the stable pinned gate parses those declarations from the compiler-accepted workspace sources instead; every new hidden signature fails before its return type can evade the rustdoc classifier. Widening any surface is a deliberate trust-boundary change and must update the matching snapshot in the same commit, with a stated reason. The gate **fails closed on an incomplete render**: it validates every item rustdoc advertises for a crate before parsing, because a partial render otherwise omits both an existing region and any new capability inside it, which would let an unapproved addition pass unseen. Methods reached through `Deref` are excised because they come from the standard library and drift with the compiler version; only that region is removed, resuming at whichever of the trait, synthetic or blanket implementation anchors exists, since rustdoc emits trait implementations after it and a capability-returning trait method is exactly what would hide there. The gate is slow - roughly 13 minutes - because completeness is proven per crate; that cost is deliberate. Seal fixtures must fail because the capability is genuinely unreachable, each asserting its own exact diagnostic: a deliberately unexported item fails with `error[E0432]` naming it, a missing method with `error[E0599]`, a private item with `error[E0603]`, reading a private field with `error[E0616]`, and constructing a struct with private fields with `error[E0451]`. The `CommittedRevisionProof` fixture additionally asserts the *absence* of `E0432`, because it must fail on private fields rather than a missing import. An earlier fixture asserted `no AdmittedComponentSession in the root` against a type the crate had renamed, so it passed for entirely the wrong reason while the mint path stayed open: a fixture must name types that currently exist unless absence is itself the invariant. |
