...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its `approved-public-api.txt`, `approved-capability-api.txt`, `approved-hidden-public-api.txt`, and `approved-capability-trait-impls.txt`, the source mutations under `tests/ui/public-api-mutations/`, the compiler-seal mutations under `packages/d2b-bus/tests/ui/external-seals/`, and the capability definitions in `packages/{d2b-bus,d2b-session,d2b-session-unix}/src/` | The **enforcing compiler leg** uses stable trait-solver ambiguity assertions in the defining crates. It rejects `Clone`, `Copy`, and `Default` for `ComponentSessionAdmission`, `VerifiedUnixPeer`, `SessionAcceptor<C>`, and `AuthenticatedComponentSession<C>`. It rejects `From<Arc<ComponentSessionAdmissionIdentity>>` and `From<()>` for `ComponentSessionAdmission`, `From<PeerCredentials>` and `From<()>` for `VerifiedUnixPeer`, and `From<C>` for both generic session types. The generic functions use an unconstrained `C`, so they catch unconditional blanket implementations. They do not catch an implementation that requires extra bounds on `C`, or one written only for a concrete parameter. Separate assertions cover `C = ()` in `d2b-session` and the workspace's `C = ComponentSessionAdmission` uses in `d2b-bus`; future concrete parameterizations are not covered until added explicitly. Every assertion names the forbidden set as `CapabilityMustNotImplementCloneCopyDefaultOrFrom` and carries local remediation. The `d2b-bus` external-seals integration test activates real `Clone` and `Default` implementations for every guarded type plus `From<()>` for the two non-generic capability types, and requires both `error[E0283]` and that actionable helper name. Because the compiler performs name resolution, selected module resolution, and macro expansion, this leg is complete for the enumerated type-and-trait pairs in each configuration that is actually compiled, regardless of source spelling. An implementation disabled by the active `cfg` is not type-checked in that build. No finite assertion enumerates every possible `From<X>` shape. A downstream crate may implement `From<LocalInput>` for a capability type because it owns `LocalInput`; a compile-pass fixture pins that coherence fact. Such an implementation cannot fabricate valid authority because the capability's construction fields and instance identity remain private; the paired compile-fail fixture proves direct construction is rejected with `error[E0451]`. A conversion can only return authority it already holds or obtained through an approved API. `SessionRegistrationCapability` is a trait and has no self-type assertion. The **best-effort source leg** scans workspace library source for the explicit impl and derive forms it models and compares them with `approved-capability-trait-impls.txt`. It has focused regression fixtures for direct and inline `#[path]`, ordinary children of path-loaded modules, raw identifiers, lexical symlink paths, renamed aliases plus plain named and `::{self}` imports of local modules that contain a discovered capability binding, alias-before-target ordering, chained aliases, chained re-exports, harmless aliases in every covered spelling, a harmless two-hop re-export that requires fixed-point convergence, direct and nested-group glob imports of visible local modules, block-local inline and path-loaded modules, inert direct and conditional module attributes, rejected conditional `path` and unrecognised direct or conditional attributes, and one canonical file reached through different logical module paths. Duplicate-logical-module diagnostics use crate-relative source locations rather than canonical host paths. The source leg resolves module aliases, re-exports, and visible module bindings imported through module-level globs to a fixed point through lexical binding candidates. It classifies a module alias as non-capability only when resolution finds one unambiguous local target with no discovered capability binding. An unresolved module alias is ignored only for an unrelated impl self type that neither uses the alias as a prefix nor names a seeded capability identity; unresolved or ambiguous aliases used as prefixes remain capability-relevant and fail closed, including plain named, `::{self}`, direct-glob, and nested-group-glob module imports. Generic or cfg-gated declared type aliases, cfg-gated renamed imports, unsupported aliases, and lexically scoped capability aliases fail closed during classification. Resolvable unrenamed cfg-gated type imports are classified normally rather than rejected. Cyclic unresolved capability aliases fail closed when used by an impl self type. Every parsed module item, including a module declared inside a function or block, reaches the same attribute validation and external-source resolution. Unresolvable external modules, including missing `#[path]` targets, fail closed rather than permitting a partial scan. Direct and conditional module attributes are validated: direct `path` and recursive `cfg_attr` receive dedicated handling, while the source-inert allowlist accepts `cfg`, `doc`, `allow`, `warn`, `deny`, `forbid`, `expect`, `deprecated`, and the exact `rustfmt::skip` tool path in their approved shapes. Procedural, unknown tool, malformed, and every other unrecognised direct or conditional module attribute fail closed with remediation. Approved trait-implementation and hidden-public snapshots retain rendered signatures for exact comparison. Failure diagnostics separately store fixed syntax-kind labels, capability or item identity, and crate-relative logical source locations, and never echo signature token streams, source text, attribute token streams, or path literals. This syntax leg does not claim general Rust name resolution, module resolution, macro expansion, or `include!` expansion; unmodelled source forms and implementations outside the workspace can escape it. The separate capability API inventory discovers workspace library crates, propagates from fixed capability and claim type identities through private field types, and compares with `approved-capability-api.txt`; the full and hidden API snapshots retain their narrower documented scopes and fail-closed rustdoc completeness check. The **primary boundary remains construction** through private types, private fields, sealed traits, instance identity, and consumed capabilities. Widening an enumerated compiler seal or an approved snapshot is a deliberate trust-boundary change and requires a stated reason. Compile-fail fixtures must name current types and assert the exact diagnostic that proves the intended boundary, rather than passing for an unrelated missing symbol. |
