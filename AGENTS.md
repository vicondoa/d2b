...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its `approved-public-api.txt`, and the compile-fail fixtures under `packages/{d2b-bus,d2b-controller-toolkit,d2b-resource-api}/tests/ui/external-seals/` | The public API that can mint a `ComponentSessionAdmission` is **allowlisted and gate-enforced**: the guard renders the real public API of `d2b-bus`, `d2b-session` and `d2b-session-unix` from `cargo doc` and fails when an unapproved constructor, factory, capability accessor, or externally-implementable producer appears. Methods reached through `Deref` are excised from the snapshot, because they come from the standard library and otherwise drift with the compiler version; only that region is removed, since the trait implementations rustdoc emits after it are exactly where a capability-returning accessor would hide. `CommittedRevisionProof` in `d2b-controller-toolkit` is not covered by this inventory - it has its own compile-fail seal - so widening any of those surfaces needs the matching gate updated in the same change, with a stated reason. Seal fixtures must fail because the capability is genuinely unreachable, and each asserts its own exact diagnostic: a deliberately unexported item fails with `error[E0432]` naming that item, a missing method with `error[E0599]`, a private item with `error[E0603]`, reading a private field with `error[E0616]`, and constructing a struct with private fields with `error[E0451]`. The `CommittedRevisionProof` fixture additionally asserts the *absence* of `E0432`, because it must fail on private fields rather than on a missing import. That distinction is load-bearing: an earlier fixture asserted `no AdmittedComponentSession in the root` against a type the crate had renamed, so it passed for entirely the wrong reason and reported a boundary as proven while the mint path stayed open. A fixture must name types that currently exist unless absence is itself the invariant being asserted. The guard exists because this defect relocated repeatedly - each fix closed one mint route while a later refactor opened another, and a compile-fail fixture can only ever prove the single route it exercises. |
