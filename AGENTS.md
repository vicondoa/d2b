...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its `approved-public-api.txt`, `approved-capability-api.txt`, `approved-hidden-public-api.txt`, and `approved-capability-trait-impls.txt`, the source mutations under `tests/ui/public-api-mutations/`, the compiler-seal mutations under `packages/d2b-bus/tests/ui/external-seals/`, and the capability definitions in `packages/{d2b-bus,d2b-session,d2b-session-unix}/src/` | The **enforcing compiler leg** uses stable trait-solver ambiguity assertions in the defining crates. It rejects `Clone`, `Copy`, and `Default` for `ComponentSessionAdmission`, `VerifiedUnixPeer`, `SessionAcceptor<C>`, and `AuthenticatedComponentSession<C>`. It rejects `From<Arc<ComponentSessionAdmissionIdentity>>` and `From<()>` for `ComponentSessionAdmission`, `From<PeerCredentials>` and `From<()>` for `VerifiedUnixPeer`, and `From<C>` for both generic session types. The generic functions use an unconstrained `C`, so they catch unconditional blanket implementations. They do not catch an implementation that requires extra bounds on `C`, or one written only for a concrete parameter. Separate assertions cover `C = ()` in `d2b-session` and the workspace's `C = ComponentSessionAdmission` uses in `d2b-bus`; future concrete parameterizations are not covered until added explicitly. Every assertion names the forbidden set as `CapabilityMustNotImplementCloneCopyDefaultOrFrom` and carries local remediation. The `d2b-bus` external-seals integration test activates real `Clone` and `Default` implementations for every guarded type plus `From<()>` for the two non-generic capability types, and requires both `error[E0283]` and that actionable helper name. Because the compiler performs name resolution, selected module resolution, and macro expansion, this leg is complete for the enumerated type-and-trait pairs in each configuration that is actually compiled, regardless of source spelling. An implementation disabled by the active `cfg` is not type-checked in that build and remains the source inventory's responsibility. No finite assertion enumerates every possible `From<X>` shape. Rust's orphan rules mean an arbitrary `From<X>` for these capability types can only be added in the capability's defining crate, where its private construction fields are available; the source inventory covers source-visible implementations there. A macro-generated or `include!`-generated arbitrary `From<X>` in that defining crate remains a documented residual limit. `SessionRegistrationCapability` is a trait and has no self-type assertion. The **best-effort breadth leg** is the source trait-implementation allowlist. It covers source-visible traits and inactive source forms outside the compiler check, compares them with `approved-capability-trait-impls.txt`, resolves direct `#[path]` attributes relative to the containing source file and inline-module `#[path]` attributes relative to Rust's logical module directory, and fails closed on generic, conditional, ambiguous, cyclic, unsupported, or lexically scoped capability aliases, direct module renames whose local target contains a capability declaration, `self as` module renames used as type prefixes, external modules carrying `cfg_attr`, and one canonical file reached through different logical module paths. A direct rename of an external module is not treated as a local capability prefix because Rust's orphan rules prevent that module's foreign types from gaining a foreign trait implementation here. Duplicate-logical-module diagnostics use crate-relative source locations rather than canonical host paths. The scan remains syntax-level: macro-generated and `include!`-generated implementations can escape it, so it must not be described as compiler-complete. The separate capability API inventory still discovers workspace library crates, propagates from the fixed capability and claim type identities through private field types, and compares with `approved-capability-api.txt`; the full and hidden API snapshots retain their narrower documented scopes and fail-closed rustdoc completeness check. The **primary boundary remains construction** through private types, private fields, sealed traits, and consumed capabilities. Widening an enumerated compiler seal or an approved snapshot is a deliberate trust-boundary change and requires a stated reason. Compile-fail fixtures must name current types and assert the exact diagnostic that proves the intended boundary, rather than passing for an unrelated missing symbol. |
