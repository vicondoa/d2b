...
| Capability mint surface allowlist | `packages/d2b-bus/tests/public_mint_surface.rs`, its `approved-public-api.txt`, `approved-capability-api.txt`, `approved-hidden-public-api.txt`, and `approved-capability-trait-impls.txt`, the mutation fixture under `tests/ui/public-api-mutations/`, and the compile-fail seals under `packages/{d2b-bus,d2b-controller-toolkit,d2b-resource-api}/tests/ui/external-seals/` | Four checks with different scopes. The **capability inventory** discovers every workspace library crate through `cargo metadata`; it seeds roots from two fixed name lists - `CAPABILITY_TYPE_IDENTITIES` (`ComponentSessionAdmission`, `AuthenticatedComponentSession`, `SessionAcceptor`, `SessionRegistrationCapability`, `VerifiedUnixPeer`) and `CLAIM_TYPE_IDENTITIES` (`ResourceRef`, `ResourceUid`) - and then propagates by **type identity**, including through private field types, so a renamed or opaque wrapper in any crate is still classified. Everything beyond those roots is discovered, not listed: `CommittedRevisionProof` is not a seeded root, it is reached through its private `ResourceUid` field and then pinned in the capability snapshot. Adding a root is therefore a much larger act than adding a snapshot entry. Its snapshot is `approved-capability-api.txt`. The **trait-implementation allowlist** recursively parses every library module source, collects declarations, named type aliases, imports, and explicit implementations across the whole crate before classification, and resolves alias and rename chains to a fixed point. This covers aliases declared after use or in another module and renamed imports of an alias; candidate generic, conditional, ambiguous, cyclic, or unsupported aliases fail closed instead of disappearing from the inventory. Direct derives and derives recursively nested inside `cfg_attr` are recorded by name, while an explicit implementation remains visible from its syntax even when `cfg` or `cfg_attr` gates it. The resulting set of every source-visible explicit trait implementation and derive on a `CAPABILITY_TYPE_IDENTITIES` declaration must exactly equal `approved-capability-trait-impls.txt`; visibility, `doc(hidden)`, target configuration, and trait-member visibility cannot weaken the comparison. This directly makes new `Default`, `From`, clone, or conversion implementations fail by name. `SessionRegistrationCapability` is itself the one trait declaration in that root list and therefore has no self-type implementation entry. The **full-API snapshot** is narrower, covering `d2b-bus`, `d2b-session` and `d2b-session-unix` in `approved-public-api.txt`. The **hidden-API snapshot** independently parses direct public `doc(hidden)` function and method signatures because stable rustdoc does not render them. The rustdoc and hidden-source signature inventories are best-effort syntax checks, not compiler-complete API inventories: `Item::Use` re-exports, associated items outside the handled function/method shapes, `cfg_attr`-hidden variants, and item or attribute macro expansion can escape them. The trait allowlist is also intentionally source-level rather than compiler-expanded, so non-derive macro-generated implementations and `include!`-generated source remain residual limits. The primary capability boundary is enforced by construction through private types, private fields, sealed traits, and consumed capabilities; all four inventories are defense in depth. Widening a covered surface is a deliberate trust-boundary change and must update the matching snapshot in the same commit, with a stated reason. The gate **fails closed on an incomplete render**: it validates every item rustdoc advertises for a crate before parsing, because a partial render otherwise omits both an existing region and any new capability inside it, which would let an unapproved addition pass unseen. Methods reached through `Deref` are excised because they come from the standard library and drift with the compiler version; only that region is removed, resuming at whichever of the trait, synthetic or blanket implementation anchors exists. The gate is slow - roughly 13 minutes - because completeness is proven per crate; that cost is deliberate. Seal fixtures must fail because the capability is genuinely unreachable, each asserting its own exact diagnostic: a deliberately unexported item fails with `error[E0432]` naming it, a missing method with `error[E0599]`, a private item with `error[E0603]`, reading a private field with `error[E0616]`, and constructing a struct with private fields with `error[E0451]`. The `CommittedRevisionProof` fixture additionally asserts the *absence* of `E0432`, because it must fail on private fields rather than a missing import. An earlier fixture asserted `no AdmittedComponentSession in the root` against a type the crate had renamed, so it passed for entirely the wrong reason while the mint path stayed open: a fixture must name types that currently exist unless absence is itself the invariant. |
