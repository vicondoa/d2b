# `modules/nixling/ext/spectrum-ch/` — update process
...
2. **Don't try the snapshot tarball.** Pull each `.patch` file from
   ...
   ```
   curl -L -o modules/nixling/ext/spectrum-ch/cloud-hypervisor/0001-...patch \
...
When bumping CH (`ext/spectrum-ch/default.nix`) or the crosvm pin used for the GPU sidecar,
