...
export function prepareVerification(input, options = {}) {
  ...
  const requests = selection.roster.map((seat) => ({
    ...
    actual_delta: {
      ...
      context: input.actual_delta ?? null,
    },
...
export function evaluateApproval(input) {
  ...
  const lateBlockingIssues = lateIssues
    ...
    .map((issue) => issue.id);
  const statusBlocks = verification.flatMap((result) =>
    Object.entries(result.verified_issue_statuses)
      .filter(([, status]) =>
        ["open", "blocked", "unresolved", "regression", "deferred"].includes(
          statusName(status),
        ))
      .map(([issueId, status]) => ({
        seat: result.seat,
...
export function validateApprovalArtifact(approval, options = {}) {
  ...
  if (options.selection) {
    ...
    if (approval.selection_sha256 !== sha256(options.selection)) {
...
export function createApprovalArtifact(input, options = {}) {
  const approval = evaluateApproval(input);
  validateApprovalArtifact(approval, {
    selection: input.current_selection ?? input.selection,
    table: options.table ?? input.table,
  });
...
export function createMetricsArtifact(input, options = {}) {
  ...
  const artifact = {
    ...
    content_id: ledger.content_id,
    snapshot_sha256: ledger.snapshot_sha256,
    discovery_ledger_sha256: sha256(input.ledger_bytes ?? ledger),
    metrics,
  };
...
function legacyBundle(input) {
  ...
  const records = Array.isArray(rawRecords)
    ? rawRecords.map((record) => ({ name: undefined, record, bytes: Buffer.from(stableStringify(record)) }))
    : Object.entries(rawRecords).map(([role, record]) => ({
        name: `${role}.json`,
        record: { ...(isPlainObject(record) ? record : {}), role: record?.role ?? role },
        bytes: Buffer.from(stableStringify({ ...(isPlainObject(record) ? record : {}), role: record?.role ?? role })),
      }));
  return {
...
export function importLegacyRound(input, options = {}) {
  ...
  if (completeEnvelope && recordEntries.length !== LEGACY_ROSTER.length) {
    error("a coherent legacy request must contain exactly ten records");
  }
  if (!completeEnvelope && recordEntries.length >= LEGACY_ROSTER.length) {
    error("a complete fixed-ten legacy import requires its panel request");
  }
  const ordered = [];
...
function usage() {
  return [
    ...
    "  panel-lifecycle.mjs adapt-verification <ledger.json> <verdicts.json> <output.json>",
    ...
    "  panel-lifecycle.mjs metrics <ledger.json> <responses.json> [<verification-results.json>] <output.json> [--implementation-history PATH] [--verification-history PATH]",
...
async function main(argv) {
  ...
  try {
    ...
    if (command === "verification") {
      ...
      const delta = deltaPath
        ? readJson(deltaPath, "actual fix delta")
        : undefined;
      ...
      const result = writeVerificationArtifacts(argv[5], {
        ...
        responses,
        self_verification: selfVerification,
        current_candidate: candidatePath
          ? readJson(candidatePath, "current candidate")
          : candidateFromSelection(selection),
        ...(priorPath ? { prior_selection: readSelection(priorPath) } : {}),
        actual_delta_paths: actualDeltaPaths,
        ...(fullContextPath
          ? { full_context: readJson(fullContextPath, "full candidate context") }
          : {}),
      });
    ...
    if (command === "adapt-verification") {
      ...
      writeCreateOrCompare(argv[3], {
        artifact_kind: VERIFICATION_ARTIFACT,
        schema_version: SELECTION_SCHEMA_VERSION,
        discovery_ledger_sha256: sha256(ledgerBytes),
        results: adaptVerificationResults(verdicts, {
          issue_ids: ledger.issues.map((issue) => issue.id),
        }),
      });
      console.log(argv[3]);
    ...
    if (command === "approval" || command === "approve") {
      ...
      const ledger = JSON.parse(ledgerBytes);
      const responses = readJson(argv[3], "implementation responses");
      const verificationResults = readJson(argv[4], "verification results");
      const candidatePath = flagValue(argv.slice(6), "--candidate");
      const currentCandidate = candidatePath
        ? readJson(candidatePath, "current candidate")
        : candidateFromSelection(selection);
    ...
    if (command === "metrics") {
      let ledgerPath = argv[1];
      let responsesPath = argv[2];
      let verificationPath;
      let outputPath;
      let optionsStart;
      if (existsSync(ledgerPath) && statSync(ledgerPath).isDirectory()) {
        const roundDir = ledgerPath;
        ledgerPath = existsSync(join(roundDir, "discovery-ledger.json"))
          ? join(roundDir, "discovery-ledger.json")
          : join(roundDir, "ledger.json");
        responsesPath = join(roundDir, "responses.json");
        verificationPath = existsSync(join(roundDir, "verification-results.json"))
          ? join(roundDir, "verification-results.json")
          : undefined;
        outputPath = argv[2] && !argv[2].startsWith("--")
          ? argv[2]
          : join(roundDir, "metrics.json");
        optionsStart = outputPath === argv[2] ? 3 : 2;
      } else if (argv[4] && !argv[4].startsWith("--")) {
        verificationPath = argv[3];
        outputPath = argv[4];
        optionsStart = 5;
      } else {
        outputPath = argv[3];
      ...
      if (!ledgerPath || !responsesPath || !outputPath) error(usage());
      const ledgerBytes = readFileSync(ledgerPath, "utf8");
      const ledger = JSON.parse(ledgerBytes);
      const responses = readJson(responsesPath, "implementation responses");
      const verificationResults = verificationPath
        ? readJsonDirectory(verificationPath, "verification results")
        : undefined;
      const optionsArgv = argv.slice(optionsStart);
