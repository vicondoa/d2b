...
- `d2b_host::nftables::BusId` now enforces the USB busid grammar at the type boundary: the inner `String` is private, `BusId::new` validates via `media::validate_usb_busid` and returns `Result`, and a `TryFrom<&str>` conversion is provided; the `#[serde(transparent)]` wire shape is unchanged.
