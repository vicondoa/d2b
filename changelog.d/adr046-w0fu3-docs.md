...
- Contributor docs: aligned the test quick-start with shipped behaviour.
  ...
  `test-runtime-ledger` Layer-1 job and its census/baseline regeneration
  workflow (`make runtime-ledger-regen`), noting that the ledger's enforcement
  scope may be rescoped and that the `Makefile` and `tests/layer1-jobs.json` are
  authoritative if the prose diverges.
