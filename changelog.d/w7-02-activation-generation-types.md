...
- The activation-runner `targetGeneration` contract field is typed as
  the nonzero `NixosGenerationOrdinal` identity instead of a bare `u64`,
  so a zero ordinal is refused when the input decodes rather than by a
  manual check in the constructor. The wire bytes are unchanged, and the
  committed EphemeralProcess schema now carries the ordinal as a
  definition reference instead of an inline minimum-1 integer.
