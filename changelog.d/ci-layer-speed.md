...
- Cut Rust build and test time by reducing development-profile debug data, retaining an explicit full-debug profile, running pull-request work as two required jobs, and caching the toolchain-keyed compiler and rustdoc scratch trees; the local comprehensive gate still runs every surface.
