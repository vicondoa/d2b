...
- Cut Rust build and test time by reducing development-profile debug data, retaining an explicit full-debug profile, running pull-request work as two required jobs, and replacing the serial HTML API inventory with one compiler-derived workspace rustdoc JSON census; the local comprehensive gate still runs every surface, and all compiler and rustdoc warnings now fail the gate.
