...
Use this only at LFG closeout, after the current pipeline has finished or
stopped for good. Its purpose is to offer continuity into a separately planned
...
the area this plan covers, one or more future areas intended for separate
...
implementation, review, PR, or merge task belongs to the current run's remaining
shipping work; it is not a next brainstorm candidate and does not by itself
suppress a valid future-area offer.
...
brief. LFG owns the recommendation: it has already chosen, so do not ask
`ce-handoff` to rediscover or rank candidates. Include:
...
- **Actual delivery state:** what finished and what remains in the current run's
  shipping steps, such as implemented, PR open, CI decided, or not merged.
...
Keep the brief short, and point at files rather than copying them. Do not paste
the prior Product Contract into it; the plan remains authoritative and the
handoff explains what the next session needs to decide.
