...
| `{skill_dir}` | Orchestrator | Absolute path of the ce-sweep skill directory, so the sub-agent can run the bundled analyzer. The sub-agent does not inherit the orchestrator's shell state. |
