...
1. **Source type**, one of:
   ...
   - `email-experimental`, an email account or folder (experimental; stored in config as `type: email`)
2. **Identity**, which depends on the type:
...
Then assign the source a **short config id**. This is a stable, lowercase, hyphenated handle that the state file and reports use to name this source (e.g. `slack-alpha`, `gh-issues`). Suggest one derived from the type and let the user override it. Ids must be unique within `feedback_sources`.
...
Every source carries two source-side actions the sweep can perform, plus a standing approval that decides whether it may perform them unattended.

**Ask the acknowledgment action.** This is what the sweep does to mark an item *seen* on its source:

- Slack: an **emoji reaction** name. Default `eyes`.
- GitHub: a **label** to apply. Default `feedback:ack`.
- Email: none. Email items are tracked only in state; there is no source-side ack. Skip this question for email sources and say so.

**Ask the close-out action.** This is what the sweep does to mark an item *resolved* on its source:

- Slack: an emoji reaction. Default `white_check_mark`.
- GitHub: a label. Default `feedback:resolved`.
- Email: none. Email items stay state-tracked only. Explain that there is no source-side close-out.
...
- **No** -> `approved: false`. The source is read-only. Its items are tracked as `ack_deferred` and no reaction or label is ever written.

For email sources there are no source-side actions, so approval does not apply. Record `approved: false` and note that the source is inherently read-only.
...
**Ask:** "Should item content from `{{source id}}` be withheld from committed state and from plan text? Say yes when the source can carry screen recordings, PII, customer data, or anything you don't want written to a file that may be committed or shared. When yes, the sweep drops item body and quote before writing state, so only titles, urls, ids, and status persist. Default is no."

- **No** (default) -> `sensitive: false`. Full item content is retained in state and available to plans.
- **Yes** -> `sensitive: true`. The state engine drops `body` and `quote` at write time for this source's items, and plans reference items by id, title, and url only.
...
- **Yes** -> ask for the file path. Then build a `--source-map`. For each legacy channel or source id in the file, pair it with the configured source id from section 1 (the short name the live connector reads by), as a JSON object like `{"C0AQLMQBGBD":"slack-alpha"}`. This map is required. Without it, an imported `C0AQLMQBGBD` cursor lands under `C0AQLMQBGBD` while the connector reads under `slack-alpha`. That orphans the cursor and re-ingests everything on the first sweep. Run the import from **this skill's directory**; set `SKILL_DIR` inline to the absolute path of the directory containing the `SKILL.md` you loaded:
...
Then show the user the resulting Sweep section in chat and offer **one round of edits**.
