...
complete -c nixling -n "__fish_nixling_using_subcommand vm; and not __fish_seen_subcommand_from start stop restart list status exec help" -f -a "exec" -d 'Run a command inside the VM over the authenticated guest-control transport (no SSH). `nixling vm exec <vm> -- <cmd...>` runs a non-interactive command; `nixling vm exec -it <vm> -- <cmd...>` allocates a guest PTY for an interactive session. Routed CLI → daemon `public.sock` (admin-only) → authenticated guest-control vsock → guestd exec RPCs'
