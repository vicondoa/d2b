...
complete -c nixling -n "__fish_nixling_using_subcommand vm; and not __fish_seen_subcommand_from start stop restart list status exec help" -f -a "list" -d 'Daemon-side runtime view (different from `nixling list`, which is the static manifest view)'
