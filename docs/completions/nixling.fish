...
complete -c nixling -n "__fish_nixling_using_subcommand help; and not __fish_seen_subcommand_from list status usb console audio audit host auth realm vm up down restart build generations switch boot test rollback gc store keys trust rotate-known-host migrate config help" -f -a "usb" -d 'USB attach / detach / probe'
