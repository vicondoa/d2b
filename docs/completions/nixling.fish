...
complete -c nixling -n "__fish_nixling_needs_command" -f -a "vm" -d 'Per-VM lifecycle verbs (start / stop / restart / list / status) plus the admin-only guest-control sub-verbs `exec` and `konsole`, which run commands or an interactive session inside a VM over the authenticated guest-control transport (no SSH)'
...
complete -c nixling -n "__fish_nixling_using_subcommand vm; and not __fish_seen_subcommand_from start stop restart list status konsole exec help" -f -a "konsole" -d 'Open an interactive guest session in a host terminal. Thin wrapper that hosts `nixling vm exec -it <vm> -- /run/current-system/sw/bin/bash -l` in the chosen terminal emulator (default `konsole`, overridable via `--terminal`) over the authenticated guest-control transport. There is no SSH; the retired SSH-only flags `--host`/`--key`/`--user` are rejected with a migration message'
...
complete -c nixling -n "__fish_nixling_using_subcommand vm; and __fish_seen_subcommand_from help" -f -a "konsole" -d 'Open an interactive guest session in a host terminal. Thin wrapper that hosts `nixling vm exec -it <vm> -- /run/current-system/sw/bin/bash -l` in the chosen terminal emulator (default `konsole`, overridable via `--terminal`) over the authenticated guest-control transport. There is no SSH; the retired SSH-only flags `--host`/`--key`/`--user` are rejected with a migration message'
...
complete -c nixling -n "__fish_nixling_using_subcommand help; and __fish_seen_subcommand_from vm" -f -a "konsole" -d 'Open an interactive guest session in a host terminal. Thin wrapper that hosts `nixling vm exec -it <vm> -- /run/current-system/sw/bin/bash -l` in the chosen terminal emulator (default `konsole`, overridable via `--terminal`) over the authenticated guest-control transport. There is no SSH; the retired SSH-only flags `--host`/`--key`/`--user` are rejected with a migration message'
