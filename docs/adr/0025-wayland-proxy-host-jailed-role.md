...
At the time this ADR was accepted, D2b graphics VMs bridged guest
...
The crosvm GPU sidecar (`d2b-<vm>-gpu`) and render-node sidecar
(`d2b-<vm>-gpu`, render-node mode) both currently bind-mount the
...
5. exposes a per-VM filter socket at `/run/d2b-wlproxy/<vm>/wayland-0`
   for crosvm to connect to instead of the real compositor socket.

## Decision

Add a new process role `wayland-proxy` to the d2b broker/daemon
role system. The role:

### Identity and isolation

- Principal: `d2b-<vm>-wlproxy` (dedicated system user + group).
...
- Dedicated per-VM runtime directory: `/run/d2b-wlproxy/<vm>`.
- Filter listen socket: `/run/d2b-wlproxy/<vm>/wayland-0` (where
  the crosvm GPU sidecar connects instead of the real compositor socket).
- In-jail upstream socket: `/run/d2b-wlproxy/<vm>/upstream` (where
...
- Writable: `/run/d2b-wlproxy/<vm>` only.
- Bind-mount: real host compositor socket (`waylandHostSock`) → in-jail
  path `/run/d2b-wlproxy/<vm>/upstream`. Read/write.
...
- Mid-life failure: the `d2bd` pidfd watchdog is extended
