...
and emits the first-party `BUILD.bazel` files. `cargo xtask gen-bazel --check`
regenerates into a scratch tree and fails on any difference; it is wired into
`test-drift`, which already exists to catch exactly this class of staleness for
the other `xtask gen-*` outputs.
...
runs it, and to a pinned minimum census count where the surface is a scan
(section 13). The map is the normative one-to-one statement:
...
in the graph that no mapped suite transitively includes; or a hand-written
Bazel fragment exists that the map does not list. The guard's own baseline set
is read from the committed reference document, not duplicated, so the two
cannot drift apart.
...
`Cargo.toml` set, its `Cargo.lock`, and its `deny.toml`. `cargo deny` shells
out to `cargo metadata`, which needs a resolvable registry; the action supplies
a `CARGO_HOME` populated from the `crate_universe` fetch, so the check is
hermetic and cacheable. The gate today falls back to `nix shell` when
`cargo-deny` is absent and fails closed when neither is available, citing ADR
0009's no-waiver rule; the Bazel form has no fallback at all, because the tool
is a declared input.

**cargo-audit (three surfaces).** Same shape, plus the advisory database
becomes a declared, pinned input rather than a network fetch. The flake already
pins a RustSec advisory database snapshot by rev and hash for its own
`rust-audit` check; the Bazel targets consume a pin of the same shape. This
...
**Schema reproducibility.** Two Bazel actions run `xtask gen-schemas` into two
distinct declared output trees, and a test compares digests. The property under
test is unchanged: two independent invocations must agree.
...
group is silently unscanned. Guard: the test asserts a pinned minimum file
count and fails closed on an empty or short scan, matching the existing gate's
refusal to infer an empty `harness = false` set from a failed listing.
...
suites is run with the libtest `--list` interface as a Bazel action; the union
of those listings is the census the committed pins under
`tests/golden/pinned/` are compared against. `harness = false` targets have no
listing interface and are covered by target-level census instead, which is the
...
### 7. Failure reporting, scheduling, and the broker
...
The Bazel output base lives under the worktree's gitignored scratch tree so
`make clean` can reclaim it, and `make clean` learns about it in the same
change. That target already reclaimed 68 GB on a working worktree before Bazel
existed; a second uncollected cache tree is not acceptable.
...
**Caches arrive at promotion, in the same change that frees the space.** The
promotion change (section 12) deletes the `Swatinem/rust-cache` configuration
and its entries, freeing about 6.5 GiB, and only then introduces:
...
over-budget snapshot, and it queries the repository cache usage API and refuses
to publish when the post-save repository total would exceed 8 GiB of the 10 GB
budget. A refused publish is a failed step, not a silent skip.
...
required context, a miss does not block unrelated work.

At promotion, the ceiling is enforced as the job's `timeout-minutes`, set to
15, rather than as a latency assertion inside the gate.
`tests/layer1-jobs.json` classifies `test-performance-budgets` as advisory for
exactly the reason that a hosted runner cannot support an enforced latency
budget, and this ADR does not contradict that committed position. A timeout is
a structural ceiling that fails closed with no percentile machinery and no
flakiness beyond the ceiling itself. During the shadow stage the timeout is 30
minutes, so a miss is measured rather than truncated.
...
4. **Census.** `//ci/rust:pinned_test_inventory` passes against the Bazel
   census, every suite's listing is nonempty, and every scan surface meets its
   pinned minimum count.
5. **Repetition.** The three broker suites pass twenty consecutive executions
   (`--runs_per_test=20`) with `exclusive` in force, evidencing that Bazel's
   scheduling has not reintroduced the process-global-state failure the current
   gate avoids by construction.
6. **Budgets.** Section 11's rule is satisfied for all three profiles, with
   every measurement recorded.
7. **Cache.** The shadow stage published no Actions cache entry, and the
   promotion change deletes the `Swatinem/rust-cache` configuration and its
   entries in the same commit that introduces the Bazel entries.
...
- keeps the eight `make test-rust-<leaf>` names as thin aliases onto the Bazel
  ...
  contributor documentation stay correct, and removes `make test-bazel-rust`
  and its four slice aliases, because two names for one thing is the drift this
  repository spends the most effort preventing.
...
- **cargo-audit stops reaching the network.** The advisory database becomes a
  pinned input. This removes the retry loop and makes the surface
  deterministic; it also means database freshness is now a property of a
  committed pin.
- **Test process topology.** Cargo runs one test binary at a time with
  intra-binary threads; Bazel runs test targets concurrently with one process
  per target. This is a real behavioural change for every suite. It is why
  section 12 requires the repetition evidence and why the broker suites are
  `exclusive`.
...
**A repository-scanning test that passes because it scanned nothing.** Twenty
test files resolve `CARGO_MANIFEST_DIR` and eleven define a `repo_root()`
helper that walks out into the working tree. Under Bazel those tests see a
runfiles tree containing only declared inputs. A policy scan that finds no
files reports no violations and exits zero, which is indistinguishable from a
clean repository. The gate stays green while the coverage evaporates. Guard:
every migrated scanning surface asserts a pinned minimum census count recorded
in the coverage map, fails closed below it, and carries one planted control
input that the scan must detect. The repository already uses both idioms, in
the `harness = false` discovery that refuses to infer an empty set and in the
nonzero-coverage assertions added for ADR 0051.

**Eviction of the required path's cache.** The Actions cache is measured at
8.44 GiB of 10 GB across six entries. Publishing a Bazel snapshot while the
Cargo path is still required evicts rust-cache entries, and the required Rust
leaves fall from a 7m46s critical path toward the documented 43-minute cold
cost, into 60-minute job timeouts, on unrelated pull requests. The failure is
worst because it appears on other people's branches with no connection to the
Bazel work. Guard: the shadow stage publishes no cache entry at all; the
promotion change deletes the rust-cache entries in the same commit that
introduces the Bazel ones; and the save step refuses to publish when the
measured entry exceeds its budget or when the post-save repository total would
exceed 8 GiB.
...
6. Pull requests never publish a shared cache entry, and a structural policy
   test enforces it. Only one default-branch job writes.
...
10. The three privileged-broker feature suites never run concurrently with each
    other or with other tests, and relaxing that requires a dedicated isolation
    review.
11. Every scanning surface fails closed on an empty or short scan against a
    pinned minimum recorded in the coverage map.
12. A missed performance ceiling blocks promotion or fails a job. It never
    licenses reducing coverage, reclassifying an enforcing surface as advisory,
    or relaxing the ceiling.
13. The required continuous-integration context name `test-rust` does not
    change across promotion.
14. `.github/workflows/pr-bazel-rust.yml` existing means promotion is
    incomplete.
