...
| Field | Detail |
| --- | --- |
| Status | **Completed for the P0 foundation, reviewed live POC, and gateway-mode REST lifecycle bridge.** The branch ships the ACA/Relay/Waypipe reference path, gateway display orchestrator/runtime seams, target routing, gateway VM declaration surface, credential/audit hardening, a reviewed live MI-authenticated ACA Wayland demo, and the follow-up REST data-plane lifecycle bridge that makes gateway-mode `nixling vm start <aca target>` create/resume the ACA sandbox instead of returning a ready-shaped stub. |
