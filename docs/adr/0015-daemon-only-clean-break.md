...
1. **Per-VM systemd templates** — `d2b@<vm>.service`,
   ...
   `d2b-otel-relay@<vm>.service`, `d2b-known-hosts-refresh@.service`,
   `d2b-vfsd-watchdog@.{service,timer}`, and the
   per-env `d2b-sys-<env>-usbipd-{proxy,backend}.{service,socket}`
...
are all true with first-run `/var/lib/d2b/validated/<wave>.json`
evidence records (P5).
...
- **Audit-surface sprawl.** Every legacy template was a separate
  ...
  `d2bd` meant auditing N+1 surfaces forever.
- **Two-writer hazard.** ADR 0007's `legacy-systemd` /
  ...
  "d2bd"` field and a `/run/d2b/locks/<vm>` filesystem lock
  to keep two writers off the same VM. Both became dead weight once
  every VM was daemon-owned.
- **Operator confusion.** With two CLIs (`d2b` Rust binary and
  `d2b-legacy` bash) and three modes, every support thread
...
`d2b.compat.legacySystemd = true` for one minor release (v0.5),
...
From v1.0.0, `d2bd` and `d2b-priv-broker` are the **only**
persistent root surfaces the d2b framework declares.
...
- The fixed root is `/sys/fs/cgroup/d2b.slice`. The systemd unit
  for `d2b-priv-broker.service` declares
  `Delegate=cpu cpuset io memory pids` against this slice.
- The broker `fchown`s the slice subtree to the non-root `d2bd`
...
- Teardown uses **broker-mediated `CgroupKill`** (v1.1-P10 op
  ...
  (see `packages/d2bd/src/supervisor/pidfd_table.rs`) is the
...
The following are removed wholesale in P6 (no `d2b.compat.*`
...
- Every per-VM systemd template enumerated in the legacy-unit
  ...
  `d2b@<vm>.service`, `microvm@<vm>.service`,
  `microvm-virtiofsd@<vm>.service`,
  `microvm-{tap-interfaces,macvtap-interfaces,pci-devices,set-booted}@.service`,
  `d2b-<vm>-{gpu,snd,video,swtpm,store-sync}.service`,
  `d2b-otel-relay@<vm>.service`,
  `d2b-known-hosts-refresh@.service`,
  `d2b-vfsd-watchdog@.{service,timer}`,
  `d2b-sys-<env>-usbipd-{proxy,backend}.{service,socket}`.
- Every host singleton framework service:
  `d2b-{ch-exporter,otel-host-bridge,net-route-preflight,audit-check}.service`,
  `d2b-audit-check.timer`, `microvms.target`.
- The `cli.nix` Nix module and every bash entrypoint under
  `scripts/` it packaged. The `/run/current-system/sw/bin/d2b`
...
- The W14c bash fallback bridge and its env-knob escape hatches:
  ...
  `D2B_NATIVE_ONLY` (the latter retained as a no-op only because
  P4 cli-up already documented it; future minor releases may delete
  the no-op too).
- The `d2b.vms.<vm>.supervisor` option. **Removed in v1.1-P2**
  ...
  "d2bd" the only valid supervisor; v1.1 completes the migration
  by deleting the option entirely.

  Migration: remove every "supervisor = ..." line from your consumer
  flake's d2b.vms.<vm>.* declarations. The daemon-only path is
...
- The `d2b-launcher` polkit allowlist and its per-VM rules. The
  group itself stays declared for permission-boundary continuity but
  no polkit rule consumes it.
- The per-VM filesystem lock at `/run/d2b/locks/<vm>` (the
...
- **Single audit surface.** The framework declares three units. Every
  root-touching code path lives behind `d2b-priv-broker` and is
...
- **Smaller TCB.** Per-VM systemd templates ran as bespoke users
  (`microvm`, `d2b-<vm>-gpu`, …) each with their own ACL surface
  ...
  and one audit trail.
- **Daemon health is the only health.** `d2b host doctor` and
  `d2b host validate` report against `d2bd` + broker only,
...
- **Operator UX.** One CLI, one config option family
  (`d2b.daemonExperimental.*` retired alongside the bash
...
- **No v2 → v3 compatibility window.** Consumer hosts on v0.4 cannot
  ...
  guide ([`docs/how-to/migrate-d2b-v0-to-v1.md`](../how-to/migrate-d2b-v0-to-v1.md))
  is required reading.
- **Hard dependency on daemon health.** With the per-VM systemd
  templates gone, there is no "systemctl can still bring a VM up if
  `d2bd` is wedged" escape hatch. Mitigations:
  - `d2b-priv-broker.socket` is socket-activated, so the broker
    cold-starts on first verb dispatch and does not require the
    daemon to be up.
  - `d2bd.service` carries `Restart=always` with a bounded
    `RestartSec`, so a crash recovers without operator intervention.
  - `d2b host doctor` (a daemon-independent CLI surface) reports
    daemon liveness and the broker `health/v1` probe.
  - The supervisor's persisted DAG state under
    `/var/lib/d2b/supervisor/state.json` survives daemon
    restarts; on restart, `d2bd` reconciles against the recorded
    state and resumes supervision without losing pidfd handoffs
    (see [ADR 0011](0011-cgroup-v2-delegation-and-pidfd-handoff.md)).
- **Single point of failure for lifecycle ops.** `d2bd` is the
...
- **`cli.nix` package retired.** The Nix module that packaged the
  ...
  (`d2b-launch-<vm>`) are regenerated by the new daemon-native
  launcher module emitting `d2b vm start --apply` calls (P6
...
- **`cli-contract.md` becomes the operator-facing surface.** With
  ...
  envelope. The bash `man d2b` is retired in P7 (`ph7-p7-rust-cli-manpage`).
- **AGENTS.md "Naming conventions" table shrinks.** Most rows
  (`d2b@<vm>.service`, `microvm@<vm>.service`,
  `d2b-<vm>-<role>.service`, host-singleton `d2b-<role>.service`)
