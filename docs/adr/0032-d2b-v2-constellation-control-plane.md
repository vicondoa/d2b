...
D2b v1 is an optimized single-host desktop microVM framework. The
operator's local `d2b` CLI talks to `d2bd` over the local public
Unix socket. `d2bd` owns per-VM lifecycle DAGs and delegates
privileged host mutations to `d2b-priv-broker`. Guests run
`d2b-guestd`; framework guest control uses authenticated ttRPC over
...
evolve d2b into a constellation of managed execution environments
...
control-plane process is still `d2bd`. A constellation is **d2bd
instances working together**. The old working label `d2bk` is retired
as a separate component name; the new work is the `d2bd`
constellation/peer protocol, provider abstraction set, and
transport/session machinery that let one `d2bd` route work to another
`d2bd` or provider adapter.
...
D2b v2 will evolve `d2bd` into a transport-neutral constellation
...
`d2bd` peer-session layer must provide end-to-end authentication,
...
The host entrypoint remains the `d2b` CLI for local VM lifecycle.
Realms have an **entrypoint mode**:

| Entrypoint mode | Where it runs | Use |
| --- | --- | --- |
| `host-resident` | Host `d2bd` local fast path | Local-only or trusted-host realms whose workloads live on this host and do not require relay/provider credentials. |
| `gateway-guest` | Dedicated local d2b guest VM | Realms with remote nodes, provider integrations, work-managed credentials, or any trust boundary that should stay inside a microVM. |

The default for cross-host, work, and provider realms is a dedicated local
d2b guest VM: the **realm gateway**. The host daemon manages that
...
Inside the gateway guest lives a realm-scoped `d2bd` instance plus
...
`d2bd` does not become a generic WAN relay daemon. Local hosts keep
the current Unix-socket fast path by default. In v2, however, `d2bd`
...
Each gateway-backed realm maps to an isolated d2b environment/network
...
```text
d2b CLI
  -> local d2bd over /run/d2b/public.sock
    -> d2b-priv-broker over /run/d2b/priv.sock
    -> local d2b-guestd over authenticated ttRPC/vsock
...
CLI-to-daemon transport is `/run/d2b/public.sock`; v2 treats that as
one `DaemonAccessTransport` binding rather than the only possible shape.

```text
d2b CLI or web UI
  -> D2b API frontend transport
    ...
    -> explicit SSH bootstrap     (setup/recovery only)
      -> d2bd ApiService
        ...
        -> operation router
          ...
          -> gateway peer         (realm-scoped d2bd in gateway guest)
          -> remote daemon peer   (remote/cloud d2bd)
          -> provider adapter     (managed sandbox/session)
```

This lets a future cloud-hosted `d2bd` be controlled by the same
`d2b` CLI without pretending the local Unix socket exists. The remote
...
arrives, that `d2bd` still re-originates privileged work through its
...
`d2bd` without inbound firewall holes. A work/provider realm can still
...
### d2bd internal architecture

To support CLI, web, local, gateway, and cloud uses without duplicating
business logic, `d2bd` grows three internal layers:
...
operation router
  target address -> dispatch target
      ...
      |
      +--> PeerDaemonExecutor
      |      constellation peer session to another d2bd
      |
      +--> GatewayExecutor
      |      local guest-control into gateway's realm-scoped d2bd
      |
      +--> ProviderExecutor
...
| --- | --- | --- |
| `ApiFrontend` | Accept connections, frame/decode requests, authenticate transport principal, produce `AuthenticatedRequest`. | Today this is `public.sock` parsing in `packages/d2bd/src/wire.rs`. |
| `ApiService` | Own typed command semantics independent of transport. | Today logic is spread across `packages/d2bd/src/lib.rs` command dispatch helpers. |
...
| `PeerDaemonExecutor` | Open/reuse constellation peer sessions to another `d2bd`, forward semantic operations, return typed results. | New v2 module using transport/session/codecs. |
| `GatewayExecutor` | Enter a local gateway VM and invoke its realm-scoped `d2bd` API. | Existing local guest-control exec/PTY path plus v2 API binding. |
| `ProviderExecutor` | Call limited/full provider adapters when no full host daemon exists. | New v2 provider modules. |

`d2bd` modes are configuration profiles over the same binary:
...
should not reimplement orchestration; it connects to a `d2bd`
...
```text
host d2b CLI
  -> local d2bd resolves target name
    -> host-resident realm:
         local d2bd applies realm policy and dispatches local workload
    -> gateway-backed realm:
         local d2bd starts/enters gateway VM
           -> gateway guest runs realm-scoped d2bd
             -> gateway transport provider opens outbound relay/direct session
               -> remote/cloud d2bd or provider adapter on the target node
                 -> node-local adapter
                   -> local d2bd/broker/guestd when the node is a full host
...
available. The gateway's `d2bd` owns the realm session and sends
constellation operations or named stream opens to peer `d2bd` instances or
provider adapters. It never sends raw local daemon frames, broker
requests, or guest ttRPC frames over the transport. A target `d2bd`
...
Operator UX should be deeply integrated into the existing `d2b` CLI.
`constellation` is the daemon-to-daemon protocol/library stack. Most operators
should not need to invoke it directly. Host-side `d2b` remains the
...
d2b vm list
d2b vm exec personal-dev -- hostname

# constellation addresses use DNS-shaped target names
d2b vm list --all
d2b vm exec build-vm.remote-dev.work.d2b -- hostname
d2b vm stop gpu-dev.homelab.personal.d2b

# interactive escape hatch; commands run inside the gateway guest
d2b realm enter work
work-gw$ d2b vm list --all
work-gw$ d2b vm exec build-vm.remote-dev.work.d2b -- hostname

# explicit low-level one-shot; equivalent to what the integrated facade
# does internally for a work-realm target
d2b realm run work -- d2b vm list --all
d2b realm run work -- d2b vm exec build-vm.remote-dev.work.d2b -- hostname
```

### Target address and name scheme

D2b v2 uses a **target address**, not a network address. It names an
...
<workload>.d2b                 local workload on this host; current-node form
...
workload as `api.build.payments.work.d2b`. Internally, policy can
...
host-local work, CLI input may omit it and use `workload.node.d2b`.
The canonical machine form still expands the local realm explicitly:

```text
d2b://build-vm.remote-dev.work.d2b  ==  build-vm.remote-dev.work.d2b
d2b://api.build.payments.work.d2b   ==  api.build.payments.work.d2b
d2b://personal-dev.laptop.local.d2b ==  personal-dev.laptop.d2b
d2b://personal-dev.laptop.local.d2b ==  personal-dev.d2b
```

`.d2b` is an internal d2b target-name suffix by default. It is
...
- `realm`, `node`, and `workload` labels use the existing d2b-style
...
- `d2b` is the reserved target-name suffix.
- `all` and `*` are list-only selectors, not valid persisted labels.
- A two-label name such as `build-vm.work.d2b` is always parsed as
  workload `build-vm` on local node `work`; it is **not** shorthand for
  realm `work`. Use `build-vm.<node>.work.d2b` or list flags.
- One label before `.d2b` means `<workload>.this.local.d2b`.
- Two labels before `.d2b` means `<workload>.<node>.local.d2b`.
- Three or more labels before `.d2b` mean
  `<workload>.<node>.<realm-path>.d2b`. The first two labels are
...
local           host-resident   this host's d2bd
personal        host-resident   this host's d2bd
work            gateway-guest   work-gateway.laptop.local.d2b
ops             gateway-guest   ops-gateway.laptop.local.d2b
```
...
dispatch: local d2bd applies personal realm policy, then local fast path
```
...
```text
personal        gateway-guest   personal-gateway.laptop.local.d2b
```

Then `dev-vm.laptop.personal.d2b` is forwarded to that gateway rather
than dispatched directly by host `d2bd`. This keeps target names stable
while allowing realm placement to evolve.

Resolution algorithm:

1. Strip the reserved `.d2b` suffix and optional `d2b://` prefix.
2. Split labels left-to-right.
3. If there is one label, expand to
   `<workload>.this.local.d2b`.
4. If there are two labels, expand to
   `<workload>.<node>.local.d2b`.
...
6. Convert the realm path to policy order by reversing it. For
   `api.build.payments.work.d2b`, the target labels are:
   - workload: `api`;
   - node: `build`;
...
8. If the entrypoint is host-resident:
   - require that the target node is this host or an explicitly configured
     local-node alias;
   - apply host-resident realm policy in `d2bd`;
...
| --- | --- | --- |
| `personal-dev` | Treat as local shorthand; dispatch directly to local `d2bd` fast path. | None. |
| `personal-dev.d2b` | Treat as current-host shorthand for routing. Machine output expands to the configured local node, e.g. `d2b://personal-dev.laptop.local.d2b`. | None. |
| `personal-dev.laptop.d2b` | Treat as local-realm node-qualified shorthand for `d2b://personal-dev.laptop.local.d2b`. If `laptop` is this host, dispatch locally through the host-resident `local` realm. Otherwise require a configured entrypoint for that local-realm node. | Local realm resolver handles non-current nodes. |
| `dev-vm.laptop.personal.d2b` | If `personal` is configured as host-resident and `laptop` is this host, apply personal realm policy locally and dispatch through local `d2bd`. | None unless the target node is not this host. |
| `build-vm.remote-dev.work.d2b` | Parse only the `work` realm label, select the local work gateway, and forward `build-vm.remote-dev.work.d2b` to it over guest-control. | The work gateway resolves `remote-dev` and `build-vm` in its realm registry and policy. |
| `api.build.payments.work.d2b` | Parse the realm path `payments.work`, select the gateway for the nearest configured realm boundary, and forward the full name. | The gateway applies parent/child realm policy and resolves `build`/`api` inside that nested realm path. |
| `d2b://build-vm.remote-dev.work.d2b` | Same as `build-vm.remote-dev.work.d2b`; canonical form for config, audit, and machine output. | Same. |
...
d2b://personal-dev.laptop.local.d2b        personal-dev.laptop.d2b       microvm   running
d2b://work-gateway.laptop.local.d2b        work-gateway.laptop.d2b       gateway   running
d2b://build-vm.remote-dev.work.d2b         build-vm.remote-dev.work.d2b  microvm   running
d2b://api.build.payments.work.d2b          api.build.payments.work.d2b   microvm   running
d2b://session-42.aca.work.d2b              session-42.aca.work.d2b       sandbox   running
```
...
For a `build-vm.remote-dev.work.d2b` target, host `d2b` parses the
address, selects the `work` gateway guest, asks local `d2bd` to open
...
nested target such as `api.build.payments.work.d2b`, the host selects
...
`d2b vm list --all` follows the same boundary. The host enumerates
...
`d2b realm run` remains the explicit low-level primitive for scripts
that want to run an exact command inside a gateway. `d2b realm enter`
remains the debugging escape hatch that gives the operator an interactive
shell inside the realm trust domain.

### Full-host node flow

For a remote machine that can run d2b as a full host:

```text
gateway guest d2bd session
  -> remote d2bd peer
    -> remote d2b-priv-broker for host mutation
    -> remote d2b-guestd over host-local ttRPC/vsock
```
...
```mermaid
flowchart LR
    operator["operator<br/>d2b CLI<br/>vm list / vm exec<br/>DNS-shaped target names"]

    subgraph host["local host (trusted; no realm relay credentials)"]
        public_sock["/run/d2b/public.sock"]
        d2bd["d2bd<br/>local lifecycle DAG supervisor"]
        broker_sock["/run/d2b/priv.sock"]
        broker["d2b-priv-broker<br/>privileged host mutations"]
        ...
        subgraph personal_env["personal realm env / isolated host bridge"]
            ...
            personal_gw["personal gateway guest<br/>realm-scoped d2bd<br/>transport adapters<br/>realm creds/config/audit"]
            personal_vm["personal workload guest<br/>d2b-guestd + apps"]
        end

        subgraph work_env["work realm env / isolated host bridge"]
            work_net["sys-work-net<br/>NAT/DHCP net VM"]
            work_gw["work gateway guest<br/>realm-scoped d2bd<br/>work relay creds/config/audit"]
            work_vm["work workload guest<br/>d2b-guestd + apps"]
        end
    end
    ...
    subgraph remote["remote/provider side"]
        remote_node["remote full d2b host<br/>d2bd -> local broker -> guestd"]
        provider["provider-managed sandbox/session<br/>limited capability adapter"]
        work_service["work-hosted node/service"]
    end

    operator -->|"local CLI facade"| public_sock --> d2bd
    d2bd -->|"local broker requests"| broker_sock --> broker
    ...
    d2bd -->|"host-to-gateway guest-control:<br/>fan-out, argv, stdio, status"| personal_gw
    d2bd -->|"host-to-gateway guest-control:<br/>fan-out, argv, stdio, status"| work_gw
    d2bd -->|"local guest-control"| personal_vm
    d2bd -->|"local guest-control"| work_vm

    personal_gw -->|"gateway d2bd opens<br/>outbound peer session"| azure
    personal_gw -->|"gateway d2bd opens<br/>alternative transport"| selfhosted
    work_gw -->|"gateway d2bd opens<br/>work realm peer session"| azure
...
1. Host-local control plane
   ...
   operator
     |
     | d2b CLI over /run/d2b/public.sock
     v
   d2bd
     |  \
...
2. Per-realm layout on the host
   ...
   personal realm env (own bridge/L2 segment)
     ...
     - personal gateway guest     realm-scoped d2bd
                                  transport adapters
                                  personal creds/config/audit
     - personal workload guest    d2b-guestd + apps

   work realm env (own bridge/L2 segment)
     - sys-work-net               NAT/DHCP net VM
     - work gateway guest         realm-scoped d2bd
                                  transport adapters
                                  work creds/config/audit
     - work workload guest        d2b-guestd + apps
...
3. How the host CLI reaches the constellation
   ...
   d2bd
     ...
     | - for exec/start/stop: call the gateway d2bd API
   ...
   work gateway guest
     |
     | realm-scoped d2bd opens a constellation peer session
     | to a remote/cloud d2bd or provider adapter
     v
...
4. Gateway-to-remote path
   ...
   personal gateway guest
     |
     | gateway d2bd opens outbound peer session
     v
   untrusted relay / rendezvous
     - Azure Relay
   ...
   remote/provider side
     - remote full d2b host:
         d2bd -> local broker -> guestd
...
D2b v2 adds one new realm/constellation protocol and one new
daemon-access abstraction. It does not replace the existing local
protocols.

| Hop | Protocol after this ADR | Changes in v2 |
| --- | --- | --- |
| `d2b` CLI -> `d2bd` daemon access | Semantic daemon API carried by a `DaemonAccessTransport`. Local binding remains existing Unix `SOCK_SEQPACKET` public socket, 4-byte little-endian length prefix, strict JSON DTOs, typed errors. Future bindings may use mTLS/QUIC/WebSocket/relay/SSH bootstrap. | The local binding is unchanged. The daemon API becomes transport-neutral so a future cloud/remote `d2bd` can be reached without assuming `/run/d2b/public.sock`. |
| Host `d2bd` -> `d2b-priv-broker` | Existing local Unix private socket with typed broker DTOs and broker-side `SO_PEERCRED`/bundle re-derivation. | Unchanged. The broker is never a WAN endpoint and never receives relay credentials. |
| Host `d2bd` -> local workload `d2b-guestd` | Existing authenticated ttRPC/protobuf over virtio-vsock. | Unchanged. Local VM health, exec, config reads, and workload control stay here. |
| Host `d2bd` local realm dispatch | Existing local daemon/broker/guest-control paths. | Used when the selected realm entrypoint is host-resident. No constellation or gateway guest is required. |
| Host `d2bd` -> realm gateway guest | Existing guest-control exec/PTY path over authenticated ttRPC/protobuf. | Used when the selected realm entrypoint is gateway-backed, including integrated `vm <op> <workload>.<node>.<realm-path>.d2b` routing and `d2b realm enter/run` fan-out. This is local host-to-guest control, not constellation. |
| `d2bd` -> relay/direct transport -> peer `d2bd` or provider adapter | **New constellation peer protocol.** | Added in v2. Runs between d2bd instances and provider adapters; gateway guests are one place a realm-scoped d2bd can run. |
...
specific `d2bd`; constellation carries peer operations between `d2bd`
...
- `packages/d2bd/src/lib.rs` is the local daemon entry point and
...
- `packages/d2b-contracts/src/lib.rs` defines public/private IPC
  constants, frame caps, feature flags, protocol versioning, and broker
  capability negotiation.
- `packages/d2b-contracts/src/public_wire.rs` and
  `packages/d2b-contracts/src/broker_wire.rs` are the DTO seams for local
  requests. They should not become WAN protocols.
- `packages/d2b-core/src/bundle.rs`,
  `packages/d2b-core/src/manifest_v04.rs`,
  `packages/d2b-core/src/processes.rs`, and the generated schema
...
- `packages/d2b-contracts/src/guest_wire.rs` defines the guest-control
  schema version, ttRPC frame cap, guest-control vsock port, chunk caps,
  and bounded `ReadGuestFile` rules.
- `packages/d2bd/src/guest_control_vsock.rs` and
  `packages/d2bd/src/guest_control_bridge.rs` are local transport and
  auth/probe plumbing. This is the bridge point: a remote constellation
  operation targets a remote `d2bd`, and that daemon re-originates the
  local guest-control call near the guest.
- `packages/d2bd/src/exec_session*.rs`,
  `packages/d2b-guestd/src/service.rs`, and
  `packages/d2b-guestd/src/exec*.rs` are the practical precedent for
...
- `nixos-modules/processes-json.nix` and
  `packages/d2b-core/src/processes.rs` model process roles,
...
| `NodeId` | A registered d2b host, remote agent, gateway, or limited provider-managed environment. |
...
Remote guest operations are re-originated by the remote `d2bd`. The
...
- Local callers authenticate to `d2bd` over the public Unix socket via
...
- `d2bd` talks to `d2b-priv-broker` over a local private Unix
...
This does not claim secrecy from a fully compromised host. D2b's
...
| --- | --- | --- | --- |
| Local principal | Local kernel `SO_PEERCRED` plus d2b allowlists | Per local socket connection | Remote principal |
...
- Relay/provider credentials grant transport reachability. They do not
  identify the constellation principal and do not authorize d2b operations.
- A full-host node may use a dedicated local node-agent service principal
  to call its own local `d2bd`, but remote principal identity remains
...
1. **Install the local host substrate.**
   The operator installs d2b on the host OS. NixOS remains Tier 0, but
   ...
   The generic host-install path lays down the local `d2b` CLI,
   `d2bd`, `d2b-priv-broker`, systemd units, daemon config, trusted
...
2. **Choose a local node name.**
   ...
   as `d2b://personal-dev.laptop.local.d2b`, while the CLI may still
   accept `personal-dev`.
...
3. **Choose the realm entrypoint mode.**
   ...
   `dev-vm.laptop.personal.d2b` through local `d2bd`. For a
   gateway-backed realm, host `d2bd` creates a gateway guest as a local
   workload in that realm's isolated env/bridge. Example: the work realm
   may create `work-gateway.laptop.local.d2b` as a local gateway VM.
...
5. **Connect the gateway to its realm when remote reachability is needed.**
   ...
   for that realm. The host still does not talk to relays. `d2b vm
   list --all` or `d2b vm exec build-vm.remote-dev.work.d2b -- ...`
   reaches this realm by asking local `d2bd` to execute the matching
   in-guest operation inside the gateway.

6. **Register or provision remote nodes.**
   Remote hosts can be onboarded in two ways:
   - **Register existing infrastructure:** install d2b/d2bd on
   ...
   - **Provision new infrastructure:** an in-gateway provider adapter asks
     ...
     bootstraps d2b components, then enrolls the resulting node.

Provisioning infrastructure, bootstrapping d2b components,
...
operator enters realm gateway guest (gateway-backed realms)
  -> in-guest realm-scoped d2bd
    -> in-guest transport provider opens relay/direct channel
    -> local and remote d2bd peers perform constellation mutual authentication
...
authenticated constellation operation
  -> node checks realm, principal, capability, and stream/op policy
    -> node re-originates through local d2bd/node adapter
...
authorized remote guest operation
  -> node-local daemon/guest-control adapter
    -> broker-minted guest-control auth material
      -> local ttRPC/vsock call to d2b-guestd
```
...
daemon-access relay egress for reaching this `d2bd`; that uses
...
```text
host local admin
  -> local d2bd/broker starts gateway VM
      ...
      -> constellation E2E peer session over untrusted relay/direct transport
        -> remote node identity and realm policy
          -> remote local node service principal
            -> remote local d2bd SO_PEERCRED gate
...
- local `d2bd` spans, metrics, and typed errors;
...
1. **Live control-plane fan-out.** Host `d2b` can ask each local
   gateway for status, inventory, and operation details, then merge the
   responses for display. This is how `d2b vm list --all`,
   `d2b op inspect <operation-id>`, and similar commands get a
...
| --- | --- | --- |
| `d2b.realm` | Realm or nested realm path, e.g. `work` or `work/payments`. | Configured realm count. |
| `d2b.gateway` | Gateway guest that exported the event. | Configured gateway count. |
| `d2b.node` | Registered node label inside a realm. | Registered node count. |
| `d2b.workload` | Registered workload label inside a node. | Registered workload count. |
| `d2b.kind` | `microvm`, `gateway`, `sandbox`, `provider-session`, etc. | Closed enum. |
| `d2b.operation` | Closed constellation operation kind. | Closed enum. |
| `d2b.stream_kind` | Closed stream kind. | Closed enum. |
| `d2b.transport` | `azure-relay`, `websocket`, `quic`, `ssh`, `mock`, etc. | Closed enum. |
| `d2b.outcome` / `error_kind` | Typed outcome/error. | Closed enum. |

Full target names such as `build-vm.remote-dev.work.d2b` are useful
...
d2b op inspect <operation-id> --all
...
`d2b realm run`, `d2b realm enter`, and integrated
...
before starting its realm-scoped `d2bd` span. If the trace context is
...
`build-vm.remote-dev.work.d2b` belong in typed audit records and human
...
d2b host doctor
d2b vm status personal-dev.d2b

# remote target status, routed through the selected gateway
d2b vm status build-vm.remote-dev.work.d2b --trace

# inspect a specific operation by correlation ID
d2b op inspect <operation-id>

# low-level realm-owned inspection inside the gateway
d2b realm run work -- constellation op inspect <operation-id>
```

For local-only operations, `d2b op inspect` reads local daemon/broker
audit and local OTel links. For realm operations, host `d2b` fans out
...
- local CLI -> local `d2bd` over `public.sock`;
- local `d2bd` -> `d2b-priv-broker` for host mutation;
- local `d2bd` -> guest `d2b-guestd` over authenticated
  ttRPC/vsock;
...
A remote machine may be a full d2b host if it has KVM, root/broker
authority, cgroup v2, bridge/TAP/network control, compatible kernel
features, and storage layout for the per-VM store model.

Shape:

- local realm gateway guest connects to a remote `d2bd` peer;
- the remote agent talks to its local `d2bd` or embeds the node-side
  control surface;
- remote `d2bd` launches and manages remote microVMs;
...
Some providers already supply the isolation boundary. D2b may run
...
- model the environment as a limited-capability node or workload
  provider;
- do not pretend it is a full d2b hypervisor host;
...
| --- | --- | --- |
| `DaemonAccessTransport` | How the `d2b` CLI reaches a specific `d2bd`. | Local Unix socket first; direct mTLS/QUIC/WebSocket, Azure Relay-backed access, and explicit SSH bootstrap later. |
| `HostSubstrateProvider` | Installing/checking/preparing d2b on the host OS. | NixOS module host; Ubuntu/generic Linux host-install. |
| `RuntimeProvider` | Running a local workload on a full d2b host. | Current Cloud Hypervisor path; crosvm full-VMM/sidecar path as the first alternate. |
...
| `InfrastructureProvider` | Creating host infrastructure and bootstrapping d2b components. | Existing SSH-accessible hosts first; Azure VMs later. |
...
2. **Bootstrap/registration:** install d2b components and enroll node
   identity.
3. **Daemon access:** reach the correct `d2bd` over the appropriate
...
- crosvm already appears in d2b as a sidecar/device tool and is the
...
- Azure Container Apps dynamic sessions/custom container sessions provide
  ...
  `RuntimeProvider`, because d2b does not own KVM, vsock, broker
...
Full d2b hosts are not assumed to be NixOS. The host substrate is its
...
| Ubuntu/generic Linux host-install | Primary v2 remote-host path: install Nix, install `d2b`, lay down systemd units/config, reconcile NetworkManager/nftables/cgroups/devices through daemon/broker host-prepare. |
...
- installing or locating `d2b`, `d2bd`, and `d2b-priv-broker`;
...
Cloud Hypervisor remains the default local runtime. It fits d2b's
...
- crosvm: best future second local provider because d2b already uses
...
- Firecracker: not a full desktop d2b provider today because it
...
- `LocalMicroVmProvider`: wraps the current `RuntimeProvider` +
  daemon/broker/guestd path for full d2b hosts.
...
the CLI and `d2bd` when an operator explicitly enables remote/cloud
...
Classify providers by what d2b can own:
...
- `d2b-constellation-transport-azure-relay` owns Hybrid Connection listener/sender
...
| --- | --- | --- |
| `d2b` | Existing CLI plus realm routing and gateway lifecycle/enter helpers | Human entrypoint for local host operations; dispatches host-resident realms locally and starts/enters gateway-backed realms without holding realm relay credentials. |
| `d2bd` | Existing daemon plus gateway workload support | Local lifecycle, local broker mediation, local guest-control bridge, local gateway VM management. |
| `d2b-priv-broker` | Existing privileged broker | Local host mutation only; never a WAN endpoint. |
| `d2b-guestd` | Existing guest daemon | Guest-local health, file read, exec, PTY/log semantics over local guest-control. |
| `d2b-constellation-core` | New pure library | IDs, realms, nodes, workloads, capabilities, executions, streams, audit, typed errors, semantic `ConstellationFrame`, and bounded `TraceContext`. |
| `d2bd` peer modules | New modules inside the existing daemon | API frontend, operation router, peer sessions, provider dispatch, realm policy, and local re-origination. |
| `d2b-daemon-access` | New library | Transport-neutral CLI-to-`d2bd` semantic API and transport bindings. |
| `d2b-constellation-provider` | New pure library | Provider traits, capability descriptors, operation result normalization, and conformance fixtures. |
| `d2b-constellation-transport-*` | New transport libraries packaged into gateway/remote nodes | Move encrypted constellation peer session bytes over Azure Relay, mock, WebSocket, QUIC, SSH, or future transports. |
| `d2b-constellation-codec-*` | New protocol codec libraries | Encode/decode semantic constellation frames behind a stable API; protobuf is first and owns the single authoritative `.proto` source. |
...
| Host substrate adapters | New libraries/modules | Install/check/prepare d2b on NixOS and generic Linux hosts, with Ubuntu as the primary v2 remote-host path. |
...
packages/
  ...
  d2b-constellation-transport/
    src/lib.rs
    src/mock.rs
  d2b-constellation-transport-azure-relay/
    src/lib.rs
  d2b-constellation-codec-protobuf/
    src/lib.rs
    src/generated.rs
    src/wire.rs
    proto/constellation.proto
  d2b-constellation-host-substrate/
    src/nixos.rs
...
adapters over existing `d2bd` and guest-control code. Remote
...
| Field | Detail |
| --- | --- |
| Status | **Completed for the P0 foundation, reviewed live POC, gateway-mode REST lifecycle bridge, and real host-daemon live proof.** The merged branch ships the ACA/Relay/Waypipe reference path, gateway display orchestrator/runtime seams, target routing, gateway VM declaration surface, credential/audit hardening, a reviewed live ACA Wayland demo using a gateway-minted short-lived Relay Send bearer, and the follow-up REST data-plane lifecycle bridge that makes gateway-mode `d2b vm start <aca target>` create/resume the ACA sandbox instead of returning a ready-shaped stub. The real switched host proof rendered a Wayland proof window through `d2b vm start/exec <aca target>` against the system `d2bd`; sanitized PR/session evidence carries the concrete run details without publishing live identifiers here. |
...
| Design decisions | This wave implements a deliberately thin vertical slice instead of waiting for every generalized realm/provider feature. ACA dynamic/custom-container sessions are `WorkloadProvider` implementations; Waypipe-style forwarding is a `DisplayProvider`; Azure Relay or another relay-compatible transport carries named `control`, `stdio/logs`, and `display` streams. The host CLI still uses `d2b vm start/exec <target>` even though the target is a provider-managed sandbox, because `vm` is the operator's workload verb. Gateway-backed targets are routed by explicit realm entrypoints and fail closed when the realm gateway is not declared. |
...
init, and `d2b vm exec <...>.aca.<realm>` runs commands through the **ACA
...
model: sandboxes that advertise persistent shell must run a d2b
...
`d2b vm exec` **never implicitly starts** a stopped workload or a
...
(`vm not running; run d2b vm start <target>` / `display unavailable`),
...
source-built merged d2b daemon, an ACA disk image containing the gated
...
- ACA sandbox lifecycle is a real data-plane contract, not an operator shell-out:
  ...
  them behind the stable d2b workload alias label.
...
- Live validation against unreleased daemon code must opt out of release
  prebuilts (`d2b.site.usePrebuiltHostTools = false`) or the host switch can
  silently keep running older `d2b`/`d2bd` binaries.
...
| Field | Detail |
| --- | --- |
| Goal | Make realm entrypoint selection explicit and make gateway-backed realms first-class local d2b workloads. |
...
| Tasks | Add semantic `ConstellationFrame` and operation DTOs in `d2b-constellation-core`; add the single authoritative `.proto` schema and generated DTOs in `d2b-constellation-codec-protobuf`; define protocol version, frame caps, operation envelopes with caller-generated idempotency keys for mutating operations, handshake outcomes, denial enums, typed errors, reserved tags, and schema fingerprints; document the protocol reference; add mock d2bd-peer handshake harness. |
...
| Design decisions | Mock transport runs between d2bd peer endpoints; conformance suite is shared by every real transport; session capacity, close/error, backpressure, reconnect, auth failure, and shutdown are tested before Azure. |
...
| Validation | Host artifact grep/canary tests for realm relay credentials; egress policy eval tests; `d2bd` no-realm-relay-socket test; daemon-access relay allowlist/canary test when opt-in daemon access is enabled; gateway TAP egress positive test; no-shared-L2/no-cross-realm-L2 test; local ops unaffected; regression proving a host-mode daemon cannot mint realm Relay Send bearers or open a realm Hybrid Connection after production gateway placement lands. |
...
| Field | Detail |
| --- | --- |
| Goal | Let a gateway guest manage a remote full d2b host through that host's local re-origination point. |
| Design decisions | Remote `d2bd` enforces realm/capability policy and then invokes its local broker/guestd; privileged mutation stays local; raw broker/ttRPC frames do not cross WAN; full-host support is substrate-independent once the host advertises required capabilities. |
| Tasks | Add remote node registration/heartbeat/capability reporting including host substrate; add local-node adapter over `d2bd`; support workload list/inspect/start/stop and exec/log/file/port streams; add node audit correlation. |
...
| Field | Detail |
| --- | --- |
| Goal | Support limited-capability environments without pretending they are full d2b hosts. |
...
(e.g. an ACA dynamic/custom-container session) advertises `d2b-guestd`,
...
- Do not turn d2b into an arbitrary network bridge.
...
- [ADR 0002: Non-root d2bd plus minimal privileged broker](0002-non-root-daemon-and-privileged-broker.md)
