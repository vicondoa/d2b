...
> **Update (U10) - exploratory/historical.** This is W0 feasibility evidence,
> not a description of the current surface. U10 removed the standalone Guest
> daemon, guest-control protobuf/ttRPC APIs, chunked-stdio path, and SSH
> compatibility transport. Current Guest behavior uses ComponentSession and
> signed target-local Processes. Read the rows below as the original
> feasibility analysis, not current behavior.
...
| Chunked stdio conformance | integrated W0 decision branch | `f309bff43a7735e020f5e76e7caa90cca6101aaf` | PASS: executable proof covers 64 MiB stdout + 64 MiB stderr offset reads, TTY stdout-only/stderr-unavailable behavior, zero-length read and append-after-EOF rejection, 16 MiB slow stdin idempotency, deterministic slow-consumer bounds, four-attached-session byte-skew fairness, mixed three-exec plus unary-Health scheduler fairness with capacity saturation, stale restart, EOF vs Ctrl-D, resize/signal/cancel ordering, bounded control-event metadata, terminal-state sealing, control/idempotency replay, close-after semantics, terminal-status cursor accounting, and signal exit mapping. |
