...
In v1.2, enabling `d2b.vms.<vm>.tpm.enable = true` on a VM that had
**no pre-existing** `/var/lib/d2b/vms/<vm>/swtpm` state directory
...
Normal `d2b vm stop <vm> --apply`, `down`, and `restart` now ask
...
then d2b falls back to the previous SIGTERM/SIGKILL cleanup path.
...
```nix
d2b.vms.<vm>.lifecycle.gracefulShutdown.enable = false;
```

Use `timeoutSeconds = 1..600` globally or per VM to tune slow shutdowns.
For emergency operations, `d2b vm stop <vm> --force --apply` skips
