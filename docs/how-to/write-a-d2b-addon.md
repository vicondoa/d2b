...
> How-to: publish a sibling flake that composes with `d2b` on one
> VM at a time.
>
...
A `d2b` addon is a **sibling flake** that exports an ordinary NixOS
...
for example `nixosModules.default`. It should not need a `d2b`
input just to define its option schema or guest-side behavior.

The contract is deliberately small:

- `d2b` owns the host-side VM lifecycle, sidecars, networking, and
  CLI.
...
`d2b`**. Do that in the **consumer flake**, not inside the addon.

```nix
inputs = {
  d2b.url = "github:vicondoa/d2b/v0.3.0";
  my-addon.url = "github:example/my-addon";

  my-addon.inputs.nixpkgs.follows = "d2b/nixpkgs";
};
```

Do **not** make the addon depend on `d2b` just to inherit its
`nixpkgs`. That would couple an ordinary NixOS module bundle to one
specific framework. The addon should stay reusable outside `d2b`.
...
**NEVER** declare addon options under `d2b.*`. That namespace
...
prove that your module and `d2b` can be imported into one NixOS
...
(import <nixpkgs/nixos/lib/eval-config.nix> {
  modules = [
    ...
    {
      d2b.vms.test-vm.config.imports = [
...
[`d2b` README](../../README.md) and say which `d2b` version or
version range it was tested against.

Good README conventions:

- link to `d2b` for the host-framework concepts the addon does not
  re-explain;
- name the concrete `d2b` release used during validation;
- point at a minimal consumer example if the addon ships one.

## Step 7: Version independently

Addons are versioned independently from `d2b`. They do not need to
share tag numbers.
...
`d2b` and the addon evaluate cleanly, the version numbers do not
need to match.
...
- keep the addon out of the `d2b.*` namespace;
- document the consumer-side `nixpkgs` follow rule;
- show the `d2b.vms.<vm>.config.imports` seam in the README;
...
- If an addon needs a kernel module loaded, declare it in the
  consumer-side `d2b.site.modules` allow-list so the module
...
- If an addon needs a NetworkManager unmanaged-config row or an
  ...
  broker applies it under the d2b ownership marker; addon-owned
  ownership markers are rejected (`nm-managed-foreign-conflict`).
- Addons that need an extra firewall rule beyond `inet d2b`'s
  ...
  (per ADR 0013), but addon-emitted rules inside `inet d2b`
