...
Run the commands as a nixling admin: either a user in the `nixling` lifecycle
group or via `sudo`, depending on how the host was configured. Do not edit
`/run/nixling/locks/usbip/<busid>`, sysfs driver links, nftables rules, or
...
- `nixling.site.yubikey.enable = true`.
- The target VM is enabled, belongs to an env, and sets
  `nixling.vms.<vm>.usbip.yubikey = true`.
- The VM declares the allowed busids with `nixling.vms.<vm>.usbip.busids`.
- If `nixling.host.usbip.allowlist` is set, the plugged device's VID/PID must
...
For one VM, `nixling status <vm>` also includes the USB summary:
...
- `SESSION-CLAIM` in `nixling usb probe` (or `session-claim=` in
  `nixling status`) is the broker claim for the current host boot/session.
  ...
  because the backing lock lives under `/run/nixling/locks/usbip`.
...
VM start, nixling replays host bind/proxy state and asks guestd to import again.
After a host reboot, `/run/nixling/locks/usbip` is recreated empty and the
...
```bash
nixling usb detach corp-vm 1-2 --apply
nixling usb probe
```

If detach reports `usbip-revocation-not-isolated`, use the busid named in the
error. Nixling could not prove that one busid stream can be revoked without
...
| Symptom from `nixling usb probe` or `nixling status` | What it means | Remediation |
| --- | --- | --- |
...
| `carrier-unavailable`, `host-bind-unavailable`, or a departed-device reason | The physical device, `usbip-host` carrier, or host bind state is not present. | Reconnect the device, run `nixling usb probe`, then `nixling usb attach corp-vm 1-2 --apply` |
| `proxy-unavailable` | The per-env proxy/backend carrier is not listening or is stale. | `nixling usb attach corp-vm 1-2 --apply`; if it repeats, inspect `/var/lib/nixling/audit/broker-<utc-date>.jsonl` for the USBIP runner/broker failure. |
| `policy-failed`, `device-reappeared-with-different-topology`, or topology mismatch | The plugged device does not match the declaration or allowlist. | Fix `usbip.busids` / `nixling.host.usbip.allowlist` or plug the approved device, then `sudo nixos-rebuild switch --flake .#desktop`, `sudo systemctl restart nixlingd.service`, and retry `nixling usb probe`. |
| `stale-host-state` or `stale-guest-state` after a removed claim | Carrier/import state outlived the session claim. | `nixling usb detach corp-vm 1-2 --apply`, then `nixling usb probe` |
| `probe-incomplete` | The daemon could not observe enough redacted identity to safely reconcile. | Retry `nixling usb probe`; if it repeats, fix the declaration or physical topology and rebuild before attaching. |
