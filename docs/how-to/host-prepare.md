...
> prepare`. This document is assembled by the W3 integrator from the
> per-scope fragments under `docs/how-to/host-prepare.d/*.md`; do not
> edit the fragments via this file or vice versa — each fragment is
> owned by its scope agent and re-assembled per round.

> **v1.0 status note (per ADR 0015).** `nixling host check`,
> `nixling host prepare --dry-run`, `nixling host destroy --dry-run`,
> and `nixling host doctor --read-only` exercise the broker's
> read-only audit path. The mutating `--apply` verbs (`host prepare
> --apply`, `host destroy --apply`) accept the flag and dispatch
> through the broker reconcile ops (`ApplyNftables`, `ApplyRoute`,
> `ApplySysctl`, `UpdateHostsFile`, `ApplyNmUnmanaged`) on Tier 1 /
> Tier 1-later / Tier 2. On Tier 0 the verb returns the typed
...
The W3 host CLI is split across seven verbs (plan §"W3 CLI host verb
scope table"); the canonical contract is:

| Verb | Mutates host (when W4 wires `--apply`) | Required flag |
| --- | --- | --- |
| `nixling host check` | no | n/a — read-only inventory + diff |
| `nixling host prepare --dry-run` | no | `--dry-run` mandatory; reports only |
| `nixling host prepare --apply` | live in v1.0 (broker reconcile ops per ADR 0015) | `--apply` mandatory |
| `nixling host destroy --dry-run` | no | `--dry-run` mandatory; reports only |
| `nixling host destroy --apply` | live in v1.0 (broker reconcile ops per ADR 0015) | `--apply` mandatory |
...
| `nixling host install --apply` | yes (W15, daemon → broker) | `--apply` mandatory; optional `--enable` + `--start`/`--no-start`; broker failures exit 78 |
...
The four W3 reconcile domains — cgroup delegation, network (bridge /
TAP / NM / IPv6 / hosts), firewall (`inet nixling` nftables coexistence
+ USBIP rule skeleton), and modules + device nodes — are each owned
by a W3 scope. The sections below are reproduced verbatim from the
...
## Section 1 — cgroup v2 delegation (W3 s1)
...
## Section 2 — network reconcile (W3 s2)
...
# the W3 s2 mutations only (bridges, TAPs, NM drop-in, /etc/hosts
...
## Section 3 — firewall coexistence (W3 s3)
...
## Section 4 — kernel modules + device nodes (W3 s4)
...
- Security boundary deltas: [`SECURITY.md`](../../SECURITY.md) § W3
