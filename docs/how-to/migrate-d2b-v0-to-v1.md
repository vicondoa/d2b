...
If you are coming from raw `microvm.nix` (no d2b at all) read
...
- `d2bd.service` and `d2b-priv-broker.{service,socket}` are
  the **only** persistent root surfaces the framework declares.
- Per-VM systemd templates (`d2b@<vm>`, `microvm@<vm>`,
  `d2b-<vm>-{gpu,snd,video,swtpm,store-sync}`, the
  `microvm-{tap-interfaces,pci-devices,set-booted,virtiofsd}@`
  templates, `d2b-known-hosts-refresh@`,
  `d2b-vfsd-watchdog@.{service,timer}`,
  `d2b-otel-relay@<vm>`, and the per-env
  `d2b-sys-<env>-usbipd-{proxy,backend}.{service,socket}`) are
  deleted.
- Host singletons (`d2b-{ch-exporter,otel-host-bridge,
  net-route-preflight,audit-check}.service`,
  `d2b-audit-check.timer`, the `microvms.target` aggregator)
  are deleted; their work moved into `d2bd` or the broker.
- The bash CLI (`d2b-legacy` / `share/d2b/cli.sh` / the
  `cli.nix` builder) is deleted. The Rust `d2b` binary is the
  only CLI surface.
- The bash fallback bridge inside the Rust CLI is removed; so is
  `D2B_LEGACY_BASH_OPT_IN` / `D2B_LEGACY_CLI`.
  `D2B_NATIVE_ONLY` is preserved as a no-op for back-compat.
- The polkit allowlist for per-VM units is retired; `d2b`
...
```bash
git -C /etc/nixos tag pre-d2b-v1
...
shipped). `d2b_core::manifest_v04::MANIFEST_VERSION_CURRENT` is
...
sudo cat /run/current-system/sw/share/d2b/vms.json \
...
already bumped the d2b flake input, also:

```bash
git -C /etc/nixos checkout pre-d2b-v1 -- flake.nix flake.lock
sudo nixos-rebuild switch --flake .#myhost
```

## 2. Bash CLI removed (`d2b-legacy` / `share/d2b/cli.sh` / `cli.nix`)

### Before

`/run/current-system/sw/bin/d2b-legacy` was the bash entrypoint;
`/run/current-system/sw/share/d2b/cli.sh` was the script body;
`nixos-modules/cli.nix` packaged both plus every per-VM
`d2b-launch-<vm>.desktop` wrapper. Operators could call either
binary; the Rust `d2b` would shell out to bash for any verb the
daemon could not serve.

### After

The bash CLI is gone. `d2b-legacy` no longer exists in
`/run/current-system/sw/bin`. `nixos-modules/cli.nix` is deleted.
The Rust `d2b` binary is the only CLI surface and never shells
...
helper aliases for `d2b-legacy` invocations:

```bash
sudo grep -rIn 'd2b-legacy\|share/d2b/cli\.sh' \
...
| --- | --- |
| `d2b-legacy up <vm> -d` | `d2b vm start <vm> --apply` |
| `d2b-legacy down <vm>` | `d2b vm stop <vm> --apply` |
| `d2b-legacy restart <vm>` | `d2b vm restart <vm> --apply` |
| `d2b-legacy list` | `d2b vm list` |
| `d2b-legacy status <vm>` | `d2b status <vm>` |
| `d2b-legacy audit --strict` | `d2b host doctor --apply` |
| `d2b-legacy console <vm>` | `d2b console <vm>` |
...
`d2b-launch-<vm>.desktop` files `cli.nix` used to generate.
...
test ! -e /run/current-system/sw/bin/d2b-legacy
test ! -e /run/current-system/sw/share/d2b/cli.sh

# The desktop wrapper contract must be present for every graphics VM:
d2b vm list --json | jq -r '.vms[].name' | while read vm; do
  test -e "/run/current-system/sw/share/applications/d2b-launch-${vm}.desktop"
done

# Layer-2 wave validators:
sudo d2b host validate --apply --wave p4
```

Cross-reference: `packages/d2b/tests/vm_verbs_contract.rs`,
...
If your consumer scripts call `d2b-legacy` you cannot run them
on a v1.0 system; pin your flake input to v0.4.x until the script
audit is complete.

## 3. Bash fallback removed + `D2B_LEGACY_BASH_OPT_IN` no-op

### Before

The Rust `d2b` binary's `dispatch_mutating_verb` first tried
the daemon, then on `not-yet-implemented` or `daemon-down` shelled
out to `/run/current-system/sw/bin/d2b-legacy`. The
`D2B_LEGACY_BASH_OPT_IN=1` env var bypassed the daemon entirely
and went straight to bash. `D2B_LEGACY_CLI` /
`D2B_LEGACY_CLI_PATH` overrode the legacy binary path.
...
```bash
sudo grep -rIn \
  -e 'D2B_LEGACY_BASH_OPT_IN' \
  -e 'D2B_LEGACY_CLI' \
  -e 'D2B_LEGACY_CLI_PATH' \
  /etc /home /root /var/spool 2>/dev/null | grep -v Binary
```
...
D2B_LEGACY_BASH_OPT_IN=1 D2B_LEGACY_CLI_PATH=/bin/false \
  d2b vm start work --apply --json
...
`D2B_LEGACY_BASH_OPT_IN` only works on v0.4.x or earlier.
...
ownership). The `d2b-launcher` polkit allowlist permitted
operator `systemctl start/stop/restart` on every per-VM unit.

### After

There are no framework-declared per-VM systemd units. `d2bd`
...
The `d2b.vms.<vm>.supervisor` option is **retained in v1.0
...
+  supervisor = "d2bd";   # v1.0 convention
   …
 };
```

You also need `d2b.daemonExperimental.enable = true` on the host
for `supervisor = "d2bd"` to evaluate (the assertion at
...
{start,stop,restart} d2b@<vm>.service` (and the per-VM sidecar
unit names) and replace them with the Rust verbs from §2.

```bash
sudo grep -rIn \
  -e 'd2b@' \
  -e 'd2b-[a-z0-9-]\+-\(gpu\|snd\|video\|swtpm\|store-sync\)\.service' \
  -e 'microvm@' \
  /etc /home /root /var/spool 2>/dev/null | grep -v Binary
```
...
sudo systemctl list-units --no-pager --all \
  | grep -E '^(d2b@|microvm@|d2b-.+-(gpu|snd|video|swtpm|store-sync)\.service)' \
  | awk '{print $1}' \
...
```bash
# Exactly three d2b-shaped units on the host:
...
`supervisor = "d2bd"` back to `supervisor = "systemd"` without
a flake rev rollback. For a full rollback to v0.4.x, pin the
d2b flake input back to v0.4.x and `nixos-rebuild switch
--rollback`.
...
```bash
sudo d2b host doctor --apply --json | jq '.checks[] | select(.name=="audit-rotation")'
```
...
for u in d2b-net-route-preflight d2b-audit-check.service \
         d2b-audit-check.timer d2b-ch-exporter \
         d2b-otel-host-bridge; do
...
for every per-VM systemd unit (`d2b@<vm>`,
`d2b-<vm>-{gpu,snd,swtpm,store-sync}`) and every per-env
usbipd triplet (`d2b-sys-<env>-usbipd-{proxy,backend}.{service,socket}`).
The `d2b` group plus `org.freedesktop.systemd1.manage-units`
let operators run `systemctl start/stop/restart` on those units
without a password.

A companion JS rule scoped to the per-VM
`d2b-<vm>-gpu` system user granted it start/stop/restart of
its paired `d2b-<vm>-snd.service`.

### After

`host-polkit.nix` allowlists exactly three units:
`d2bd.service`, `d2b-priv-broker.service`,
`d2b-priv-broker.socket`. The per-VM / per-env entries and
...
`systemctl start d2b@<vm>` is a member of `d2b`
(daemon socket access), not just `d2b` (polkit
singleton restarts):
...
```nix
{ d2b.site.launcherUsers = [ "alice" "bob" ]; }
```
...
sudo grep -E 'd2b[^"]*\.service|d2b[^"]*\.socket' \
  /etc/polkit-1/rules.d/*d2b* \
  | sort -u
...
sudo grep -E 'd2b@|d2b-[^"]+-(gpu|snd|swtpm|store-sync)|d2b-sys-[^"]+-usbipd' \
  /etc/polkit-1/rules.d/*d2b* \
  || echo "no per-VM polkit entries (expected on v1.0)"

# Daemon socket accept-time authz works for an operator:
sudo -u alice d2b vm list --json | jq '.vms | length'
```
...
- `d2b.daemonExperimental.enable` flips to `true` automatically
...
- The framework emits exactly three persistent root-visible d2b
  systemd units: `d2bd.service`, `d2b-priv-broker.service`,
  `d2b-priv-broker.socket`.
...
d2b host validate --dry-run --json \
...
# persistent d2b/microvm root-visible units.
systemctl list-units --no-pager --all \
  | grep -E '^(d2b|microvm)' | wc -l
...
reason and re-run; do not edit `/var/lib/d2b/validated/<wave>.json`
by hand.
...
d2bd.service -u d2b-priv-broker.service` window, and the
last broker audit log under `/var/lib/d2b/audit/broker-<utc-date>.jsonl`.
...
- **With `--strict`** (`d2b audit --strict`): **truly deferred**
...
```
sudo cat /var/lib/d2b/audit/broker-$(date -u +%Y-%m-%d).jsonl | jq .
```
...
`/run/d2b/vms/<vm>/console.sock` (read-only operator
diagnostic; `socat - UNIX-CONNECT:/run/d2b/vms/<vm>/console.sock`).

**v1.1 plan**: see the v1.1 deliverables for the
**migration-guide-rendering** lift (the Rust `Display` impl
update in `packages/d2b-core/src/error/remediation_rendering.rs`
...
runtime toggling via `d2b audio mic|speaker|off` is not
exposed in v1.0.
...
### `d2b keys list` / `d2b keys show` (daemon-down rendering pointers)

`d2b keys list` and `d2b keys show` are **v1.0
...
(The mutating subcommands `d2b keys rotate`, `d2b trust`,
`d2b rotate-known-host` are also v1.0 daemon-backed and follow
