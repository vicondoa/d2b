...
- applied: redacted-Debug mechanism folded onto one crate-internal `redacted_debug!` (service.rs; exported via pub(crate) use); resolver.rs byte-identical macro copy deleted; 11 hand-rolled impl Debug bodies (service ×3, serving ×1, enrollment ×5, engine ×2) replaced with one-line macro invokes. Canonical output form kept as `(<redacted>)` (the 11 hand-rolled bodies' exact form) so the enrollment `<redacted>` assertions stay green; service/resolver "redacted" substring asserts still pass. router.rs's 2 hand-rolled impls were NOT named by the lane and were left untouched.
