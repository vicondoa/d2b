...
skip (stale claim): finding 1 names types absent at HEAD - `BoundSubject`, `SessionSubject`, `TranscriptHash`, `RingDigest` do not exist in this crate (grep-verified across src/; `authz.rs:36-49` at HEAD holds a single pure-form Debug impl for `Subject`, plus `ExecutionTarget`, `OpenSessionRequest`, `Attachment`, `SupervisorCandidate`; `session/ring.rs` Debug impls at 63-72/92-98 are structured `debug_struct` with real fields, explicitly out of scope per the lane's own note). The named six-line-per-impl macro replacements are therefore not present; no cut applied.

Finding 2 refused (#S39) - no-op per ledger.

No files changed. No tests run (no changes).