...
- applied: 1 dnsmasq.rs deleted (+lib.rs arm); 2 ssh_keygen.rs deleted (+lib.rs arm); 3 activation-helper dead verbs (ensure-regular-file/setfacl-on-path/clear-acl-on-path/chown-if-orphan) + Args fields + help lines + fs_posture tests deleted; 4 routes.rs preflight cut (kept render_hosts_block/extract_managed_block/HOSTS_MANAGED_*); 6 ifname.rs folded to d2b-contracts-resource v3 re-export; 7 hardlink_farm legacy u32-token path (swap_current_symlink/current_generation/sweep_live_pool/reconcile_stale_swap_tmp) + tests; 8 replace-live-paths surface (hardlink_farm + broker store_view_farm wrappers + helper verb + tests); 11 host_prep_dag pub->private + HostPrepStepFailed deleted; 12 cgroup create_d2b_slice/create_vm_role_leaf deleted (tests reworked to delegated_slice helper; EnabledControllers kept - live via require_controllers); 14 ioctl_policy is_allowed/ioctl_allowlist pub(crate), dead TUNSETPERSIST/TUNSETOWNER/TUNATTACHFILTER constants + negative assertions deleted; 16 activation-helper canonical_json/framed_digest replaced by framed_canonical_digest; 17 stdin-JSON verb bodies folded into run_stdin_json_verb.
- skipped: 5 netlink fake trailer - STALE at HEAD: d2b-broker/src/ops/tap.rs:30-31 imports NetlinkBackend/fake::FakeBackend/ipv6_off_sequence/readback_bridge_port_flags/LinkKind/LinkSpec/TapOwner/NetlinkError (create_tap/set_bridge_port_flags generic fns + tests); lane's "no production implementor" claim refuted by live broker imports. 9 atomic-write helper twins - NOT APPLIED this wave (leaf shrink of live code; deferred, no consumer conflict). 10 ownership_matrix recursive machinery - STALE: ChildDrift matched by d2bd-runtime ownership_preflight.rs:390 (render_drift_message) and recursive field constructed at :284 - out-of-scope crate consumers. 13 host_generation - STALE: host_generation.rs is now the live activation protocol (broker host_generation_handoff.rs); HostGenerationMeta/write_host_meta live in hardlink_farm.rs and write_host_meta is called by live build_store_view (broker store_sync.rs:811 asserts state meta.json). 15 ioctl_allowlist fold - STALE: ioctl_allowlist is called by live compile_ioctl_policy_to_bpf (seccomp.rs:123, broker live_handlers.rs:1096).
- gate note: check-provider-crate-layout red on 4 stale SHARED_FAMILY_KNOWLEDGE_RATCHET rows in packages/xtask/src/provider_crate_policy.rs (dnsmasq/nftables tokens on packages/d2b-host/src/{dnsmasq.rs,lib.rs,routes.rs}, retiresWith "U12 network-fds step") - rows are stale because the dnsmasq surface they exempted was deleted by finding 1 and routes.rs no longer carries dnsmasq/nftables tokens after finding 4. xtask is outside this unit's crate scope; rows need retiring by the U12 lane or orchestrator.
