...
> **Status: historical landing page.** The "default switch / bash
> deprecation cycle" rollout this file used to describe is closed.
> P5 [`ph5-w18-flip`] flipped `nixling.daemonExperimental.enable`
> to `true` by default, and P6 deleted the legacy bash CLI and the
> pre-P6 per-VM `nixling@<vm>.service` (retired) / `microvm@<vm>.service`
> systemd templates outright — there is no longer a "default mode" vs
> "native-only mode" axis, and there is no multi-step deprecation
> timeline to track. See
...
1. The **post-clean-break per-verb matrix** — every CLI verb ships
   exactly one path (daemon-native). The "legacy bash path" column
   collapses to a single `no` cell.
2. The **W18 auto-flip gate** — still the mechanism that decides
   whether `nixling.daemonExperimental.enable` evaluates to `true`
   on a fresh consumer host. It is no longer a rollout valve; it
   is now a guard that refuses to flip on a host that has not
   recorded the per-wave evidence files.
3. Cross-references to the docs that own the live surface
   (`cli-contract.md`, `wave-evidence-schema.md`, ADR 0015,
   `host-validate.md`).

Anything else that used to live on this page — the W14c three-mode
bridge, the `NIXLING_NATIVE_ONLY` / `NIXLING_LEGACY_BASH_OPT_IN`
escape hatches, the `W10-fu + 30/90/180 days` bash-warning /
fail-loud / removal calendar — is gone with the clean break.

## Post-clean-break compatibility matrix

Every public CLI verb ships exactly one path. The fallback column
exists only so that downstream readers cross-referencing pre-P5
prose can see at a glance that the answer is now uniformly
**no**[^bash-rm].

[^bash-rm]: The bash CLI binary (`scripts/nixling`,
    `nixos-modules/cli.nix`) and the pre-P6 per-VM `nixling@<vm>.service`
    (retired) + `microvm@<vm>.service` systemd templates were deleted in P6
    (`ph6-remove-systemd-emission` and `ph6-p6-cli-nix-migrations`;
    see [ADR 0015 § Scope](../adr/0015-daemon-only-clean-break.md)).
    P4 had already removed the W14c bash fallback bridge for
    `vm start/stop/restart/list`; P6 finished the job by removing
    the bash dispatcher and the systemd templates outright.
...
| `vm start`, `vm stop`, `vm restart`, `vm list` | daemon-native | no | P4 removed the W14c bash fallback; P6 removed the bash dispatcher and the pre-P6 `nixling@<vm>.service` template (retired). Failures surface as typed daemon / broker envelopes (`daemon-down` exit 1, `not-yet-implemented` exit 78). |
| `up`, `down`, `restart` (top-level aliases) | daemon-native | no | P4 promoted these to first-class aliases for `vm start/stop/restart`. Same envelope shape. |
| `host prepare`, `host destroy` | daemon-native | no | Daemon-side host-prepare DAG (P2); the bash host-prepare entrypoint is removed in P6. |
| `host install` | daemon-native (broker `RunHostInstall`) | no | — |
| `host validate` | daemon-native | no | P5 deliverable; writes the per-wave evidence files this page references below. See [`host-validate.md`](./host-validate.md). |
...
| `usb attach`, `usb detach`, `usb probe` | daemon-native | no | W6 USBIP live executors via the broker (`UsbipBindFirewallRule` + per-busid lock). |
| `console`, `audio status`, `audio mic`, `audio speaker`, `audio off` | daemon-native | no | Rust CLI owns the surface; the previous "Rust shim → bash helper" bridge is removed in P6. |
| `debug bundle` | daemon-native | no | — |

There is no `NIXLING_NATIVE_ONLY` and no `NIXLING_LEGACY_BASH_OPT_IN`.
Both environment variables are unrecognised after P6; setting them
has no effect.
...
- [`wave-evidence-schema.md`](./wave-evidence-schema.md) — JSON
  schema for the W18 evidence files this page's gate consumes.
...
- [`../explanation/default-switch-and-deprecation.md`](../explanation/default-switch-and-deprecation.md)
  — historical record of the W10 / W14c / W18 rollout shape that
  preceded the clean break.
