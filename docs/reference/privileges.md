...
| `CheckSystemdUserManager` | systemd user manager | per user | live | no | metadata-only | `d2bd` | yes | deny | Bounded manager reachability and readiness metadata; no unit path or raw bus output. | [ADR-046 system-systemd](../specs/providers/ADR-046-provider-system-systemd.md) |
| `ObserveSystemdUnit` | systemd unit | per VM / role | live | no | metadata-only | `d2bd` | yes | deny | Bounded unit lifecycle state and identity verification metadata. | [ADR-046 system-systemd](../specs/providers/ADR-046-provider-system-systemd.md) |
| `OpenSystemdUnitPidfd` | systemd unit pidfd | per VM / role | live | no | no | `d2bd` | yes | deny | Re-verified process identity only; the pidfd crosses through `SCM_RIGHTS`. | [ADR-046 system-systemd](../specs/providers/ADR-046-provider-system-systemd.md) |
| `StopSystemdUnit` | systemd unit | per VM / role | live | yes | no | `d2bd` | yes | deny | Typed transient-unit identity and bounded stop outcome; no caller-selected unit name or properties. | [ADR-046 system-systemd](../specs/providers/ADR-046-provider-system-systemd.md) |
...
| `MigrateLegacySwtpmState` | fs (`${stateDir}/swtpm-legacy` -> `${stateDir}/swtpm`) | per VM | live | yes for migration; no for `probeOnly` inventory classification | metadata-only | `d2b-admin` | yes | deny | `vm_id`, `probe_only`, `outcome` (`migrated`/`already-migrated`/`not-applicable`/`pending`/`failed`/`ambiguous`/`adoption-required`/`never-provisioned`). Probe-only is broker-locked and non-mutating; Core uses its closed result to seal the TPM decision. Raw source, destination, journal, and marker paths never cross the wire. | [0034](../adr/0034-storage-lifecycle-restart-and-synchronization.md) |
...
| `DeletePersistentTap` | network | per env / VM / TAP | reserved | yes (link destroy) | no | `d2b-admin` | yes | deny | `ifname_derived`, `role` | [0046](../adr/0046-d2b-3-provider-control-plane.md) |
| `CreateBridge` | network | per env | reserved | possible (link create) | no | `d2b-admin` | yes | deny | `ifname_derived`, `scope_id` | [0046](../adr/0046-d2b-3-provider-control-plane.md) |
| `DeleteBridge` | network | per env | reserved | yes (link destroy) | no | `d2b-admin` | yes | deny | `ifname_derived`, `scope_id` | [0046](../adr/0046-d2b-3-provider-control-plane.md) |
...
| `ApplyNftablesProjection` | network host | global / per env | reserved | yes | no | `d2b-admin` | yes | deny | `action` (closed enum `apply` / `remove`), `table_hash_before`, `table_hash_after` | [0046](../adr/0046-d2b-3-provider-control-plane.md) |
...
- Contract coverage:
  the owner-local GPU and broker tests. This is advisory until
...
| `host-prep.known-hosts-refresh` | `d2b-known-hosts-refresh@<vm>.service`; current replacement is the planned `SshKeygenProbe` broker dispatch | `SshKeygenProbe` (future work) | after `vm.component-session-health`, not in the cold-start chain |
...
- owner-local daemon tests - check current
...
| `Usbip` | per-env singletons `d2b-sys-<env>-usbipd-{proxy,backend}.{service,socket}`; replaced by broker `SpawnRunner{role: Usbip, vm_id: sys-<env>-usbipd, …}` | backend: scoped host-root carve-out with `CAP_NET_RAW`; proxy: empty | **per env** - two runners per USBIP-enabled env (`vm_id` = `sys-<env>-usbipd`, roles `backend` and `proxy`) | The host runner contract remains defined here, but non-qemu `d2b device usb attach|detach --apply` currently fails closed before `UsbipBind`, `UsbipBindFirewallRule`, proxy reconciliation, or claim release because the signed target-local USBIP Process/ComponentSession effect path is unavailable. Host kernel module is `usbip-host` (not `vhci_hcd`, which is the guest module). |
| `CloudHypervisor` (NixOS VM) | `d2b@<vm>.service` runner; replaced by broker `SpawnRunner{role: CloudHypervisor, vm_id: <vm>, ...}` | empty | per VM | The broker validates the signed runner intent and the enrolled ComponentSession endpoint before the daemon or target-local Process can use it; no token share or fallback transport is present. |

### Metrics transport

The daemon does not serve an HTTP metrics endpoint. Core processes emit
bounded telemetry to the per-Zone Unix socket, and the optional
observability Provider or an external collector owns any exposition
endpoint. The transports and sandbox posture are documented here alongside
...
| --- | --- | --- | --- | --- | --- |
| External collector URL configured by the operator | external collector | Prometheus exposition is owned by the external collector; `d2bd` serves no HTTP metrics listener | no daemon capability surface | Configure collector transport and access policy outside d2b. | Daemon metric descriptors remain canonical and bounded; collection is an external deployment concern. |
...
| `d2b-ch-exporter.service` | Retired daemon-served scrape unit. Host telemetry egress to the obs VM uses the configured Unix socket when enabled. | not emitted | [`docs/reference/components-observability.md`](components-observability.md) |
...
The owner-local privilege checks ensure that every
...
| `d2b-ch-exporter.service` | Retired; no daemon-served Prometheus endpoint. | not emitted |
...
- `sync` is **admin-only**: it dispatches the ConfigNixos service's
  `ReadGuestConfig` operation, which is gated to the `d2b-admin` role at
  `SO_PEERCRED` accept time. A launcher-role caller is rejected with the
  typed `authz-not-admin` error (exit `75`) before any guest read. The local
  review sub-verbs (`diff` / `approve` / `reject` / `status`) operate on the
  host-side staging copy and dispatch no daemon verb, so a launcher can run
  them - which is why the schema models the `config` group as
  `d2b-launcher` + `d2b-admin`. `sync` reads the guest-editable config through
  the authenticated ConfigNixos ComponentSession service and writes only into
  the host-side staging copy; it never evaluates or imports the guest bytes.
  It fails closed when the target-local service or session is unavailable.
...
ops over the authenticated ComponentSession to the target-local Process. It
is in the machine-readable public-operation matrix
([`schemas/v2/privileges.json`](schemas/v2/privileges.json),
...
- `exec` is **admin-only**: the daemon checks the `SO_PEERCRED` admin
  ...
  caller is rejected first, with no ComponentSession established.
...
terminal operations over the authenticated ComponentSession to the target-local
shell Process. It
is in the machine-readable public-operation matrix
([`schemas/v2/privileges.json`](schemas/v2/privileges.json),
...
- `shell` is **admin-only**: the daemon checks the `SO_PEERCRED` admin role
  before ComponentSession contact or owner-session handoff.
...
- Raw shell names, terminal session handles, terminal bytes, helper diagnostics,
  argv, env, paths, and ComponentSession evidence never appear in metric labels,
...
| `gpu` | `vm-<vm>-gpu` | empty (per-role smoke proves virgl/venus/cross-domain run under SCHED_OTHER) | device binds: `/dev/kvm`, `/dev/dri/renderD128`, `/dev/nvidiactl`, `/dev/nvidia0`, `/dev/nvidia-uvm`, `/dev/udmabuf`; mount `/run/user/<uid>/wayland-0` → `/run/d2b-gpu/<vm>/wayland-0`; ioctls: full `DRM_IOCTL_VIRTGPU_*` family (via DeviceClass::Dri) | owner-local GPU and broker tests |
| `audio` | `vm-<vm>-audio` | `CAP_NET_RAW` (vhost-user-sound bind on PipeWire mediation path; AF_NETLINK for virtio-snd) | RO bind of `/run/user/<uid>/pipewire-0`; RW bind of `/run/d2b/vms/<vm>/snd.sock`; seccompPolicyRef = `w1-audio` | owner-local audio and broker tests |
| `video` | `vm-<vm>-video` | empty | `deviceBinds = [ "/dev/dri/renderD128" ]` by default for virtio-media decode (virtio-media wire contract: `virtio_id=48`, 2×256 queues, 256 MiB SHM, `vring_base=0`); `graphics.videoNvidiaDecode = true` additionally allows `/dev/nvidiactl`, `/dev/nvidia0`, and `/dev/nvidia-uvm` for the proprietary NVIDIA VA-API/NVDEC backend; `/dev` is masked and no broad bind is allowed; RW bind of `/run/d2b-video/<vm>/`; seccompPolicyRef = `w1-video`; principal is the dedicated `d2b-<vm>-video` uid | owner-local video and broker tests |
| `vsock-relay` | `vm-<vm>-vsock-relay` | empty (pre-opened fds only, no AF_VSOCK socket creation in-role) | bind: per-VM `/var/lib/d2b/vms/<vm>/vsock.sock` (the inherited UDS); seccompPolicyRef = `w1-vsock-relay` (denies `socket(AF_VSOCK)` + `ptrace`) | owner-local transport and broker tests |
