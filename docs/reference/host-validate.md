...
1. Iterates the default-switch readiness waves in the deterministic catalog
   ...
   `nixos-modules/options-daemon.nix` by
   [`packages/d2b/tests/host_validate_verb.rs`](../../packages/d2b/tests/host_validate_verb.rs)
   in `test-rust`.
2. For each wave, inventories the per-wave Layer-2 validator
   basenames embedded in the catalog (including retired names such as
   `tests/minijail-validator-*.sh`,
   `tests/per-vm-state-ownership-eval.sh`,
   and `tests/d2bd-startup-smoke.sh`). A wave is `ready` only when
...
The verb does NOT execute the per-wave shell validators itself. It only checks
for catalogued files and writes umbrella evidence for waves whose files are
present. Retired validators cannot emit their former per-role evidence.
`host validate --apply` produces per-wave `<wave>.json` files only for waves
that pass this presence check.

> **Current catalog limitation:** most validator basenames in
> `WAVE_CATALOG` name retired shell scripts that are absent from this tree.
...
- [`packages/d2b/tests/host_validate_verb.rs`](../../packages/d2b/tests/host_validate_verb.rs) - enforcing `test-rust` regression coverage.
