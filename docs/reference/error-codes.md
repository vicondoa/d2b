...
| <a id="broker-unimplemented"></a>`#broker-unimplemented` | `broker-unimplemented` | `30` | `daemon-api/broker` | broker operation {operation} is reserved for wave {target_wave} and is not callable in this build | The v1.0 daemon-only contract (ADR 0015) returns the typed `broker-unimplemented` envelope; no bash fallback exists. Wait for the target wave to land. |
