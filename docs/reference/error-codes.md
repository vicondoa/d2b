...
| <a id="guest-shell-disabled"></a>`#guest-shell-disabled` | `guest-shell-disabled` | `70` | `shell` | persistent guest shell is not available for this VM | Enable nixling.vms.<vm>.guest.shell when the shell runtime is available, rebuild the guest, and retry. |
