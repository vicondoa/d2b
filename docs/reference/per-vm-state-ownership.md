...
Reference for the typed ownership matrix that the d2b daemon
...
as `d2b.daemon.perVmStateOwnershipMatrix` and enforced by
[`d2b_host::ownership_matrix`](../../packages/d2b-host/src/ownership_matrix.rs)
via a VM-start preflight wired into d2bd
...
`/var/lib/d2b/vms/<vm>/store/` propagated a default ACL onto the
...
2. The Rust enforcer
   (`d2b_host::ownership_matrix::should_recurse`) hard-rejects
...
The live readiness marker `store-view/live/.d2b-marker-<vm>` is a
...
`hardlink_farm_carve_out_holds_for_legacy_store`](../../packages/d2b-host/src/ownership_matrix.rs).
...
| `host-keys` | `d2bd` | `d2b` | `0750` | dir | yes | false | Known-hosts pin store for per-VM ssh host key fingerprints. |
| `store` | `d2bd` | `users` | `0755` | dir | **no (legacy)** | **false (carve-out)** | Legacy pre-store-view per-VM `/nix/store` hardlink farm retained only during migration. `required = false`: posture-if-present on native VMs. |
| `store-meta` | `d2bd` | `users` | `0755` | dir | **no (legacy)** | false | Legacy metadata sibling retained only during migration. |
| `store-view` | `d2bd` | `users` | `0755` | dir | yes | false | Canonical store-view root; holds the served `live/` pool, the guest-readable `meta/` subtree, and the host-only `state/`, `gcroots/`, `sync.lock`. |
...
| `store-view/state/integrity-unknown.json` | `d2bd` | `d2b` | `0640` | **file** | **no** | n/a | **HOST-ONLY** VM-level integrity fallback record. File-kind, created lazily; absence before first use must not fail preflight. |
| `store-view/live/.d2b-marker-<vm>` | `d2bd` | `users` | `0644` | **file** | **no** | n/a | Guest-readable live readiness marker. Zero-length, file-kind single-inode check; exempt from the `live/` no-recursion carve-out. Absent before first sync. |
...
| Role | `writablePaths` under `/var/lib/d2b/vms/<vm>/` | Matrix entry covering it |
|---|---|---|
...
| `virtiofsd` (per-share) | the per-VM state dir, `/run/d2b/...` | `.` |
...
- Integration (shell):
  ...
  `store-view/state` + `store-view/gcroots` at `d2b:d2b 0750`,
  file-kind `sync.lock` and live marker), the retired
  `store-view/generations` path is gone, and the `d2b-store-sync`
  directory ownership fix-up matches the matrix (directory-only
  `chown d2bd:users` / `chmod 0755`, no `root:kvm`, no `2775`).
