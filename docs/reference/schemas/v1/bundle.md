...
`bundle.json` is the private bundle index. It identifies the bundle version, records the artifact set that belongs to that version, and provides the hashes and compatibility metadata that `nixlingd` and the broker use before trusting sibling artifacts.

Producer: `nixos-modules/manifest-bundle.nix` emits this artifact; `packages/nixling-core` parses it.
...
| `producer` | object | yes | Nixling producer identity: package/version metadata and the source revision when available. |
...
| --- | --- | --- |
| `name` | string | Producer name; this schema uses `nixling`. |
| `version` | string | Nixling package or release version when available. |
...
root:`nixlingd` `0640`; `vms.json` remains outside this private map.
