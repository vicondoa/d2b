...
Schema: [`bundle.json`](./bundle.json) (forward reference; generated by W1 s1 with `cargo xtask gen-schemas`).
...
| `hashAlgorithm` | string | yes | Hash algorithm used for every hash in this artifact set. W1 uses one value across the bundle. |
...
| --- | --- | --- |
| `name` | string | Producer name; W1 uses `nixling`. |
...
| `manifestVersion` | integer | Public manifest contract version. W1 preserves `2`. |
| `sha256` | string | Hash of the rendered `vms.json`. |

## `artifacts`

The artifact map is closed over these W1 keys:
