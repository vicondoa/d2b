...
| `IdentityScope` | enum | [`IdentityScope`](../../packages/d2b-contracts/src/v2_state.rs#L309) | `LocalRoot`; `Realm` — struct { `realm_id`: `RealmId` }; `Workload` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId` }; `Provider` — struct { `realm_id`: `RealmId`; `provider_id`: `ProviderId` }; `Role` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId`; `role_id`: `RoleId` } |
| `AuthorityRef` | enum | [`AuthorityRef`](../../packages/d2b-contracts/src/v2_state.rs#L336) | `Pid1`; `LocalRootAllocator`; `LocalRootBroker`; `RealmController` — struct { `realm_id`: `RealmId` }; `RealmBroker` — struct { `realm_id`: `RealmId` }; `WorkloadController` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId` }; `WorkloadBroker` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId` }; `WorkloadRole` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId`; `role_id`: `RoleId` }; `Provider` — struct { `realm_id`: `RealmId`; `provider_id`: `ProviderId` } |
| `StorageCategory` | enum | [`StorageCategory`](../../packages/d2b-contracts/src/v2_state.rs#L428) | `LocalRoot`; `Realm`; `Workload`; `Provider`; `Runtime`; `Lock`; `Lease`; `Quarantine`; `Audit`; `Projection` |
| `LogicalLocation` | enum | [`LogicalLocation`](../../packages/d2b-contracts/src/v2_state.rs#L460) | `HostAllocator`; `HostBroker`; `HostAudit`; `RealmController`; `RealmBroker`; `RealmAudit`; `WorkloadState`; `WorkloadDisks`; `WorkloadStoreView`; `WorkloadTpm`; `WorkloadMedia`; `WorkloadAudio`; `WorkloadKeys`; `ProviderState`; `RuntimeRealm`; `RuntimeWorkload`; `RuntimeRole`; `RuntimeLocks`; `RuntimeLeases`; `Quarantine`; `Projection` |
| `ResourceKind` | enum | [`ResourceKind`](../../packages/d2b-contracts/src/v2_state.rs#L486) | `Directory`; `JsonDocument`; `JsonLinesSegment`; `UnixSocket`; `OfdLockFile`; `LeaseRecord`; `DiskImage`; `TpmState`; `StoreView`; `ProjectionDocument` |
| `CreationPolicy` | enum | [`CreationPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L501) | `CreateIfMissing`; `MaterializeGeneration`; `BindPrecreated`; `ObserveOnly` |
| `ReconcilePolicy` | enum | [`ReconcilePolicy`](../../packages/d2b-contracts/src/v2_state.rs#L510) | `VerifyOnly`; `ReconcileExact`; `AdoptAfterProof`; `Regenerate` |
| `RepairPolicy` | enum | [`RepairPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L519) | `FailClosed`; `RepairExact`; `Quarantine`; `Regenerate`; `NoRepair` |
| `DeletePolicy` | enum | [`DeletePolicy`](../../packages/d2b-contracts/src/v2_state.rs#L529) | `Never`; `AfterOwnerAbsenceProof`; `GenerationRetirement`; `RetentionOnly`; `FactoryResetOnly` |
| `PersistenceClass` | enum | [`PersistenceClass`](../../packages/d2b-contracts/src/v2_state.rs#L539) | `Configuration`; `Persistent`; `BootScoped`; `ProcessScoped`; `Regenerable` |
| `SecretClass` | enum | [`SecretClass`](../../packages/d2b-contracts/src/v2_state.rs#L549) | `PublicMetadata`; `PrivateMetadata`; `SecretAdjacent`; `Secret` |
| `FileMode` | enum | [`FileMode`](../../packages/d2b-contracts/src/v2_state.rs#L557) | `Mode0600`; `Mode0640`; `Mode0660`; `Mode0700`; `Mode0710`; `Mode0750`; `Mode0770` |
| `GroupPolicy` | enum | [`GroupPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L576) | `OwnerPrimaryGroup`; `LocalRootControl`; `RealmControl`; `RolePrivate` |
| `StorageRestartPolicy` | enum | [`StorageRestartPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L593) | `PreserveAndVerify`; `RecreateAfterOwnerAbsence`; `RegenerateFromConfiguration`; `QuarantineOnAmbiguity`; `NotAdoptable` |
| `StorageAdoptionPolicy` | enum | [`StorageAdoptionPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L603) | `VerifyIdentityAndGeneration`; `VerifyLease`; `ReopenFromPersistentState`; `QuarantineOnAmbiguity`; `NotApplicable` |
| `AtomicWritePhase` | enum | [`AtomicWritePhase`](../../packages/d2b-contracts/src/v2_state.rs#L964) | `Initial`; `TemporaryCreated`; `CompleteDocumentWritten`; `TemporaryFileSynced`; `Renamed`; `ParentDirectorySynced` |
| `CrashRecoveryOutcome` | enum | [`CrashRecoveryOutcome`](../../packages/d2b-contracts/src/v2_state.rs#L1020) | `PriorDocument`; `CompleteNewDocument` |
| `EvidenceVerdict` | enum | [`EvidenceVerdict`](../../packages/d2b-contracts/src/v2_state.rs#L1046) | `Match`; `Mismatch`; `Missing`; `Ambiguous` |
| `PidfdPersistence` | enum | [`PidfdPersistence`](../../packages/d2b-contracts/src/v2_state.rs#L1055) | `ProcessLocalNonPersistent` |
| `RecoveryOrdering` | enum | [`RecoveryOrdering`](../../packages/d2b-contracts/src/v2_state.rs#L1253) | `RecoverBeforeCleanup` |
| `QuarantineReason` | enum | [`QuarantineReason`](../../packages/d2b-contracts/src/v2_state.rs#L1259) | `MissingIdentityEvidence`; `MultipleCandidates`; `CgroupMismatch`; `ExecutableMismatch`; `ConfigurationMismatch`; `GenerationMismatch`; `OwnerAmbiguous`; `CorruptState`; `AuditGap` |
| `DegradedReason` | enum | [`DegradedReason`](../../packages/d2b-contracts/src/v2_state.rs#L1273) | `AdoptionPending`; `AdoptionQuarantined`; `StorageDrift`; `StorageRepairFailed`; `LockContended`; `LockOwnerAmbiguous`; `LeaseExpired`; `AuditChainInvalid`; `AuditGapDetected`; `RestartRequired` |
| `Remediation` | enum | [`Remediation`](../../packages/d2b-contracts/src/v2_state.rs#L1288) | `RetryAdoption`; `InspectQuarantine`; `RestartWorkload`; `RepairStorage`; `ReleaseLease`; `ExportAudit`; `AcknowledgeAuditGap` |
| `AbsenceEvidence` | enum | [`AbsenceEvidence`](../../packages/d2b-contracts/src/v2_state.rs#L1300) | `CompletedDiscovery` |
| `CleanupTarget` | enum | [`CleanupTarget`](../../packages/d2b-contracts/src/v2_state.rs#L1311) | `Resource` — struct { `resource_id`: `ResourceId` }; `Role` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId`; `role_id`: `RoleId` }; `Workload` — struct { `realm_id`: `RealmId`; `workload_id`: `WorkloadId` } |
| `AdoptionDecision` | enum | [`AdoptionDecision`](../../packages/d2b-contracts/src/v2_state.rs#L1381) | `Adopt` — struct { `fresh_pidfd_opened`: `bool` }; `Quarantine` — struct { `reason`: `QuarantineReason`; `remediation`: `Remediation` }; `Cleanup` — struct { `target`: `CleanupTarget`; `owner_absence_proof`: `OwnerAbsenceProof` } |
| `LockClass` | enum | [`LockClass`](../../packages/d2b-contracts/src/v2_state.rs#L1461) | `LocalRoot`; `Realm`; `Workload`; `Provider`; `Role` |
| `LockKind` | enum | [`LockKind`](../../packages/d2b-contracts/src/v2_state.rs#L1471) | `Ofd`; `InProcess`; `KernelObject`; `FdBackedLease` |
| `FdTransferPolicy` | enum | [`FdTransferPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L1480) | `Never`; `ComponentSessionAttachment`; `ScmRightsLeaseHandoff`; `ExplicitFdMapping` |
| `ContentionPolicy` | enum | [`ContentionPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L1489) | `FailFast`; `BoundedWait` |
| `CancellationPolicy` | enum | [`CancellationPolicy`](../../packages/d2b-contracts/src/v2_state.rs#L1496) | `Cancellable`; `CompleteAtomicSection` |
| `LeaseRevocation` | enum | [`LeaseRevocation`](../../packages/d2b-contracts/src/v2_state.rs#L1645) | `Active`; `RevokedByOwner`; `RevokedByGenerationChange`; `Expired` |
| `AuditStream` | enum | [`AuditStream`](../../packages/d2b-contracts/src/v2_state.rs#L1690) | `LocalRoot`; `Realm` — struct { `realm_id`: `RealmId` } |
| `AuditOwner` | enum | [`AuditOwner`](../../packages/d2b-contracts/src/v2_state.rs#L1702) | `LocalRootBroker`; `RealmBroker` — struct { `realm_id`: `RealmId` } |
| `AuditEvent` | enum | [`AuditEvent`](../../packages/d2b-contracts/src/v2_state.rs#L1709) | `StorageCreate`; `StorageReconcile`; `StorageRepair`; `StorageDelete`; `RestartDiscover`; `RestartAdopt`; `RestartQuarantine`; `LockAcquire`; `LockRelease`; `LeaseGrant`; `LeaseRevoke`; `ProviderOperation`; `SessionOperation`; `SegmentSeal`; `Checkpoint`; `GapDetected`; `RetentionPrune`; `Export` |
| `AuditOutcome` | enum | [`AuditOutcome`](../../packages/d2b-contracts/src/v2_state.rs#L1732) | `Succeeded`; `Denied`; `Failed`; `Degraded`; `Quarantined`; `Cancelled` |
| `AuditReason` | enum | [`AuditReason`](../../packages/d2b-contracts/src/v2_state.rs#L1743) | `PolicyAllowed`; `PolicyDenied`; `IdentityVerified`; `IdentityMismatch`; `GenerationMismatch`; `OwnerAmbiguous`; `StorageDrift`; `LockContended`; `LeaseExpired`; `CorruptState`; `SequenceGap`; `RetentionLimit`; `OperatorRequested` |
| `AuditActor` | enum | [`AuditActor`](../../packages/d2b-contracts/src/v2_state.rs#L1774) | `LocalRootAllocator`; `LocalRootBroker`; `RealmController` — struct { `realm_id`: `RealmId` }; `RealmBroker` — struct { `realm_id`: `RealmId` }; `Provider` — struct { `provider_id`: `ProviderId` }; `WorkloadRole` — struct { `role_id`: `RoleId` } |
| `PruneStatus` | enum | [`PruneStatus`](../../packages/d2b-contracts/src/v2_state.rs#L1973) | `Retained`; `EligibleAfterCheckpoint`; `PrunedWithCheckpoint` |
| `AuditRetentionDecision` | enum | [`AuditRetentionDecision`](../../packages/d2b-contracts/src/v2_state.rs#L2289) | `Retain`; `SealCurrentSegment`; `CreateCheckpoint`; `PruneCheckpointedSegment` |
| `AuditExportFormat` | enum | [`AuditExportFormat`](../../packages/d2b-contracts/src/v2_state.rs#L2379) | `RedactedJsonLines`; `CheckpointBundle` |
| `ProjectionAuthority` | enum | [`ProjectionAuthority`](../../packages/d2b-contracts/src/v2_state.rs#L2453) | `DiagnosticsOnly` |
| `ProjectionStatus` | enum | [`ProjectionStatus`](../../packages/d2b-contracts/src/v2_state.rs#L2459) | `Ready`; `Starting`; `Stopped`; `Degraded`; `Quarantined`; `Unknown` |
