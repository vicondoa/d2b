...
- [`../../nixos-modules/options-daemon.nix`](../../nixos-modules/options-daemon.nix) -
  `validationEvidencePresent`, the gate-checked predicate this
  doc mirrors.

## Wave 7 recovery-point attestation

Wave 7 delivery uses a separate, strict version-1 attestation for the
operator-owned external full-host recovery point. The machine-readable schema
is [`schemas/delivery/recovery-point-attestation-v1.schema.json`](./schemas/delivery/recovery-point-attestation-v1.schema.json).
It is generated by `bazel run //packages/xtask:xtask -- gen-schemas`.

The attestation is imported into the existing candidate evidence directory by
the `recovery-import` delivery stage. It is never a replacement for the
host-local readiness file above and it never stores the recovery payload,
restore text, host identity, operator subject, or external locator.

### Exact attestation contract
...
`artifactKind` is `d2b-recovery-point-attestation`, `schemaVersion` is `1`,
and `program` is `ADR046`, the current d2b-3.0 delivery identifier.
The explicit `d2b-3.0` release spelling is also accepted. `recoveryPointKind` is either
`full-host-snapshot` or `full-host-backup`; its verification method is,
respectively, `snapshot-readback` or `backup-verify`. Both verification
results and the final result must be `passed`.

`qualification` contains exactly these five boolean members, all set to
`true`: `bootAndSystemStateCovered`, `affectedArtifactInventoryCovered`,
`preservedIdentityStateCovered`, `sameHostRestoreTarget`, and
`readOnlyUntilExpiry`. The candidate, commit, tree, pinned closure store path
digest, bundle generation, canonical preview, host digest, operator digest,
restore-instruction digest, and locator digest are compared again at every
delivery boundary. The locator is represented only by its digest.

All six time values are bounded integer Unix seconds. The validator samples
its clock once per invocation and requires:

```text
previewed <= captured <= verified <= attested <= verifier-now < expires
expires = min(captured + 86400, verified + 86400, retentionUntil)
```

The additions are checked against the bounded timestamp range. A caller must
also provide the remaining lifetime required for apply, verification, guarded
