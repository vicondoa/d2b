...
Authoritative emitter: [`nixling_host::ifname::derive_from_env_vm`](../../packages/nixling-host/src/ifname.rs).

| Resource          | Pattern                                  | Example         |
| ----------------- | ---------------------------------------- | --------------- |
| Per-VM tap        | `<prefix>t<HASH8>`                       | `nl-tWORK010`   |
| Per-env bridge    | `<prefix>b<HASH8>`                       | `nl-bWORK000`   |

Where:

- `<prefix>` is the configurable nixling ifname prefix. Default
  `nl-` (see `nixling_host::ifname::DEFAULT_PREFIX`). Prefix must
  satisfy `validate_prefix` (≤ 8 bytes, ends in `-`, alphabet
  `[A-Za-z0-9_-]`).
- The single role tag character is `t` for taps and `b` for
  bridges (`nixling_host::ifname::{TAP_TAG, BRIDGE_TAG}`).
...
- With the default `nl-` prefix the total length is 12 bytes
  (`nl- + t + 8 hash chars`), well within IFNAMSIZ-1.

Reverse-lookup: [`nixling_host::ifname::looks_nixling_owned`]
...
never touches a tap that does not pass `looks_nixling_owned` with
the configured prefix.

The mapping `(env, vm?, role) → derived_ifname` is emitted into
the trusted bundle as `host.json::ifNameMapping`
(`nixling_core::host::IfNameMapping`) and re-validated by the
broker via `nixling_host::ifname::detect_collisions`. Two distinct
...
[`nixling_host::host_prep_dag::build_host_prep_dag`](../../packages/nixling-host/src/host_prep_dag.rs):
...
1. `<vm>:apply-nftables-rules` — declared `depends_on`. The
   `inet nixling` table must hold the per-VM/per-busid carve-outs
...
2. `ApplyNmUnmanaged` (host-wide, not per-VM) — enforced inside
   the broker handler via [`TapCreateGate`](../../packages/nixling-priv-broker/src/ops/tap.rs).
...
ioctl sequence (`nixling_host::netlink::ipv6_off_sequence`), the
tap is bound to its env bridge with `bridge_slave` master + the
per-role port flag matrix from
[`nixling_host::bridge_port::BridgePortFlagSet::defaults_for`](../../packages/nixling-host/src/bridge_port.rs).
...
[`nixling_core::host::ChNetHandoffMode`](../../packages/nixling-core/src/host.rs)
...
2. Broker applies the per-link IPv6-off sysctl sequence
   ...
   so no IPv6 SLAAC/link-local packet ever leaves a nixling tap.
...
1. Enumerate every link whose name matches
   `looks_nixling_owned(name, prefix)`.
...
5. If a tap exists for a VM that is also alive (pidfd present
   in `/run/nixling/state/runner-pidfds/<vm>`): the daemon
   leaves it alone. Reconciliation never tears down a tap whose
   CH is still live.

Foreign links (no `nl-` prefix, or `nl-` prefix but failing
`looks_nixling_owned`) are ignored. The daemon NEVER deletes a
link it does not own.

## Failure semantics

Tap creation failure surfaces as the typed
[`HostPrepStepFailed`](../../packages/nixling-host/src/host_prep_dag.rs)
...
- **Bridge port flags**: [`packages/nixling-host/src/bridge_port.rs`](../../packages/nixling-host/src/bridge_port.rs).
- **Tap broker handlers**: [`packages/nixling-priv-broker/src/ops/tap.rs`](../../packages/nixling-priv-broker/src/ops/tap.rs).
- **Derived ifname emitter**: [`packages/nixling-host/src/ifname.rs`](../../packages/nixling-host/src/ifname.rs).
