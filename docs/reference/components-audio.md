...
> CLI: [`nixos-modules/cli.nix`](../../nixos-modules/cli.nix) (`nixling audio …`)
...
record is a per-VM JSON file at
`/var/lib/nixling/vms/<vm>/state/audio-state.json`; the
`nixling audio` CLI subcommand mutates it under flock. The sidecar
publishes the resolved mic/speaker state as custom PipeWire
properties (`nixling.mic`, `nixling.speaker`); a host-side
`client.conf.d` rule reads those properties and null-routes the
corresponding stream direction with `target.object = "-1"` when it's
`off`. Setting `audio.enable = true` only enables the *capability* —
...
| `nixling.vms.<vm>.audio.allowMicByDefault` | bool | `false` | Initial value of the `mic` field when the per-VM state file is first materialised. Consulted at creation time only; subsequent edits via `nixling audio …` persist. |
| `nixling.vms.<vm>.audio.allowSpeakerByDefault` | bool | `false` | Initial value of the `speaker` field when the per-VM state file is first materialised. Same edit-survives semantics. |
...
- **`nixling-<vm>-snd.service`**
  ...
  — vhost-user-sound sidecar, system service (not user-session
  scoped, not a template). Runs as `nixling-<vm>-snd:nixling-<vm>-snd`,
  `SupplementaryGroups = [ "audio" ]`. Started on demand by
  `nixling audio mic|speaker on <vm>` (or by the audioArgsScript
...
- **Lock file** `/run/nixling/audio-<vm>.lock`, mode 0660,
  `root:nixling-launcher`. The CLI takes flock on it before
  mutating the state file.
...
- **VM silent / no soundcard in `aplay -l`.** Both mic and speaker
  ...
  and restart the VM (`nixling down <vm> && nixling up <vm>`) so
...
- **`autostart = true` + `audio.enable = true` eval failure.**
  ...
  bound to `nixling up <vm>`, which `microvm@<vm>.service` doesn't
  trigger.
