...
| B6 | d2b-resource-runtime peripheral dead paths | not applied | `-` | `ManagerCall` and `ChannelManagerEndpoint` (`context.rs:143`), the write-only audit-log read path, and the identity re-export module remain. |
| B7 | d2b-resource-compiler's compile_provider_artifact alias | not applied | `-` | `packages/d2b-resource-compiler/src/lib.rs:1325` still declares it; nothing in the tree names it. |
| B8 | BindingChildReconciler | not applied | `-` | `packages/d2b-core-controller/src/binding_children.rs` still declares it and `lib.rs` re-exports it. |
