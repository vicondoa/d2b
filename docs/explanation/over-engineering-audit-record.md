...
| 5 | The `EffectPortAdapter` layer in both process provider crates is never constructed | applied | `-` | Deleted: `minijail/src/effect_port.rs` and `systemd/src/effect_port.rs` with their `MinijailProcessEffectPort`/`SystemdProcessEffectPort` declarations, their re-export arms, and their dossier destination lines were retired in the legacy-effect-port-removal lane. The live conformance `ProcessLaunchEffectPort` remains the providers' generic spawn boundary. |
