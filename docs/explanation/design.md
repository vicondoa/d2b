...
- [1. The problem d2b solves](#1-the-problem-d2b-solves)
...
Pre-v1.1, d2b composed on top of [microvm.nix] by translating
`d2b.vms.<vm>` into `microvm.vms.<vm>` in `host.nix`'s
mapAttrs pass. v1.1 retired this translation in favour of the
d2b-owned per-VM evaluator + d2b-owned option set; the
...
```
          ...
          │   │  compositor + d2b CLI invocations         │   │
          │   └──────────────────┬────────────────────────────┘   │
...
1. **Host kernel ↔ guest userspace + guest kernel.** Enforced by
   ...
   microvm.nix. This is the strongest boundary d2b has and
...
runs under a dedicated per-VM user (`d2b-<vm>-gpu`,
`d2b-<vm>-snd`, `d2b-<vm>-swtpm`; declared in
...
under `/var/lib/d2b/vms/<vm>/swtpm/` and is owned by
`d2b-<vm>-swtpm:d2b-<vm>-swtpm` mode 0700. No other VM's
...
ACL granting `d2b-<vm>-gpu` rw at `ExecStartPost` time
([`nixos-modules/host-sidecars.nix:79-83`](../../nixos-modules/host-sidecars.nix)).

### Threats *not* addressed

D2b is deliberately not a defense against any of the following.
...
- **Side channels (cache timing, branch predictor, Rowhammer,
  ...
  microcode provide; d2b adds nothing.
...
- **Multi-user trust separation on the host.** D2b assumes a
  ...
  member of `d2b`. A second untrusted human on the
...
**Boot chain.** No d2b component runs as part of the host's
UEFI or bootloader chain. D2b is a NixOS module activated
after the host has already booted. Lanzaboote signs the host
kernel and initrd; d2b's per-VM kernels run later inside KVM
guests and are not part of the host Secure Boot chain.

**TPM state ownership.** Lanzaboote uses the host TPM for measured
boot. D2b's per-VM `swtpm` emulators are software TPMs with
state under `/var/lib/d2b/vms/<vm>/swtpm/`. They are
...
`/var/lib/d2b/vms/<vm>/swtpm/` does not depend on the host's
Secure Boot keys and is not sealed to host PCRs.

**Recovery boundary.** If host Secure Boot breaks, d2b VMs are
...
`/run/d2b/public.sock` combined with membership in the
...
- **Do not turn d2b into an arbitrary network bridge.** The
...
`microvm@.service`. Before v1.0, d2b wrapped that with its own
template, `d2b@.service`, declared in
...
D2b pins `microvm.autostart = [ ]` so the upstream template
...
`d2b.vms.<vm>.autostart`. This eliminates a class of double-
...
every d2b-managed file under `/var/lib/d2b/` instead of the
...
> store-sync, otel-relay@). All other "d2b-<vm>-{gpu,snd,video,
...
gated by the per-VM component toggles. `d2bd` starts each long-lived
...
- (legacy only) `d2b-sys-<env>-usbipd-backend.service` and
  ...
  `d2b.slice/sys-<env>/usbipd-{backend,proxy}`), so the prior
...
- Per-tap networkd rules that route taps named `<env>-u*` to the
  ...
  Setting `d2b.envs.<env>.lan.allowEastWest = true` clears
...
other VM's closure) into each VM. D2b replaces this with a
per-VM farm at `/var/lib/d2b/vms/<vm>/store/` containing only
...
The `d2b` command (the Rust crate at
[`packages/d2b`](../../packages/d2b), see also the
...
- At host activation, a single shell block (under `flock` on
  ...
  `d2b` group `r` on the private key.
- The same activation script stages the per-VM pubkey + the
  resolved `userAuthorizedKeys` content into
  `/var/lib/d2b/vms/<vm>/host-keys/`, which `host.nix`
  mounts into the guest via virtio-fs as
  `/run/d2b-host-keys/`. The guest baseline runs
  `d2b-load-host-keys.service` at boot, reads that share,
...
Everything d2b owns lives under `/var/lib/d2b/`:
...
flowchart TD
    ...
    root --> knownhosts["known_hosts.d2b<br/>CLI-owned known_hosts file"]
    root --> knownhostslock["known_hosts.d2b.lock"]
```
...
├── known_hosts.d2b         CLI-owned known_hosts file
└── known_hosts.d2b.lock
```
</details>

`/var/lib/d2b/tmp/` is the framework's per-boot transient state
root. Components SHOULD place reboot-safe scratch data under
`/var/lib/d2b/tmp/<vm>/` (temporary sockets, swtpm proxy paths,
...
Net VMs land at `/var/lib/d2b/vms/sys-<env>-net/` for now;
...
| `d2b-sys-<env>-net`                   | reserved auto-system VM name            | network.nix                 |
| `d2b` group                 | SO_PEERCRED authorisation surface at `d2bd`'s public.sock (mode 0660, group `d2b`) | host-users.nix |
| `d2b-<vm>-{gpu,snd,swtpm,usbip}` users | dedicated per-VM runner uids (broker-spawned in v1.0) | host-users.nix |
...
- **D2b owns:** VM / env / sidecar lifecycle, network isolation
  (per-env bridges + net VM + NAT), per-VM `/nix/store` hardlink
  farm, the `d2b` CLI, lifecycle group authorization, and key
...
- **The seam** is one line in the consumer's flake:
  ...
  ```nix
  d2b.vms.work-vm.config.imports = [
...
declares one user per sidecar per VM (`d2b-<vm>-gpu`,
`d2b-<vm>-snd`, `d2b-<vm>-swtpm`), all `isSystemUser =
...
### Launcher authorisation (v1.0: SO_PEERCRED + d2b group)
...
lifecycle dispatch goes through `d2bd`'s public socket at
`/run/d2b/public.sock` (mode `0660`, owner `d2bd`, group
`d2b`). Authorisation is enforced via
`SO_PEERCRED` at `accept(2)` time: the daemon reads the peer's
uid/gid, refuses requests from peers outside the `d2b` group /
`d2b.site.launcherUsers` (the *connection* gate), and gates
...
On every startup, `d2bd` probes each env's LAN bridge under
...
3. The net VM start (`d2b@sys-<env>-net.service`) is
...
[`nixos-modules/host-keys.nix`](../../nixos-modules/host-keys.nix),
written to `/var/lib/d2b/keys/<vm>_ed25519` (mode 0640, root-
...
- **VM-to-VM east-west traffic within the same env is not
  ...
  (`d2b.envs.<env>.intraLanIsolation = false`) is on the
...
- **(legacy only) USBIP per-env systemd units materialised even when no VM in the env opted in.**
  Before v1.0, each declared env emitted `d2b-sys-<env>-usbipd-{backend,proxy}.service`
  ...
  USBIP runners are broker-spawned on demand via `d2b.slice/sys-<env>/usbipd-*`
...
- **No static lint for the `mkOption { default = …; readOnly =
  ...
  reviewer-found bug (the `d2b.manifest` "set multiple
...
- **`d2b.site.stateDir` / `d2b.store.stateDir` stay fixed at
  their defaults for now.** Several modules still hardcode
  `/var/lib/d2b` and `/var/lib/d2b/vms`, so eval rejects
...
- **Audio mic/speaker enforcement is via PipeWire stream rules, not
  ...
  trusted boundary, and `d2b audio` is the explicit toggle.
...
- **Host-eval namespace policy lint (fail-closed).** The guest file is
  ...
  `microvm.*` / `d2b.*` option. Detection evaluates the guest file
...
its level. microvm.nix gives you a VM; d2b gives you the
...
better answer to that question than d2b could ever be.
...
is its own OS, with its own assumptions; d2b is a flake
...
threat models. D2b's pitch is "you already run NixOS,
...
the same way d2b already materialises other cross-cutting
...
`d2b-otel-vsock-out.service` forwards the guest collector's Unix
...
The host's own collector uses `/run/d2b/otel/host-egress.sock` and
...
broker, which is already the trust posture d2b accepts for
...
does not give d2b a native guest-to-guest vsock transport. A small
...
`d2b-otel-vsock-in-*` listener per host/workload source, each
...
- **Custom Rust relay instead of `socat`**: same topology, smaller
  ...
  when a native relay lands, d2b should ship the new interface first
...
ClickHouse; if d2b emits high-cardinality strings or sensitive paths,
...
If d2b ever grows into a multi-host framework, the scaling shape is
...
- Per-VM `microvm@<vm>.service` and `d2b@<vm>.service` units
  (legacy wrappers; retired in v1.0 per ADR 0015) went away for
  daemon-owned VMs; `d2bd` supervises children directly.
  Single-writer is enforced by `/run/d2b/locks/<vm>` (ADR 0001).
- `polkit` runtime gating is replaced by `SO_PEERCRED` + groups on the
  ...
  start d2bd`, but per-VM lifecycle no longer touches polkit.
- The Cloud Hypervisor command line is generated by `d2bd` from
  evaluated `microvm.nix`/`d2b` configuration; `declaredRunner`
...
- Telemetry posture stays "none". `d2bd` makes no outbound
...
3. **Daemon state on disk:** the supervisor persists a per-(vm,
   ...
   `/var/lib/d2b/daemon-state/<vm>/runtime.<role_id>.json`
   (root-owned `0640 root:d2bd`). The snapshot carries enough
...
4. **Daemon version file:** the daemon writes `/run/d2b/version`
