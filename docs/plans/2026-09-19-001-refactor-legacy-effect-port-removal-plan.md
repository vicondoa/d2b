...
Recorded implementation state, 2026-09-21: every family lane has delivered its move, and each lane is either merged or in its final review pass. The problem statement above describes the state the programme opened against, not the current tree. The host, process, and network-local families are converted and merged on the mainline and their registration rows ride the generated table, as do the enabling units U2-U4. Every remaining family lane is converted on its branch and awaits merge; none is counted as landed until its head is in the tree, and the per-unit inventory lives in the unit table below. Two kinds of partial conversion exist and are named per unit: families whose substantive launch and reconcile work remains daemon-side behind declared runtime facets (process, network-local), and families whose lane retires the port while a documented daemon-side runtime or composition surface stays (credential, interaction and desktop). The process providers' spawn ports and the transport effect-port module are retired rather than converted (U1's retirement half); the neutral volume contract retires with the volume lane (KD3) and still stands on the mainline until that lane merges.
...
| U5 host | Converted and merged | `kernel_module_check.rs` and `pidfs_probe.rs` deleted; the probe serves from `packages/d2b-provider-host/` over daemon-supplied facets |
...
| U9 device | Converted on branch, awaiting merge | `refactor/device-families-lane` (head 08b6f40a4): deletes `tpm_effect_port.rs` and `usbip_production.rs`, and moves the GPU, security-key, and USBIP arms out of `shared_provider_effects.rs` (the network arm remains there on the branch too) |
