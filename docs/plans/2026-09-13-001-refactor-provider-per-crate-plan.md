...
- **Re-scoped during execution (realm-core tree, verified):** `packages/d2b-realm-core/` stays; the deletion instruction is withdrawn because the tree is not orphaned. It is outside the workspace member list, `Cargo.lock`, and every Bazel label, but the standalone `labs/window-chrome/proxy` prototype (ADR 0047) declares it by path (`labs/window-chrome/proxy/Cargo.toml`) and imports `WorkloadProviderKind` from it in six modules, and `docs/reference/constellation-core.md` records that the former model's identifiers remain in standalone prototype sources. Deleting the directory would break the prototype build, so it survives as the lab's read-only dependency; the main graph already excludes it.
...
- **The invariant is stronger than "no guard across an await": no synchronous lock is taken on an async path at all.** A scoped `std::sync::Mutex` guard that is dropped before every suspension point still serialises an executor's worker and still blocks it while another thread holds the lock - the lint cannot see it, and it is not acceptable. Async paths are **lock-free by design** where they can be: ownership moved to the task that needs it, message passing instead of shared mutation, atomics for counters and flags, and immutable values published by reference. Where shared mutation genuinely cannot be avoided, the answer is a thread-safe type designed for the purpose (an atomic, a channel, a concurrent map) or an async-aware primitive - and a synchronous lock is not one. Where an async-aware lock is genuinely the only option, that is a design question to state and defend in the code, not a default. The synchronous-only remainder of the system - command-line paths, the daemon's own dispatch threads, test scaffolding - is unaffected and keeps its locks; a synchronous lock on a synchronous path is correct and the denylist's global reach is why those need a tracked allow rather than a rewrite. The practical enforcement for the async half cannot come from the await lint alone: it needs a check that flags a synchronous lock acquisition lexically inside an async function, which is the kind of source-level rule this repository's policy checks already implement.
...
- **The effect-port split, which decides most of the verdicts:** every effect port has two halves and they have opposite answers. The **adapter half** - translating a family's effect intent into a typed broker request, then decoding the reply - is glue that the generic path makes unnecessary: it collapses when that family's call sites migrate onto the forwarding rendezvous, and it retires with its typed arm. The **executor half** - the privileged syscall, the subprocess, the fd handed back, the trusted resolution of an intent against the broker's own bundle copy - is the broker's job by design and does not move into a provider, because a provider is not the trusted party and the carrier cannot yet carry descriptors or broker-owned state. So the audit classifies each port into adapter and executor, and expects most adapters to be scheduled for removal while most executors are kept with a reason. Where an effect has no trustworthy broker-side implementation and the provider could perform it under its own posture, that is a design change worth naming rather than assuming.

- **Audit result (read-only, 123 pockets):** 78 keep with a stated reason, 32 move, 13 delete, plus the typed-broker construction class counted separately. The split that decides sequencing: **the deletes are actionable now** - dead pockets with no caller, about twelve hundred lines of test-only wave-and-legacy simulation, fifteen callerless admin-default broker wrappers, and the residues named per file - while **the moves are mostly adapter halves that wait on the generic path for their family**, so they belong with the migration rather than here. The largest single payoffs are the shared provider effects' network child port and input handling, the usbip and guest-media block, the display supervisor's durable-process realisation, and the activation block. Two contradictions worth keeping: no dead arms from the deleted shell, launcher, and runner families survive in these files (those wire families are retired at parse), and the textual sweep counts sixty-four typed broker constructions against the census's sixty-seven - a small discrepancy to reconcile when the migration unit starts rather than a disagreement about the shape.
