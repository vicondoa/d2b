...
**Verification:** Owner-local broker contract/runtime tests, broker Nix module evaluation, ACL checks, and socket-activation tests cover shared wire/runtime behavior and the current broker service boundary. The existing broker service-posture case file is not scheduled by a live Nix surface and is not current evidence. No live owner-local target asserts complete Host/Guest profile or cross-instance separation; U11 must schedule the posture cases and add the missing separation cases before citing strict separation proof.
