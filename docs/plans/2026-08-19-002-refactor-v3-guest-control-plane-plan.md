...
| Cross-target aggregate controllers directly spawn children | Process lifecycle bypasses providers | Add an owner-local controller boundary test and independent review; treat source hygiene and Provider conformance as supporting evidence only |
...
**Verification:** Owner-local broker contract/runtime tests, broker Nix module evaluation, ACL checks, and socket-activation tests cover shared wire/runtime behavior and the current broker service boundary. The existing broker service-posture case file is not scheduled by a live Nix surface and is not current evidence. U11 must schedule it through `tests/unit/nix/surfaces/daemon.nix` so `//bazel/checks/nix:nix-unit-daemon` executes it. No live owner-local target asserts complete Host/Guest profile or cross-instance separation; U11 must add `//packages/d2b-broker:host_profile`, `//packages/d2b-broker:guest_profile`, and `//packages/d2b-broker:profile_separation` from the exact files above before citing strict profile, catalog, authority-root, or cross-instance separation proof.
...
| `D2B_VM_CHECK=guest-control-plane make test-host-integration` | U3-U4, U9-U10 | The named x86_64 runNixOSTest Guest proves daemon/broker mode separation, vsock enrollment, assignment fencing, controller launch, Process readiness, reconnect, handoff, and teardown; an unsupported-platform skip is not evidence |
| `make test-integration` | U9-U10 | The required `ubuntu-host-check` runner executes both shared binaries in the supported foreign-userland container path; an empty inventory, missing Nix, or skipped runner is not evidence |
...
- Broker service posture: U11 schedules `tests/unit/nix/cases/broker-service-posture.nix` through `//bazel/checks/nix:nix-unit-daemon`; strict profile and cross-instance separation are owned by the planned `//packages/d2b-broker:host_profile`, `//packages/d2b-broker:guest_profile`, and `//packages/d2b-broker:profile_separation` targets.
