...
- Before U9 cutover, bind a family inventory of every legacy caller, method, generated artifact, Nix input, Bazel target, and replacement owner into the `d2b-cutover` candidate and preview. Stop when any row lacks a validated replacement.
- During U10, use only the rollback path and boundary recorded by `d2b-cutover`. After irreversible finalization, rollback is a whole-head revert; do not restore guest-control methods, feature flags, or dual routing.
...
1. Freeze the pre-cutover inventory and bind a validated replacement owner for every legacy caller, method, generated artifact, Nix input, and Bazel target into the existing `d2b-cutover` candidate and preview.
2. Drive the synchronized transition through the existing cutover consent, journal, hold, recovery, rollback-boundary, verification, and finalization contracts; do not add a migration ledger or second cutover engine.
...
- Cutover restart reopens the exact journal and candidate, preserves incident holds, and cannot cross the recorded rollback boundary twice.
...
- The final transition uses the existing `d2b-cutover` candidate, consent, journal, hold, recovery, rollback-boundary, verification, and finalization contracts with no second migration ledger.
