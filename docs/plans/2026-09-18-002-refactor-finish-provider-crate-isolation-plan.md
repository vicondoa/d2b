...
- **Sequencing (established during execution):** the type authority generates now, because the declarations already carry every type fact it needs. The role and principal authority cannot, because the census shows only four declaring crates hold role vocabulary in their own source; the other twelve role families still live in shared crates, and the principals live in `packages/d2b-core/src/storage.rs`, `packages/d2b-contracts-zone-session/src/v3/role.rs`, `packages/d2bd/src/principal_allocation.rs`, and `packages/d2b-resource-types/src/provider.rs`. So the role authority generates as each lane moves its own vocabulary, and the authority bound applies to a crate once its role facts are declared rather than at the end of this unit.
- **Test scenarios:**
...
- **Sequencing (established during execution):** the structural signals live in their own `SHARED_STRUCTURAL_KNOWLEDGE_RATCHET`, keyed by module, class, and symbol and pinned two ways so a new signal fails and a stale row fails; a signal the family ratchet already covers stays in that ratchet instead of getting a second row. The Nix surface is monitored through the shared modules only, with `nixos-modules/generated/` excluded because a generated artifact is checked by its producer annotation instead. The ratchet's rows retire with the lanes that move the vocabulary they name.
- **Test scenarios:**
