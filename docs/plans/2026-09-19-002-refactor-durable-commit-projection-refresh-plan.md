...
- **Objective:** Make a Volume or VolumeBinding child row resolvable no matter which writer committed it, by driving the plane's anchor projection from the manager's existing durable-change stream instead of from per-writer refresh calls, and by making the one failure that stream leaves - a resolution that races its own commit - a retry the runtime owns. This closes issue #531 by removing its premise: no provider family is asked to supply an anchor-refresh hook, so none can omit one.
...
- **Stop condition:** `make check` passes on the change's head, the anchor projection re-materializes from every named commit route with no writer-specific refresh call, a resolution that misses an anchor is a failure the runtime retries (a deferral on the delay its driver names, an operational error on the actor's escalating ladder), the per-family hook is gone from the tree, and no test was deleted or weakened except those whose subject is retired.
...
The retirement this plan specified (R7, R8) rests on one assumption: that a resolution attempted in the same pass as a commit can tolerate the projection's delivery latency, because the refresh has become asynchronous. **The host lane showed what that costs, and the answer is not to restore the refresh** - it is to make the failure a retry the runtime owns.

- **Evidence.** In `runtime-cloud-hypervisor-guest-preflight`, the volume root resolver missed an anchor **3.2 ms before** the projection registered it (`21:24:46.077732Z` miss, `21:24:46.080961Z` registration, uid `9fe7ff2b-…`), and a second volume repeated the pattern thirty seconds later. Downstream of that miss the check lost its waits: one head failed at `binding-worker-ready` (t=168.4 s), another at `volume-controller-processes` (t=203.1 s), with the broker's own spawn invocation timing out behind it (`kernel transport: reply timeout`, 7 occurrences against 0 in a passing run) and 27 launch failures against the passing run's 11. A passing log of the same check carries the same three anchor misses and recovers - the difference between the two is retry behaviour, not anchor visibility.
- **A driver failure must be a failure the runtime can retry.** Two laws now hold, and they are separate on purpose:
  - **A `NotYet` verdict is a deferral, not a failure.** The driver may name how long to wait (`DriverFailure::with_retry_after`, surfaced as `DriverVerdict::NotYet { retry_after }`), and the actor schedules at that delay; a deferral that names none keeps the configured backoff unchanged. A deferral never escalates.
  - **A retryable operational *error* escalates.** The actor's ladder doubles the wait per consecutive error, from the configured backoff to `DEFAULT_REQUEUE_BACKOFF_MAX` (30 s), and a pass that converges resets it. A transient error is therefore retried within milliseconds while a persistent one cannot spin.
- **R7 and R8 stand.** No commit path refreshes the projection; the subscription remains its only law, and the recovery reload is now zone-open so its coverage matches the durable load it stands in for. What the retirement needs is not a writer-side obligation but a driver contract that reports failures the runtime can act on - which is what the two laws above are.
- **Consequence for the tests.** `an_api_applied_volume_resolves_through_the_projection` and `a_bridge_committed_volume_resolves_without_the_bridge_refresh` both state the projection's contract (eventual, notice-driven) and keep their waits; the retry laws are pinned by `the_retry_ladder_doubles_and_stops_at_its_ceiling`, `a_deferral_keeps_its_cadence_while_an_error_escalates`, and `a_driver_named_retry_delay_paces_the_deferral` in `d2b-resource-runtime`.
...
- R6. A Volume or VolumeBinding row becomes resolvable whichever writer committed it, with no writer-specific refresh call: a driver effect through the child surface, a driver's declared-children pass, the controller child-mutation bridge, a plain API apply, or a bundle ingest. Resolution may lag the commit by one drain, and a resolution that misses for that reason is a *retryable* failure: the row's actor requeues it on the delay the driver named or on its own error ladder, so the miss costs a retry rather than the row.

**The retirement**

> R7 and R8 stand as written; what R8's first consequence needs is the retry contract on the other side of it, not a restored refresh. See **Implementation Correction (2026-09-19)** for the two laws that make a deferred resolution a retry the runtime owns.
