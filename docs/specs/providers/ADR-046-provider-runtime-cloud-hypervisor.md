...
| Integration | Zone ResourceClient + system-minijail Process Provider + fake MinijailProcessEffectPort; `runtime-cloud-hypervisor-guest-preflight` is discovered through the public heavy-gated target and exercises real-KVM Guest adoption only when nested TAP/cgroup prerequisites are available |
