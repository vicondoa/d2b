...
| Integration | Zone ResourceClient + system-minijail Process Provider + fake MinijailProcessEffectPort; `runtime-cloud-hypervisor-guest-preflight` is discovered through the public `make test-host-integration` target and exercises real-KVM Guest adoption only when nested TAP/cgroup prerequisites are available |
| Data migration | None (spike) |
| Validation | Unit: reconcile state machine, fast-path latency, adoption/ambiguity, finalize ordering. Integration: exact attr `vmChecks.x86_64-linux.runtime-cloud-hypervisor-guest-preflight` is discovered and built through `make test-host-integration`; it proves real Guest adoption only on a builder that grants nested TAP/cgroup posture and otherwise records an explicit external block without claiming adoption. |
