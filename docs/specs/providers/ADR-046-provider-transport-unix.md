...
Every Host unconditionally proves Linux 6.9 or newer, pidfs, accepted-socket
`SO_PEERPIDFD`, cgroup v2 delegation, and delegated-leaf `cgroup.kill` before
Host Ready, even when this Provider is absent. Before Provider/transport-unix
is Ready, its core/broker path additionally reruns the pidfs self-pidfd and
`SO_PEERPIDFD` socketpair probes, including pidfs object identity, liveness,
close-on-exec, and exact descriptor ownership. A kernel version string is not
proof. Missing or inconclusive support returns an actionable
`pidfs-unavailable` or `peer-pidfd-unavailable` refusal, keeps both Host or
Provider placement not Ready as applicable, and launches no production Unix
admission path. There is no production soft-fail, numeric-PID fallback,
capability-claim opt-out, or reduced-identity mode.
...
| Dependency/owner | `ADR046-transport-unix-002`, `ADR046-session-001`, `ADR046-bus-001`, `ADR046-exec-003`, and `ADR046-activation-001`. ADR046-exec-003 owns unconditional Host readiness; the activation item is the first writer of the shared broker/contract catalogue and dispatch files; this item is their dependency-ordered second writer and exclusively owns the accepted-socket peer-pidfd operation, session evidence adapter, and registrar issuance correction. |
