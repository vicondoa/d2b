...
Version 2 prospectively freezes the serialized handler contract. In the
indexed source, `packages/d2b-contracts/src/v3/zone.rs` still has the earlier
closed `ZoneHandlerName` set and generic handler-list validation: it does not
yet define `SystemCoreHost` or `SystemCoreUser`, and it does not yet require
their pair. The underscore spellings already present in metrics policy remain
current telemetry labels, not serialized Zone handler values.
...
| Dependency/owner | `ADR046-provider-001`, `ADR046-core-001`, `ADR046-zone-control-001`, `ADR046-cli-009`, `ADR046-exec-003`, `ADR046-exec-004`, `ADR046-exec-005`, `ADR046-pstate-012`, `ADR046-telem-001`, and `ADR046-audit-001`; `ADR046-system-core-001` is the exclusive W6 owner of both normative spec revisions, the listed implementation and focused tests, and the compiler-derived API snapshots; it is the serialized later writer of `packages/d2b-contracts/src/v3/zone.rs` after the generic Zone contract owner and of `packages/d2b/src/zone.rs` after the Zone CLI owner |
