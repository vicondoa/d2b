...
| `workload.rs` - `WorkloadSelector` | deleted | Zero production callers; deleted with the ponytail U3/U97 cleanup | DELETED | `ResourceListFilter` in resource-API; no current production caller | `ADR046-api-001` |
...
| `d2b-guest-shell-runner` | removed | Dormant static guest helper for persistent shell feasibility; its systemd unit had no in-tree socket client and the v3 shell family serves persistent sessions | DELETE | Removed with its crate, flake output, and guest-image wiring; persistent shell is a `ShellPool`/`ShellSession` of `Provider/shell-terminal` | `ADR046-primitives-003` |
...
| `d2b-session-unix/src/vsock.rs` framing header | 4-byte big-endian u32 length-prefix encode/decode | Bounded length-prefix framing; wire-shape baseline for the Provider transport | `packages/d2b-provider-transport-vsock/src/framing.rs` | **DONE (2026-09-24, U5/KTD2)**: Provider framing aligned to this 4-byte u32 header; round-trip tests green; session-unix baseline untouched |
