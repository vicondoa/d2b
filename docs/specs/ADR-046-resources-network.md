...
projection erase another's, and erase device-usbip rules installed by
`UsbipBindFirewallRule` - directly violating the ownership-marker preservation
...
`ADR-046-provider-device-usbip.md`) requires the same projection-scoped
semantics for its per-Network/per-busid marker; the shipped whole-table
`UsbipBindFirewallRule` op does not provide them, so that path must be reconciled
onto the same projection-scoped contract (integrator cross-slice item; see the
finding report).
...
**Proposed decision-register row** (integrator lands in
`ADR-046-decision-register.md`; this slice does not edit the register):
`D119 | Per-ownership-projection host firewall mutation | New closed broker op ApplyNftablesProjection replaces the whole-table ApplyNftables mapping for per-Network firewall; mutates exactly one generation-fenced ownership projection and preserves all other Network and device-usbip markers | RESOLVED | ADR-046-resources-network.md D-NETWORK-004`
