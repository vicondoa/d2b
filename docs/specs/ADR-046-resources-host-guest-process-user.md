...
| Reuse action | adapt |
| Destination | `packages/d2b-contracts/`, `packages/d2b-contracts/`, `packages/d2b-contracts/`, `packages/d2b-contracts/`, `packages/d2b-contracts/`, `packages/d2b-contracts/`, `packages/d2b-contracts/` |
...
| Current source | `packages/d2b-core/src/processes.rs`: `ReadinessPredicate`, `VmProcessInvariants`; `packages/d2b-core/src/processes.rs`: `SpawnRunnerPlanOp`; `packages/d2b-broker/src/ops/` SpawnRunner |
| Reuse action | adapt |
| Destination | `packages/d2b-contracts/`: LaunchTicket, ProcessIdentityDigest, AdoptionCandidate, PidfdEvidence, WaitReapOwner, BrokerTerminalResult, ProcessOutcome, ExitClass |
...
| Current source | `packages/d2b-unsafe-local-helper/src/systemd.rs`; `packages/d2b-guestd/src/exec.rs`: `SystemdRunUnitManager`, `ManagedUnit`, `ExecPolicy`; `packages/d2b-guestd/src/exec_linux.rs`; `packages/d2bd/src/` (transient unit management) |
| Reuse action | adapt |
| Destination | `packages/d2b-provider-process-systemd/src/`: launch.rs (opaque EffectPort requests), adoption.rs (typed outcomes], sandbox.rs (semantic validation]; core/ProviderSupervisor owns DBus, unit-name, systemd-property,, pidfd, wait,,and user-supervisor adapters; `packages/d2b-process-conformance/src/port.rs` names the live `ProcessLaunchEffectPort` boundary; `packages/d2b-provider-process-systemd/tests/`: hermetic lifecycle/conformance/fault tests with fake EffectPorts; framework integration covers transient-unit-and user-supervisor scenarios through the core adapter; `packages/d2b-provider-process-systemd/README.md`: Provider identity,, `spec.provider.settings` schema,, ResourceTypes,, placement,, RBAC,, security posture,, telemetry labels,, build/test commands (provider crate standard layout - see §Provider crate standard layout) |
| Detailed design | system-systemd Process/EphemeralProcess provider conformance per this spec's system-systemd conformance section; Provider validates semantic SandboxSpec,and requests start/adopt/stop through ProcessLaunchEffectPort using opaque IDs and handles. Core maps signed policy to a transient `Type=exec` unit, binds InvocationID+cgroup+MainPID+start-time before pidfd_open, owns systemd wait/reap observation and user-manager access, re-verifies adoption, and enforces the runtimeDeadline/drainTimeout stop sequence. No DBus connection, systemd property fragment, PID, pidfd, unit name, cgroup path, or user-manager handle enters the Provider. Primary reuse disposition: `adapt`. Preserved source-plan detail: extract and adapt. |
| Integration | Zone-installed Provider/system-systemd; ProviderSupervisor owns LaunchTicket and production ProcessLaunchEffectPort adapter; Provider uses ResourceClient UpdateStatus |
...
| Current source | `packages/d2b-core/src/processes.rs`: `ProcessNode`, `RoleProfile`, `NamespaceSet`, `MountPolicy`, `CgroupPlacement`; `packages/d2b-core/src/minijail_profile.rs`: full; `packages/d2b-broker/src/ops/spawn_runner.rs` (if present at baseline); `packages/d2bd/src/` pidfd/wait; `packages/d2b-core/src/process_builder.rs` |
| Reuse action | adapt |
| Destination | `packages/d2b-provider-process-minijail/src/`: sandbox_compiler.rs, launch.rs, adoption.rs, effect_result.rs, user_ns.rs (semantic validation only); core/ProviderSupervisor owns LaunchTicket, pidfd, wait, `/proc`, cgroup, user-namespace, and broker adapters; `packages/d2b-provider-process-minijail/tests/`: hermetic sandbox-compilation/lifecycle/conformance/fault tests with fake EffectPorts; framework integration scenarios cover clone3/user-namespace and broker spawn through the core adapter; `packages/d2b-provider-process-minijail/README.md`: Provider identity, `spec.provider.settings` schema, ResourceTypes, placement, RBAC, security posture (capabilities, namespaces, seccomp), telemetry labels, build/test commands (provider crate standard layout - see §Provider crate standard layout) |
...
| Current source | `packages/d2b-core/src/processes.rs` test coverage; `packages/d2bd/src/` tests; minijail/seccomp test vectors |
| Reuse action | adapt |
...
| Reuse action | adapt |
| Destination | `packages/d2b-provider-process-systemd/` (guest-domain EphemeralProcess launch via systemd-run inside guest); `packages/d2b-session/` (ComponentSession replacing ad-hoc guest ttrpc); runtime Provider guest bootstrap Process |
...
| Reuse action | create |
| Destination | `packages/d2b-core-controller/`: EphemeralProcess TTL cleanup controller handler |
...
| Reuse action | adapt |
| Destination | `packages/d2b-core-controller/` (or a core/user-agent per-session agent Process under `Provider/system-systemd`); `AuthorityDescriptor` on the session authority |
...
| Reuse action | adapt |
| Destination | `packages/d2b-bus/src/`: all above modules verbatim; `packages/d2b-bus/tests/`: all above tests verbatim |
...
| Reuse action | adapt |
| Destination | `packages/d2b-bus-contracts/src/generated_v3_services/`: v3 generated ttrpc stubs for Zone service methods (Resource CRUD, Watch, ComponentSession service verbs); `packages/d2b-zone-routing/src/`: Zone runtime service handler adapted from `DaemonServiceV2<H>` pattern; `packages/d2b-zone-routing/`, `handler.rs`, `routing.rs` |
...
| Main reuse source (daemon-access) | main `a1cc0b2d` `packages/d2b-unsafe-local-helper/src/component_session.rs`: `LocalDaemonSession` (one authenticated local daemon session wrapping `DaemonClient`; connection via `connect_component_session` using `HostSocketConnector` + `local_daemon_endpoint_identity` + peer uid verification against `d2bd` uid); `connect_seqpacket` (blocking connect helper) |
| Reuse action | adapt |
| Destination | `packages/d2b-client/`: all above modules; `DaemonClient` → `ZoneClient` (v3 Resource CRUD/Watch verbs); `GuestClient` → `ProcessAttachClient`; `HostSocketConnector` → `ZoneSocketConnector`; `LocalDaemonSession` → `LocalZoneSession` |
...
| Removal proof | Supersedes the baseline `packages/d2b-realm-router/src/` routing owner after `packages/d2b-zone-routing/` passes idempotency, dispatch-limit, and principal-binding tests; ADR45 realm/workload route tables are not imported into v3. |
