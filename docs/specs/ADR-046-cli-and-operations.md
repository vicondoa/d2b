...
| `WorkloadId`, `WorkloadTarget` (`= RealmTarget`) | `d2b-realm-core/src/ids.rs`, `d2b-contracts/src/workload_identity.rs` | `Guest/<name>` ResourceRef for VM/sandbox/cloud/remote; `Host/<name>` for unsafe-local (NEVER `Guest`) |
...
| `debug-read-refused` | The debug report could not read the named row's resource type |
