...
- `RealmControllersJson` / `realm-controllers.json` → runtime-created Zone
  self-resource, compiler-only `parentZone` topology sealed into allocator
  bootstrap state, and child-local ZoneLink transport/route state.
- `RealmWorkloadsLauncherV2Json` / `realm-workloads-launcher-v2.json` →
  target Process resource annotations in the Zone `resource-bundle.json`.
...
2. `resource_name` matches `^[a-z][a-z0-9-]{0,62}$` - the `RealmId`/`WorkloadId`
   LABEL_PATTERN grammar from `d2b-realm-core/src/ids.rs`, bounded to 1..63
   bytes per D113 (deliberately tighter than the v2 128-byte identifier
   ceiling, which is a different contract). Empty names and names of 64 bytes
   or more reject at eval, so a name Nix accepts is always admissible to Rust.
...
`<name>` matches `^[a-z][a-z0-9-]{0,62}$` - the `RealmId` LABEL_PATTERN grammar
bounded to 1..63 bytes per D113 (the `ZoneId` bound). Empty names and names of
64 bytes or more reject at eval.

```nix
d2b.zones.local-root = {
  label = "Local root";    # parentZone is forbidden on this Zone
};

d2b.zones.dev = {
  label      = "Development";   # optional; defaults to attribute name
  parentZone = "local-root";    # required for every non-root Zone
};
```

Zone-level Nix options (`label`, `parentZone`, `retainedGenerations`,
`trustedPublishers`) are compiler/configuration-service settings. They control
build behavior and are not emitted into the Zone ResourceSpec. `parentZone` is
a plain Zone name, not a `ResourceRef`. It is required for every non-root Zone,
must resolve to another declared `d2b.zones.<name>`, and is forbidden on the
distinguished local-root Zone. The compiler turns the resulting child→parent
map into the sealed private allocator-bootstrap topology; it never emits
`parentZone` into a Zone resource bundle or `Zone.spec`, which remains exactly
`{}`.

The scalar `parentZone` setting selects exactly one allocator owner for the
child. One optional child-local uplink `ZoneLink` supplies that edge's transport
binding and local route/session state. A non-root Zone with no enabled ZoneLink
still has an authored parent but remains disconnected. There is no `parentRef`
in the Zone spec and no reciprocal parent-store ZoneLink resource.
...
store. The compiler resolves the enclosing Zone's `parentZone`, and the sealed
bootstrap allocation binds this local ZoneLink UID and child identity to that
one selected parent edge. There is no `parentRef` in either Zone's spec and no
cross-Zone ResourceRef. Current source for zone hierarchy:
...
The compiler validates the `parentZone` map before publication: `local-root`
must omit the setting, every other Zone must have one resolved scalar parent,
no Zone may name itself, the graph must be acyclic, and every ancestry path
may contain at most 16 Zone names (the retained `MAX_REALM_LABELS` bound).
Conflicting scalar definitions fail through normal Nix module merging, so one
child cannot acquire two parents. There is no bidirectional `parentRef` or
undefined reciprocal link; `parentZone` is the topology input and the
child-local ZoneLink is the sole resource declaration for its transport and
local route state.
...
# Export only the owner Service, never its Device/Endpoint backing.
...
# Compiler-only topology selects the allocator owner; this is not emitted into
# Zone.spec or any ResourceSpec.
d2b.zones.work.parentZone = "local-root";

# The consumer's uplink is a local resource. It supplies transport/local route
# state while the selected parent allocator owns all privileged effects.
...
`purpose` is exactly `credential-entra-login`, its schema
...
d2b.zones.dev.resources.wayland-proxy = {
  ...
  spec = {
    ...
    restartPolicy = {
      ...
      backoffBase = "1s";
      backoffMax = "60s";
      backoffMultiplierMilli = 2000;   # integer fixed-point: multiplier x 1000 (2.0). Floats are rejected at eval (D101/D121).
      maxRestarts = 5;
      resetAfter = "300s";
    };
    readiness = {
...
| Rule | Assertion |
| --- | --- |
| ResourceRef format | `<Type>/<name>` where name matches `^[a-z][a-z0-9-]{0,62}$` (1..63 bytes, D113); the whole canonical ref string is at most 201 bytes (137-byte qualified type, separator, 63-byte name) |
...
| `parentZone` topology | Compiler-only plain Zone name, not a ResourceRef; omitted on `local-root`, required everywhere else, resolves to one declared Zone, is not self, forms no cycle, and keeps every ancestry path within 16 Zone names |
...
| `spec` must not contain any floating-point number; every numeric field is a JSON integer (D101/D121). Fixed-point fields such as `backoffMultiplierMilli` carry the scaled integer | Eval |
| `resource_name` matches `^[a-z][a-z0-9-]{0,62}$` (1..63 bytes, D113; empty and 64-byte-or-longer names rejected) | Eval |
...
This section is the single source of truth (D119) for the per-Zone generation
layout, its installation paths, and its digest chain. Every other ADR 0046
spec that references a Zone bundle, generation, retention setting, index, or
artifact catalog defers to this section; where an older description elsewhere
disagrees, this section governs.

#### One monolithic resource bundle per Zone

Each Nix evaluation produces one immutable configuration generation. For each
declared Zone the build derivation emits exactly one monolithic bundle
document; there is no per-resource-type split (no `zone.json`,
`providers.json`, `hosts.json`, `guests.json`, `processes.json`,
`volumes.json`, `networks.json`, `devices.json`, or `roles.json` bundle
files). Every authored resource for the Zone is one element of the bundle's
single canonical `resources` array. This matches the current v2
`nixos-modules/bundle.nix` monolithic bundle and the generic bundle contract
in the resource-network spec.

Where any work item, disposition table, or prose elsewhere in this spec refers
to a per-resource-type file (`zone.json`, `providers.json`, `hosts.json`,
`guests.json`, `processes.json`, `volumes.json`, `networks.json`,
`devices.json`, `roles.json`) or to a per-Zone `bundle.json`, that reference
denotes the corresponding resources inside this single `resource-bundle.json`;
no per-type bundle file and no per-Zone `bundle.json` install file is emitted.
The derived low-level views that remain separate files
(`closures/<guest>.json`, `minijail-profiles/<id>.json`, `privileges.json`) are
broker and sandbox inputs, not resource declarations, and are not part of the
resource bundle.

The store-derivation filename is `bundle.json`; the installed and
runtime-read name is `resource-bundle.json`:

```
/nix/store/<hash>-d2b-zone-<zone>-bundle/bundle.json          (fixed-output; installed as resource-bundle.json)
/nix/store/<hash>-d2b-artifact-catalog/artifact-catalog.json  (site-wide private catalog; storePath per entry for staging)
```

The bundle is a fixed-output derivation: identical Nix inputs produce an
identical store path and an identical `contentHash`; any resource-spec change
produces a different store path and `contentHash`.
...
```json
{
  ...
  "resources": [
    { "apiVersion": "resources.d2bus.org/v3", "type": "<Type>",
      "metadata": { "name": "<name>", "zone": "dev" }, "spec": {} }
  ],
...
- `resources` is sorted canonically by `(type, name)`.
- `managedBy`, `configurationGeneration`, `uid`, `generation`, `revision`,
  timestamps, and `status` are absent from the Nix-emitted bundle; core
  assigns the runtime-owned fields when it applies each resource.
...
| Artifact | Installed path | Owner/mode | Origin |
| --- | --- | --- | --- |
| Zone resource bundle | `/etc/d2b/zones/<zone>/resource-bundle.json` | `root:d2bd 0640` | Nix build (one per declared Zone) |
| Site provider catalog | `/etc/d2b/provider-catalog.json` | `root:d2bd 0640` | Nix build (one, site-wide) |
| Site artifact catalog | `/etc/d2b/artifact-catalog.json` | `root:d2bd 0640` | Nix build (one, site-wide, private) |
| Cross-Zone index | `/etc/d2b/index.json` | `root:d2bd 0640` | Nix build (one, site-wide, derived) |
| Per-Guest closure map | `/etc/d2b/closures/<guest>.json` | `root:d2bd 0640` | Nix build (derived low-level view) |
| Minijail profiles | `/etc/d2b/minijail-profiles/<id>.json` | `root:d2b-priv-broker 0640` | Nix build (derived low-level view) |
| Broker op catalog | `/etc/d2b/privileges.json` | `root:d2bd 0640` | Nix build (derived low-level view) |
| `Zone/<name>` self-resource | Zone runtime store | n/a | Controller-created; never emitted in any bundle |
| Prior generation bundles | `/var/lib/d2b/zones/<zone>/configuration/prior/<contentHash>.json` | `root:d2bd 0640` | Runtime (retention ring) |
| Generation state | `/var/lib/d2b/zones/<zone>/configuration/generation.json` | `root:d2bd 0640` | Runtime (active/prior pointers + retention metadata) |

The `d2b.artifacts.<id>` option and both catalogs are site-wide, not per-Zone;
`d2b:v3:artifact-catalog` (D101) is a single document digest pinned once for
the whole program. The cross-Zone `index.json` is derived, drift-gated
(`xtask gen-index` + `git diff --exit-code`), and cross-checked at activation;
...
The cross-Zone `index.json` is not a digest-chain member; its correctness is
enforced by the drift gate and the activation-time cross-check against the
bundle. The bundle and catalog derivations are hermetic (same Nix inputs
produce byte-identical output; no timestamps, randomness, or network access)
and offline (all required Provider JSON Schemas are already in Provider package
closures in the Nix store at build time). Store paths are excluded from all
...
Activation is fail-closed and crash-safe. The active pointer, the prior
pointer, and the retention metadata are persisted together, atomically, before
any reconciliation is notified, so a crash can never leave a new generation
active with no durably recorded rollback target.

```
Nix eval
  -> derivation build (hermetic, offline)
  -> /nix/store/<hash>-d2b-zone-<zone>-bundle/  (immutable)
  -> system activation (d2b-activation-helper)
      1. verify the digest chain (contentHash, providerSchemaDigests,
...
A normal daemon restart is a continuation event, not a teardown (ADR 0034). On
startup the Zone runtime reads `generation.json`, adopts the recorded
`activeContentHash` when its staged bundle is present and integrity-verifies,
and otherwise quarantines the ambiguous generation and continues serving from
the recorded `priorContentHash`. Recovery adopts or quarantines before any
cleanup: no prior-generation bundle is pruned while a rollback target or an
in-flight Delete from its generation could still be needed. Because step 4 is
atomic, a crash before it leaves the previous generation active with its prior
pointer intact, and a crash after it leaves the new generation active with the
new prior pointer intact.

### Retention

Generation retention is a per-Zone Nix compiler setting, `retainedGenerations`
(default 3, range 1..16), declared at the Zone option level outside the Zone
ResourceSpec:
...
An eval assertion enforces `1 <= retainedGenerations <= 16`. The minimum of 1
guarantees at least one prior generation is available for rollback; values
above 16 reject with a structured error. `retainedGenerations` is consumed
only during build and runtime generation bookkeeping and is never emitted into
any Zone ResourceSpec or bundle JSON. There is exactly one name for this
setting; `retainedConfigurationMax` is not used.
...
Rollback re-stages a retained prior bundle and reapplies it as a new, higher
generation; the atomic step-4 commit and the restart-recovery rules above
apply unchanged. The configuration-publication controller reconciles affected
resources.

### Cross-Zone generation ordering

When child Zone A selects parent Zone B with `A.parentZone` and has a local
uplink `ZoneLink`, neither bundle carries the other Zone's generation or a
cross-Zone `cursorRef`.
...
1. The configuration-publication controller reads the new generation's
   `resource-bundle.json` `resources` array to form the new configuration-owned
   name set.
...
3. **Activation does not block on cleanup.** Step 4 of the activation path
   (atomic generation-pointer commit) completes before cleanup begins.
...
| Field | Values |
| --- | --- |
| `Zone.status.phase` | `Ready` - all configuration-owned resources reconciled; `Degraded` - deletion pending or ZoneLink lagging; `Pending` - atomic generation commit complete, reconciliation in progress |
...
| --- | --- | --- |
| Two-generation bundle diff | nix-unit | Generation 1 declares resource R; generation 2 omits R. Verify R absent from generation 2 `resource-bundle.json` `resources` array; generation 1 bundle retains R. |
...
Zone bundles the activation tool rejects the generation. It is not a
digest-chain member (see Bundle contract (canonical)); its correctness is
enforced by the drift gate (`xtask gen-index` + `git diff --exit-code`) and the
activation-time cross-check.
...
| --- | --- | --- |
| `provider-catalog.json` | `root:d2bd 0640` | Offline Provider catalog (site-wide) |
| `artifact-catalog.json` | `root:d2bd 0640` | Artifact catalog (site-wide, private) |
| `index.json` | `root:d2bd 0640` | Cross-Zone normalized index |
| `zones/<name>/resource-bundle.json` | `root:d2bd 0640` | Monolithic Zone bundle (canonical `resources` array + digest chain); see Bundle contract (canonical) |
...
There is no per-resource-type bundle file and no separate `zone.json`; the
`Zone/<name>` self-resource is controller-created and never emitted (see Bundle
contract (canonical)).
...
| Missing/extra Zone parent | Any non-root Zone omits `parentZone`, or `local-root` defines it |
| Conflicting Zone parent | Separate module definitions resolve one Zone's scalar `parentZone` to different values; normal Nix option merging rejects the conflict |
| Unknown/self Zone parent | `parentZone` does not resolve to a declared Zone or equals the child Zone |
| Zone topology cycle/depth | The compiler-only `parentZone` graph cycles or an ancestry path exceeds 16 Zone names |
...
| `nixos-modules/processes-json.nix` | `VmProcessDag`/`ProcessRole`/`binaryPath`/argv | Yes | `resources-zones-processes.nix`; Process/EphemeralProcess resources in `zones/<z>/resource-bundle.json` | ADR046-nix-006 |
...
| `nixos-modules/allocator-json.nix` | `AllocatorJson` - realm/env bridge assignments, socket paths | Yes | Privileged effects remain allocator-owned; parent edges come from compiler-only `parentZone`; Network/Host resources carry declarative intent | ADR046-nix-008 |
| `nixos-modules/realm-controller-config-json.nix` | `RealmControllersJson`/`realm-controllers.json` | Yes (read by live d2bd `realm_access_resolver`) | Runtime-created Zone self-resource + sealed `parentZone` allocator topology + child-local ZoneLink | ADR046-nix-008 |
| `nixos-modules/realm-workloads-launcher-v2-json.nix` | `RealmWorkloadsLauncherV2Json`/`realm-workloads-launcher-v2.json` | Yes | Provider/display-wayland + Provider/shell-terminal Process configs in `zones/<z>/resource-bundle.json` | ADR046-nix-009 |
...
| `nixos-modules/manifest.nix` | `manifest.json` `manifestVersion` contract | Yes | `zones/<z>/resource-bundle.json` `schemaVersion`; `manifestVersion` -> `schemaVersion` | ADR046-nix-013 |
| `nixos-modules/host-json.nix` | `host.json` host-side config | Yes | Folded into Host resource in `zones/<z>/resource-bundle.json` | ADR046-nix-013 |
...
| --- | --- | --- | --- | --- |
| `RealmId`, `RealmPath` | `d2b-realm-core/src/ids.rs`, `realm.rs` | Live in d2bd | `ZoneId` (`^[a-z][a-z0-9-]{0,62}$`, 1-63 bytes per D113), `ZonePath` in `d2b-contracts/src/v3/identity.rs` | ADR046-identities-001 |
...
| `RealmControllersJson`, `RealmControllerMetadataSummary` | `d2b-core/src/realm_controller_config.rs`; read live by d2bd `realm_access_resolver` | Live | Runtime Zone self-resource + compiler-only `parentZone` allocator topology + child-local ZoneLink transport/route state; `realm-controllers.json` retained during migration | ADR046-nix-008 |
| `RealmWorkloadsLauncherV2Json`, `LauncherWorkloadSummary` | `d2b-core/src/realm_workloads_launcher.rs` | Live | Process resource annotations in `zones/<z>/resource-bundle.json` | ADR046-nix-009 |
...
| `OperationRouter`, `DurableExecTable`, `TargetResolver` | `d2b-realm-router/src/`; `d2bd/src/realm_stubs.rs` | COMPILE-ONLY at baseline (`dead_code`-allowed, not called) | `d2b-bus` routing; adapt `RealmSessionAuthority`/`CredentialCustody`/`RealmServiceLimits` from `main:packages/d2b-realm-router/src/service_v2.rs` | ADR046-nix-033 |
...
| --- | --- | --- |
| `d2bd.service` | PID1 local-root controller | Retained; becomes fixed Zone core controller launcher. `Type=notify`; `d2bd` binds, listens on, and owns `public.sock` itself and is not reported ready until that socket is bound. No separate framework socket unit is declared for it (AGENTS.md permits exactly three root-visible units: `d2bd.service`, `d2b-priv-broker.socket`, `d2b-priv-broker.service`). |
...
| Proof | Description |
| --- | --- |
| Zone self-resource round-trip | Nix Zone declaration → runtime-created Zone self-resource → resource API GET returns `spec = {}` with no compiler-only settings |
...
| Cross-Zone ZoneLink | Child Zone's `parentZone` selects one allocator owner; its local uplink has self-matching `childZoneName` and local `transportProviderRef`; the selected allocator binds the private edge and owns privileged route creation; no reciprocal resource, `parentRef`, or emitted `parentZone` |
...
| Conflict detection | `parentZone` missing/unknown/self/cycle/depth, CIDR overlap, owner cycle, and duplicate type/name rejection at eval time |
...
| Bundle integrity | Byte-identical rebuild from identical inputs; bundle `contentHash` matches the digest recomputed over the canonical `resources` array; any digest-chain member mismatch fails activation |
...
| `d2b-client` | `client.rs`, `session.rs`, `target.rs`, `service.rs`, `daemon_service.rs`, `guest_service.rs`, `host_socket.rs` | `packages/d2b-client/src/` (adapt in place) | ADR046-nix-032 |
| `d2b-realm-router` | `service_v2.rs` | `packages/d2b-bus/src/routing/zone_service.rs` | ADR046-nix-033 |
| `d2bd` | `provider_registry.rs` | `packages/d2bd/src/provider_registry.rs` (adapt in place) | ADR046-nix-034 |
| `d2bd` | `provider_effects.rs` | `packages/d2bd/src/provider_effects.rs` (adapt in place) | ADR046-nix-035 |
...
contract work item (ADR046-nix-034/ADR046-nix-035). Cross-reference:
...
| Reuse action | adapt |
| Destination | `nixos-modules/options-zones.nix` (Zone-level options: `label`, `parentZone`, `retainedGenerations`, `trustedPublishers` - compiler settings, not Zone spec fields); `nixos-modules/options-zones-resources.nix` (unified `resources` attrset) |
...
| Detailed design | `d2b.zones.<z>.retainedGenerations` (default 3, range 1..16, compiler setting - not emitted in Zone spec); `d2b.site.stateDir` maps to Zone storage roots; `d2b.site.usePrebuiltHostTools` retained Primary reuse disposition: `adapt`. Preserved source-plan detail: retain and extend. |
...
| Detailed design | Cross-Zone normalized index: zone/host/guest/network/closure entries; executionIndex; networkIndex; closureIndex; sorted output; `cfg._index` attribute tree retained as internal helper during migration Primary reuse disposition: `adapt`. Preserved source-plan detail: rewrite. |
...
| Reuse action | adapt |
| Destination | `nixos-modules/resources-zones-processes.nix`; serializes Process/EphemeralProcess resources into the monolithic `zones/<z>/resource-bundle.json` |
| Detailed design | Process/EphemeralProcess resource serialization per disposition table and the frozen `ExecutionSpec`: exact `providerRef`/`executionRef`/domain/user/processClass/template/config/credential/mount/sandbox/budget/`networkUsage`/`deviceUsage`/telemetry names; semantic owner from `metadata.ownerRef` with template fallback through the Process Provider; no `packageRef`, free-form `binaryPath`, `argv`, renamed network/device fields, or inline endpoints; stable produced endpoints are separate owned `Endpoint` resources with `producerRef`; Process uses canonical `restartPolicy` fields; EphemeralProcess uses `runtimeDeadline` and has no restart policy; VsockRelay → `Process` under `Provider/transport-vsock`; GuestSshReadiness retired at v3 cutover; Usbip long-lived backend/proxy → `Process`, Usbip per-busid attach/detach → `EphemeralProcess`, all owned by `Provider/device-usbip` Primary reuse disposition: `adapt`. Preserved source-plan detail: extract and adapt. |
| Integration | Process/EphemeralProcess resources in `resource-bundle.json` replace `cfg._bundle.processesJson`; Process Providers read the new format |
...
| Reuse action | adapt |
| Destination | `nixos-modules/resources-zones-volumes.nix`; serializes Volume resources into the monolithic `zones/<z>/resource-bundle.json`; OFD lock rows move to `d2b-contracts` internals |
| Detailed design | Volume layout/views/ACL/no-follow/repair preserving current policy; `PrincipalRef { kind: "uid" }` → `User/<name>` typed ref only; OFD rows removed from Nix artifacts Primary reuse disposition: `adapt`. Preserved source-plan detail: extract storage policy → adapt; retire sync rows. |
| Integration | Volume resources feed the monolithic `zones/<z>/resource-bundle.json`; the Volume Provider consumes them for path/view/ACL reconciliation; `d2b-contracts` internals own OFD lock rows. |
...
| Reuse action | adapt |
| Destination | Compiler-only `parentZone` map in `nixos-modules/options-zones.nix`; local-root allocator bootstrap compiler/sealer; runtime-created Zone self-resource with `spec = {}`; child-local ZoneLink resource; `realm-controllers.json` RETAINED during migration (live d2bd reads it) |
| Detailed design | Resolve each non-root `parentZone` to one declared Zone; reject a root value, missing/unknown/self/conflicting/cyclic parent, and ancestry deeper than 16 names. Canonicalize sorted child→parent rows and feed them to the local-root allocator's private bootstrap sealer. Parent allocators derive/pre-bind privileged socket/listener state from that topology; no socket path or `parentZone` enters `Zone.spec` or a ResourceRef. Child-local ZoneLinks provide transport/local route state. `realm-controllers.json` remains published until `realm_access_resolver` is replaced. Primary reuse disposition: `adapt`. Preserved source-plan detail: adapt and retire. |
| Integration | Nix Zone options → validated topology compiler → local-root allocator bootstrap sealer; ZoneLink controller consumes only its sealed selected-parent binding. During migration, `realm-controllers.json` remains available to the legacy resolver until the Zone topology/ZoneLink path takes over. |
| Data migration | Topology is regenerated from `d2b.zones.<zone>.parentZone`; full d2b 3.0 reset; no allocator/realm-controller state import. |
| Validation | Zone self-resource round-trip with exact empty spec; `parentZone` required/root-forbidden/existence/one-parent/self/cycle/16-name-depth vectors; topology seal excludes `parentZone` from Zone/resource bundles; bootstrap socket/listener regression tests |
...
| Reuse action | adapt |
| Destination | Provider/display-wayland and Provider/shell-terminal Process configs in `zones/<z>/resource-bundle.json`; `Provider/credential-entra` Credential resource; `realm-identity.json` RETAINED during migration |
...
| Validation | Launcher metadata shape regression; Entra identity-Guest fixture requires login Endpoint purpose `credential-entra-login`, `visibility = "provider"` (and rejects `"zone"`), exact `consumerPolicy.allowedSubjects` containing both `Provider/credential-entra` and the Credential's configured `consumerRef`, and exact canonical `resolve` operation; no-secret assertion vectors |
...
| Detailed design | Broker op catalog is not Zone-scoped; no structural change required Primary reuse disposition: `copy-unchanged`. Preserved source-plan detail: retain. |
...
| Destination | Per-Zone `zones/<z>/resource-bundle.json` (`schemaVersion`); Host resource in `zones/<z>/resource-bundle.json` |
...
| Integration | `bundle-zones.nix` writes `schemaVersion` into the per-Zone `resource-bundle.json`; activation-helper validates it and Host resource consumers replace `host.json` readers. |
...
| Detailed design | Migrate existing assertions to Zone/Resource terminology; add ref validation, owner cycles, CIDR overlap (Zones), provider resolution, RoleBinding verb set assertions Primary reuse disposition: `adapt`. Preserved source-plan detail: extend in place. |
...
| Detailed design | `d2b-activation-helper` updated to validate/stage per-Zone bundles; `d2bd.service` updated to read Zone bundle; `d2b` group retained for `SO_PEERCRED` Primary reuse disposition: `adapt`. Preserved source-plan detail: retain and adapt. |
...
| Detailed design | Current bridge/NAT/DHCP/firewall Nix units retained; `Provider/network-local` controller emits equivalent configuration from Network resources; `lib.mkForce` neutralization preserved Primary reuse disposition: `copy-unchanged`. Preserved source-plan detail: retain until Provider successor. |
...
| Detailed design | `store.nix` retained; `Provider/volume-virtiofs` controller creates equivalent EphemeralProcess; per-VM store path derived from Zone stateDir + Guest name via Provider, not raw path in spec Primary reuse disposition: `copy-unchanged`. Preserved source-plan detail: retain until Provider successor. |
...
| Detailed design | `d2b.vms.<v>.tpm.enable = true` → `d2b.zones.<z>.resources.vm-tpm = { type = "Device"; spec = { providerRef = "Provider/device-tpm"; ... }; }`; all component eval assertions migrated to `assertions.nix`; GuestSshReadiness retired at v3 cutover; Usbip long-lived backend/proxy → `Process`, per-busid attach/detach → `EphemeralProcess`, both owned by `Provider/device-usbip` Primary reuse disposition: `replace`. Preserved source-plan detail: each component becomes a Provider install resource + Device/Guest spec field. |
...
| Detailed design | `cargo xtask gen-schemas` emits one JSON Schema per ResourceType under `docs/reference/schemas/v3/`; Nix derivation reads these schemas from `pkgs.d2b-resource-schemas` and validates every emitted `spec` JSON before producing the Zone bundle; Provider-settings validation reads `settingsSchemaDigest` from `provider-catalog.json` and resolves the schema from the Provider package closure; Credential ref enforcement: eval assertion rejects any `spec` string field matching `-----BEGIN`, `eyJ`, or a hex string >= 32 bytes in a secret-typed field; `managedBy` in any input spec rejected at eval (core-set runtime field, never in Nix input); bundle integrity: `contentHash` computed over the canonical sorted `resources` array (see Bundle contract (canonical)) Primary reuse disposition: `adapt`. Preserved source-plan detail: extend xtask schema generation; new Nix eval/build validation hooks. |
...
| Reuse action | create |
| Destination | Configuration-publication controller handler in `packages/d2b-core-controller/`; `ConfigurationOwnedClassifier`; `AbsentResourceReaper`; `Zone` status conditions in `d2b-contracts/src/v3/zone_status.rs`; cleanup audit emission through the shared audit subsystem |
| Detailed design | `ConfigurationOwnedClassifier`: classify resources by core-set `managedBy` field only - `managedBy=configuration` resources are owned by config publication; `managedBy=controller` and `managedBy=api` resources are never touched. At activation, diff new-generation bundle name+type set against all resources with `managedBy=configuration` in the Zone store. ADR046-routing-013 performs the D122 atomic durable commit before the controller applies that diff; only after the commit returns are absent resources enqueued for Delete and added or changed resources persisted with core-set `managedBy=configuration` and `configurationGeneration`. Never infer ownership from `ownerRef`, labels, or absence from emitted files. `AbsentResourceReaper`: processes the Absent queue asynchronously; does not block the atomic durable activation commit; sets `status.phase=Pending` + `PendingDeletion` condition (`reason: AbsentFromConfiguration`); waits for all finalizers to clear; commits a single store transaction that appends the `Deleted` revision and removes the resource row and its indexes; subsequent `Get` returns not-found; the `ResourceDelete` audit event is appended afterward with dedup/exactly-once recovery. Zone phase: `Pending` from the durable commit until first reconcile; `Degraded` while any `managedBy=configuration` resource carries `PendingDeletion` or a ZoneLink lags; `Ready` when all reconciled. Generation pruning: prune when `generationIndex ≤ activeIndex - retainedGenerations` AND all enqueued resources from that generation have reached `Deleted`. Rollback: re-adopt `managedBy=configuration` resources in `Pending/PendingDeletion` back to the rollback target generation's owned set |
| Integration | `d2b-activation-helper` only verifies, stages, installs, and notifies; it writes no resource metadata. After ADR046-routing-013 commits `generation.json`, the configuration-publication controller sets `managedBy=configuration` and `configurationGeneration` while applying resources, then uses those core-set fields to determine the owned set - never `ownerRef` or bundle membership alone |
...
| Reuse action | create |
| Destination | `packages/d2b-contract-tests/`; workspace scan in `packages/xtask/src/main.rs` (extend `check-workspace` or add `check-provider-layout` subcommand) |
...
| Detailed design | Per-generation `RequestRegistry`, request-wide cancellation, `DeadlineBudget::admit_metadata()` as the inbound metadata gate, and single-use `BootstrapAdmission::consume()` replay protection. Primary reuse disposition: `adapt`. Preserved source-plan detail: Reuse from main commit `a1cc0b2d`; adapt `CancelRequest`/`CancelResponse` module paths if v3 services move the common proto types.. |
...
| v3 destination | `packages/d2b-bus/` - adapt as the Unix transport backend for `d2b-bus` sessions; keep the `host-socket`/`native-vsock` feature gates intact; the transport code itself has no ADR45 realm bindings |
...
| Detailed design | Canonical wire values, bounded vectors, binary size constants, closed-enum tag codecs, and fail-closed validation for the ComponentSession protocol. Primary reuse disposition: `adapt`. Preserved source-plan detail: Reuse wire constants/types from main commit `a1cc0b2d`; rename ADR45 realm-oriented variants only as directed by the bus owning spec and preserve numeric tags.. |
...
| v3 destination | `packages/d2b-contracts/`; `COMPONENT_SESSION_MAJOR` stays 2 unless the wire handshake format changes; KAT vectors in `docs/reference/component-session-v2-vectors.json` must still pass after copy |
...
| Reuse action | adapt |
| Destination | `packages/d2b-contracts/`. |
| Detailed design | Versioned ttrpc service modules preserve server dispatch tables, shared request/cancel/outcome/error types, and service fingerprint inventory. Primary reuse disposition: `adapt`. Preserved source-plan detail: Reuse generated ttrpc service tables from main commit `a1cc0b2d`; adapt ADR45 Realm/Workload fields and service names to v3 Zone/Resource naming per owning bus spec.. |
...
| v3 destination | `packages/d2b-contracts/` (versioned sub-path); service interfaces adopted as-is initially; breaking method changes require a new proto major version |
...
| Field | Value |
| --- | --- |
| Dependency/owner | ADR046-provider-004, ADR046-zone-control-019, ADR046-zone-control-020, ADR046-zone-control-024; Nix resource compiler owner |
...
| v3 destination | `packages/d2b-client/` (adapt in place); client becomes the primary CLI and controller access path for Zone-local and cross-Zone ComponentSession services |
...
| v3 destination | `packages/d2b-bus/`; `RealmSessionAuthority` renames to `ZoneSessionAuthority`; `CredentialCustody` is behavior-stable and requires no rename; `REALM_SERVICE_NAME` updates per ADR-046-componentsession-and-bus owning spec |
...
| Detailed design | Named ProviderCompositionError variants, first-party factory instantiation, bundle resolver validation, version/schema constants, and monotone lifecycle operation IDs. Primary reuse disposition: `adapt`. Preserved source-plan detail: Reuse fail-closed provider composition from main commit `a1cc0b2d`; adapt v2 identity/bundle artifact types to v3 provider registry contracts and bump bundle versions.. |
...
| Field | Value |
| --- | --- |
| Dependency/owner | ADR046-nix-034; ADR046-nix-029; GuestLifecycleRequest ResourceRef addressing per ADR-046-componentsession-and-bus owning spec |
...
| Detailed design | DaemonEffectAdapters bind semantic effect ports; ProviderLifecycleDispatch bounds lifecycle mutations and routes broker start/stop through blocking adapters with idempotency-key deduplication. Primary reuse disposition: `adapt`. Preserved source-plan detail: Reuse descriptor-bound effect dispatch from main commit `a1cc0b2d`; adapt lifecycle request wire type from VM name to Zone-scoped Guest ResourceRef.. |
