...
| D096 signed Service/State projection factory, Service-only export, deterministic same-type projection, authored State graph/finalizers, and real cross-Zone streams | `ADR-only` | Yes — SPIKE-20 |
...
| Field | Value |
| --- | --- |
| Hypothesis | (D096) Deterministic cross-Zone sharing requires a signed Provider projection factory binding a qualified `*Service` type, matching qualified `*State` type, allowed Service backing refs, allowed State target refs, and projection schema/fingerprints. Export targets only the one owner Service; import creates exactly one same-qualified-type non-authoritative projection Service; operator-authored local States reference it and own Process/Endpoint children. No Device/Endpoint/State projection, auto-State, or FD/secret/path/locator transfer is needed. |
| Minimal disposable artifact | `proofs/cross-zone-singleton-share-spike/` — an in-memory two-Zone resource store/catalog, signed/tampered factory fixtures, fake owner/import/State controllers, deterministic owner/dependency graph, fake ZoneLink/clock/stream, and a real encrypted named-stream feature. The fake tier exercises all metadata/admission/lifecycle rules without processes, sockets, sleeps, or network; the integration feature swaps in the production stream from SPIKE-06/SPIKE-07. |
| Inputs | (a) audio owner Service backed by local Device+Endpoint, export to work Zone, same-type projection Service, and two authored States targeting a Guest/User; (b) security-key Service with exclusive fair queue; (c) observability Service with producer backpressure; (d) USBIP Service under allow/deny policy combinations; (e) every other Provider forbidden; (f) negative factories: absent, unsigned, wrong Service/State type, disallowed backing/target, schema mismatch, fingerprint tamper; (g) direct Device/Endpoint/State/Credential export, attempted Device/Endpoint/State projection, forged State authority status, stale generation, and FD/secret/path/socket/device-handle transfer. |
...
| Metrics | (1) factory signature/type/ref/schema/fingerprint validation is exact and fail-closed; (2) only owner Service accepted as export target; (3) exactly one same-type projection Service with import ownerRef and zero Device/Endpoint/State projections; (4) State exists only when authored, validates service/target refs, and cannot write remote authority; (5) graph exactly import→Service→State→owned Process/Endpoint; (6) revoke/update propagates through that graph; import finalizer waits on State refs and never deletes them; (7) audio/security-key/observability admitted, USBIP policy-gated, every other family forbidden; (8) Nix lowering/resource names stable across three evaluations; (9) quota/fairness/deadline/cancel/reconnect behavior exact; (10) no FD/secret/Credential/backing/remote Ref/path/locator/bytes leak and intermediaries see ciphertext; high-churn sessions/streams never enter the store. |
...
| Failure interpretation | A non-Service export, wrong projection type/cardinality, auto-State, State authority spoof, factory bypass, forbidden-Provider admission, plaintext/FD/secret/path leak, or stale reconnect is severity-blocking and requires structural contract/controller repair. Quota/fairness misses revise arbitration, never these invariants. |
