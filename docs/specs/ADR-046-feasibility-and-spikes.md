...
| Inputs | (a) `audio.d2bus.org.AudioService/AudioBinding`, initially implemented by `audio-pipewire`: owner backed by local Device+Endpoint, export to work Zone, same-type projection Service, and two authored Bindings targeting a Guest/User; (b) `security-key.d2bus.org.SecurityKeyService/SecurityKeyBinding` with `device-security-key` and exclusive fair queue; (c) `telemetry.d2bus.org.TelemetryService/TelemetryBinding` with `observability-otel` and producer backpressure; (d) `usb.d2bus.org.UsbService/UsbBinding` with `device-usbip` under allow/deny policy combinations; (e) every other Provider forbidden; (f) canonical minimal base without `spec.provider`, independently selected conformant owner/consumer implementations, and negatives for implementation-qualified type, implementation detail in base, absent/unsigned/wrong Service/Binding factory, disallowed backing/target, schema mismatch, and fingerprint tamper; (g) direct Device/Endpoint/Binding/Credential export, attempted Device/Endpoint/Binding projection, observed state smuggled into Binding spec, forged Binding authority status, stale generation, and FD/secret/path/socket/device-handle transfer. |
...
| Metrics | (1) factory signature/type/ref/schema/fingerprint validation is exact and fail-closed; (2) only owner Service accepted as export target; (3) exactly one same-semantic-type projection Service with import ownerRef and zero Device/Endpoint/Binding projections, including when local Provider implementations differ; (4) Binding exists only when authored, validates service/target refs, keeps desired intent in spec and observations only in status, and cannot write remote authority; (5) graph exactly import→Service→Binding→owned Process/Endpoint; (6) revoke/update propagates through that graph; import finalizer waits on Binding refs and never deletes them; (7) the four exact semantic pairs are admitted under their initial Provider mapping, USB policy is gated, and every other family is forbidden; (8) Nix lowering/resource names are stable across three evaluations and canonical minimal base works without `spec.provider`; (9) quota/fairness/deadline/cancel/reconnect behavior exact; (10) no FD/secret/Credential/backing/remote Ref/path/locator/bytes or PipeWire/CTAPHID/OTEL/USBIP base detail leaks, and intermediaries see ciphertext; high-churn sessions/streams never enter the store. |
