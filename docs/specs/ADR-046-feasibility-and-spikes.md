...
| Field | Value |
| --- | --- |
| Hypothesis | (D096) Deterministic cross-Zone sharing requires a signed Provider projection factory binding a qualified `*Service` type, matching qualified `*Binding` type, allowed Service backing refs, allowed Binding target refs, and projection schema/fingerprints. Export targets only the one owner Service; import creates exactly one same-qualified-type non-authoritative projection Service; operator-authored local Bindings express desired consumer intent, reference it, and own Process/Endpoint children. Observed realization exists only in status. No Device/Endpoint/Binding projection, auto-Binding, or FD/secret/path/locator transfer is needed. |
...
| Inputs | (a) audio owner Service backed by local Device+Endpoint, export to work Zone, same-type projection Service, and two authored Bindings targeting a Guest/User; (b) security-key Service with exclusive fair queue; (c) observability Service with producer backpressure; (d) USBIP Service under allow/deny policy combinations; (e) every other Provider forbidden; (f) negative factories: absent, unsigned, wrong Service/Binding type, disallowed backing/target, schema mismatch, fingerprint tamper; (g) direct Device/Endpoint/Binding/Credential export, attempted Device/Endpoint/Binding projection, observed state smuggled into Binding spec, forged Binding authority status, stale generation, and FD/secret/path/socket/device-handle transfer. |
...
| Metrics | (1) factory signature/type/ref/schema/fingerprint validation is exact and fail-closed; (2) only owner Service accepted as export target; (3) exactly one same-type projection Service with import ownerRef and zero Device/Endpoint/Binding projections; (4) Binding exists only when authored, validates service/target refs, keeps desired intent in spec and observations only in status, and cannot write remote authority; (5) graph exactly import→Service→Binding→owned Process/Endpoint; (6) revoke/update propagates through that graph; import finalizer waits on Binding refs and never deletes them; (7) audio/security-key/observability admitted, USBIP policy-gated, every other family forbidden; (8) Nix lowering/resource names stable across three evaluations; (9) quota/fairness/deadline/cancel/reconnect behavior exact; (10) no FD/secret/Credential/backing/remote Ref/path/locator/bytes leak and intermediaries see ciphertext; high-churn sessions/streams never enter the store. |
