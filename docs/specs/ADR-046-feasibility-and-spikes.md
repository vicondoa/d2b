...
| Field | Value |
| --- | --- |
| Hypothesis | (D096) A scarce singleton resource can be shared across a parent/child Zone boundary only through one owner-Zone Provider authority and one consumer-Zone local projection: both sides opt in, `ResourceExport` references only local `resourceRef`/`endpointRef`, `ResourceImport` references only local `zoneLinkRef` plus bounded `exportKey`, schema fingerprints match, quotas/fairness/deadlines are enforced, reconnect revalidates generation/fingerprint, revocation degrades the projection, and no FD/device/socket/resource grant crosses a Zone. |
| Minimal disposable artifact | `proofs/cross-zone-singleton-share-spike/` — a fake in-memory resource store for two Zones, a fake ZoneLink, a fake owner Provider export adapter, a fake consumer Provider import adapter, and a bounded encrypted named-stream harness. The hermetic tier uses a deterministic fake stream that exposes ciphertext-only bytes to an intermediate observer; the integration tier swaps in the real `d2b-session` named-stream implementation copied/adapted per SPIKE-06/SPIKE-07 once that transport substrate exists. |
| Inputs | (a) one audio-like singleton export (`arbitration=multiplexed`) with two child imports competing under quotas; (b) one security-key-like export (`arbitration=exclusive`) with a fair queue, deadline, cancel, and revoke; (c) one observability-ingest-like export (`arbitration=multiplexed`) with backpressure; (d) negative cases: direct cross-Zone `ResourceRef`, fingerprint mismatch, unauthorized consumer Zone, `requestedCapabilities` outside the capability ceiling, Credential/token `resourceRef`, stale generation on reconnect, and an attempted FD/socket/device handle transfer. |
...
| Metrics | (1) opt-in: absent export or absent import yields no advertisement/projection; (2) cross-Zone `ResourceRef` rejection for both types; (3) `expectedBaseSchemaFingerprint` match required before binding; (4) unauthorized consumer and capability-ceiling violations fail closed; (5) quota/fairness/deadline/cancel order matches the configured arbitration; (6) reconnect requires generation/fingerprint revalidation and stale reconnect degrades the projection; (7) revoke/export delete/ZoneLink loss releases leases and degrades or deletes the local projection per policy; (8) intermediate observer sees ciphertext only; (9) FD/device/socket/token transfer attempts are rejected before any lease is admitted; (10) metrics/audit samples use only closed labels and carry no raw bytes/path/device/token/`exportKey`. |
| Pass/fail threshold | All metrics binary pass in the hermetic fake-stream tier; the real-stream tier must pass the same metrics before production `ADR046-zone-control-019`/`020` can be considered complete. Metrics (2), (3), (8), and (9) are zero-tolerance security gates. |
...
| Failure interpretation | A cross-Zone ref acceptance, fingerprint bypass, plaintext/FD/device/socket/token leak, or stale reconnect binding is a severity-blocking finding against D096 and must revise the export/import controller or Provider adapter contract structurally; quotas/fairness misses revise the arbitration algorithm, never the security invariants. |
| Affected decisions/work items | D096; `ADR046-zone-control-019`, `ADR046-zone-control-020`, the `audio-pipewire`, `device-security-key`, and `observability-otel` Provider export/import adapter work items. |
| Cleanup | Deleted once the real ResourceExport/ResourceImport controller, projection lifecycle, provider conformance kit, and at least one fake-stream plus one real-stream integration test reproduce the same opt-in/ref/fingerprint/quota/reconnect/revocation/no-FD/no-plaintext metrics. |
| Status | Specified — not yet executed. |
