...
Every pull request to `v3` runs the `security-scan` job in
...
#586). `main` carries the same required context, but the producing job
only exists once the workflow lands there via a `v3`-derived promotion;
until then, pull requests to `main` show the check as Expected rather
than running it. In-flight pull requests to `v3` opened before this
workflow merged into `v3` likewise stay Expected until the base gains
the job on merge.
...
- Scope: added lines of in-tree Rust source files (vendored `pkgs/` code
  ...
  tree. The changed-line diff is taken from the merge base against the
  working tree (two-dot), so a local run also sees uncommitted lines;
  the diff keeps rename/copy statuses (`--diff-filter=ACMR`), so a
  violation that arrives with a rename or copy is still a finding. An
  all-zeros base (a first push) is rejected with exit 2 - a changed-line
  scan against it cannot prove the rule, and a scan that could not run
  is never reported clean.
- Rule: a log-emitting macro invocation (`tracing::*!`, `log::*!`, bare
  `trace!`/`debug!`/`info!`/`warn!`/`error!`, or
  `println!`/`eprintln!`/`print!`/`eprint!`) that references one of the
  ...
  set.
- The rule is evaluated against the whole macro invocation: the scanner
  buffers from the log-macro opening line until the invocation's closing
  `);` and matches the identifier anywhere in the record, including a
  mention in message text. A multi-line record (macro opened on one
  line, fields on continuation lines) is therefore a finding when any
  part of the record references a pinned identifier. A false positive is
  resolved by rewording the record, never by allowlisting.
- The scanner engine pinned here is the deterministic rule above; the
  external scanner that produced the original findings remains
  owner-confirmed. When the owner names that engine, this job is updated
  to pin it.
...
```
The changed-line local run diffs the merge base against the working
tree, so uncommitted lines are scanned too.
