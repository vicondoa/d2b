...
used the productionized `d2b-relay` endpoint binary
(`packages/d2b-provider-relay`) on both ends and the sandbox's MI Entra
...
`hyco-websocket` SDK omits the id; `d2b-provider-relay::build_connect`
...
- Bicep already deployed (see `live-demo-checklist.md` §1): ACR, the relay
  namespace + `hc-d2b-display`, the sandbox group, and the
...
./build-and-push.sh "$REGISTRY" d2b-wayland:mi   # builds d2b-relay from the workspace
```
`image.nix` built `d2b-relay` from `../../../packages` and the historical
agent (`bridge/d2b-sandbox-agent.sh`) fetched the MI token from
`$IDENTITY_ENDPOINT` and ran `d2b-relay sender`.
...
aca sandboxgroup disk create --region "$REGION" \
  --image "$LOGIN_SERVER/d2b-wayland:mi" --name d2b-mi-fixed \
...
D2B_RELAY_NAMESPACE=<ns>.servicebus.windows.net \
D2B_RELAY_ENTITY=hc-d2b-display \
D2B_RELAY_KEY_NAME=gateway-listen D2B_RELAY_KEY=<listen-key> \
...
export D2B_RELAY_NS='<ns>.servicebus.windows.net'
export D2B_RELAY_ENTITY='hc-d2b-display'
export D2B_APP_CMD='foot --title=ACA-MI-RELAY'
( /bin/d2b-sandbox-agent </dev/null >/tmp/agent.log 2>&1 & )
sleep 16; cat /tmp/agent.log
EOF
)"
```
Expect: `MI token acquired (... chars); starting d2b-relay sender (no
...
#   -> foot, d2b-relay, waypipe x2 all alive
```
