...
#   2. `nixling-relay-bridge listen`, which accepts the sandbox's outbound
...
#
# Env:
#   NIXLING_RELAY_NS        Relay namespace FQDN (…servicebus.windows.net)
#   NIXLING_RELAY_ENTITY    hybrid connection name           (default: hc-nixling-display)
#   NIXLING_RELAY_KEYNAME   Listen SAS rule name             (default: gateway-listen)
#   NIXLING_RELAY_KEY       Listen SAS key
#   NIXLING_RG              resource group                   (default: rg-nixling-centralus)
#   NIXLING_WP_COMPRESS     waypipe -c value                 (default: zstd)
#   WAYLAND_DISPLAY/XDG_RUNTIME_DIR  the operator's compositor
set -euo pipefail

here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RG="${NIXLING_RG:-rg-nixling-centralus}"
ENTITY="${NIXLING_RELAY_ENTITY:-hc-nixling-display}"
KEYNAME="${NIXLING_RELAY_KEYNAME:-gateway-listen}"
COMPRESS="${NIXLING_WP_COMPRESS:-zstd}"
: "${XDG_RUNTIME_DIR:?XDG_RUNTIME_DIR must point at the operator runtime dir}"
WPC_SOCK="${XDG_RUNTIME_DIR}/wpc.sock"
PIDFILE="${XDG_RUNTIME_DIR}/nixling-aca-poc-host.pids"

log() { printf '[host-display] %s\n' "$*" >&2; }

bridge_bin() {
  if command -v nixling-relay-bridge >/dev/null 2>&1; then
    command -v nixling-relay-bridge
  else
    echo "$here/../relay-bridge/target/release/nixling-relay-bridge"
  fi
}
...
up() {
  ...
  "$bin" --namespace "$NIXLING_RELAY_NS" --entity "$ENTITY" \
