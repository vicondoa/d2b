...
| copilotw6fu21 | build | 2026-07-31 | The agent tool's default working directory is a different worktree, and `&` detaches the leading `cd`, so a diff staged for panel review was computed in the wrong checkout and looked plausible rather than failing | 1 | resolved |
