...
# pipeline; for the general d2b onboarding story see
...
# write-up.
#
# One-shot migration: moves /var/lib/d2b state to the new
# vicondoa/d2b v0.1.0 layout. PRESERVES TPM enrollment.
...
#
#   sudo -A bash $(nix flake archive --json github:vicondoa/d2b \
#     | jq -r .path)/scripts/migrate-d2b-v0.1.0.sh --dry-run
#
...
# Workload VMs: per-VM state dir under /var/lib/d2b/<vm>/ that
# moves to /var/lib/d2b/vms/<vm>/. Order matters: we move the
# workload VM dir FIRST, then nest the swtpm/<vm>/ inside it.
WORKLOAD_VMS=( work-aad personal-dev d2b-test )
...
# Net VMs: /var/lib/d2b/<env>-router/ -> /var/lib/d2b/vms/sys-<env>-net/.
...
PRIVATE_DIR="/var/lib/private/d2b"
BACKUP_ROOT="/var/lib/d2b-migration-backup"
...
OLD_USBIPD_UNITS=(
  "usbipd-d2b.service"
  "usbipd-d2b-work.service"
  "usbipd-d2b-work.socket"
  "usbipd-d2b-work-backend.service"
  "usbipd-d2b-personal.service"
  "usbipd-d2b-personal.socket"
  "usbipd-d2b-personal-backend.service"
)
...
usage() {
  cat <<'EOF'
Usage: migrate-d2b-v0.1.0.sh [OPTIONS]
...
What to do:
  ...
  2. If the file is empty/zero-bytes, a previous run crashed mid-write.
     Decide based on the actual state of /var/lib/d2b/:
       - If /var/lib/d2b/vms/ exists with the expected VMs, the
         rename completed; restore the marker by hand:
           ...
           EOF
       - If /var/lib/d2b/<vm>/ still exists (pre-migration layout),
...
snapshot_swtpm_state() {
  ...
  for vm in "${TPM_VMS[@]}"; do
    ...
    if [[ "$DRY_RUN" -eq 0 ]]; then
      ...
      # safety. See https://github.com/vicondoa/d2b/issues/<TBD>.
      if [[ -d "$dir1" ]]; then
...
rename_swtpm_public() {
  ...
  # If /var/lib/d2b/swtpm/ is now empty, clean it up so it doesn't
...
# usbipd-d2b-*) and the NEW naming introduced by
# (d2b-sys-<env>-usbipd-{backend,proxy}.{service,socket}). On a
...
discover_new_usbipd_units() {
  ...
  systemctl list-units --all --plain --no-legend --no-pager \
      'd2b-sys-*-usbipd-*.service' \
      'd2b-sys-*-usbipd-*.socket' \
      2>/dev/null \
    | awk '{print $1}' \
    | grep -E '^d2b-sys-.*-usbipd-.*\.(service|socket)$' \
...
stop_old_sidecars() {
  ...
  local svcs_per_vm=(
    ...
    "d2b-snd@%s.service"
    "d2b-store-sync@%s.service"
    "d2b-known-hosts-refresh@%s.service"
  )
...
# F4: discover any d2b-sys-*-usbipd-* units that exist as static
...
unit_disable_phase() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    dry "enumerate d2b-sys-*-usbipd-* unit files and disable any present"
  else
    local nu
    while IFS= read -r nu; do
      [[ -z "$nu" ]] && continue
  ...
  for vm in "${NET_VMS_OLD[@]}"; do
    unit="d2b-vfsd-watchdog-${vm}-enable.service"
    disable_unit_if_present "$unit"
  done
...
symlink_cleanup_phase() {
  ...
  remove_back_compat_symlink "/var/lib/microvms" "/var/lib/d2b"
  remove_back_compat_symlink "/var/lib/swtpm"   "/var/lib/d2b/swtpm"
}
...
Next steps (you, manually):
  ...
  1. Update /etc/nixos to consume the new vicondoa/d2b flake
...
main() {
  ...
  # /var/lib/d2b which non-root users can't create.
  check_root
