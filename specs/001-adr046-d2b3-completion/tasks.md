...
- [ ] T035 [US1] W3 GATE - run the native Copilot pre-panel procedure below FIRST: dispatch a read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`, then run `/d2b-panel-round work`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; clear every CRITICAL finding before snapshot, import validation evidence, panel-request, panel-attest (10/10 unanimous), seal (every wave item Merged), merge-target, and merge-eligibility. Also confirm for every item in this wave: reference docs landed with their behavior (FR-019), no change contradicts a decision in the register (FR-047), and every removal proof for a path retired in this wave passed (FR-023). From round 9, LOW/MEDIUM may be deferred to deferred-findings.md; CRITICAL/HIGH never (FR-051, FR-052). At wave close, review both registers and log this wave friction in friction-log.md (FR-053)
...
- [ ] T072 [US1] `adr046w5` ENTRY - confirm every prior-wave work item is Merged, destinations uncontended, stack proposed against the exact parent commit, heavy-gate free, fast hermetic suite green
...
T595 -> {T596,T597,T598,T599,T604} -> freeze candidate
freeze candidate -> {T600,T601} -> T602 -> T219 -> T220
```
...
T595 is the only daemon-composition writer. T596-T599 and T604 are file-disjoint. T600 and
T601 are read-only evidence lanes and write only delivery evidence outside the repository.
...
- [ ] T603 [US1] **`adr046w5` RESUME RECONCILIATION - bind the amended plan gate before T589.** Writes no feature artifact and owns no implementation file. Audit every T073-T218 obligation against the current converged Wave 5 commit and delivery records; code presence alone is never completion evidence. Atomically write the closed machine receipt defined in `plan.md` to `.scratch/autopilot/adr046w5/reconciliation.json` at directory/file modes `0700`/`0600`. It must bind the exact 28-file length-framed feature snapshot SHA-256, Git tip, branch, candidate, current analysis receipt, qualified `adr046w5-r<n>` plan panel with ten record locators, and exactly 146 unique ordered T073-T218 rows carrying the obligation identity, `satisfied|open`, evidence kind, and qualifying commit/receipt locator. Reject unknown fields/statuses, missing or extra rows, stale or mismatched identities, nonlocal locators, symlinks, weak permissions, partial or non-atomic writes, and receipt payloads containing diffs, transcripts, command/validation output, secrets, credentials, store paths, or raw sink details. If any row is `open`, leave T603 unchecked and change no checkbox. If and only if all rows are `satisfied`, analysis has no unresolved HIGH or CRITICAL finding, and unanimous plan signoff is current on the same snapshot/tip, prepare and route one explicit `/d2b-spec-edit` batch citing the exact receipt and SHA-256. The editor must recompute every binding before applying a batch whose only feature changes check T073-T218 and T603; no integrator or autopilot lane edits `tasks.md` directly. **Done when** `.scratch/autopilot/adr046w5/progress-editor-receipt.json` binds the reconciliation receipt, pre/post feature snapshots, unchanged tip, and exact changed-ID set T073-T218 plus T603, and all 147 checkboxes are checked. C1 is approved and fully assigned in the artifacts; normal analysis, unanimous plan signoff, and this 146-row reconciliation still gate the transition.
- [ ] T589 [US1] **`adr046w5` INTEGRATOR PREP - freeze the shared production contracts before parallel work.** Depends directly only on T603 and semantically on all T073-T218 obligations reconciled there. Refuse unless the reconciliation receipt has 146 `satisfied` rows, the editor progress receipt is current, and `tasks.md` shows T073-T218 plus T603 checked. Its immediate contract inputs include T165, T170-T172, T182-T184, T195, and T208-T218. Owned files: `packages/d2b-resource-store/src/lib.rs`, `packages/d2b-resource-api/src/adapter.rs`, `packages/d2b-core-controller/src/runtime.rs`, `packages/d2b-resource-store-redb/src/audit.rs`, `packages/d2b-bus/src/router.rs`, and the dependency edges in `packages/{d2bd,d2b-bus,d2b-resource-api,d2b-core-controller,d2b-resource-store-redb}/Cargo.toml`. Establish only the sealed interfaces for: registrar-consumed session admission; the private, non-`Clone`, one-shot `PolicyBootstrapRead`; authenticated post-install policy revision reads; an engine-neutral bounded controller-ledger port; a durable audit-outbox drainer whose `CommittedPendingAudit` state uses the existing `ResourceStatus` composite (`phase = ResourcePhase::Degraded`, `outcome.code = StatusCode("committed-pending-audit")`, `update.state = UpdateState::Blocked`, and `update.operation_id = Some(original_operation_id)`); and aggregate readiness observations backed by the `d2b-core-controller`-owned `Provider/system-core` registration plus live `HostReconciler` and `UserReconciler` handles that T595 will project as exactly one `Zone.status.handlers[]` record named `system-core-host` and one named `system-core-user`. The policy bootstrap surface reads only policy-input envelopes, carries no Resource API subject, exposes no general read/mutation method, and is consumed by one install attempt. The controller-ledger value is opaque to store crates. **Done when** the prep commit compiles every affected crate, both T603 receipts and all prerequisite checkboxes are current, no interface exposes a subject constructor, reusable bootstrap reader, independent mutation path, new audit enum/field/schema version, or production-ready boolean setter, and every T590-T594 and T605 branch is cut from that exact commit. These files may be edited later only by their named serialized successor; no parallel slice shares one.
- [ ] T590 [P] [US1] **Install and recover the single-owner Zone resource policy without a bootstrap cycle.** Depends on T589. Owned files: `packages/d2b-resource-api/src/authz.rs`, `packages/d2b-core-controller/src/rbac.rs`, and new focused tests under `packages/d2b-resource-api/tests/production_policy.rs`. `ZoneResourceRuntime` owns each `PolicyBootstrapRead` and requests installation, but `d2b-resource-api` alone parses and compiles policy into the immutable `PolicySet` interpreted by `NativeAuthorizer`. For initial install and restart, consume the one-shot capability to read only this Zone's policy-input envelopes at the exact durable nonzero `policy_revision`; it has no public subject, general read/mutation operation, clone, or reuse path. After installation, perform every normal policy read/update through an authenticated Resource API session. On revision advance, compile the exact committed revision before atomic replacement, invalidate cached allows, and report ready only when installed revision and Zone UID equal live durable metadata. Refuse revision zero, stale/missing/cross-Zone/invalid policy, a caller claim, reusable bootstrap access, and any fallback to a constant or partial set. **Done when** focused tests cover first install, authenticated revision advance, restart recovery of the advanced revision, capability consumption/non-reuse, no public subject or general access, and every failure leaving only the affected Zone unpublished, degraded, and denied.
- [ ] T591 [P] [US1] **Restore the D106 store boundary and make it exhaustive.** Depends on T589. Owned files: `packages/d2b-resource-store-redb/src/transaction.rs`, `packages/d2b-resource-store/tests/d106_policy.rs`, and `packages/d2b-contract-tests/tests/policy_resource_mutation_seal.rs`. Remove redb deserialization or ownership of `RoleSpec`, `RoleBindingSpec`, `PolicySet`, and all other RBAC DTOs. Move policy-shape interpretation to the Resource API policy owner while retaining policy-neutral canonical-envelope, installed-schema, structural, atomicity, revision, and seal checks in the store. Expand the guard from three hand-picked source files to the full store/redb crate source and dependency graph. **Done when** the policy test proves neither store crate depends on the Resource API or contains/imports/deserializes an RBAC policy DTO, the native evaluator remains the only allow issuer, and authorized Role/RoleBinding mutations still pass through the sealed generic envelope path.
- [ ] T592 [P] [US1] **Complete durable store identity recovery and the mutation-audit drain.** Depends on T589. Owned files: `packages/d2b-resource-store-redb/src/{lib.rs,actor.rs,audit.rs,migration.rs,backup.rs,tests.rs}` and `packages/d2b-audit/src/{lib.rs,sink.rs}`. Separate immutable store/Zone identity from mutable policy, active-configuration, and controller revisions so reopen reads the latter from durable metadata after any advance. Replace the production-disabled audit callback with one owner-supplied sink/drainer; replay pending outboxes by operation ID plus mutation ordinal; durably append and sync each logical record; and durably clear the outbox before returning ordinary mutation success. If the mutation committed first, preserve the outbox and stored final result and return semantic `CommittedPendingAudit` through the existing layered `ResourceStatus`: `phase = ResourcePhase::Degraded`; `outcome.code = StatusCode("committed-pending-audit")` with retryable safe remediation and no raw sink detail; `update.state = UpdateState::Blocked`; and `update.operation_id = Some(original_operation_id)`. Existing bounded, redacted condition/outcome/update fields carry only same-ID retry/status instructions. Add no enum variant, wire field, schema version, phase member, or status-code member to `ResourceUpdateStatus`, and never imply rollback. Same-ID retry resumes or observes that mutation without reapplying it and returns its eventual one final result; different-ID retry follows normal revision/conflict rules. Keep the affected Zone unpublished and degraded until drain recovery. **Done when** crash tests at commit, append, sync, and outbox-clear boundaries prove the exact composite and redaction bounds, same-ID apply count stays one, different-ID conflicts are unchanged, restart yields no missing or duplicate logical audit record, advanced revisions reopen, and production construction cannot select `NoopMutationAudit` or `enabled() == false`.
- [ ] T593 [P] [US1] **Publish the authenticated Resource API and watch route.** Depends on T589. Owned files: `packages/d2b-bus/src/{router.rs,registry.rs,operations.rs,streams.rs}`, `packages/d2b-resource-api/src/{adapter.rs,watch.rs}`, and new focused tests under `packages/d2b-bus/tests/production_resource_route.rs`. Replace the unregistered production seam with a route whose registration consumes the authenticated ComponentSession admission. `ZoneRegistrar` exclusively derives and propagates the subject from verified peer evidence; requests and stream frames carry no subject claim. Register exact-Zone ResourceService and controller routes, admit watch replay/live delivery through ZoneBus, and expose one registration/readiness observation from actual owned handles. **Done when** same-Zone authenticated Get/List/Watch reaches the real service, cross-Zone, self-named, unregistered, reused-admission, and direct-WatchService paths are denied, and neither `UnregisteredBusAdapter` nor a fixed endpoint can satisfy production publication.
...
- [ ] T605 [P] [US1] **Correct and pin the system-core Zone handler contract.** Depends on T589. Sole writable ownership: `packages/d2b-contracts/src/v3/zone.rs`; compiler-regenerated public and private snapshots under `tests/golden/api-surface/`, regenerated only with `make api-surface-pin`; the ownership-compatible existing lowest-layer guard `packages/d2b-contract-tests/tests/policy_contracts.rs`; and the existing paired reference page `docs/reference/resource-plane-runtime.md` (adapt). Add `ZoneHandlerName::SystemCoreHost` and `ZoneHandlerName::SystemCoreUser`; the existing kebab-case serialization MUST round-trip them as `system-core-host` and `system-core-user`. The Zone status-handler contract MUST accept exactly one `Zone.status.handlers[]` record with each required name, phase, and `lastReconciledAt`, reject duplicate or missing required records and wrong-name substitution, and preserve `ZoneHandlerName::ProviderLifecycle` as a distinct allowed value that cannot substitute for either. Treat `packages/xtask/src/zone_schema.rs` and `docs/reference/schemas/v3/core.d2bus.org_Zone.schema.json` as read-only proof inputs: because `ZoneSpec` is unchanged, generator execution MUST leave the committed desired-state schema byte-identical. **Done when** focused `d2b-contracts` tests prove both variant round-trips, exactly-one-each list acceptance, duplicate/missing/wrong-name rejection, and `ProviderLifecycle` preservation/non-substitution; the targeted `policy_contracts.rs` contract test and paired reference page pin the same list shape; public/private API snapshots are current after `make api-surface-pin`; the Zone desired schema is byte-identical before and after its existing generator; and the targeted contract, `make test-rust-api-surface`, and `make test-drift` gates pass.
- [ ] T595 [US1] **Compose the production Zone runtime and one readiness projection.** Depends explicitly on T590, T591, T592, T593, T594, and T605 and begins only after all six slices converge. Sole owned files: `packages/d2bd/src/resource_runtime.rs`, `packages/d2bd/src/lib.rs`, `packages/d2bd/Cargo.toml`, and focused unit tests inside `resource_runtime.rs`. Replace hard-coded mutable revision identities and independent booleans with the recovered store metadata and owned handles from T590-T594. Establish the verified Unix ComponentSession through the registrar, consume `PolicyBootstrapRead` for the first `NativeAuthorizer` install, switch all later policy access to the authenticated Resource API, register ResourceService and controller endpoints, admit the watch cursor, start the audit drainer, recover controller effects, and only then publish the Zone. Consume T605's `ZoneHandlerName::SystemCoreHost` and `ZoneHandlerName::SystemCoreUser` variants to project exactly one `Zone.status.handlers[]` record named `system-core-host` and exactly one named `system-core-user`, each with `phase` and `lastReconciledAt`, from the live owned `HostReconciler` and `UserReconciler` health handles for the `d2b-core-controller`-owned `Provider/system-core` registration. `ProviderLifecycle` is distinct and cannot satisfy either record. Do not wait for other Wave 6 Provider dossiers. Public resource requests must traverse that session/router path and must never promote `SO_PEERCRED` role to a Resource API subject. Refactor startup and shutdown to visit every Zone and return a per-Zone report: a failed Zone stays unpublished and actionable, while unrelated Zones continue; close aggregates errors and never drops later runtimes. **Done when** no public path calls `dispatch_authenticated_cli_request` with a constructed subject, `require_ready` derives every member from live owned production handles rather than a boolean, duplicate/missing/wrong-name required records or substitution by `ProviderLifecycle` degrade only that Zone, partial readiness is impossible, advanced revisions reopen, and one Zone's open/close failure does not abort or discard another.
- [ ] T604 [P] [US1] **Prove exact-candidate operator activation through effect and cleanup.** Depends on T595 and is file-disjoint with T596-T599. Sole owned files: new `packages/d2b-contract-tests/tests/resource_operator_activation.rs`, new `packages/d2bd/tests/resource_operator_activation.rs`, and new `tests/host-integration/resource-operator-activation.nix`. The fixture-backed contract test proves that an operator declaration emits the exact pinned `zones/<zone>/resource-bundle.json` generation and that removing one declaration emits the corresponding generation delta; run it through `D2B_ENABLE_FIXTURE_BUILD=1 make test-fixture-contracts`. The Type-3 d2bd test is the lowest feasible production-boundary test: consume those exact generation bytes through the daemon activation/reload entry and production store/controller path, never through a direct ResourceService/WatchService call, and run it through `make test-rust`. The Type-10 test is additionally required because NixOS activation, systemd, broker mutation, and a real owned host effect cannot be proved below `runNixOSTest`; run it only through the public heavy-gated `make test-host-integration` target. In the host test, declare one Zone with the specification's representative Guest, Volume, Network, and Device resources in Nix, activate the emitted system and bundle through the production daemon boundary, and observe durable resource creation, controller reconciliation, and for each resource either its real owned effect/readiness or a precise typed actionable failure. A status-only path is ineligible. Remove the declared Guest, activate the next emitted generation, observe dependency-safe cleanup of its owned effect, and prove the Volume, Network, Device, and unrelated resources remain intact and are not recreated. Bind all three test records and both bundle content hashes to the same exact candidate. **Done when** the emitted declaration and removal generations are pinned, the Type-3 leg proves production daemon ingestion/reconciliation, at least one real broker/controller-owned effect is observed in Type 10, every declared resource has ready/effect evidence or an exact actionable refusal, removed-resource cleanup completes in dependency-safe order, unrelated resources and durable identities are unchanged, and all three public commands pass on the same candidate.
- [ ] T596 [P] [US1] **Add authenticated publication, watch, readiness, and Zone-isolation acceptance coverage.** Depends on T595. Sole owned file: new `packages/d2bd/tests/resource_plane_authenticated.rs`. Enter through the production daemon Unix session boundary, registrar, ZoneBus route, ResourceService, store, and controller endpoint. Consume T605's contract evidence and cover authoritative same-Zone Get/List/Watch, cross-Zone denial and audit, caller-supplied subject rejection, consumed-admission reuse, partial-readiness non-publication, exact `Provider/system-core` registration ownership, and an actual `Zone.status.handlers[]` list containing exactly one `system-core-host` and one `system-core-user` record with `phase` and `lastReconciledAt`, backed by active, initialized, current handlers. Reject duplicate, missing, or wrong-name required records and `provider-lifecycle` substitution. Run the three-Zone open/close matrix with failures in the first and middle positions; remove the Provider registration and each required list record in turn and prove only that Zone degrades. No Wave 6 dossier is required. The test must assert every Zone was visited and later healthy Zones remain operable. Direct service calls, `ProductionWatchHarness`, fake endpoints, status-only Provider substitutes, and readiness mutation helpers are forbidden in this file. **Done when** all cases pass against production owners, the emitted list shape matches T605, and removing or corrupting any required readiness owner makes the affected Zone return its specific actionable refusal.
...
- [ ] T598 [P] [US1] **Add durable audit acknowledgement, pending-result, and replay acceptance coverage.** Depends on T595. Sole owned file: new `packages/d2bd/tests/resource_plane_audit.rs`. Mutate through the authenticated production Resource API, crash at every commit/outbox/append/sync/clear boundary, reopen, and compare the authoritative audit sink with store operations by operation ID and mutation ordinal. Include sink unavailable, disabled callback, undrained outbox, hash-chain mismatch, and duplicate-replay negatives. After a committed mutation whose audit cannot finish, require semantic `CommittedPendingAudit` as the existing `ResourceStatus` composite: `phase = ResourcePhase::Degraded`, `outcome.code = StatusCode("committed-pending-audit")` with safe retryable remediation and no raw sink detail, `update.state = UpdateState::Blocked`, and `update.operation_id = Some(original_operation_id)`. Assert only bounded, redacted condition/outcome/update detail, no ordinary success, no rollback claim, and no new enum variant, field, or schema version; assert the Zone is unpublished/degraded until recovery. Retry the same ID before and after restart and prove zero reapplication plus one eventual stored final result. Retry a different ID and prove normal revision/conflict behavior. **Done when** ordinary success is never returned before durable append plus outbox completion, every committed mutation eventually has exactly one logical record, the exact composite round-trips through the current schema, same-ID apply count is one, status observability is stable across restart, and every audit failure leaves the affected Zone unpublished with an actionable typed refusal.
- [ ] T599 [P] [US1] **Reconcile Wave 5 CLI and reference promises with emitted behavior (FR-019, FR-074).** Depends on T595. Sole owned files: `docs/reference/{zone-cli-contract.md,desktop-wrapper.md,companion-contracts.md,cli-contract.md,components-audio.md,components-usbip.md,components-usb-security-key.md,resource-client.md}` and `packages/d2b-contract-tests/tests/{policy_cli_consumers.rs,policy_docs.rs}`. Compare exact `d2b --help`, subcommand help, JSON output, capability keys, typed refusals, and public wire fields. Resource status documentation must expose committed-pending-audit through `ResourceStatus.phase`, `ResourceStatus.outcome.code`, `ResourceStatus.update.state`, and `ResourceStatus.update.operation_id` with the exact C2 values; show only bounded, redacted same-ID retry/status remediation; never claim `ResourceUpdateStatus` owns phase/code; and never claim success, rollback, a new enum variant/field, or a schema version. It must consume T605's paired contract/reference evidence and attribute system-core readiness to `Provider/system-core` plus the actual `Zone.status.handlers[]` list: exactly one record named `system-core-host` and one named `system-core-user`, each with `phase` and `lastReconciledAt`; `provider-lifecycle` is distinct and cannot substitute. Implemented surfaces are documented exactly; absent surfaces lose command/field promises and are described only by the actual emitted capability or refusal. Do not add a fallback or claim companion verification. **Done when** every documented desktop-wrapper, companion, audio, USB, security-key, and resource operation is observed in emitted behavior or explicitly unavailable through its emitted typed state, pending-audit and system-core list text matches emitted behavior and T605's evidence, and the docs/schema drift gate is clean.
- [ ] T600 [US1] **Capture exact-candidate production-boundary evidence.** Depends on T596-T599 and T604 and runs read-only after the candidate is frozen. Owns no repository files; import delivery evidence records only. Run the authenticated same-Zone/cross-Zone watch matrix, restart crash-window/effect replay matrix, cleanup stale/zero/wrong-UID negatives, durable audit drain/replay matrix including the exact `ResourceStatus` committed-pending-audit composite and same-ID/different-ID behavior, exact `Provider/system-core` registration and handler-health failure matrix, three-Zone startup/close isolation matrix, and T604's operator Nix-activation-to-effect-and-cleanup result. Import T605's variant round-trip, exactly-one-each `Zone.status.handlers[]` list, duplicate/missing/wrong-name rejection, `ProviderLifecycle` non-substitution, current API snapshot, paired-reference, targeted-contract-test, and unchanged desired-schema drift evidence. Every record must name the same candidate commit and production entry point. Reject direct ResourceService/WatchService calls, `ProductionWatchHarness`, fixed/fake endpoints, constructed subjects, status-only Provider/readiness substitutes, manually set readiness, and evidence from an earlier commit. **Done when** each FR-072 behavioral evidence kind appears exactly once for the frozen commit, the T604 result identifies its emitted bundle and real owned effect, T605's complete contract/drift result is candidate-bound, same-ID audit retry applies once and returns one eventual final result, duplicate/missing/wrong-name system-core records or `provider-lifecycle` substitution degrade only their Zone, and every command passed.
- [ ] T601 [US1] **Capture exact-candidate RSS, owner fan-in, removal, operator-effect, and reference evidence.** Depends on T596-T599 and T604 and runs read-only in parallel with T600 subject to the heavy-gate limit. Owns no repository files; import delivery evidence records only. Measure the full daemon-owned publication path at 10,000 resources and 100 authenticated watches with no baseline subtraction; prove one store owner, one policy owner, one ResourceService route, one controller endpoint/fan-in, one audit drainer, and one `d2b-core-controller`-owned `Provider/system-core` registration per Zone, with exactly one `Zone.status.handlers[]` record named `system-core-host` and one named `system-core-user`, each carrying `phase` and `lastReconciledAt` from a live active/initialized/current handler. Reference the same T605 contract/API-snapshot/paired-reference/unchanged-schema evidence and T604 exact-candidate activation receipt; verify T604's declaration/removal bundle identities, owned effect/readiness or actionable failure per resource, dependency-safe cleanup, and unrelated-resource preservation. Re-run every manifest-label W5 removal proof at the candidate instead of citing `removal-proof-w5.md`'s historical `a7f4a6a4` snapshot. Compare emitted CLI/help/JSON/wire behavior with all T599 pages. **Done when** RSS is <=24,576 KiB, owner counts are exactly one, both required list records are unique and active/initialized/current, T605 and T604 results are candidate-bound and complete, all current removal-proof predicates are true, docs match emitted behavior, and all records name the frozen commit.
- [ ] T602 [US1] **`adr046w5` PRODUCTION COMPLETION CONVERGENCE - mechanically unblock T219.** Depends on T600 and T601. Owns no implementation files. Verify T603, T589-T599, T604, and T605 are complete; `tasks.md` shows T073-T218 and T603 checked; `.scratch/autopilot/adr046w5/reconciliation.json` and `progress-editor-receipt.json` pass their closed schema, permissions, exact-set, hash, tip, branch, candidate, analysis, panel, and pre/post snapshot checks; all 146 rows are `satisfied`; the exact-candidate evidence inventory contains each FR-072 kind once; both T600/T601 manifests reference T604's exact emitted-bundle activation/effect/cleanup result and T605's variant/list/API-snapshot/paired-reference/unchanged-schema result; every evidence record names the same post-T599/T604/T605 commit; and no evidence command names an ineligible direct/fake boundary. RSS, owner counts, the exact `Provider/system-core` registration, exactly one `Zone.status.handlers[]` record named `system-core-host` and one named `system-core-user` with phase/timestamp, duplicate/missing/wrong-name and `ProviderLifecycle` non-substitution evidence, the exact `ResourceStatus` committed-pending-audit composite and same-ID no-reapply/final-result behavior, current removal predicates, audit durability, dependency-safe cleanup, unrelated-resource preservation, targeted contract/API/drift gates, and docs comparison must all pass, and `git diff --exit-code <candidate>..HEAD` must be empty. Any false conjunct blocks T219 and names the failed remediation. Historical validation remains historical and is not reclassified.

- [ ] T219 [US1] `adr046w5` GATE - depends on T602. Run the native Copilot pre-panel procedure below FIRST: dispatch a read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`, then run `/d2b-panel-round work` at qualified round address `adr046w5-r<n>`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; clear every CRITICAL finding before snapshot, import only the T600/T601 exact-candidate evidence, panel-request, panel-attest (10/10 unanimous), seal (every wave item Merged), merge-target, and merge-eligibility. Mechanically confirm the T603 reconciliation/editor receipts and checkbox transition, authenticated registrar/ZoneBus publication, one-shot bootstrap then authenticated policy revision flow, registered controller endpoint, admitted watch, durable effect/adoption and cleanup ledger, the exact existing `ResourceStatus` committed-pending-audit composite with no new enum/field/schema version, caught-up mutation audit and same-ID semantics, the T604 operator Nix-activation-to-owned-effect-and-cleanup result, and T605's variant round-trip, targeted contract tests, current public/private API snapshots, paired reference, and byte-identical Zone desired-schema result. The readiness projection must contain the exact `Provider/system-core` registration plus exactly one `Zone.status.handlers[]` record named `system-core-host` and one named `system-core-user`, each with `phase` and `lastReconciledAt`; duplicates, a missing or wrong name, or `ProviderLifecycle` substitution fail. Also confirm per-Zone failure isolation, D106 store neutrality, exact-tip RSS and owner fan-in, current removal proofs, and CLI/reference consistency (FR-066-FR-074, SC-030-SC-034). A reusable bootstrap reader, readiness flag, status-only Provider substitute, direct service call, fake endpoint, disabled audit callback, undrained outbox, ordinary success for pending audit, missing T604 effect/cleanup evidence, incomplete T605 evidence, stale receipt/evidence, or a `ResourceUpdateStatus` phase/code claim blocks the gate. Also confirm for every item in this wave: reference docs landed with their behavior (FR-019), no change contradicts a decision in the register (FR-047), and every removal proof for a path retired in this wave passed (FR-023). From round 9, LOW/MEDIUM may be deferred to deferred-findings.md; CRITICAL/HIGH never (FR-051, FR-052). At wave close, review both registers and log this wave friction in friction-log.md (FR-053)
- [ ] T220 [US1] `adr046w5` CONVERGE + MERGE - merge every slice branch into the wave integration branch; run integration tests, `/d2b-panel-round work` with an `adr046w5-r<n>` address, and CI against the converged tree only; open one PR against `v3`; merge after eligibility; rebase the next wave onto the updated `v3`; fold changelog fragments; then clean up in order: delete each worktree packages/target, remove worktrees, delete local branches, delete remote branches, nix-collect-garbage, and audit `git worktree list` plus `git branch -a` for residue
...
| W4 | 6 | **6** |
| W5 | 12 manifest groups + completion graph | **12** for the original manifest groups; after T589 the completion graph has **5** file-disjoint implementation slices, then one serial daemon composition slice, then **5** file-disjoint acceptance/docs slices including T604 |
...
| W5 (`adr046w5`) | 12 manifest groups + completion graph | T603 is the receipt/editor-mediated resume gate and sole direct prerequisite of T589; the original store chain remains semantically ahead of T589; T589 precedes six disjoint completion slices T590-T594 and T605, T595 serializes daemon composition after all six, T596-T599 plus T604 fan out, and T600-T601 share one frozen candidate |
...
T603 reconciles the amended resume boundary through the external reconciliation and editor
receipts; T589-T602 plus T604 ensure the production risk is retired by operator and
production-boundary evidence rather than a readiness substitute, and T605 supplies the
approved contract and no-drift evidence. C1 is fully assigned but not yet implemented;
analysis, unanimous plan signoff, and T603 still gate resume. W6 carries the highest volume
(257 items).
