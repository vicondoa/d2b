...
- [ ] T569 [US4] Verify each companion by exercising it against the release candidate on a live host - `d2b-toolkit`, `d2b-wlterm`, `d2b-wlcontrol`, `d2b-clip-picker`; `weezterm` consumes no d2b contract (FR-040, SC-024). Per FR-061 the record must name the exact candidate, the companion revision, every surface exercised, and the result; source inspection, a matching version number, and the fact that the contracts were published at W5 are each explicitly not evidence here. Per FR-063 classify **every named surface** as Conformant, Blocked, or Retired, and default to Blocked whenever the outcome cannot be classified: a row with one Blocked surface is Blocked. A capability-conditional refusal is Conformant only if it names the false capability key or refusal state and at least one concrete operator action - a silently greyed control is Blocked. A failure at this point is the detection event FR-062 names, and its response is to hold the release or amend FR-045, never to relax FR-040
