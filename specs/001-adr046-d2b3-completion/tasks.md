...
- [ ] T558 [US4] W8 PLAN PANEL + ENTRY - depends on T557 and requires W7 exit criteria to hold. After T557 fixes the triaged work-item set, file ownership, and validation map, run `/d2b-panel-round plan` against the exact clean W8 implementation base and feature snapshot and require 10/10 sign-off with zero recommendations before any W8 implementation lane is dispatched. A task-map, ownership, feature, or base change invalidates the receipt and requires another plan review. This plan gate is distinct from T565's post-convergence work panel.
