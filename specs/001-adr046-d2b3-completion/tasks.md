...
- **[P]**: Free to start immediately within its wave - no incoming dependency or file-overlap
  edge. 91 of 545 manifest work items qualify. Across the full task list, 100 of 605 total
  tasks carry `[P]`.
...
- [ ] T559 [US4] W8 IMPLEMENT + CONVERGE - execute the triaged items (count known only after T557), merge every W8 slice into the wave integration branch, rebase after the sealed W7 merge, run integration tests and CI on the converged tree, and resolve every content-changing result. No slice branch may remain to merge after T559, and T559 MUST NOT issue a binding panel request, panel-attest, or seal.
