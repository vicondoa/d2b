...
Constitution 2.1.0 permits pipelined dispatch, but `ADR-046-validation-and-delivery` section 4 still
...
- [ ] T035 [US1] W3 GATE - run the native Copilot pre-panel procedure below FIRST: dispatch a read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`, then run `/d2b-panel-round work`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; clear every CRITICAL finding before snapshot, import validation evidence, panel-request (refused unless every prior-wave work item is Merged and W3 is rebased after the predecessor merge), panel-attest (10/10 unanimous), seal (every prior-wave and wave item Merged), merge-target, and merge-eligibility. Also confirm for every item in this wave: reference docs landed with their behavior (FR-019), no change contradicts a decision in the register (FR-047), and every removal proof for a path retired in this wave passed (FR-023). From round 9, LOW/MEDIUM may be deferred to deferred-findings.md; CRITICAL/HIGH never (FR-051, FR-052). At wave close, review both registers and log this wave friction in friction-log.md (FR-053)
...
- [ ] T070 [US1] W4 GATE - run the native Copilot pre-panel procedure below FIRST: dispatch a read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`, then run `/d2b-panel-round work`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; clear every CRITICAL finding before snapshot, import validation evidence, panel-request (refused unless every prior-wave work item is Merged and W4 is rebased after the predecessor merge), panel-attest (10/10 unanimous), seal (every prior-wave and wave item Merged), merge-target, and merge-eligibility. Also confirm for every item in this wave: reference docs landed with their behavior (FR-019), no change contradicts a decision in the register (FR-047), and every removal proof for a path retired in this wave passed (FR-023). From round 9, LOW/MEDIUM may be deferred to deferred-findings.md; CRITICAL/HIGH never (FR-051, FR-052). At wave close, review both registers and log this wave friction in friction-log.md (FR-053)
...
```text
T073-T218 obligations -> T603 -> T589 -> {T590,T591,T592,T593,T594,T605} -> T595
...
T590-T594 and T605 are six file-disjoint slices and launch together from the T589 prep commit.
...
- [ ] T592 [P] [US1] **Complete durable store identity recovery and the authoritative mutation-audit journal/export drain.** Depends on T589. Owned files: `packages/d2b-resource-store-redb/src/{lib.rs,actor.rs,audit.rs,migration.rs,backup.rs,tests.rs}`, `packages/d2b-audit/src/{lib.rs,sink.rs}`, and `docs/specs/ADR-046-telemetry-audit-and-support.md`. `transaction.rs` is read-only after T589 freezes its journal hook. Separate immutable store/Zone identity from mutable policy, active-configuration, and controller revisions so reopen reads the latter from durable metadata after any advance. In the same redb transaction as every privileged mutation, create immutable authoritative journal rows keyed by a fixed domain-separated operation digest plus bounded mutation ordinal; export completion is separate mutable state and cannot clear or rewrite the authoritative row. Keep raw operation, correlation, authoritative subject, Zone, and resource identifiers only in private replay state. Audit rows, segments, and exports carry fixed 32-byte digests derived with distinct `d2b:audit:<class>:v1` domains; every custom `Debug` is redacted. Persist only syntactically and semantically validated propagated trace context, never an unvalidated trace value or a resource/operation identifier relabelled as trace context. Persist a replay-binding digest over the registrar-derived subject, Zone, canonical semantic request, target, verb, exact expected revision, operation ID, and idempotency data. Same-ID status or resume MUST match that digest before returning pending/final state; mismatch is audited and denied without reapplication. The drainer exports each immutable row durably once, keyed by operation digest plus ordinal, then durably marks export completion before ordinary success. Incomplete export returns T589's protobuf-representable `CommittedPendingAudit` composite and never implies rollback. Enforce the governing configured retention days, maximum records per segment, and maximum segment bytes at startup and rotation; a prune or limit-enforcement failure produces typed degraded Zone health and blocks publication. Bump the governing audit specification `Version` and move its authority/export/retention language with the implementation. **Done when** crash tests at mutation/journal commit, append, sync, completion, rotation, and prune boundaries prove no committed privileged mutation lacks an immutable authoritative row, multi-mutation replay yields one exported record per ordinal, same-ID apply count stays one, cross-subject/Zone/request/target/verb/revision/idempotency and restart mismatches deny, validated absent/present trace context round-trips, raw-identifier canaries are absent from journal/segments/exports/errors/logs/metrics/spans and `Debug`, configured limits are enforced, prune failure degrades health, advanced revisions reopen, and production construction cannot select `NoopMutationAudit` or `enabled() == false`.
- [ ] T593 [P] [US1] **Publish the authenticated Resource API and watch route.** Depends on T589. Owned files: `packages/d2b-bus/src/{router.rs,registry.rs,operations.rs,streams.rs}`, `packages/d2b-resource-api/src/{adapter.rs,watch.rs}`, `packages/d2b-session-unix/src/{pidfd.rs,subject.rs,zone_admission.rs}`, and new focused tests under `packages/d2b-bus/tests/production_resource_route.rs`. Replace the unregistered production seam with a route whose registration consumes the authenticated ComponentSession admission. At Unix accept, bind `SO_PEERCRED` to a live `pidfd_open` handle and expected process generation/cgroup evidence before issuing the single-owner peer capability; the numeric PID alone is never identity, the pidfd is `CLOEXEC`, and admission consumes the bound evidence. `ZoneRegistrar` exclusively derives and propagates the subject from that evidence; requests and stream frames carry no subject claim. Reject `ESRCH`, a dead pidfd, PID reuse/start-time mismatch, expected generation/cgroup mismatch, or ambiguous evidence. Register exact-Zone ResourceService and controller routes, admit watch replay/live delivery through ZoneBus, and expose one registration/readiness observation from actual owned handles. **Done when** same-Zone authenticated Get/List/Watch reaches the real service, cross-Zone, self-named, unregistered, reused-admission, direct-WatchService, numeric-PID-only, reused-PID, mismatch, `ESRCH`, and ambiguity paths are denied, and neither `UnregisteredBusAdapter` nor a fixed endpoint can satisfy production publication.
...
- [ ] T598 [P] [US1] **Add authoritative audit, pending-result, replay-binding, retention, and redaction acceptance coverage.** Depends on T595. Sole owned file: new `packages/d2bd/tests/resource_plane_audit.rs`. Mutate through the authenticated production Resource API, including a multi-mutation batch; crash at every mutation/journal commit, segment append, sync, export-completion, rotation, and prune boundary; reopen; and compare immutable authoritative journal rows with exported logical records by fixed operation digest plus mutation ordinal. Include sink unavailable, disabled callback, incomplete export, hash-chain mismatch, duplicate replay, configured retention/record/byte-limit enforcement, and prune-failure typed-health negatives. Prove the journal row commits transactionally with the privileged mutation before any effect is success-shaped; segment export and its completion cursor are separate and cannot rewrite or delete it. After committed export-pending state, require `CommittedPendingAudit` through T589's `PendingAuditStatus` protobuf field, including `DeleteResponse` and batch ordinals, with the exact canonical `ResourceStatus` composite and no ordinary success or rollback claim. Retry/status the same operation ID before and after restart only with an exact replay-binding match to the original registrar-derived subject, Zone, canonical semantic request, target, verb, expected revision, and idempotency data; prove cross-subject, cross-Zone, altered-request/target/verb/revision/idempotency, and restart mismatches are denied and audited without observation or reapplication. Retry a different ID and prove normal revision/conflict behavior. Inject distinct raw operation, correlation, subject, Zone, and resource canaries; require only domain-separated fixed digests in journal rows, audit segments, and exports, and require no raw canary in errors, logs, metrics, spans, or redacted `Debug`. Cover absent and valid propagated trace context plus invalid-context rejection. **Done when** every committed privileged mutation has an immutable authoritative row at commit, ordinary success waits for durable export completion, multi-mutation restart yields exactly one export per ordinal, same-ID apply count is one, all replay-binding mismatches deny, the exact composite round-trips through every mutation response, canaries remain absent, configured limits prune correctly, prune failure degrades health, status observability is stable across restart, and every audit/export failure leaves the affected Zone unpublished with an actionable typed refusal.
- [ ] T599 [P] [US1] **Reconcile Wave 5 CLI and reference promises with emitted behavior (FR-019, FR-074).** Depends on T595. Sole owned files: `packages/d2b/src/{dispatch.rs,resource.rs,context.rs}`, `docs/reference/{zone-cli-contract.md,desktop-wrapper.md,companion-contracts.md,cli-contract.md,components-audio.md,components-usbip.md,components-usb-security-key.md,resource-client.md}`, and `packages/d2b-contract-tests/tests/{policy_cli_consumers.rs,policy_docs.rs}`. Implement and test the exact recovery contract in `contracts/operator-cli.md`: every mutating generic and typed resource verb accepts `--operation-id <OPAQUE_ID>`; an initial call emits its operation ID; the identical command plus that flag reuses the original operation/idempotency binding; and `d2b --zone <ZONE> op inspect --operation-id <OPAQUE_ID> [--watch]` is the sole status query. Pin mutation and inspection exit codes plus exact human/JSON pending/final/not-found/refusal shapes. Compare exact `d2b --help`, subcommand help, JSON output, capability keys, typed refusals, and public wire fields. Resource status documentation must expose committed-pending-audit through T589's additive protobuf status field and the exact `ResourceStatus.phase`, `outcome.code`, `update.state`, and `update.operation_id` composite; show only bounded, redacted same-ID retry/status remediation; and never claim success or rollback. Reconcile every downstream status consumer owned by this task with T605's paired contract and T595's emitted `Zone.status.handlers[]`: system-core readiness is attributed to `Provider/system-core` plus exactly one `system-core-host` and one `system-core-user`; underscore labels and `provider-lifecycle` cannot substitute. Compare candidate output to the frozen documented surface. Candidate absence of a command or field is a defect, not permission to delete its promise, unless the same change follows the explicit parity or FR-042 retirement path with replacement, migration guidance, owner, release treatment, and contract tests. Do not add a fallback or claim companion verification. **Done when** every documented desktop-wrapper, companion, audio, USB, security-key, and resource operation is present in emitted behavior or has an approved parity/retirement record; pending-audit recovery and exact exits/forms match the operator contract; T595's emitter and all T599-owned consumers match T605's exact names and non-substitution rule; and the focused docs/schema checks owned by T599 are clean. T220 performs the final integrator-owned generated-manifest reconciliation and full drift gate.
- [ ] T220 [US1] `adr046w5` CONVERGE + FREEZE - depends on T596-T599 and T604. Before any exact-candidate evidence or binding panel, merge every slice branch into the wave integration branch, reconcile T605's normative version changes into the integrator-owned generated spec manifests, verify T595's emitter and T599's consumers against T605, fold changelog fragments, then, after W4 is sealed and merged, rebase onto the updated `v3` and record that exact `v3` commit as the panel-base eligibility proof. Run integration tests, full `make test-drift`, and CI on the converged tree, and resolve every content-changing result. Open or update one PR against `v3`, then freeze its clean HEAD and tree as final candidate F. T220 MUST NOT invoke a panel command, panel-request, panel-attest, or seal. After F is frozen, any content change or rebase invalidates F and requires T220 plus T600-T602 to rerun before T219 may issue the one binding panel request.
- [ ] T600 [US1] **Capture exact-candidate production-boundary evidence.** Depends on T220 and runs read-only after F is frozen. Owns no repository files; import delivery evidence records only. T600 exclusively owns these five closed `EvidenceRecord.validation` identifiers: `production-session-watch`, `effect-replay-cleanup`, `audit-drain-replay`, `system-core-handler-contract`, and `operator-nix-activation-cleanup`. Run the authenticated same-Zone/cross-Zone watch matrix with pidfd-bound admission, fresh restart pidfd, and PID-reuse/mismatch/`ESRCH`/ambiguity refusals; restart crash-window/effect replay; cleanup stale/zero/wrong-UID negatives; and the authoritative journal/export matrix including all crash points, multi-mutation ordinals, replay-binding cross-subject/Zone/request/restart denials, retention/prune health, raw-identifier canaries, the exact protobuf-represented `ResourceStatus` pending composite, and same-ID/different-ID behavior. Also run the exact `Provider/system-core` registration/handler-health matrix, three-Zone startup/close isolation, and T604's public-switch activation-to-effect-and-cleanup result. Import T605's exact wire round-trip, underscore and duplicate/missing/wrong-name rejection, `ProviderLifecycle` non-substitution, current API snapshot, paired normative/reference/version result, targeted contract test, and unchanged desired-schema drift evidence. Every record must name F, F's tree, and the production entry point. Reject direct ResourceService/WatchService calls, `ProductionWatchHarness`, fixed/fake endpoints, constructed subjects, numeric-PID-only identity, status-only Provider/readiness substitutes, manually set readiness, evidence from an earlier commit, an unknown identifier, or a duplicate identifier. **Done when** T600 emits exactly its five assigned identifiers once each for F; the operator record identifies emitted bundles and all four representative owned effects; the handler-contract record is candidate-bound and complete; same-ID audit retry applies once and replay-binding mismatches deny; no raw canary escapes; PID negatives pass; malformed status fields cannot round-trip; and every command passed.
- [ ] T601 [US1] **Capture exact-candidate RSS, owner fan-in, removal, and reference evidence.** Depends on T220 and runs read-only in parallel with T600 subject to the heavy-gate limit. Owns no repository files; import delivery evidence records only. T601 exclusively owns these three closed `EvidenceRecord.validation` identifiers: `resource-plane-rss-owner-fanin`, `wave5-removal-proofs`, and `cli-reference-conformance`. Measure the full daemon-owned publication path at 10,000 resources and 100 authenticated watches with no baseline subtraction; prove one store owner, one policy owner, one ResourceService route, one controller endpoint/fan-in, and one authoritative audit journal/export owner per Zone. The `Provider/system-core` registration and handler records belong only to T600's `system-core-handler-contract`. Re-run every manifest-label W5 removal proof at F instead of citing `removal-proof-w5.md`'s historical `a7f4a6a4` snapshot. Compare emitted CLI/help/JSON/wire behavior, including exact same-ID retry/status commands, exit codes, and human/JSON forms, with all T599 pages. Do not re-emit T600's operator-activation or handler-contract kinds. **Done when** T601 emits exactly its three assigned identifiers once each for F; RSS is <=24,576 KiB, owner counts are exactly one, all current removal-proof predicates are true, docs match emitted behavior, and every record names F and F's tree.
- [ ] T602 [US1] **`adr046w5` PRODUCTION COMPLETION CONVERGENCE - mechanically unblock T219.** Depends on T600 and T601. Owns no implementation files. Verify T603, every T589-T599 task, T604, T605, and T220 are complete and `tasks.md` shows T073-T218 and T603 checked. Validate immutable authorization receipt R against repository identity, relative feature path, resume base B, tree B, and pre-edit snapshot P; validate progress receipt E against R's digest, authorized post-edit snapshot Q, dedicated checkbox commit C, exact parent `C^ = B`, and exact 147-token `B..C` diff; require C to be an ancestor of final candidate F. Do not compare R or E to final HEAD as though either were final-candidate evidence. Read the imported records and compare the multiset of `(lane, validation)` pairs byte-for-byte with the closed table in `plan.md`: exactly the five T600 identifiers and three T601 identifiers, each at its assigned lane and exactly once, with no unknown, duplicate, missing, extra, or conflated record. Require all eight records to bind F and F's tree after T220, with T604's public-switch/all-representative-effect/cleanup result present only under `operator-nix-activation-cleanup`, T605/T595/T599 coordinated contract evidence present only under `system-core-handler-contract`, and no ineligible direct/fake boundary. Require HEAD exactly equals F, `git diff --cached --exit-code` is empty, `git diff --exit-code` is empty, and `git status --porcelain=v1 --untracked-files=all` reports no staged, unstaged, or non-ignored untracked path; ignored external receipts under `.scratch/` are not candidate content. RSS, owner counts, pidfd-bound admission negatives, the exact `Provider/system-core` registration, unique `system-core-host`/`system-core-user` records, underscore and `ProviderLifecycle` non-substitution, transactional authoritative audit rows, export/retention health, raw-identifier canaries, protobuf-represented pending status, same-ID no-reapply/final-result plus replay-binding mismatch denial, all four representative owned effects, current removal predicates, dependency-safe cleanup, ready unrelated resources, targeted contract/API/drift gates, and exact CLI recovery/docs comparison must all pass. Any false conjunct blocks T219 and names the failed remediation. Historical validation remains historical and is not reclassified.
...
- [ ] T479 [US2] W6 GATE - run the native Copilot pre-panel procedure below FIRST: dispatch a read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`, then run `/d2b-panel-round work`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; clear every CRITICAL finding before snapshot, import validation evidence, panel-request (refused unless every prior-wave work item is Merged and W6 is rebased after the predecessor merge), panel-attest (10/10 unanimous), seal (every prior-wave and wave item Merged), merge-target, and merge-eligibility. Also confirm for every item in this wave: reference docs landed with their behavior (FR-019), no change contradicts a decision in the register (FR-047), and every removal proof for a path retired in this wave passed (FR-023). From round 9, LOW/MEDIUM may be deferred to deferred-findings.md; CRITICAL/HIGH never (FR-051, FR-052). At wave close, review both registers and log this wave friction in friction-log.md (FR-053)
...
- [ ] T580 [US3] **Implement and prove the recovery-point attestation gate** (FR-043, SC-025). Depends on T494, T500, and T502. The cutover MUST refuse to execute any step past its rollback boundary until the operator has attested that a host recovery point exists, and every attestation MUST be recorded. This remains program-local and deliberately outside the work-item manifest because it is stricter than `ADR-046-reset-and-cutover`; the manifest census therefore cannot prove it. Run T502's `tests/integration/live/cutover-real-host.sh` FR-043 matrix against the exact converged W7 candidate and import exactly one candidate-bound `EvidenceRecord` whose `validation` is `recovery-point-attestation`. That record may exist only when the no-attestation case proves that no post-boundary step executed and the attested case proves the attestation was recorded. **Done when** the matrix exits zero, the record names the exact candidate commit and tree, and T580 is checked; absent, failed, duplicate, or stale evidence leaves T580 incomplete and blocks T555.

- [ ] T555 [US3] W7 GATE - depends on T580. Before dispatching either native pre-panel lane, require T580 checked and import exactly one passing `recovery-point-attestation` record bound to the current W7 candidate commit and tree. Missing, failed, duplicate, stale, or wrong-candidate evidence refuses the pre-panel dispatch and panel request; recheck the same record before panel-attest, seal, merge-target, and merge-eligibility. Then run the native Copilot pre-panel procedure: dispatch a read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`, then run `/d2b-panel-round work`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; clear every CRITICAL finding before snapshot, import validation evidence, panel-request (refused unless every prior-wave work item is Merged and W7 is rebased after the predecessor merge), panel-attest (10/10 unanimous), seal (every prior-wave and wave item Merged), merge-target, and merge-eligibility. Also confirm for every item in this wave: reference docs landed with their behavior (FR-019), no change contradicts a decision in the register (FR-047), and every removal proof for a path retired in this wave passed (FR-023). From round 9, LOW/MEDIUM may be deferred to deferred-findings.md; CRITICAL/HIGH never (FR-051, FR-052). At wave close, review both registers and log this wave friction in friction-log.md (FR-053)
- [ ] T556 [US3] W7 CONVERGE + MERGE - depends on T555 and refuses PR merge unless T580 remains checked and the one passing `recovery-point-attestation` record still names the exact candidate commit and tree approved by T555. Any content or history change after that record was produced invalidates it and returns the flow to T580 and T555. Merge every slice branch into the wave integration branch; run integration tests, panel, and CI against the converged tree only; open one PR against `v3`; merge after eligibility; rebase the next wave onto the updated `v3`; fold changelog fragments; then clean up in order: delete each worktree packages/target, remove worktrees, delete local branches, delete remote branches, nix-collect-garbage, and audit `git worktree list` plus `git branch -a` for residue
...
2.1.0** to permit pipelined dispatch under exactly these four conditions. The ADR-046 delivery
spec and its tooling still enforce the stricter rule and must be amended before the pipeline
is executable - see T585-T588.
...
| W5 (`adr046w5`) | 12 manifest groups + completion graph | T603 is the two-pass receipt/editor-mediated resume gate and sole direct prerequisite of T589; the original store chain remains semantically ahead of T589; T589 precedes six disjoint completion slices T590-T594 and T605, T595 serializes daemon composition after all six, T596-T599 plus T604 fan out, T220 converges and freezes F, and T600-T601 share that immutable candidate before T219's one binding close |
