...
non-authorizing, and lands a versioned double-opt-in migration plus a work-item amendment that
assigns the production implementation and all four Network/Host cases to owners that merge
before T220. The current W6 assignment T336-T355 cannot satisfy that pre-T220 dependency.
A feature-local matrix, single-opt-in assertion, declaration-only fixture, fake adapter, or
evidence from the old env surface cannot resolve the conflict. T220 and T219 must fail closed
until the accepted amendment, real emitter/controller/broker/net-VM implementation, and all
four cases are ancestors of final F. T604 remains W6 acceptance-only and reuses that landed
path; no feature-local status correction can unblock any boundary.
...
must assign the production implementation and four-case matrix to pre-T220 owners; current
W6 rows T336-T355 are a blocking manifest conflict until moved or replaced. Wave 5 does not
claim T604's positive operator activation result. T604 remains W6 acceptance-only and
consumes the Network path already required before T220. Guest runtime-effect acceptance
...
- [ ] T220 [US1] **`adr046w5` CONVERGE + SELECTED-ROSTER PHASE LIFECYCLE + FREEZE.** Depends on T596-T599 and on the accepted external Network contract/work-item amendment assigning the production Network remediation before T220. Merge every Wave 5 slice, including the amendment-generated Network owners, reconcile owned generated manifests and the twelve source-writing changelog fragments, and verify that accepted Version 2 plus `ADR-046-validation-and-delivery-traceability.{json,md}` were complete before T589. Require an exact generated bijection for all Wave 5 `VD2-SC002-*` identifiers and the complete T599 runbook/action mapping; copied SC-002 prose and declaration-only fixtures fail. Before freezing F, require the accepted double-opt-in migration, the real emitter/controller/broker/net-VM implementation, and enforcing results for Network false/Host false, Network false/Host true, Network true/Host false, and Network true/Host true. The current T336-T355 W6 assignment cannot satisfy this dependency; until the external manifest moves or replaces that ownership, T220 refuses with remediation to amend the Network contract and work-item graph. T604 remains W6 acceptance-only and is excluded from the exact-seven Wave 5 profile. Run that exact-seven profile and all named enforcing gates. For each provisional candidate, create one current lifecycle selection and dispatch exactly its selected roster and profiles through comprehensive discovery and scoped verification. Freeze final F only when every selected seat is unanimous with no recommendations. T220 creates no binding delivery request, attestation, or seal; the retained request still requires T219 external disposition. Any content change or rebase invalidates F and reruns T220 plus T600-T602. **Done when** generated manifests and traceability are clean, the Network amendment/implementation/four-case matrix are ancestors of F, twelve fragments are folded, every selected lifecycle seat approves exact F, T600-T602 can bind F and its tree, and no unresolved SC-002, Network, runbook, or retained-request prerequisite remains.
...
**Current generated ownership is blocked and must be amended before T220:** these rows still
record the external manifest's W6 assignment, but that assignment cannot satisfy final R9.
Before any Network implementation starts, an accepted versioned
`ADR-046-resources-network` contract/work-item amendment must require
`Network.spec.isolation.allowEastWest && d2b.site.allowUnsafeEastWest`, default both false,
and move or replace the production adapter, site-gate transport, schema migration, and all
four real emitter/controller/broker/net-VM cases with generated owners that merge before
T220. Until regeneration, T336-T355 are not executable and T220 refuses rather than freezing
around them. T604 remains W6 acceptance-only, owns no Network implementation, and consumes
the already landed result; T219 and T479 revalidate its ancestry.
...
- [ ] T479 [US2] W6 CONVERGE + FREEZE + OPERATOR/GUEST ACCEPTANCE - depends on T039, T604, and every W6 work item in the authoritative manifest regenerated by the accepted Network amendment. Derive that exact set from the manifest, reject missing/extra/duplicate/unchecked/unreachable rows, and require every member to reach T479 in the dependency graph; no stale numeric interval or pre-amendment census is authority. After `adr046w5` merges, rebase W6, confirm the accepted double-opt-in migration, pre-T220 production Network path, and four-case evidence remain ancestors, merge every remaining W6 slice plus T604, run integration/CI, fold W6 fragments, and freeze clean proposed F6. On that exact candidate, run no-skip `make test-host-integration` for `resource-operator-activation`, `runtime-cloud-hypervisor-guest-acceptance`, and `daemon-restart-vm-survival`. Require real Volume/Network/Device effects and cleanup, real Cloud Hypervisor/KVM and authenticated guest-control readiness, and FR-075 continuity. Import exactly one F6-bound `operator-nix-activation-cleanup` record from T604 and one `w6-cloud-hypervisor-guest-acceptance` record from T384; fake, status-only, refusal, skipped, empty, stale, wrong-family, or wrong-candidate evidence is ineligible. T479 issues no binding request, attestation, or seal. Any content, merge, generated-output, fold, rebase, or evidence-identity change invalidates F6 and reruns both acceptance lanes.
...
| W5 (`adr046w5`) | 12 manifest groups + completion graph + amendment-generated Network scope | T603 -> T589 -> production composition -> T595 -> T596-T599; the accepted double-opt-in migration, real production Network path, and all four cases must also merge before T220 -> `{T600,T601}` -> T602 -> T219 |
| W6 | groups remaining after the accepted Network work-item amendment + T604 acceptance | T221 gates all remaining Provider dossiers; T604 stays acceptance-only and consumes the Network implementation already required before T220, then T479/T480 bind its W6 result |
...
C1 is fully assigned but not yet implemented. The current W6 manifest still contains
T336-T355, but final R9 makes that placement a hard external-amendment blocker: regenerated
owners for the production Network path and four-case matrix must merge before T220.
...
- **After `adr046w5`** - a partial US1 production-plane checkpoint exists only if T602 and
  ...
  controller, audit, restart, CLI, reference, and required double-opt-in Network production
  boundaries without claiming the later three-resource operator positive. A ready-looking
  skeleton, direct service fixture, sole Network opt-in, or post-T220 Network owner has no
  incremental value and does not satisfy this checkpoint. T604 consumes the already landed
  Network path on proposed F6 without owning production files, and T479/T480 accept the
  operator and Guest results together.
