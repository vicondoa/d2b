...
evidence from the first-dispatch base. Otherwise, before any further implementation,
...
`w4-plan-review-remediation` receipt. It guards future dispatch only, leaves the historical
...
completion. The "Wave 5 acceptance set" means the representative Volume, Network, and Device
that T604 exercises together. It does not assign implementation ownership: Network
...
- [ ] T604 [P] [US1] **Prove exact-candidate operator activation through effect and cleanup.** Depends on T595 and is file-disjoint with T596-T599. Sole owned files: new `packages/d2b-contract-tests/tests/resource_operator_activation.rs`, new `packages/d2bd/tests/resource_operator_activation.rs`, new `tests/host-integration/resource-operator-activation.nix`, and only the host-integration discovery/build recipe in `Makefile`. The fixture-backed contract test proves that an operator declaration emits the exact pinned `zones/<zone>/resource-bundle.json` generation and that removing one declaration emits the corresponding generation delta; run it through `D2B_ENABLE_FIXTURE_BUILD=1 make test-fixture-contracts`. The Type-3 d2bd test is the lowest feasible production-boundary test: consume those exact generation bytes through the daemon startup/change-ingestion entry and production store/controller path, never through a direct ResourceService/WatchService call, and run it through `make test-rust`. The Type-10 test is additionally required because NixOS activation, systemd, broker mutation, and real owned host effects cannot be proved below `runNixOSTest`; run it only through the public heavy-gated `make test-host-integration` target. Preserve that existing target rather than adding a top-level gate, but make empty `vmChecks` discovery fail and emit the enumerated and built attr names. T604 evidence is ineligible if the target skips, discovers an empty set, omits, or does not build `vmChecks.x86_64-linux.resource-operator-activation`; a non-x86 skip is never passing evidence. In the positive host test, declare one Zone with the Wave 5 acceptance set - the representative Volume, Network, and Device - in Nix, switch through T595's public target-closure deployment entrypoint rather than raw `nixos-rebuild`, and require each of the three to reach its real broker/controller-owned effect and readiness. This is acceptance scope only; Network implementation remains owned by Wave 4. A status-only path or actionable refusal is ineligible for the positive story; refusal cases run separately as negative tests. Repeat an identical deployment and prove no duplicate ingestion or effect. Remove the declared Device, deploy the next emitted generation without a manual `systemctl restart`, `reload`, private RPC, or test-only hook, observe dependency-safe cleanup of its owned effect, and prove the Volume, Network, and unrelated resources remain ready, intact, and unrecreated. Guest runtime-effect ownership is deferred to the Wave 6 Guest Provider: Guest emission, ingestion, status, or refusal is not positive T604 evidence and cannot close FR-072 or SC-034. Bind all test records and both bundle content hashes to the same exact candidate. **Done when** declaration and removal generations are pinned; startup and both public deployments automatically reach production daemon ingestion; all three representative resources have positive owned-effect/readiness evidence; the identical deployment is idempotent; Device cleanup completes in dependency-safe order; Volume, Network, unrelated resources, and durable identities are unchanged; no Guest success is claimed; no extra unit exists; all three public gates pass on the same candidate; and host evidence records exact enumeration and a successful build of `vmChecks.x86_64-linux.resource-operator-activation` with no skip. Passing T604 proves only the Wave 5 partial US1 production-plane checkpoint, not full US1 completion.
...
- [ ] T221 [US2] W6 PLAN PANEL + ENTRY - before any W6 implementation lane is dispatched, confirm Gate 0 passed, destinations are uncontended, the stack is proposed against the exact named parent commit, the heavy-gate semaphore is available, and the fast hermetic suite is green on the entry tree; then run `/d2b-panel-round plan` against that exact clean base and feature snapshot and require 10/10 sign-off with zero recommendations. If `adr046w5` is not yet merged, implementation entry additionally requires at least 5 of its 10 work reviews returned and green integration on its converged tree. A base or feature change before dispatch invalidates the plan receipt. This authorizes implementation only: T480's distinct work panel, seal, and merge eligibility remain blocked until `adr046w5` is sealed and merged and W6 is rebased onto the updated integration lineage (FR-057).
...
- [ ] T479 [US2] W6 CONVERGE + FREEZE - depends explicitly on T039 and on every W6 work-item row T222-T478. Do not use the numeric interval alone: derive the exact W6 work-item ID set from the authoritative manifest, require 258 unique rows, compare it with `{T039} union {T222-T478}`, and fail on any missing, extra, duplicate, unchecked, or unreachable member. Mechanically traverse the task dependency graph from every manifest-derived W6 task and require each to reach T479 before freeze. After `adr046w5` is sealed and merged, rebase W6 onto updated `v3`; merge every slice branch into the wave integration branch, run integration tests and CI on the converged tree, resolve every content-changing result, then reconcile and fold all changelog fragments. Before freezing, confirm reference docs landed with behavior (FR-019), no change contradicts the decision register (FR-047), every required removal proof passed (FR-023), and the deferred-findings and friction registers are current (FR-051, FR-052, FR-053). Open or update one PR against `v3`, then freeze its clean HEAD and tree as F6. T479 MUST NOT issue a binding panel request, panel-attest, or seal. Any content change, slice merge, generated-output change, changelog fold, or rebase after F6 is frozen invalidates F6 and requires T479 to rerun.
- [ ] T480 [US2] W6 SINGLE BINDING WORK GATE + MERGE - depends on T479. Require HEAD and tree to equal clean F6, revalidate T221's unanimous plan-panel receipt and reviewed feature snapshot, and prove the reviewed entry base is an ancestor of every W6 implementation head. T480's work panel is not a substitute. Against F6, first dispatch the read-only reviewer Task lane and rubber-duck Task lane in parallel, each bound to `gpt-5.6-luna` / `max` / `long_context`; a content defect from either lane abandons F6 and returns to T479 before any binding panel request. Only after both pre-panel lanes are clear, run F6's one binding `/d2b-panel-round work`, whose ten seats bind `github-copilot` / `gpt-5.6-sol` / `xhigh` / `default`; import F6-bound validation evidence, panel-request (refused unless every prior-wave work item is Merged and W6 is rebased after the predecessor merge), panel-attest (10/10 unanimous), seal (every prior-wave and wave item Merged), register merge-target, pass merge-eligibility, and then merge the already-open PR. A nonunanimous result permanently fails F6 but preserves scoped recovery: retain its records, route only its recommendations through a fix round, return to T479, rerun convergence and validation, freeze a distinct successor, and run that candidate's delta/full-context follow-up panel before its single binding request. Never re-request or modify F6; a policy/tooling refusal is an integrator escalation, not a waiver. From binding panel request through disposition of F6, F6 and its tree are immutable. The merge MUST preserve the successful candidate's tree byte-for-byte. After merge, rebase the next wave onto updated `v3`, then clean up in order: delete each worktree `packages/target`, remove worktrees, delete local branches, delete remote branches, run `nix-collect-garbage`, and audit `git worktree list` plus `git branch -a` for residue.

**Checkpoint**: W6 converged, panelled, sealed, merged to `v3`, rebased, and cleaned up.
Full US1 completion is placed here, after the Wave 6 Guest Provider, and is recognized only
when its positive production-boundary acceptance shows the declared Guest reaching a real
Provider-owned runtime effect and ready state alongside the previously accepted Volume,
Network, and Device. Missing, skipped, status-only, fake-boundary, or actionable-refusal
evidence leaves US1 incomplete. Successor entry criteria satisfied.
...
- **After W6** - full US1 completes only after the Wave 6 Guest Provider has positive
  production-boundary runtime-effect and readiness acceptance for the declared Guest;
  missing, skipped, status-only, fake-boundary, or refusal evidence leaves US1 incomplete.
