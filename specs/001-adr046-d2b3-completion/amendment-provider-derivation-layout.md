...
3. the program is verified by the **same bounded walk, applied independently and
   anchored to its own store output**: the same relative-link, no-`..`,
   no-magic-link, 8-link ceiling and same-output confinement rules, terminating
   at a regular file. It is not required to be ELF or to carry an execute bit,
   since the interpreter reads it rather than executing it. The two walks share
   no state, and a failure names which of the two inputs failed; the resolved
...
The eighteen scenarios gated by neither, named so the list can be checked:
...
and `nix-runtime-launcher-anchored-resolution`. Twenty-three in total.
...
| `provider-layout-entry-unexpected` | `share/d2b/provider/` holds an entry other than the three of 4.9.2, violating the closed-directory invariant | artifact ID, up to four offending entry names in sorted order each truncated to 64 bytes, and the total count |
