...
contract/work-item amendment must place the double-opt-in production path and all four cases
before T220; current W6 rows T336-T355 are a blocking ordering conflict until moved or
replaced. T604 remains the W6 operator activation boundary and consumes that already landed
path after T221. The feature task total remains 605 until the external manifest authority
accepts and regenerates its own census.
...
- separate W6 T604 exact-F6 coverage, after T221 and authoritative T336-T355, from an operator Nix declaration and emitted bundle through
  ...
  evidence-profile member. The accepted double-opt-in migration, production Network path,
  and four-case matrix remain separate pre-T220 Wave 5 prerequisites;
...
| FR-066 - FR-072, FR-074 | Operator-approved Wave 5 production-completion amendment plus analysis remediation | T603 uses the exclusive editor plus checkbox-only Git commit; accepted Version 2 and generated `VD2-SC002-*` traceability own all SC-002 protocol detail; T599 owns the versioned recovery runbook/action mapping and client-side response-loss operation ID; T220 freezes only after Wave 5 generated traceability, selected-roster lifecycle approval, the exact seven evidence identifiers, and the accepted Network amendment plus pre-T220 production implementation and all four Network/Host cases. Current W6 rows T336-T355 are an explicit external-manifest blocker, not a post-freeze substitute. T604 remains W6 acceptance-only and emits `operator-nix-activation-cleanup` for T479 on exact F6. The manifest census changes only through that accepted external work-item amendment; this feature does not silently reassign rows. |
