...
| --- | --- |
| Host authorization and recovery | Initial public-socket `SO_PEERCRED`/group Admin classification is consumed into one sealed nonfabricable capability. T592 owns a durable broker coordinator before first mutation and the exact bootstrap-to-target-broker ownership transfer; target broker/daemon startup and bootstrap crash boundaries run without daemon recovery ownership or a new unit. |
