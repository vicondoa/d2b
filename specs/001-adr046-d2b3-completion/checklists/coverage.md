...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt; a failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import and cleanup share one exclusive candidate OFD lock. Ordinary cleanup empties both ephemeral namespaces. Identity ambiguity never unlinks and instead retains a durable incident quarantine entry or ambiguous name that blocks publication and close. |
