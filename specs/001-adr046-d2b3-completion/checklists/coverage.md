...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt through the shared typed hash oracle; no raw locator hash exists. A failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import, cleanup, incident transitions, successor admission, and the retention guard share one exclusive candidate OFD lock; cleanup loses before namespace access against every live owner, and only private nonserializable `SidecarCleanupOwner` can open or mutate a cleanup namespace. Ordinary cleanup empties both ephemeral namespaces and never uses check-then-unlink. Every incident state remains inspectable by stable id, closed cause/remediation, human/JSON projection, and exits. One immutable structured preimage carries every kind-specific component and is repeated byte-identically across every durable path/status record. A verified expected payload reaches `parked`; authenticated no-unlink residue reaches `mismatch-retained`; unusable or unstable primary evidence reaches a separate resolution through a complete recursively enumerated census or identity-bearing bounded-failure commitment. The frozen primary scope binds every descendant and canonical failure-path digest, excludes resolution/request/disposition/freeze leaves, raw `01ff` is non-authorizing, and copied or changed commitments block successor admission. The successor triplet is durably frozen before the canonical authority request is signed, and request/disposition/apply/admit preserve one binding. A rename/reopen race is exactly resumable or irreconcilable with all names preserved and no false terminal status. Source-floor issuer authentication produces a private nonserializable result consumed into the private validated-floor result; copied authority/key digests produce neither. The exact 61 receipt, 45 malformed-census, source-floor 32/20/21, and mutation post-first 15 negative registries are independent. All incident states block parked-candidate publication and close while the authenticated request/apply plus fresh-successor path remains available. |
