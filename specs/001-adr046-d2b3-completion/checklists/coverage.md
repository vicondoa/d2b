...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt through the shared typed hash oracle; no raw locator hash exists. A failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import, cleanup, incident transitions, successor admission, and the retention guard share one exclusive candidate OFD lock; cleanup loses before namespace access against every live owner. Private `CandidateSidecarGuard` solely owns that lock fd, and `SidecarCleanupOwner<'guard>` exclusively borrows it. Current publication uses unnamed-inode zero-capability procfs-fd direct-final no-replace linking. Named temporary/quarantine state is legacy-observation-only; cleanup never renames, hardlinks, or unlinks it, and private `CandidateNamespaceWriteOwner` runtime negatives block parent-boundary loss, source replacement, and destination-reopen mismatch before later mutation. Only direct-final ordinary terminals require both ephemeral namespaces empty; every terminal legacy incident retains every original legacy source name at its frozen locator under the exact frozen recursive census. Every cause maps to a closed inspect/action/status/successor row; coverage denial exposes a bounded failure/root class, one owner procedure, and no disposition-request command until repair. One immutable structured preimage carries every kind-specific component before any direct-final payload/residue evidence copy. A verified payload reaches `parked`; authenticated no-unlink residue reaches `mismatch-retained` only with that exact frozen census; source absence is irreconcilable and can advance only through separate authenticated resolution. New observation emits stage `3`, while stages `1` and `2` remain historical decode-compatible inputs. The successor triplet is durably frozen before the exact 19-field request. Request output prepares an unnamed inode before candidate publication, procfs-fd links the exact inode directly to the final after candidate durability, and retains the internal pair on unsupported link. Source-floor validation uses one anchored stable close-on-exec lock inode, OFD-claims one non-clonable protected origin, and commits consumption only with durable dispatch; lock replacement and exec inheritance refuse. The exact 61 receipt, 73 malformed-census, 35 direct-final publication, source-floor 32/26/21, and mutation post-first 15 negative registries plus the seven-stage and seventeen-row recovery redaction registries are independent. All incident states block parked-candidate publication and close until their exact safe recovery path completes. |
...
| Round 23 closure | Round 23 planning used zero-capability procfs-fd exact-inode direct-final linking with no `AT_EMPTY_PATH`, linked temporary, or create-and-unlink probe; treated unsupported request-output link as a retained-internal output failure; guarded every existing-name cleanup move; rejected serialized unavailable nodes; consumed one protected source-floor origin into one validated capability; exposed exact coverage failure/root classes and owner procedures with no signing command until repair; provided selector-free coordinator pointer repair plus immutable-audit escalation; and pinned its then-current 61/73/26 SC-002 registries, seventeen recovery canaries, all six remediation rows, and 135-case handoff matrix over seven rollback members, 30 audit members, and 15 transitions. All external non-authorizing dispositions and external artifact escalations remained unchanged. |
| Round 24 closure | Current planning supersedes the Round 23 move/claim/repair shorthand: every new SC-002 immutable publication is direct-final, named cleanup state is legacy-only and never renamed/unlinked, and source-origin consumption commits only with durable dispatch under an anchored stable close-on-exec lock inode. The publication registry is 35 ids, including importer post-link/final-reopen mismatch. T592 owns Admin-only typed repair/restoration broker ops, coordinator lock, 32 handoff audit members plus a separate two-edge restoration audit fixture, immutable pre/outcome/provenance audit, and bounded retained backup; T595 owns both public clients. The exact 156-case handoff matrix covers status/pointer repair while the separate 62-case restoration broker registry covers authorization, artifact integrity, supersession, backup order, retention, conflicts, every crash boundary, and no-write replay. External blockers and non-authorizing dispositions remain unchanged. |
| Round 26 closure | Active planning retains the 156-case handoff and 35-case SC-002 publication registries and expands the separate restoration broker registry to 145 literal ids. It adds request-shape coverage, durable hierarchy replay, stable-root fd-relative publication/pruning, root capacity reservation, checked retention epochs, a sealed typed prune op with immutable pre/outcome audit, private body evidence plus digest-only audit provenance, closed wire failures, under-lock binding revalidation, per-record final-sync/response-loss cases, a pinned Type-3 real-binary client test, and shrinkage poisons. Task ids remain exactly 605; external blockers and dispositions are unchanged. |
| Round 27 closure | Active planning retains the 156-case handoff and 35-case SC-002 publication registries and expands the restoration broker registry from the historical R26 value 145 to 168 literal ids. It independently pins all nine publication boundaries for prune pre and prune outcome, restart reservation reconstruction, both durable-prune and immutable-zero-mutation release paths, and dedicated prune/reservation shrinkage poisons. It also assigns typed broker-owned source/target publication-root activation, crash-stable retention age anchors with unsafe-forward quarantine, sealed prune API negatives, bounded aggregate private-state retention, nested restoration responses, a resumable degraded-to-repaired settlement chain, exact 19-class CLI mapping, and release-binary Type-3 root projection with only socket injection. Task ids remain exactly 605; external blockers and dispositions are unchanged. |
| Round 29 closure | Active planning leaves all 168 broker ids unchanged and narrows their claim to their literal legacy cases. The mandatory read-independent 207 record-boundary ids own the listed amendment classes at all nine boundaries, including continuity-repair pre/evidence/outcome; the mandatory 78 lifecycle ids own standing-reserve taxonomy, cycle-unique capacity success/refusal/retry and malformed prefixes, retention-anchor conflict, continuity crash prefixes and permit seals, exact transport-loss resubmission, all private publication identifier canaries, and shrinkage. The CLI response-loss form exits 4 without a publication class; continuity repair is typed and pre-audited before watermark advance; restoration attempt hashing has one canonical two-step formula. Task ids remain exactly 605; external blockers and dispositions are unchanged. |
| Round 30 closure | Active planning preserves all 168 broker ids and expands the supplemental registries to exactly 216 record-boundary ids and 88 lifecycle ids. The added watermark class owns all nine publication boundaries; added lifecycle ids own continuity-evidence limits, both release malformed-prefix machines, continuity ordering/pairing conflicts, and evidence export/compaction restart. Continuity pre-only replay is reconstructible from durable fields and a deterministic handle, outcome variants couple deadline/prune posture, publication failures and pending settlement are total, standing exhaustion is a no-write pre-audit refusal, and prune/restoration-settlement identities plus continuity body fields have independent canaries and poisons. Task ids remain exactly 605; external blockers and dispositions are unchanged. |
