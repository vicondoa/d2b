...
| Round 24 closure | Current planning supersedes the Round 23 move/claim/repair shorthand: every new SC-002 immutable publication is direct-final, named cleanup state is legacy-only and never renamed/unlinked, the publication registry is 34 ids, and source-origin consumption commits only with durable dispatch. T592 owns Admin-only typed repair/restoration broker ops, coordinator lock, 32 audit members, immutable pre/outcome audit, and authenticated retained backup; T595 owns the CLI. The exact 155-case handoff matrix covers clean/repairable absence, forbidden inputs, conflicts, every crash boundary, second-run no-write, bounded restoration diagnostics, and separate integrity escalation while retaining 15 transition edges. External blockers and non-authorizing dispositions remain unchanged. |
