...
must place double opt-in, the production Network path, and all four cases before T220.
The three-resource operator boundary remains separate W6 acceptance: T604 consumes that
already landed path for exact `Volume/acceptance-state`, `Network/acceptance-net`, and
...
| --- | --- |
| COV1 | Exact-F6 operator acceptance covers `Volume/acceptance-state`, `Network/acceptance-net`, and `Device/acceptance-tpm`. The accepted external amendment places the real double-opt-in Network path and four-case matrix before T220; T604 remains W6 acceptance-only and T479/T480 bind its result plus distinct Guest acceptance. |
...
| Finding | Feature-artifact disposition |
| --- | --- |
| A1 | The operator acceptance set is exact and non-selectable. `spec.md` pins the three resources and their Providers/effects. Support resources cannot substitute. T604 consumes the pre-T220 Network implementation as W6 acceptance-only evidence, and T479/T480 bind the operator record to exact F6. |
...
| Network escalation | The untouched external Network spec and absent production adapter make W4 sole opt-in nonconforming and non-authorizing. T070 and T071 preserve the historical bytes but cannot complete from sole opt-in. Before T220 freezes F, an accepted external contract/work-item amendment must land double opt-in, assign the production emitter/controller/broker/net-VM path to pre-T220 owners, and pass all four Network/Host cases. Current W6 rows T336-T355 cannot satisfy that ordering. T219 revalidates the same ancestry before seal or merge; T604 remains W6 acceptance-only. |
