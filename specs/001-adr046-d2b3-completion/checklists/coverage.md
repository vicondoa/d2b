...
- [x] CHK052 Does the Constitution Check classify the W0/W1 and W2-W4 Principle VI gaps as non-authorizing violations, reject every feature-local exception or waiver, and require a separately accepted external FR-036 amendment before continuation, while distinguishing FR-043 and constitution-authorized pipelined-dispatch risks? [Consistency, Plan "Constitution Check"]
...
| CHK052 | The Constitution Check and Complexity Tracking classify both historical Principle VI gaps as non-authorizing violations, accept no feature-local exception, and require the separate external FR-036 amendment before continuation; FR-043 and constitution-2.0.0-authorized pipelining remain distinct program risks. |
...
| UND1 | T592 owns typed `OpenPeerPidfdFromAcceptedSocket`, accepted-socket/pidfd `SCM_RIGHTS`, and the sole raw syscall wrapper in the approved broker `sys.rs` FFI quarantine with narrow allowances and per-block `SAFETY:` comments. T593 remains unsafe-forbid and consumes only the safe typed result. The `nix` wrapper, a new project FFI crate, raw/numeric-PID fallback, and a local session syscall are ineligible. |
...
| U1 | FR-075 and SC-035 promote W2-W6 host continuity to a fail-closed exact-candidate close gate. T028 owns the existing host VM case and its sole Makefile recipe before W2 freeze; the no-skip evidence requires explicit Ready/Stopped states, same PID/start-time through a newly acquired pidfd, PID reuse/mismatch/ambiguity quarantine, and exact enumeration of only the three ADR-0015 units. The task preamble maps that result to every W2-W6 freeze/close pair without a new task ID or W5 evidence identifier. CHK038 is closed. |
