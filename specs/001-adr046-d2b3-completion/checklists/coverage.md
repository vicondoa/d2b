...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt through the shared typed hash oracle; no raw locator hash exists. A failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import, cleanup, incident transitions, successor admission, and the retention guard share one exclusive candidate OFD lock; cleanup loses before namespace access against every live owner. Ordinary cleanup empties both ephemeral namespaces and never uses check-then-unlink. Every incident state remains inspectable by stable id, closed cause/remediation, human/JSON projection, and exits. A verified expected payload reaches `parked`; authenticated no-unlink residue reaches `mismatch-retained`; unusable or unstable primary evidence reaches a separate resolution through a complete census or identity-bearing bounded-failure commitment. The frozen primary scope excludes resolution leaves, raw `01ff` is non-authorizing, and copied or changed commitments block successor admission. A rename/reopen race is exactly resumable or irreconcilable with all names preserved and no false terminal status. The exact 61 receipt, 45 malformed-census, and 32 source-floor receipt negative registries are independent. All incident states block parked-candidate publication and close while the authenticated apply plus fresh-successor path remains available. |
