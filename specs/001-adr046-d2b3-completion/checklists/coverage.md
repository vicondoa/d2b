...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt through the shared typed hash oracle; no raw locator hash exists. A failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import, cleanup, incident transitions, successor admission, and the retention guard share one exclusive candidate OFD lock; cleanup loses before namespace access against every live owner. Private `CandidateSidecarGuard` solely owns that lock fd, and `SidecarCleanupOwner<'guard>` exclusively borrows it. Existing-name cleanup additionally requires the private namespace write owner and blocks on boundary loss. Every cause maps to a closed inspect/action/status/successor row; coverage denial exposes a bounded failure/root class, one owner procedure, and no disposition-request command until repair. One immutable structured preimage carries every kind-specific component and is procfs-fd linked as the exact complete opened inode directly to its final no-replace name before every other incident record. A verified payload reaches `parked`; authenticated no-unlink residue reaches `mismatch-retained`; unusable primary evidence reaches a separate resolution through one recursive grammar that rejects serialized unavailable nodes. The successor triplet is durably frozen before the exact 19-field request. Request output prepares an unnamed inode before candidate publication, procfs-fd links the exact inode directly to the final after candidate durability, and retains the internal pair on unsupported link. Source-floor validation consumes one non-clonable protected origin through private provenance into one validated-floor result; later boundaries borrow/attenuate it, and copied digests, origin replay, repeated mint, or serialized revalidation produce no authority. The exact 61 receipt, 73 malformed-census, 26 request-output, source-floor 32/26/21, and mutation post-first 15 negative registries plus the seventeen-row recovery redaction registry are independent. All incident states block parked-candidate publication and close until their exact safe recovery path completes. |
...
| Round 23 closure | Current planning uses zero-capability procfs-fd exact-inode direct-final linking with no `AT_EMPTY_PATH`, linked temporary, or create-and-unlink probe; treats unsupported request-output link as a retained-internal output failure; guards every existing-name cleanup move; rejects serialized unavailable nodes; consumes one protected source-floor origin into one validated capability; exposes exact coverage failure/root classes and owner procedures with no signing command until repair; provides selector-free coordinator pointer repair and immutable-audit escalation; pins 61/73/26 SC-002 registries, seventeen recovery canaries, all six remediation rows, and the 135-case handoff matrix over seven rollback members, 30 audit members, and 15 transitions. All external non-authorizing dispositions and external artifact escalations remain unchanged. |
