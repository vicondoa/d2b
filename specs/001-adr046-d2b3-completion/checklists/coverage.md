...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt; a failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import, cleanup, and the retention guard share one exclusive candidate OFD lock. Ordinary cleanup empties both ephemeral namespaces. The sole identity-ambiguity terminal has immutable preimage metadata, a revalidated payload in durable incident quarantine outside those namespaces, and append-only parked status. A rename/reopen race remains recovery-pending with all names preserved and no parked status. Neither path unlinks; both block publication and close. |
