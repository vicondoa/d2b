...
| SC-002 compatibility | The global `EvidenceRecord` schema and v2 decoder remain unchanged. A passed operator record references one typed version-1 receipt through the shared typed hash oracle; no raw locator hash exists. A failed record imports without one but cannot satisfy panel, seal, eligibility, or close. Import, cleanup, incident transitions, successor admission, and the retention guard share one exclusive candidate OFD lock; cleanup loses before namespace access against every live owner. Private `CandidateSidecarGuard` solely owns that lock fd, and `SidecarCleanupOwner<'guard>` exclusively borrows it, so stale cleanup authority cannot outlive or be paired with another guard. Ordinary cleanup empties both ephemeral namespaces and never uses check-then-unlink. Every cause maps to a closed inspect/action/status/successor row. One immutable structured preimage carries every kind-specific component and is repeated byte-identically across every durable path/status record. A verified expected payload reaches `parked`; authenticated no-unlink residue reaches `mismatch-retained`; unusable primary evidence reaches a separate resolution through one recursive absent-root/directory/regular-file census grammar. A bounded failure authorizes only after two stable walks cover every descendant within the hard ceiling; otherwise request, apply, and admission deny. The frozen scope excludes resolution/request/disposition/freeze leaves, raw `01ff` is non-authorizing, and copied or changed commitments block successor admission. The successor triplet is durably frozen before the exact 19-field request; authority transformation to the 22-field disposition is closed, and request output is anchored, atomic, crash-safe, and CLOEXEC. Source-floor issuer authentication produces a private nonserializable result consumed into the private validated-floor result; copied authority/key digests produce neither. The exact 61 receipt, 72 malformed-census, 26 request-output, source-floor 32/20/21, and mutation post-first 15 negative registries plus the fifteen-row recovery redaction registry are independent. All incident states block parked-candidate publication and close while the authenticated request/apply plus fresh-successor path remains available after the full-coverage guard. |
...
| Round 22 closure | The current SC-002 contract has six remediation values; complete preimage and request-output unnamed-inode write-ahead publication; twelve root/root-instance pairs; total invalid-node encoding; full-sequence stable bounded failure; depth-64 success/depth-65 denial; 72 census ids, 26 output ids, and fifteen recovery canaries including raw `st_uid`/`st_gid`; and payload sync before parked-status temp creation under the candidate lock. Handoff status is a private validated variant set with authenticated current/terminal pointer selection, failed-transfer and rollback partitions, exact valid/error outputs and exits, incomplete-proof refusal, T589 schema/golden ownership, and T595 hermetic implementation plus dedicated Type-10 handoff ownership; T604 consumes the public handoff in separate exact-candidate operator acceptance. NixOS and Network findings remain explicit no-ops with no ownership or contract change. External source/tests/schemas/normative/reference/ADR/constitution/contributor/changelog/panel artifacts remain unedited and escalated. |
