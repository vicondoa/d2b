...
- Host activation positive coverage starts from an installed 3/1 generation whose accepted
  external compatibility floor atomically provides both numeric-protocol-4 source peers,
  exact `source-handoff-v1` operation-catalogue negotiation, matching source
  schemas/catalogues/fingerprints/snapshots/fixtures, and the broker-managed immutable source
  apply object under the existing service lifecycle. Separate negatives start from bare
  ...
  exec, PID reuse, mismatch, or ambiguity before mutation without persisting a pidfd. The
