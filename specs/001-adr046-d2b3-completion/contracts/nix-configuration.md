...
| NIX-8 | Upgrade an installed 3/1 host through the target closure's `system.build.d2bHostGenerationDeploy` entrypoint. The first procedure is built from parameterized target inputs, fail-fast validates the exact flake/configuration grammar and 2048-byte composition, resolves the installable, and stops before public-socket authorization or `sudo` on any failure. It does not read `/etc/d2b/host-generation-rebuild-ref`. The unprivileged `--authorize-handoff` invocation passes the existing public-socket `SO_PEERCRED` plus `d2b`-group Admin classification and causes the broker to consume that one-shot classification into a durably sealed nonfabricable handoff capability bound to the staged intent; it emits no authority token. The privileged `--apply-authorized-handoff` invocation may only resume that exact capability. Bootstrap, stable-reference, and parameterized rollback procedures all use this exact pair. The entrypoint may only validate, build and verify the complete 4/2 closure, durably stage immutable transition bytes, probe capability, and submit one opaque intent. It cannot publish a profile, control a service, mutate 3/1 bootstrap state, or initiate rollback. A capability-authorized protocol-5 broker, or the target closure's one-shot typed bootstrap-broker mode when the installed broker lacks the operation, performs every profile, service, bootstrap, publication, repair, and stock rollback mutation with immutable pre-mutation and outcome audit. Daemon identity and euid 0 never independently authorize. The exact order is durable intent/capability, broker coordinator before first mutation, target broker, durable ownership transfer, target daemon, exact-generation protocol-5 Hello while unready, phase-attenuated authenticated publication request, broker-durable d2b pointer/reference publication, daemon ingestion, then readiness. On failure, the broker-owned coordinator is durable before first mutation, transfers exactly once from bootstrap to target broker before daemon activation, and reopens through the matching bootstrap owner or existing target broker service after entrypoint, broker-startup, or daemon-startup failure; prior pointer/reference bytes or absence are restored before typed stock rollback and source-service restoration. Identical deployment and crash replay produce no duplicate ingestion or effect, and no new unit exists | FR-070, SC-032 | W5 |
