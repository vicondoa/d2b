...
| F009 | 2026-08-06 | `signoff` | W2-W5 | The external ADR/spec/tooling still says one binding request per wave while the repository panel contract requires durable failed-candidate closure and a distinct successor after findings | W2-W4 cannot rely on T589's future W5 implementation, and a feature-local assertion cannot change external policy | `contracts/README.md` defines versioned prerequisite `adr046-candidate-recovery-prerequisite/v1`; T008 owns the historical W2 entry attestation and exact nonempty commands. Current reruns cannot close it. F2 must carry the named remedial requalification before W2 close if contemporaneous entry evidence is absent. The external scopes remain escalated and deliberately unedited by this batch |
