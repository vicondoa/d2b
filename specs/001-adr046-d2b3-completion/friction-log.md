...
| F012 | 2026-08-06 | `signoff` | W4/W5 | Feature-local W4 prose asserted Host/site plus Network double opt-in, while untouched external `ADR-046-resources-network` normatively defines sole Network opt-in and committed code lacks the production adapter | Historical W4 close cannot honestly claim the four-case implementation, and accepting sole opt-in would let T220 freeze before the required security boundary exists | W4 bytes remain preserved but sole opt-in is nonconforming and non-authorizing. Final R9, recorded in F021, supersedes the earlier implementation-timing disposition: T220/T219 require the accepted double-opt-in migration and removal of stale sole-opt-in paths, while T336-T355 and all four production cases remain W6 work under T221 |
| F013 | 2026-08-07 | `signoff` | W5 | The source-generation compatibility floor had no closed external owner or authoritative generated mapping | An incomplete or mixed source actor could become success-shaped | Accepted Version 2 must own `SourceGenerationCompatibilityFloorV1` through `VD2-SC002-SOURCE-FLOOR`, `VD2-SC002-REGISTRIES`, and `VD2-SC002-TRACEABILITY`; T589 and T592 consume only generated owner rows |
...
| F021 | 2026-08-08 | `merge` | W5/W6 | Separate post-round-22 consistency analysis found T604 trying to pull authoritative T336-T355 work and T221 panel ownership from W6 into W5 | The feature graph could violate wave and panel ownership | Final R9 keeps T336-T355 as authoritative W6 implementation under T221 and T604 as W6 acceptance-only after those rows merge. Before T220, the accepted external amendment must remove every current-facing sole Network-opt-in path, install double opt-in, and regenerate that settled W6 ownership plus four-case obligations without moving implementation into Wave 5. This supersedes F012's earlier implementation-timing disposition |
