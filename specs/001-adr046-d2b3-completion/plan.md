...
| W5 (`adr046w5` delivery address) | 7 plus the externally amended Network production scope | 146 + 15 local completion/evidence tasks + T603, plus the manifest rows assigned by the required Network amendment | 12 manifest groups + the serialized completion graph + one generated Network scope before T220 | FR-036 and T072 gate T603's exclusive-editor reconciliation. Before T220 may freeze F, an accepted Network contract/work-item amendment must replace sole Network opt-in with double opt-in, assign the production implementation to a pre-T220 W5 owner, and land all four real Network/Host cases. The current T336-T355 W6 assignment cannot satisfy this gate and creates a hard stop until the external manifest is amended. The retained binding request remains externally dispositioned only. |
| W6 | 27 manifest dossiers after the Network work-item amendment | 258 manifest items adjusted by the accepted amendment + T604 operator acceptance | 5 families plus the file-disjoint T604 acceptance slice | FR-036 first; T221 selected-roster plan lifecycle before remaining W6 implementation. T604 remains W6 acceptance-only, consumes the Network implementation already required before T220, and emits `operator-nix-activation-cleanup` only for F6; T479 requires that result plus `Provider/runtime-cloud-hypervisor` Guest acceptance, and T480 revalidates both before close. |
...
| Integrator prep | T589 | Depends on T603 plus accepted Version 2 and the installed source-floor disposition. Owns the shared registrar, policy-bootstrap, operation-status, audit hook, exact-seven evidence validator, and only implementation/schema/fixture/API rows assigned to T589 by generated `VD2-SC002-*` traceability. It verifies but does not produce the external source floor. No feature-local SC-002 or source-floor copy is authority. |
| Serialized implementation | T590-T594, T605 | T590, T591, and T594 start from T589. T592 follows T591 and solely owns `packages/d2b-resource-store-redb/src/{lib.rs,actor.rs,audit.rs,migration.rs,backup.rs,tests.rs,transaction.rs}`, `packages/d2b-audit/src/{lib.rs,sink.rs,record_types.rs,segment.rs,export.rs}`, `packages/d2b-contracts/src/{lib.rs,broker_wire.rs}`, `packages/d2b-core/src/{privileges.rs,privileges_w3.rs}`, `packages/d2b-priv-broker/src/{bootstrap.rs,lib.rs,main.rs,protocol.rs,runtime.rs,live_handlers.rs,sys.rs,audit.rs,ops/mod.rs,ops/audit_op.rs}`, `packages/d2bd/src/{lib.rs,daemon_version.rs}`, `nixos-modules/{resources.nix,resources-bundle.nix,privileges-json.nix}`, and both Cargo lockfiles; its generated Version 2 rows own their exact schema/fixture/snapshot paths. T593 follows T592 and consumes its peer-pidfd operation; T605 follows T593 for shared API snapshots. T592 consumes source-floor and SC-002 inputs only through generated `VD2-SC002-SOURCE-FLOOR`, `VD2-SC002-REGISTRIES`, and `VD2-SC002-TRACEABILITY` rows. T605 owns the Zone enum correction and paired normative/test/reference/API artifacts. |
| Serial daemon composition | T595 | Sole writer after T592 for `d2bd/src/{resource_runtime.rs,lib.rs}`, the deployment binary and Cargo owners, `nixos-modules/{bundle-zones,host-daemon,host-broker,options-site}.nix`, and the owned Nix/VM tests. It composes the production Zone runtime and unprivileged opaque-intent deployment path, while the existing broker service owns privileged continuation. Source-floor schemas, fixtures, poison registries, and transitions come only from T595-assigned generated `VD2-SC002-*` rows. No new unit, daemon rollback owner, caller-selected privileged executable, or feature-local registry is permitted. |
| External Network correction and pre-T220 production path | external contract/work-item amendment, then its generated W5 owners | The accepted amendment must set `effectiveEastWest = Network.spec.isolation.allowEastWest && d2b.site.allowUnsafeEastWest`, default both inputs false, assign the production adapter and net-VM path to owners that merge before T220, and assign all four Network/Host combinations to enforcing production tests. Existing code keeps `NetworkEffectPort` in `d2b-provider-network-local::controller` and lacks the production adapter. Current generated rows T336-T355 are W6 and therefore cannot satisfy a pre-T220 gate; no W5 implementation or freeze may begin until the external manifest resolves that ordering. |
...
| Integrator convergence and freeze | T220 | Merges every Wave 5 slice and generated artifact, verifies accepted Version 2 and generated Wave 5 `VD2-SC002-*` traceability, folds exactly twelve fragments, requires the T599 runbook/action map, and runs integration, CI, and drift. It also rejects F unless the accepted Network amendment, its pre-T220 production implementation, and all four real Network/Host cases are ancestors of F. T604 remains W6 acceptance-only and is not part of the Wave 5 exact-seven profile. Each provisional candidate uses one current selected-roster lifecycle; every selected seat must approve before F. |
...
must assign and land the double-opt-in production path and all four cases. The Type-10 test
