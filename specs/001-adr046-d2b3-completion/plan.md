...
external owner must atomically install both protocol-4 source peers whose Hello
`operation_catalogue_sha256` exactly equals the `source-handoff-v1` operation-catalogue
fingerprint, every matching source wire/privilege
schema, catalogue, fingerprint, snapshot and positive/negative fixture, and the exact
immutable broker-managed apply object used under `sudo`; the caller-flake target entrypoint
...
directory. Creation, publication, loser cleanup, and restart cleanup are serialized by one
verified candidate-scoped OFD write lock held through parent `fsync`, a zero-residue census,
and record publication or return. A live importer retains that lock, so cleanup cannot touch
its temp; restart cleanup proceeds only after acquiring the released lock. Under the lock,
...
uses fd-relative `unlinkat` and `fsync`s the leaf parent. A mismatch is restored and refused,
never unlinked. Every winner, loser, refusal, and restart must leave both reserved temp and
quarantine namespaces empty before record publication or return. Crash recovery may reuse an
...
7. Installed-host migration begins with the target closure's
   ...
   mutation. Peer exit, exec, PID reuse, mismatch, or ambiguous identity refuses. The pidfd
   and executable fds are closed with the connection and are never serialized or persisted.
   ...
   cannot route authority to a target-closure mode. The installed source receiver derives
...
both numeric-protocol-4 source peers, exact `source-handoff-v1` catalogue negotiation,
matching source schemas/catalogues/fingerprints/snapshots/fixtures, the source apply object,
and pre-transfer coordinator behavior under the existing `d2b-priv-broker.service`. Bare
...
and durably adopts coordinator ownership once before daemon activation. Tests inject source
fingerprint mismatch, target/apply executable and symlink substitution, apply-peer
exit/exec/PID-reuse/mismatch/ambiguity, target broker startup failure, target daemon
...
accepted external source-generation compatibility disposition
  ...
  -> broker-managed GC root plus exact target-object pin and separately resolved installed
     apply-object pin
  -> accepted apply connection binds a direct peer pidfd and live executable identity to the
     apply-object pin, with immediate pre-mutation revalidation and no persisted pidfd
...
atomically installed in the 3/1 source generation: both numeric-protocol-4 peers, exact
`source-handoff-v1` catalogue negotiation, matching source
schemas/catalogues/fingerprints/snapshots/fixtures, and the source apply object. Bare
...
ambiguity; every case refuses before mutation, and no peer pidfd persists. Accepted
...
| Earlier handoff prose claimed the target closure's compatibility broker could run under the existing broker service before profile publication and allowed the caller-flake target executable to run under `sudo`. | Committed `d2b_contracts::PROTOCOL_VERSION` is 4 and its `BrokerRequest` and operation-catalogue fingerprint have no host-generation handoff operation. Committed `nixos-modules/host-broker.nix` makes `d2b-priv-broker.service` execute the installed generation's `brokerPackage`, not a target-closure binary. Immutability does not make caller-selected executable code a trusted root entrypoint. | No executable authorized source actor exists at this base. T589 and downstream Wave 5 implementation are blocked on an accepted external source-generation compatibility disposition that atomically installs both source peers, exact `source-handoff-v1` catalogue negotiation, all matching source schemas/catalogues/fingerprints/snapshots/fixtures, and a broker-managed immutable apply object before migration. That external owner is outside this feature. T592 consumes that frozen fingerprint and owns only target protocol-5 adoption and target artifacts; T595 owns target-generation behavior. The caller-flake target entrypoint runs only unprivileged. Only the separately pinned installed apply object runs under `sudo`, and its live connection peer pidfd/executable identity must match the pin before every mutation. Target/apply/GC-root substitution and peer exit/exec/PID reuse/mismatch/ambiguity refuse. Target-only code, a new unit or override, an entrypoint child or mutation path, daemon recovery, and a synthetic starting image are not accepted substitutes. |
| Earlier follow-up prose called the new source handoff row simply "protocol 4" and assigned its compatibility artifacts to T592. | The committed numeric protocol-4 peers negotiate a closed catalogue that does not contain the row; silently changing that catalogue under the same undifferentiated handshake is incompatible. | External scope escalation: the source-generation disposition must add actual two-peer Hello negotiation whose `operation_catalogue_sha256` equals the exact `source-handoff-v1` catalogue fingerprint and move the source peers, schemas, catalogues, fingerprints, snapshots, fixtures, and installed source apply object atomically. Bare protocol 4 refuses. T592 treats that external source set as read-only and changes only target-v5 adoption and target outputs. |
