...
T589's private `CandidateRetentionOwner` is the sole retirement owner. It may delete these
leaves only as part of an fd-anchored whole-candidate retirement after the candidate is
terminal, every delivery transition is terminal, every incident is absent or
`successor-admitted`, no external record references the candidate, the shared lock is held,
both ephemeral namespaces are empty, and the exact durable census is valid and bounded.
There is no per-leaf or clock-driven deletion.
...
`docs/reference/schemas/delivery/sc002-incident-{status,disposition}-v1.schema.json`,
`tests/golden/delivery/sc002-incident-{human,json}-v1.txt`, and
`tests/golden/delivery/sc002-incident-disposition-v1.json`. Its existing
...
commands, stable IDs, exits, human/JSON remediation, private canonical-disposition validator,
collision-safe bounded retirement, candidate-retention owner, and focused tests. That
...
quarantine; no sidecar data leaf is unlinked. The private retention owner may remove them
only through the whole-candidate predicate. Overlap tests latch both orderings for
