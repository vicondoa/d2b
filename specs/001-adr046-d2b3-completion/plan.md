...
extra, empty, stale, or cross-disposition members refuse. The caller-flake target entrypoint
...
performs a final name/identity check before fd-relative `unlinkat` and leaf-parent `fsync`.
An identity mismatch never calls `unlinkat` or restores the temp. It moves the currently
named suspect with `renameat2(RENAME_NOREPLACE)` into durable
...
member omission, stale and cross-disposition members, fingerprint mismatch, target/apply
