...
cleanup may once-open and verify a retained regular single-link current-effective-uid `0600`
legacy source, but it never renames, hardlinks, or unlinks that existing name. The temporary
and cleanup-quarantine namespaces are legacy-observation-only, and current publication never
creates either one. Before any payload or residue evidence copy, cleanup direct-final
publishes and syncs the complete incident preimage. It then copies exact bytes only from the
retained source fd into a new unnamed inode and direct-final publishes immutable evidence;
the original source name remains at its frozen locator in the recursive census. The retired
subtree is read-only compatibility evidence, and the current protocol creates no new retired
member. Historical retirement ids bind the candidate triplet, content digest, and recorded
device/inode without rendering raw inode identity. Two identical historical orphan leaves
with distinct inodes have distinct retirement ids. A retained destination `EEXIST`, a 65th
retired leaf, more than 1,048,576 retired bytes, or an invalid retired census preserves every
name and enters the incident transition instead of overwriting, reusing, deleting, or growing
the set. An identity mismatch never restores, renames, hardlinks, or unlinks the suspect. It
first durably direct-final publishes immutable structured `Sc002IncidentPreimageV1` with
every kind-specific component, then a kind-bearing anchor and preimage-complete metadata,
with file and ancestor-directory durability at each publication boundary. It copies the
metadata-bound bytes from the retained source fd into a new unnamed inode and direct-final
publishes
`evidence-sidecars/sc002/incidents/payload/sha256/<incident-id>.bin`, syncs the payload
parent and every changed ancestor, reopens and verifies the copied inode, and `fsync`s that
reopened payload fd before append-only publishing and syncing `parked` status. Only that
fully verified and payload-durable copy may enter the verified-payload terminal branch. A
replacement, `ENOENT`, nonidentical `EEXIST`, or post-copy
...
direct-final publishes immutable residue evidence from retained fds while preserving every
original legacy source name at its frozen locator and binding the exact frozen retained-name
and residue censuses, or binds
...
historical retired orphans remain at their frozen locators under distinct retirement ids in
the bounded durable census; the current protocol creates no new retired member. Incidents use
an immutable complete structured preimage plus preimage-complete metadata; a direct-final
copied, revalidated, file-synced payload reaches `parked`; and an irreconcilable mismatch
reaches `mismatch-retained` only after authenticated disposition direct-final publishes
immutable residue evidence from retained fds while every original legacy source name remains
at its frozen locator under the exact frozen retained-name and residue censuses. Zero-residue,
malformed-metadata, conflicting-status, invalid-census, and
unstable-census cases instead bind the complete recursively enumerated frozen census or identity-bearing
bounded-failure commitment in the append-only resolution branch. A raced source-copy/final-reopen remains
...
The former quarantine/reopen/retirement wording for
`operator-nix-activation-cleanup` is historical, superseded, and non-authorizing. The active
row requires
