...
| Panel reviews run on `gemini-3.1-pro-preview` rather than the model pinned by the delivery contract and by `packages/xtask/src/delivery/model.rs` | Operator direction. Panels run as subagent lanes dispatched by the coding agent executing this plan. | Not a documentation change. The model is enforced in five places: the spec's §12.3 record shape, `PANEL_MODEL_POLICY`, the `panel-attest` rejection at `panel.rs:674`, a unit-test assertion of the exact strings, and `.opencode/opencode-swarm.json`. Changing it requires a specification amendment (re-opening evidence per FR-046) plus a code change owned by `ADR046-delivery-005`. Tasks T581-T584 carry it. **Until those land, `panel-attest` will reject every record produced on the new model and no wave can seal.** |
