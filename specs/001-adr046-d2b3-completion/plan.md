...
external owner must install both the versioned protocol-4 source-daemon/source-broker bridge
and the exact immutable broker-managed apply object used under `sudo`; the caller-flake
target entrypoint remains unprivileged. T589 and all downstream Wave 5 implementation remain
...
directory. A no-replace loser or restart cleanup removes only its verified temp with
fd-relative `unlinkat`, `fsync`s the leaf parent, and leaves zero temporary residue before
success or refusal. Crash recovery may reuse an identical already-durable sidecar after a
full reopen check but never replaces it; mismatched bytes or binding refuse. T602 and T219
...
other slice, target, service, socket, timer, path, or template is filtered. The same case injects PID reuse,
...
7. Installed-host migration begins with the target closure's
   ...
   only the externally installed source-generation protocol-4 compatibility actor may receive
   exactly one accepted public-socket evidence fd over the authenticated daemon/broker
   channel. Committed protocol 4 without that actor refuses; it cannot route authority to a
   target-closure mode. The installed source receiver derives authority only from that
   consumed fd and its broker-sealed staged-intent binding. It accepts no serialized uid,
   gid, role, provenance, root, daemon, or caller claim.
...
lifecycle owner. This subcontract is conditional on the unresolved external compatibility
floor: the source generation must already have installed a protocol-4 handoff-capable broker
whose ordinary `serve` process runs under the existing `d2b-priv-broker.service`. The existing
service starts and restarts that installed actor before transfer. That owner transfers the coordinator exactly
...
T592 solely owns the target protocol-5 half and
...
accepted external source-generation compatibility disposition -> installed source 3/1 compatibility floor
{fresh C/Q analysis + plan panel, installed source 3/1 compatibility floor} -> T589 -> {T590,T591,T594}
T591 -> T592
...
accepted external source-generation compatibility disposition
  -> source 3/1 generation installs protocol-4 handoff-capable broker `serve`
  ...
  -> source daemon transfers exactly one accepted-socket evidence fd over the authenticated
     protocol-4 channel
...
installed in the protocol-4 3/1 source generation; committed protocol 4 without its handoff
operation is a refusal case, not the positive control. It obtains `d2bHostGenerationDeploy`
...
evidence crosses as exactly one fd over the authenticated protocol-4 channel and no root,
provenance, daemon, or caller claim substitutes for it. The source broker coordinator is
...
For the `operator-nix-activation-cleanup` row, the later corrections are binding over its
compressed legacy wording. The caller-flake target executable runs only unprivileged; the
separately broker-pinned installed apply object alone runs under `sudo` and receives no URI or
reference to reevaluate. Source-v4 bridge implementation and the source apply object are
external-scope prerequisites; T592 owns target-v5 adoption and both sides' compatibility
artifacts. Zero-output/multi-output resolution and target/apply/GC-root/symlink substitutions
refuse. Sidecar durability includes every ancestor-directory `fsync`, fd-relative verified
temp cleanup with parent `fsync`, and zero temporary residue. Full-namespace unit equality is
computed after excluding exactly canonical `d2b.slice`; no other lifecycle unit is filtered.
...
| Earlier handoff prose claimed the target closure's compatibility broker could run under the existing broker service before profile publication and allowed the caller-flake target executable to run under `sudo`. | Committed `d2b_contracts::PROTOCOL_VERSION` is 4 and its `BrokerRequest` has no host-generation handoff operation. Committed `nixos-modules/host-broker.nix` makes `d2b-priv-broker.service` execute the installed generation's `brokerPackage`, not a target-closure binary. Immutability does not make caller-selected executable code a trusted root entrypoint. | No executable authorized source actor exists at this base. T589 and downstream Wave 5 implementation are blocked on an accepted external source-generation compatibility disposition that installs the versioned protocol-4 source-daemon/source-broker bridge and a broker-managed immutable apply object before migration. That external owner is outside this feature. T592 owns only target protocol-5 adoption and two-sided compatibility artifacts; T595 owns target-generation behavior. The caller-flake target entrypoint runs only unprivileged, privileged apply gets no URI or reference to reevaluate, and target/apply/GC-root substitution refuses. Target-only code, a new unit or override, an entrypoint child or mutation path, daemon recovery, and a synthetic starting image are not accepted substitutes. |
