...
the complete frozen primary-evidence census or identity-bearing bounded-failure commitment
when names are absent or metadata/status/census is unusable or unstable. The frozen scope
excludes resolution leaves and a raw `01ff` sentinel cannot authorize. It never restores,
...
`tests/golden/delivery/sc002-activation-receipt-negative-case-ids.txt`, and
`tests/golden/delivery/sc002-census-negative-case-ids.txt`; production enumerators and poison
...
| --- | --- | --- |
| External Version 2 delivery-contract amendment | no feature task | Before T589 dispatch, a separate external specification-amendment workflow bumps accepted `ADR-046-validation-and-delivery` from Version 1 to Version 2. It normatively owns both contracts that T589 consumes: (1) the four incident commands; every inspectable cause; distinct `recovery-resumable` and `recovery-irreconcilable` states; exact thirteen-line human and closed 17-field JSON projections; five closed remediation values; stable IDs/exits and inspect/recover/apply/successor convergence; typed domain-separated, length-framed SC-002 receipt content locator; the shared sixteen-digest/one-signature SC-002 golden; `Sc002IncidentDispositionV1` canonical encoding and Ed25519 authority/key/signature binding; immutable anchor/metadata and 15-field durable status containing the complete incident-id preimage; payload-file sync before `parked`; durable precreation and bottom-up sync of every changed retirement, incident, status, resolution-evidence, and resolution ancestor; `parked`, no-unlink `mismatch-retained`, and separate frozen-primary-evidence resolution branches; complete census and identity-bearing bounded-failure commitment forms whose scope excludes resolution/disposition leaves and for which raw `01ff` is non-authorizing; the canonical zero-residue path for `incident-names-absent`; invalid/unstable census resolution; retired source/destination residue slots; branch-aware status-prefix repair; append-only status and resolution paths; temporary-write/file-sync/no-replace/reopen/parent-and-every-ancestor-sync publication and idempotent recovery; residue staging/finalization; separate anchor, metadata, durable-status, resolution, and CLI-status schemas; fresh-successor disposition; the complete `CanonicalRetiredCensusV1` and `CanonicalIncidentPrimaryEvidenceCensusV1` contracts and vectors; the exact 61 receipt and 45 malformed-census negative registries; collision-safe retirement identity; one candidate lock excluding cleanup against every importer/cleanup/incident/successor/retention live owner; private zero-mutation candidate-retention owner; and typed validator; and (2) the source-floor canonical JSON policy, `SourceGenerationIdentityV1`, complete typed digest/domain/length-framing and exact 15-digest/four-signature `hash-vectors-v1.json`, ordering, integer/text, disposition-pinned issuer signatures, copied-digest rejection, unknown-field/tag/version refusal, strict schemas, independently authored literal 13-role expectations, the exact independent 32-id receipt/transition negative registry, and independently recomputed checked-in golden vectors. The amendment must receive the parent ADR's required pre-panel and post-panel approvals, regenerate `docs/specs/ADR-046-spec-set.json`, `ADR-046-work-items.json`, and `ADR-046-implementation-graph.{json,md}`, pass Gate 0 and drift validation on the exact amendment commit, and be an ancestor of T589's base. This row owns no feature artifact or implementation file. T589 cannot author, approve, regenerate, or substitute for it. |
...
thirteen-line human projection, persisted complete preimages, three terminal entry branches,
...
has complete and identity-bearing bounded-failure forms, binds incident/parked identity,
excludes resolution leaves, persists its canonical bytes outside the frozen scope, and
rejects raw `01ff` as authority. T220 independently encodes and verifies all retired and
primary-evidence census vectors, the shared sixteen-digest/one-signature SC-002 oracle, all four incident vectors,
the strict anchor, metadata, primary-status, resolution, and CLI-status schemas, both nonterminal
...
the disposition-pinned verifier. A separate literal 32-id receipt/transition negative
...
namespace access. The lock serializes cleanup against every importer, cleanup,
...
unstable-census cases instead bind the complete frozen census or identity-bearing
...
delivery-history namespace remains immutable. Overlap tests latch both orderings for every
writer/cleanup and writer/retention pair and recover two identical orphan leaves without
collision. The exact 61 receipt, 45 malformed-census, and 32 source-floor receipt negative
registries are independent of production and their poison builders. T595/T604 own the independent closed 15-edge registry, six
...
complete frozen primary-evidence census or identity-bearing bounded-failure commitment in a
separate append-only resolution branch when names, metadata, primary status, or census
stability is irreconcilable. The frozen scope excludes resolution leaves and raw `01ff` is
...
correction expands the overlap matrix to every importer/cleanup/incident/successor/retention
live-owner pair;
pins literal source-floor and mutation-edge constants independently from fixtures and
production; adds the exact receipt/census negative registries and shared SC-002 hash oracle;
...
list in `tasks.md`. T604 retains sole ownership of the separate unit-census fixture. No
