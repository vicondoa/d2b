...
publication and every close stage until restart or T589's owned
`sc002-incident-recover` command completes the same protocol. Recovery-pending is a
validated nonterminal prefix, never a second durable status or successful cleanup result.
Ordinary winners/losers/refusals and terminal incidents leave both ephemeral namespaces empty;
...
T589 freezes the independent flat expected-set files
...
generators may consume none of them. T604 separately owns
`tests/golden/delivery/host-generation-unit-census-case-ids.txt`. `tasks.md` pins every
literal axis, ordering rule, and cardinality for these files. A generated expected set, a
runtime count, or a fixture that imports the production registry is ineligible.

#### Serial dependencies and file ownership

| Stage | Task(s) | Ownership and concurrency |
| --- | --- | --- |
| External Version 2 delivery-contract amendment | no feature task | Before T589 dispatch, a separate external specification-amendment workflow bumps accepted `ADR-046-validation-and-delivery` from Version 1 to Version 2. It normatively owns both contracts that T589 consumes: (1) the four incident commands including recovery of the validated nonterminal prefix, exact twelve-line human projection, five closed remediation values, stable IDs/exits, `Sc002IncidentDispositionV1` canonical encoding and Ed25519 authority/key/signature binding, immutable preimage-complete incident metadata and append-only status paths, file-and-ancestor-directory-durable publication/recovery protocol, separate durable-status and CLI-status schemas, fresh-successor disposition, the complete `CanonicalRetiredCensusV1` framing/tag/unavailable/sentinel/ordering contract and vectors, collision-safe retirement identity, private zero-mutation candidate-retention owner, and typed validator; and (2) the source-floor canonical JSON policy, `SourceGenerationIdentityV1`, complete typed digest/domain/length-framing, ordering, integer/text, issuer-signature, unknown-field/tag/version refusal, strict schemas, and independently recomputed checked-in golden vectors. The amendment must receive the parent ADR's required pre-panel and post-panel approvals, regenerate `docs/specs/ADR-046-spec-set.json`, `ADR-046-work-items.json`, and `ADR-046-implementation-graph.{json,md}`, pass Gate 0 and drift validation on the exact amendment commit, and be an ancestor of T589's base. This row owns no feature artifact or implementation file. T589 cannot author, approve, regenerate, or substitute for it. |
...
rename/reopen remains recovery-pending with every still-named leaf preserved until restart
or the owned recovery command completes. No sidecar
...
an alternate "unmovable name" terminal. It preserves every still-named leaf and blocks
publication and close until restart or `sc002-incident-recover` completes that same
metadata-bound protocol. Inspect projects the validated prefix without inventing a durable
status, and only the revalidated payload plus parked status is terminal. The same correction expands
