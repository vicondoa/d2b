...
- [ ] T024 When the fingerprint precheck reports drift, run `make api-surface-pin` before `make test-lint`; then run focused xtask test/clippy/fmt, `make check-tier0`, `make test-changelog`, and `make test-policy`, recording exact results in panel evidence
