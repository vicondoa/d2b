...
- [ ] T008 [P] [US1] Add failing static contract assertions for the Make-owned Rust DAG, grouped keep-going execution, redacted actionable `D2B_RUST_BUDGET` validation, effective-budget logging, cache-aware cgroup default, unreadable-controller fallback to budget `1`, immediate Make metadata removal before setup spawns without closing stale descriptor numbers, removal of a prior requested manifest before per-leaf fragments, deterministic atomic finalization, and a simulated failed run that publishes only current partial evidence with `run_status = "failed"` and no stale entries in `tests/unit/meta/ci-runner-regression.py`
...
- [ ] T011 [US1] Implement the bounded recursive GNU Make Rust DAG, cache-aware cgroup `D2B_RUST_BUDGET`, runtime active-lane and quota calculation valid down to budget `1`, effective-budget logging, grouped output, keep-going behavior, recursive-Make jobserver propagation, and stable public shard targets in `Makefile`
- [ ] T012 [US1] On leaf-dispatcher entry, unset `MAKEFLAGS`, `MFLAGS`, and `MAKELEVEL` before any setup spawn without parsing or closing stale jobserver descriptor numbers; then pass each runtime quota through Cargo `--jobs` and nextest threads, dependency-order same-target operations, write uniquely named completed-leaf fragments, and have the top-level target remove the prior requested manifest plus only its temporary fragment directory before dispatch, run the internal scheduler, then unconditionally sort and atomically publish a passed, failed, or handled-interruption `D2B_EXECUTION_MANIFEST` before returning the original status in `Makefile` and `tests/test-rust.sh`
- [ ] T013 [US1] Replace monolithic and background `tests/test-rust.sh` aggregate calls with `make test-rust` in `tests/static.sh`, and explicitly preserve the aggregate's existing fixture-dependent and policy coverage with the authoritative `make test-fixture-contracts` and `make test-policy` targets
...
- [ ] T019 [P] [US2] Add runner-neutral failing assertions for complete multi-failure reporting, pre-evaluation removal of a prior requested manifest, deterministic atomic partial replacement on a simulated failed run without stale entries, retained `D2B_NIX_UNIT_CHECK`, explicit failure on empty discovery, bounded resource controls, actionable handling of any retired knob, and absence of any new repository-specific scheduler in `tests/unit/meta/ci-runner-regression.py`
...
- [ ] T025 [US2] Implement the selected runner/evaluator and only its measured supporting changes in `flake.nix`, `tests/unit/nix/`, `tests/test-nix-unit.sh`, and `tests/unit/meta/ci-runner-regression.py`, retaining CI shard selection and fail-closed pin/shard integrity while removing a prior requested manifest before any evaluation or runner process, installing failure and handled-interruption finalization, collecting run-specific execution entries, and atomically publishing the partial or complete `D2B_EXECUTION_MANIFEST` before returning the original status
...
- [ ] T030 [P] [US3] Add failing policy assertions for one local `nix flake check --no-build --keep-going`, retained CI shard/output selectors, pre-evaluation removal of a prior requested manifest followed by deterministic atomic partial replacement on a simulated failed run without stale entries, and narrow realized-check execution in `packages/xtask/tests/policy_ci.rs`
...
- [ ] T032 [US3] Remove the local process-per-check scheduler and implement one native flake check followed by one multi-installable realized-check build, remove a prior requested manifest before any evaluation, install failure and handled-interruption finalization, collect evaluated and realized classes in a run-specific temporary file, and atomically publish the partial or complete `D2B_EXECUTION_MANIFEST` before returning the original status; reject retired local flake knobs with migration messages and preserve CI modes plus segfault diagnostics in `tests/test-flake.sh`
...
- [ ] T049 [US5] Add the user-facing test-speed and orchestration changes without internal process markers to `changelog.d/test-orchestration-speed.md`, including the migration note that top-level Make `-j` does not cap inner Cargo concurrency and `D2B_RUST_BUDGET` is the supported local Rust budget control
