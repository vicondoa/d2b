...
- [ ] T007 Author the execution-manifest v1 prose and JSON schema in `docs/reference/test-execution-manifest.md` and `docs/reference/schemas/test-execution-manifest-v1.json`, then record the final Rust execution-leaf ownership, target-directory conflicts, broker serial chain, Nix installable sets, and realized flake check set in `specs/002-optimize-test-orchestration/benchmark-results.md`
...
- [ ] T012 [US1] Enter each ordinary leaf after GNU Make has closed its jobserver descriptors, immediately unset `MAKEFLAGS`, `MFLAGS`, and `MAKELEVEL`, then pass each runtime quota through Cargo `--jobs` and nextest threads and dependency-order same-target operations. For requested evidence, lock a persistent mode-0600 `<manifest>.lock`, securely create and verify a mode-0700 current-user fragment directory, remove prior evidence before dispatch, atomically rename complete leaf fragments, run the scheduler in a dedicated process group, and on handled `INT` or `TERM` forward the signal, wait at most 10 seconds, `SIGKILL` and reap survivors, then idempotently publish a versioned passed, failed, or interrupted `D2B_EXECUTION_MANIFEST`, clean current and stale state through verified fd-relative operations, and return the original status in `Makefile` and `tests/test-rust.sh`; keep the production grace fixed while exposing only an internal injectable clock/process boundary to tests
...
- [ ] T019 [P] [US2] Add runner-neutral failing assertions for complete multi-failure reporting, pre-evaluation removal of a prior requested manifest, secure atomic versioned fragments, persistent-lock contention failure, symlink/owner/mode rejection, anchored cleanup that skips invalid paths, bounded process-group shutdown through injected test boundaries, deterministic partial replacement on simulated failed and handled-interruption runs without stale entries or live children, retained `D2B_NIX_UNIT_CHECK`, explicit failure on empty discovery, bounded resource controls, actionable handling of any retired knob, and absence of any new repository-specific scheduler in `tests/unit/meta/ci-runner-regression.py`
...
- [ ] T030 [P] [US3] Add static policy assertions for one local `nix flake check --no-build --keep-going`, retained CI shard/output selectors, versioned manifest wiring with pre-evaluation invalidation, and narrow realized-check execution in `packages/xtask/tests/policy_ci.rs`
- [ ] T031 [P] [US3] Replace local-shard scheduler expectations with runner-regression assertions that the local manifest no longer sets `D2B_FLAKE_LOCAL_SHARDS`, retired local flake knobs fail with migration messages, persistent-lock contention fails, symlink/owner/mode violations fail, anchored cleanup skips invalid paths, injected-boundary simulated failed and handled-interruption runs publish current versioned partial evidence without stale entries or live children, and the realized video check remains enforced in `tests/unit/meta/ci-runner-regression.py`

### Implementation for User Story 3

- [ ] T032 [US3] Remove the local process-per-check scheduler and implement one native flake check followed by one multi-installable realized-check build, applying the secure manifest lifecycle from T012 before any evaluation, atomically publishing evaluated and realized entries, bounding and reaping handled-interruption shutdown, publishing partial or complete versioned evidence, cleaning run-specific and verified stale temporary state, and returning the original status; reject retired local flake knobs with migration messages and preserve CI modes plus segfault diagnostics in `tests/test-flake.sh`
...
- [ ] T045 [P] [US5] Add documentation-policy assertions for the three stable Make targets, benchmark method, warm hard gate, cold non-blocking result, CI/local distinction, and execution-manifest prose/JSON-schema version agreement in `packages/d2b-contract-tests/tests/policy_docs.rs`
