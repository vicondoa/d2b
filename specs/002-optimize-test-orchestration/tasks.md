...
- [ ] T008 [P] [US1] Add failing assertions for the Make-owned Rust DAG, grouped keep-going execution, redacted actionable `D2B_RUST_BUDGET` validation, effective-budget logging, cache-aware cgroup default, unreadable-controller fallback to budget `1`, immediate Make metadata removal before setup spawns without closing stale descriptor numbers, actionable rejection of the removed no-argument `all` scheduler, secure atomic versioned fragments, and simulated failed plus handled-interruption runs that publish only current partial evidence with the correct `run_status`, preserve the original status, leave no child process, and contain no stale entries in `tests/unit/meta/ci-runner-regression.py`
...
- [ ] T012 [US1] On leaf-dispatcher entry, unset `MAKEFLAGS`, `MFLAGS`, and `MAKELEVEL` before any setup spawn without parsing or closing stale jobserver descriptor numbers; then pass each runtime quota through Cargo `--jobs` and nextest threads and dependency-order same-target operations. For requested evidence, exclusively own the manifest path, securely create and verify a mode-0700 current-user fragment directory, remove prior evidence before dispatch, atomically rename complete leaf fragments, run the scheduler in a dedicated process group, and on handled `INT` or `TERM` forward the signal, wait at most 10 seconds, `SIGKILL` and reap survivors, then idempotently publish a versioned passed, failed, or interrupted `D2B_EXECUTION_MANIFEST`, clean temporary state, and return the original status in `Makefile` and `tests/test-rust.sh`
...
- [ ] T019 [P] [US2] Add runner-neutral failing assertions for complete multi-failure reporting, pre-evaluation removal of a prior requested manifest, secure atomic versioned fragments, bounded process-group shutdown, deterministic partial replacement on simulated failed and handled-interruption runs without stale entries or live children, retained `D2B_NIX_UNIT_CHECK`, explicit failure on empty discovery, bounded resource controls, actionable handling of any retired knob, and absence of any new repository-specific scheduler in `tests/unit/meta/ci-runner-regression.py`
