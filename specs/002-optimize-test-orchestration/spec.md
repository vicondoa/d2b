...
- **FR-027**: Each optimized target MUST record CPU use, effective CPU budget, peak memory, applicable cgroup memory events, and worker bounds during representative warm runs. Rust measurement MUST cover the target process scope. Nix measurement MUST combine the client scope with work delegated to the Nix daemon when daemon-cgroup counters are readable, otherwise it MUST use baseline-adjusted host counters; both Nix modes require an externally idle benchmark and invalidate on overlapping daemon or host activity.
...
- **FR-029**: Resource acceptance MUST fail on orchestration-attributable out-of-memory events, sustained memory-pressure stalls, swap thrashing, peak memory beyond the calculated envelope, worker growth beyond the declared bound, or configured CPU-consuming quotas whose active frontier exceeds the effective CPU budget.
- **FR-030**: When an execution manifest is requested, the target MUST remove the prior manifest before any work begins and atomically publish the current run status plus partial completion evidence on success, failure, or a handled interruption. An uncatchable termination MAY leave no manifest but MUST NOT leave stale success evidence.
- **FR-031**: Manifest publication MUST anchor the manifest parent before opening a persistent separate lockfile with a non-inheritable open-file-description lock, mark every evidence file descriptor close-on-exec, use securely created current-user-owned same-filesystem temporary paths and atomic fragments, and perform anchored fd-relative cleanup. Handled `INT` or `TERM` MUST stop the scheduler process group with a bounded graceful wait, escalate remaining children, reap them, publish partial evidence, and clean temporary state.
