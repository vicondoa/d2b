...
- **FR-031**: Manifest publication MUST use a persistent separate lockfile with a non-inheritable open-file-description lock, securely created current-user-owned same-filesystem temporary paths, atomic fragments, and anchored fd-relative cleanup. Handled `INT` or `TERM` MUST stop the scheduler process group with a bounded graceful wait, escalate remaining children, reap them, publish partial evidence, and clean temporary state.
