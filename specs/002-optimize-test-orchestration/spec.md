...
- **FR-031**: Manifest fragment storage MUST use securely created, current-user-owned temporary paths and atomic fragment publication. Handled `INT` or `TERM` MUST stop the scheduler process group with a bounded graceful wait, escalate remaining children, reap them, publish partial evidence, and clean temporary state.
