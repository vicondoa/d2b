...
W3_ROWS = {
    ...
    ("host-check", "cgroup-delegation-refused"): (
        "host-check-error", 1,
        "Whether systemd delegated +cpu +memory +io +pids to the nixling host slice.",
        "Delegation request refused; controllers missing from cgroup.subtree_control.",
        "Add `Delegate=cpu memory io pids` to /etc/systemd/system/nixling-host.slice and `systemctl daemon-reload`.",
    ),
    ...
    ("host-check", "cgroup-controllers-missing"): (
        ...
        "Required controllers missing from /sys/fs/cgroup/nixling.slice/cgroup.controllers.",
        "Enable the missing controllers via systemd `Delegate=` on nixling-host.slice and reload.",
    ),
    ("host-check", "cgroup-kill-on-ancestor-refused"): (
        ...
        "cgroup.kill probe targeted an ancestor outside the nixling delegated subtree.",
        "Re-issue cgroup.kill against a leaf inside nixling.slice; never write cgroup.kill on an ancestor.",
    ),
    ("host-check", "ifname-too-long"): (
        ...
        "Shorten the env/vm name in nixling.envs.<env>.name so the derived ifname stays <= 15 bytes.",
    ),
    ("host-check", "ifname-collision"): (
        "host-check-error", 78,
        "Whether two derived ifnames collide after hash truncation.",
        "Two distinct env/vm names produced the same nl-* ifname suffix.",
    ...
    ("host-check", "ipv6-sysctl-drift"): (
        ...
        "Re-run `nixling host prepare --apply` so the broker re-asserts the declared sysctl set.",
    ),
    ("host-check", "nm-managed-foreign-conflict"): (
        "host-check-error", 1,
        "Whether a foreign NetworkManager profile is managing a nixling-owned interface.",
        "NetworkManager reports a managed connection on an interface declared as nixling-owned.",
        "Add the interface to /etc/NetworkManager/conf.d/nixling-unmanaged.conf and `nmcli general reload`.",
    ),
    ...
    ("host-check", "foreign-nft-rule-shadows-nixling"): (
        "host-check-error", 1,
        "Whether a foreign nft rule in a higher-priority hook shadows the inet nixling table.",
        "Foreign nft rule found at a hook priority lower than nixling's declared priority.",
        "Move the foreign rule to a higher priority or document it in nixling.firewallCoexistence.allow.",
    ),
    ("host-check", "firewall-coexistence-mismatch"): (
        ...
        "Update nixling.firewallCoexistence.manager to match the actual host firewall stack.",
    ),
    ...
    ("host-check", "modprobe-denied-not-in-matrix"): (
        ...
        "Add the module to nixling.kernelModules.allowed or remove its consumer.",
    ),
    ...
    ("host-check", "single-writer-conflict"): (
        ...
        "Both `nixling-legacy.service` and `nixlingd.service` are active on a Tier-0 mixed host.",
    ...
    ("host-check", "tier-0-legacy-uses-nixos-module"): (
        ...
        "host.tier == 0 / legacy but nixlingd.service was reached.",
    ...
    ("host-check", "host-lan-cidr-ambiguous"): (
        ...
        "Set nixling.site.hostLanCidr explicitly in the bundle.",
    ),
    # ---- host prepare --apply -----------------------------------------
    ...
    ("host-prepare", "route-preflight-no-default-route"): (
        "host-prepare-apply-error", 1,
        "Whether the host has a default route before nixling adds NAT routes.",
        "ip route show default returned empty; nixling refuses to add routes without an upstream.",
    ...
    ("host-prepare", "route-preflight-foreign-default-route"): (
        "host-prepare-apply-error", 1,
        "Whether the host's default route is on a nixling-owned interface.",
        "Default route observed via a nixling-owned bridge; nixling refuses to claim it.",
    ...
    ("host-prepare", "dnsmasq-not-bound"): (
        ...
        "ss -lnp probe shows no dnsmasq listening on the nl-* bridge IP.",
        "Inspect systemctl status nixling-dnsmasq@<env>.service and retry `host prepare --apply`.",
    ),
    ...
    ("host-prepare", "nft-foreign-rule-flush-attempted"): (
        ...
        "Broker observed an nft ruleset replace targeting a non-nixling table; refused.",
        "Restrict nft batches to `table inet nixling`; never `flush ruleset` from nixling.",
    ),
    ...
    ("host-prepare", "tier-0-legacy-uses-nixos-module"): (
        ...
        "host.tier == 0 / legacy but nixlingd.service was asked to mutate the host.",
        "Use the NixOS module on Tier-0 all-legacy hosts; the W3 daemon path is for Tier 1+.",
    ),
    ("host-prepare", "single-writer-conflict"): (
        "host-prepare-apply-error", 78,
        "Whether legacy + daemon writers are both targeting host state at apply time.",
        "Both legacy units and nixlingd.service hold an exclusive writer claim.",
    ...
    ("host-prepare", "legacy-no-prepare-apply"): (
        ...
        "Legacy bash `nixling host prepare --apply` invoked; refused by design.",
        "Re-run via the Rust path: `nixling host prepare --apply` (no legacy shim).",
    ),
    # ---- host destroy --apply -----------------------------------------
    ("host-destroy", "vm-still-running-refused"): (
        ...
        "Stop the listed VMs with `nixling down <vm>` then retry `host destroy --apply`.",
    ),
    ("host-destroy", "tier-0-legacy-uses-nixos-module"): (
        ...
        "host.tier == 0 / legacy but nixlingd.service was asked to tear the host down.",
    ...
    ("host-destroy", "legacy-no-destroy-apply"): (
        ...
        "Legacy bash `nixling host destroy --apply` invoked; refused by design.",
        "Re-run via the Rust path: `nixling host destroy --apply` (no legacy shim).",
    ),
    # ---- host install -------------------------------------------------
    ("host-install", "not-yet-implemented"): (
        "host-install-error", 70,
        "Whether `host install` is implemented in this nixling release.",
        "host install is a W4 deliverable; W3 ships the schema + CLI surface only.",
        "Use `nixling switch` or the NixOS module integration path until W4 ships.",
    ),
    # ---- Inherited W3-relevant onboarding rows (host check) -----------
    ("host-check", "daemon-down"): (
        "host-check-error", 1,
        "Whether nixlingd.service is active on the host.",
        "systemctl is-active nixlingd reports inactive/failed.",
        "Start the daemon: `systemctl start nixlingd.service`.",
    ),
    ("host-check", "socket-perms-wrong"): (
        "host-check-error", 1,
        "Whether /run/nixling/public.sock has the expected mode/owner/group.",
        "Observed mode/owner/group diverges from the declared SocketSpec.",
        "Inspect `systemctl status nixlingd.socket`; the unit re-asserts perms on restart.",
    ),
    ("host-check", "missing-group"): (
        "host-check-error", 1,
        "Whether the `nixling` group exists on the host.",
        "getent group nixling returned nothing.",
        "Create the group via the NixOS module (it manages users.groups.nixling).",
    ),
    ...
    ("host-check", "no-kvm"): (
        ...
        "/dev/kvm missing or not accessible to the nixling group.",
        "Load the appropriate kvm_intel/kvm_amd module and ensure the nixling group has access.",
    ),
    ...
    ("host-check", "nftables-conflict"): (
        ...
        "Probe detected an unmanaged nft framework conflicting with nixling's policy.",
    ...
    ("host-check", "tap-creation-denied"): (
        ...
        "Ensure nixling-priv-broker.service carries CAP_NET_ADMIN.",
    ),
    ("host-check", "stale-lock"): (
        "host-check-error", 1,
        "Whether /run/nixling/locks/<vm> is owned by a live process.",
