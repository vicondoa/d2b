...
#
# P2fu1 test-r1-1 closure: assert every P2 deliverable in the plan's
# §"P2" todo list has at least one Layer-1 gate (cargo test, eval
# script, or doc-drift gate) the integrator can run on every change.
#
...
# Mapping: ph2-* todo id → gate(s). Each gate is either a script
# under tests/ or a cargo test target. The asserter checks the
