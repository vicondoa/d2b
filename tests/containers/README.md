...
OCI image under **podman**. Each test is a standalone `tests/containers/*.sh`
script that sources `lib.sh`, builds its image
(`containerImages.<system>.<name>`, auto-discovered from
`tests/containers/images/*.nix`), loads it into podman, runs it, and asserts.
...
| Unit shape + `Wants=`/ordering, broker capability set, tmpfiles, evidence-record shape | `tests/nix-unit/cases/{broker-socket-activation,nixlingd-startup-smoke}.nix` | nix eval, fail-closed |
