...
# shellcheck source=tests/containers/lib.sh
. "$HERE/lib.sh"
...
if ! artifact=$(nlc_build_image ubuntu-host-check); then
  ...
  if [ -n "$flake_out" ] \
    && [ -e "$NLC_ROOT/tests/containers/images/ubuntu-host-check.nix" ] \
    && [ ! -e "$flake_out/tests/containers/images/ubuntu-host-check.nix" ]; then
