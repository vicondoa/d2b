#!/usr/bin/env bash
# tests/ci-coverage.sh — structural gate asserting every tests/*.sh is
...
LIVE_VM_SMOKE="$ROOT/tests/live-vm-smoke.sh"
...
is_referenced() {
  ...
  if grep -qF "tests/$name" "${ref_files[@]}" 2>/dev/null; then
    return 0
  fi
...
if [ ${#orphans[@]} -eq 0 ]; then
  ...
  echo "OR add it to the EXCLUDE_FROM_CI list in tests/ci-coverage.sh with a rationale comment." >&2
  echo "" >&2
