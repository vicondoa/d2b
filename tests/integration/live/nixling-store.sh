...
#
# Usage:
#   modules/nixling/tests/integration/live/nixling-store.sh                 # full run
#   modules/nixling/tests/integration/live/nixling-store.sh --quick         # smoke subset
#   modules/nixling/tests/integration/live/nixling-store.sh --only test_X   # one test
#
...
# VM up first (`nixling up <vm>`) and re-run.
...
#
# Override with NL_VMS="dev-vm test-vm" for a subset run.
if [ -z "${NL_VMS:-}" ]; then
  _manifest=/run/current-system/sw/share/nixling/vms.json
  if [ -r "$_manifest" ]; then
    NL_VMS=$(jq -r '
      to_entries
  ...
  # flake examples with `nixling-test` so a CI-style runner can
  # smoke-test without a live host.
  NL_VMS=${NL_VMS:-corp-vm dev-vm nixling-test}
fi
...
NL_RUNNING_VM=""
for v in $NL_VMS; do
  if vm_running "$v"; then NL_RUNNING_VM=$v; break; fi
done
...
# ---------------------------------------------------------------------
# #6 build idempotent: second `nixling build` does no work.
# ---------------------------------------------------------------------
test_build_idempotent() {
  log "test_build_idempotent"
  local vm=${NL_RUNNING_VM:-corp-vm}
  local first second
  first=$(nixling build "$vm" 2>&1 | grep 'closure →' | awk '{print $NF}')
  second=$(nixling build "$vm" 2>&1 | grep 'closure →' | awk '{print $NF}')
...
test_build_gc_root() {
  ...
  local vm=${NL_RUNNING_VM:-corp-vm}
  nixling build "$vm" >/dev/null 2>&1 || true
...
test_generations_list() {
  ...
  for vm in $NL_VMS; do
    local out
    out=$(nixling generations "$vm" 2>&1)
...
test_host_rebuild_rehydrates() {
  ...
  # entry exists that invokes nixling-store-sync.
  if grep -q nixlingStoreSync /run/current-system/activate; then
    ok "activate script references nixlingStoreSync"
  else
    fail "activate script lacks nixlingStoreSync hook"
  fi
  # And per-VM nixling-<vm>-store-sync.service units exist (one per declared VM).
  if compgen -G '/etc/systemd/system/nixling-*-store-sync.service' >/dev/null 2>&1 \
     || systemctl list-unit-files 'nixling-*-store-sync.service' --no-pager --no-legend 2>/dev/null \
        | grep -q 'nixling-.*-store-sync.service'; then
    ok "nixling-<vm>-store-sync.service unit(s) installed"
  else
    fail "no nixling-<vm>-store-sync.service unit installed"
  fi
}
...
# ---------------------------------------------------------------------
# #23 legacy `nixling status` still works (regression).
# ---------------------------------------------------------------------
test_legacy_status() {
  log "test_legacy_status"
  local out
  out=$(nixling status 2>&1)
...
test_error_missing_ssh_creds() {
  ...
  # \`nixling generations <net-vm>\` should print the unreachable-VM
  ...
  local out
  out=$(nixling generations "$net_vm" 2>&1 || true)
...
main() {
  ...
  log "nixling test suite — log: $NL_LOG  running-vm: ${NL_RUNNING_VM:-<none>}"
