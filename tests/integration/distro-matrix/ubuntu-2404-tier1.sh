...
# d2b via `d2b host install --apply`, prepare the host,
...
#
# Set D2B_UBUNTU_TIER1_STRICT=1 to fail closed when expected
...
#   4. vm start: d2b vm start minimal-vm --apply (
#      SpawnRunner real wire, pidfd via SCM_RIGHTS).
#   5. probe: SSH into the VM, confirm reachability.
#   6. vm stop: d2b vm stop --apply, confirm pidfd table drain.
...
DISTRO="ubuntu-24.04"
ARCH="x86_64-linux"
D2B_UBUNTU_TIER1_CLEANUP_ARMED=0
export DISTRO ARCH D2B_UBUNTU_TIER1_CLEANUP_ARMED
...
strict_missing_ok_or_fail() {
    local reason=$1
    if [ "${D2B_UBUNTU_TIER1_STRICT:-0}" = "1" ]; then
        fail "$reason"
    fi
    log "$reason (D2B_UBUNTU_TIER1_STRICT unset)"
}
...
preflight_or_skip() {
    ...
    if ! grep -q '^ID=ubuntu' /etc/os-release; then
        ...
        export D2B_UBUNTU_SCAFFOLD_ONLY=1
    fi
    if ! grep -q '^VERSION_ID="24.04"' /etc/os-release \
        && [ -z "${D2B_UBUNTU_SCAFFOLD_ONLY:-}" ]; then
        skip "expected Ubuntu 24.04 (set D2B_UBUNTU_SCAFFOLD_ONLY=1 to force scaffold-only)"
    fi
    ok "preflight"
}
...
cleanup() {
    trap - EXIT
    if [ "${D2B_UBUNTU_TIER1_CLEANUP_ARMED:-0}" != "1" ]; then
        return 0
    fi
...
phase_audit_replay() {
    ...
    local audit_date
    audit_date=$(date -u +%F)
    local audit_log="/var/lib/d2b/audit/broker-${audit_date}.jsonl"
    local artifact
    if ! [ -f "$audit_log" ]; then
...
main() {
    ...
    D2B_UBUNTU_TIER1_CLEANUP_ARMED=0
    export D2B_UBUNTU_TIER1_CLEANUP_ARMED
    log "tier-1 smoke complete (scaffold_only=${D2B_UBUNTU_SCAFFOLD_ONLY:-0})"
}
