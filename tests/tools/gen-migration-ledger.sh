...
ORCH=(static static-fast-tier0 static-timing runner preflight-disk-space cli-rust-native-common lib
  test-lint test-rust test-proofs test-flake test-flake-list test-nix-unit test-drift test-policy test-integration)
...
script_rel_for_name() {
  local name="$1"
  case "$name" in
    ci-coverage|ci-uses-make|layer1-self-inventory|deliverable-gate-inventory|no-new-deferral|adr-index-coverage)
    ...
    hardware-smoke-gpu-yubikey)
      printf 'tests/host-integration/hardware/%s.sh' "$name"
      ;;
    rust-workspace-checks|stub-no-socket|static-timing|preflight-disk-space)
...
emit_ledger() {
  local destination="$1" name group rel status successors exercised make_target tier
  declare -A emitted_missing=()
  {
    ...
    for group in "${GROUP_NAMES[@]}"; do
      ...
      for name in "${group_ref[@]}"; do
        ...
        status=$(status_for "$rel")
        successors=$(successors_for "$rel")
        exercised=$(exercised_for "$rel" "$ROOT/$rel")
