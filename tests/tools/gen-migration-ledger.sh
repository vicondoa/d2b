...
load_existing_state() {
  local ledger="$1" name status successors exercised
  [ -f "$ledger" ] || return 0
  ...
  while IFS=$'\t' read -r name status successors exercised; do
    [ -n "$name" ] || continue
    ...
    function flush() {
      if (name != "") {
        print name "\t" status "\t" successors "\t" exercised
      }
    }
    /^\[\[script\]\]/ { flush(); name=""; status=""; successors=""; exercised=""; next }
...
self_check() {
  local fail=0 file name hits group item
  ...
  for group in "${GROUP_NAMES[@]}"; do
    ...
    for name in "${group_ref[@]}"; do
      [ -f "$ROOT/tests/$name.sh" ] || {
        echo "LEDGER NAMES MISSING FILE: tests/$name.sh ($group)" >&2
        fail=1
      }
    done
  done
...
emit_ledger() {
  local destination="$1" file name group rel status successors exercised
  {
    ...
    for file in "$ROOT"/tests/*.sh; do
      ...
      echo "name = \"$rel\""
      echo "group = \"$group\""
      echo "make_target = \"$(target_of "$group")\""
      echo "tier = \"$(tier_of "$group")\""
      echo "status = \"$status\"      # legacy | porting | ported"
