...
policy="$trusted_root/tests/unit/nix/eval-cases/w8-integration-component-policy.nix"
...
[ -z "$(git_safe -C "$trusted_root" status --porcelain=v1 --untracked-files=all --ignore-submodules=none)" ] || {
  echo "component ownership: trusted W8 worktree is dirty" >&2
  exit 1
}
[ "$(git_safe -C "$trusted_root" branch --show-current)" = "adr0045-w8-integration" ] || {
  echo "component ownership: trusted worktree is not adr0045-w8-integration" >&2
  exit 1
}
...
git_safe -C "$candidate_root" merge-base --is-ancestor "$base" "$head" || {
  echo "component ownership: candidate does not descend from trusted W8 HEAD" >&2
  exit 1
}

branch=$(git_safe -C "$candidate_root" branch --show-current)
[[ "$branch" =~ ^adr0045-w8-integration-[a-z0-9-]+$ ]] || {
